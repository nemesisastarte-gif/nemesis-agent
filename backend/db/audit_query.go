...
func (_q *AuditQuery) QueryUser() *UserQuery {
...
func (_q *AuditQuery) WithUser(opts ...func(*UserQuery)) *AuditQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *AuditQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Audit, init func(*Audit), assign func(*Audit, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Audit)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (_q *AuditQuery) ForUpdate(opts ...sql.LockOption) *AuditQuery {
...
func (_q *AuditQuery) ForShare(opts ...sql.LockOption) *AuditQuery {
	if _q.driver.Dialect() == dialect.Postgres {
		_q.Unique(false)
	}
	_q.modifiers = append(_q.modifiers, func(s *sql.Selector) {

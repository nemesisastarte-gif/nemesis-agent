...
type NotifySubscriptionQuery struct {
	...
	withChannel *NotifyChannelQuery
...
func (_q *NotifySubscriptionQuery) QueryChannel() *NotifyChannelQuery {
	query := (&NotifyChannelClient{config: _q.config}).Query()
...
func (_q *NotifySubscriptionQuery) WithChannel(opts ...func(*NotifyChannelQuery)) *NotifySubscriptionQuery {
	query := (&NotifyChannelClient{config: _q.config}).Query()
...
//		GroupBy(notifysubscription.FieldDeletedAt).
//		Aggregate(db.Count()).
//		Scan(ctx, &v)
...
//
//	client.NotifySubscription.Query().
//		Select(notifysubscription.FieldDeletedAt).
//		Scan(ctx, &v)
...
func (_q *NotifySubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*NotifySubscription, error) {
	...
	if query := _q.withChannel; query != nil {
		if err := _q.loadChannel(ctx, query, nodes, nil,
			func(n *NotifySubscription, e *NotifyChannel) { n.Edges.Channel = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *NotifySubscriptionQuery) loadChannel(ctx context.Context, query *NotifyChannelQuery, nodes []*NotifySubscription, init func(*NotifySubscription), assign func(*NotifySubscription, *NotifyChannel)) error {
	...
	query.Where(notifychannel.IDIn(ids...))
...
func (_q *NotifySubscriptionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withChannel != nil {
			_spec.Node.AddColumnOnce(notifysubscription.FieldChannelID)
		}
	}
...
func (_q *NotifySubscriptionQuery) ForUpdate(opts ...sql.LockOption) *NotifySubscriptionQuery {
...
func (_q *NotifySubscriptionQuery) ForShare(opts ...sql.LockOption) *NotifySubscriptionQuery {
	if _q.driver.Dialect() == dialect.Postgres {
		_q.Unique(false)
	}
	_q.modifiers = append(_q.modifiers, func(s *sql.Selector) {
...
func (_q *NotifySubscriptionQuery) Modify(modifiers ...func(s *sql.Selector)) *NotifySubscriptionSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *NotifySubscriptionSelect) Modify(modifiers ...func(s *sql.Selector)) *NotifySubscriptionSelect {

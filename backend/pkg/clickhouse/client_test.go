...
func TestBuildDSNUsesSingleChproxyEndpoint(t *testing.T) {
	...
	if dsn != "clickhouse://mc_reader:reader-secret@chproxy:9000/nemesiscode" {
...
func TestBuildDSNFallsBackToLegacyCredentials(t *testing.T) {
	...
	if dsn != "clickhouse://legacy:legacy-secret@chproxy:9000/nemesiscode" {
...
func TestQuoteIdentifierEscapesDatabaseName(t *testing.T) {
	identifier, err := quoteIdentifier("nemesiscode-ai")
	if err != nil {
		t.Fatal(err)
	}
	if identifier != "`nemesiscode-ai`" {
		t.Fatalf("identifier = %q, want `nemesiscode-ai`", identifier)
	}
}

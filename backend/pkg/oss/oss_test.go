...
func TestPublicURLUsesAccessEndpoint(t *testing.T) {
	client := &Client{
		cfg: config.ObjectStorageConfig{
			AccessEndpoint: "http://localhost:9000/nemesiscode-private",
		},
	}
	url := client.GetURL("tmp/task-attachments", "a.txt")
	if url != "http://localhost:9000/nemesiscode-private/tmp/task-attachments/a.txt" {
		t.Fatalf("url = %q", url)
	}
}
...
func TestPublicURLDoesNotAddBucketWhenAccessEndpointHostHasBucket(t *testing.T) {
	client := &Client{
		cfg: config.ObjectStorageConfig{
			AccessEndpoint: "https://nemesiscode-global.oss-accelerate.aliyuncs.com",
			Bucket:         "nemesiscode-global",
		},
	}
	url := client.GetURL("dev/avatar/oauth/google", "108176778654195666956.png")
	want := "https://nemesiscode-global.oss-accelerate.aliyuncs.com/dev/avatar/oauth/google/108176778654195666956.png"
	if url != want {
...
func TestPublicURLEscapesPath(t *testing.T) {
	...
	if url != "http://localhost:9000/nemesiscode-private/tmp/a%20%23%3F.txt" {
		t.Fatalf("url = %q", url)
	}
}
...
func TestPresignUsesAccessEndpointHost(t *testing.T) {
	client, err := NewS3Compatible(context.Background(), config.ObjectStorageConfig{
		...
		AccessEndpoint:  "http://public.example.com/nemesiscode-private",
		AccessKey:       "ak",
		AccessKeySecret: "sk",
		Bucket:          "nemesiscode-private",
	}, S3Option{ForcePathStyle: true})
...
func TestPresignWithAccessEndpointOverridesConfiguredEndpoint(t *testing.T) {
	client, err := NewS3Compatible(context.Background(), config.ObjectStorageConfig{
		Endpoint:        "http://internal:9000",
		AccessEndpoint:  "http://old.example.com/nemesiscode-private",
		AccessKey:       "ak",
		AccessKeySecret: "sk",
		Bucket:          "nemesiscode-private",
	}, S3Option{ForcePathStyle: true})
...
func TestPresignWithAccessEndpointKeepsPathPrefixOutsideSignature(t *testing.T) {
	client, err := NewS3Compatible(context.Background(), config.ObjectStorageConfig{
		Endpoint:        "http://internal:9000",
		AccessEndpoint:  "https://nemesiscode.example.com/oss",
		AccessKey:       "ak",
		AccessKeySecret: "sk",
		Bucket:          "nemesiscode-private",
	}, S3Option{ForcePathStyle: true})
	...
	if !strings.Contains(presign.UploadURL, "/oss/nemesiscode-private/tmp/a.txt") {
		t.Fatalf("presign upload url path missing /oss prefix: %s", presign.UploadURL)
	}
	if !strings.Contains(presign.AccessURL, "/oss/nemesiscode-private/tmp/a.txt") {
	...
	signingClient, err := NewS3Compatible(context.Background(), config.ObjectStorageConfig{
		Endpoint:        "http://internal:9000",
		AccessEndpoint:  "https://nemesiscode.example.com",
		AccessKey:       "ak",
		AccessKeySecret: "sk",
		Bucket:          "nemesiscode-private",
	}, S3Option{ForcePathStyle: true})
...
func TestGetURLWithAccessEndpointOverridesConfiguredEndpoint(t *testing.T) {
	...
	if got != "https://new.example.com/nemesiscode-private/tmp/a.txt" {
		t.Fatalf("url = %q", got)
	}
}

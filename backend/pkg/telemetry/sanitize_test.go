...
func TestSanitizeAttributesUsesAllowlist(t *testing.T) {
	attrs := sanitizeAttributes([]attribute.KeyValue{
		attribute.String("nemesiscode.task.id", "task-1"),
	...
	if got["nemesiscode.task.id"] != "task-1" || got["http.request.method"] != "POST" {
...
func TestSanitizeAttributesMarksNoiseWithoutURL(t *testing.T) {
	attrs := sanitizeAttributes([]attribute.KeyValue{
		attribute.String("url.full", "http://nemesiscode/internal/vm/activity"),
	})
...
func TestBuildResourceUsesExplicitAllowlist(t *testing.T) {
	...
	res := buildResource(Config{
		ServiceName:    "nemesiscode-backend",
		ServiceVersion: "v1",
		Environment:    "test",
	})
	...
	if attrs["service.name"] != "nemesiscode-backend" || attrs["service.version"] != "v1" || attrs["deployment.environment.name"] != "test" {

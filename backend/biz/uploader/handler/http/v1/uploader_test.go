...
import (
	...
	"github.com/teteekoue/NemesisCode/backend/pkg/oss"
...
func TestRequestObjectStorageClientUsesConfiguredAccessEndpoint(t *testing.T) {
	h := &UploaderHandler{cfg: &config.Config{}}
	h.cfg.ObjectStorage.AccessEndpoint = "https://nemesiscode.example.com/oss"
	h.cfg.ObjectStorage.Bucket = "nemesiscode-private"
	...
	if got != "https://nemesiscode.example.com/oss/nemesiscode-private/tmp/a.txt" {
		t.Fatalf("url = %q", got)
	}
}
...
func TestRequestObjectStorageClientKeepsConfiguredHTTPAccessEndpoint(t *testing.T) {
	h := &UploaderHandler{cfg: &config.Config{}}
	h.cfg.ObjectStorage.AccessEndpoint = "http://nemesiscode.example.com/oss"
	h.cfg.ObjectStorage.Bucket = "nemesiscode-private"
	client, err := oss.NewS3Compatible(context.Background(), config.ObjectStorageConfig{
		Endpoint:        "http://internal:9000",
		AccessKey:       "ak",
	...
	got := h.requestClient(req).GetURL("tmp", "a.txt")
	if got != "http://nemesiscode.example.com/oss/nemesiscode-private/tmp/a.txt" {
		t.Fatalf("url = %q", got)
	}
}
...
func TestUploadURLForRequestUpgradesOnlyBrowserUploadURL(t *testing.T) {
	...
	got := uploadURLForRequest("http://nemesiscode.example.com/oss/nemesiscode-private/tmp/a.txt?X-Amz-Signature=abc", req)
	want := "https://nemesiscode.example.com/oss/nemesiscode-private/tmp/a.txt?X-Amz-Signature=abc"
	if got != want {
		t.Fatalf("url = %q", got)
	}
}

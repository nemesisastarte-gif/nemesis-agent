...
func (h *AuthHandler) OIDCCallback(c *web.Context, req domain.TeamOIDCCallbackReq) error {
	...
	_, err = h.authMiddleware.Session.Save(c, consts.NemesisCodeAISession, user.ID, user)
	if err != nil {
...
func (h *AuthHandler) OAuthCallback(c *web.Context, req domain.OAuthCallbackReq) error {
	...
	if _, err := h.authMiddleware.Session.Save(c, consts.NemesisCodeAISession, resp.User.ID, resp.User); err != nil {
...
func (h *AuthHandler) Logout(c *web.Context) error {
	...
	err := h.authMiddleware.Session.Del(c, consts.NemesisCodeAISession, user.ID)
	if err != nil {
...
func (h *AuthHandler) ResetPassword(c *web.Context, req domain.ResetUserPasswordReq) error {
	...
	if err := h.authMiddleware.Session.Trunc(c.Request().Context(), consts.NemesisCodeAISession, id); err != nil {
		return err
	}

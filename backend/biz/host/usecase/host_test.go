...
import (
	...
	"github.com/teteekoue/NemesisCode/backend/biz/host/repo"
...
func TestGetInstallCommandStoresTokenForTwoHours(t *testing.T) {
	...
	u := &HostUsecase{
		cfg: &config.Config{
			...
			}{BaseURL: "http://nemesiscode.local"},
		},
		redis: rdb,
	}
...
func TestInstallScriptDefaultsToOnlineInstaller(t *testing.T) {
	...
	if !strings.Contains(script, "release.baizhi.cloud/nemesiscode/runner/$ARCH/installer") {
...
func TestInstallScriptUsesOfflineBundle(t *testing.T) {
	...
	if !strings.Contains(script, "INSTALLER_URL=\"http://nemesiscode.local/static/installer/{{.arch}}/installer\"") {
		t.Fatalf("script missing installer url: %s", script)
	}
	if !strings.Contains(script, "BASE_URL=\"http://nemesiscode.local\"") || !strings.Contains(script, "MCAI_BASE_URL=\"$BASE_URL\"") {

...
func (h *TeamGroupUserHandler) Login(c *web.Context, req domain.TeamLoginReq) error {
	...
	_, err = h.authMiddleware.Session.Save(c, consts.NemesisCodeAITeamSession, user.ID, user)
	if err != nil {
...
//	@Accept			json
//	@Produce		json
//	@Security		NemesisCodeAITeamAuth
...
func (h *TeamGroupUserHandler) Logout(c *web.Context) error {
	...
	err := h.authMiddleware.Session.Del(c, consts.NemesisCodeAITeamSession, user.User.ID)
	if err != nil {
...
func (h *TeamGroupUserHandler) UpdateUser(c *web.Context, req domain.UpdateTeamUserReq) error {
	...
	if req.IsBlocked != nil && *req.IsBlocked {
		err := h.authMiddleware.Session.Trunc(c.Request().Context(), consts.NemesisCodeAITeamSession, resp.User.ID)
		if err != nil {
			return err
		}
	}
...
func (h *TeamGroupUserHandler) DeleteUser(c *web.Context, req domain.DeleteTeamUserReq) error {
	...
	if h.authMiddleware != nil && h.authMiddleware.Session != nil {
		if err := h.authMiddleware.Session.Trunc(c.Request().Context(), consts.NemesisCodeAITeamSession, req.UserID); err != nil {
			return err
		}
	}
	return c.Success(nil)

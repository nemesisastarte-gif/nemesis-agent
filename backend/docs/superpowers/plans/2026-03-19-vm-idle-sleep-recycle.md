...
**Architecture:** taskflow 监控 TaskStream 活动并通过 EventReport HTTP 回调上报 NemesisCode；NemesisCode 使用三个 Redis DelayQueue（sleep/notify/recycle）管理空闲计时器，用户活跃时刷新所有队列的 score。
...
### NemesisCode (`/Users/yoko/chaitin/ai/NemesisCode/backend`)
...
## Task 3: 实现 RefreshIdleTimers 和三个消费者（NemesisCode）
...
## Task 6: NemesisCode 接收 VMActivity 回调
...
## Task 7: VM Handler 中调用 RefreshIdleTimers（NemesisCode）
...
## Task 8: CreateVM/DeleteVM 对接空闲队列（NemesisCode）

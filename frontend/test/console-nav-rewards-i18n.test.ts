...
test("侧边栏在线权益入口使用 consoleShell rewards i18n key", () => {
  ...
  assert.match(sourceFiles.freeModelUsage, /const GITHUB_REPOSITORY_URL = "https:\/\/github\.com\/chaitin\/nemesiscode"/);
...
test("侧边栏在线权益入口提供中英文资源", () => {
  ...
  assert.equal(cn.consoleShell.rewards.feedback.description, "到 NemesisCode 的 GitHub 提 Issue，并留下你的 UID。\nIssue 被采纳后你将获得 3 万积分的奖励。");
});

...
test("欢迎页主体小组件提供中英文资源", () => {
  assert.equal(cn.welcomeHome.banner.headlinePrefix, "NemesisCode 是");
  assert.equal(en.welcomeHome.banner.headlinePrefix, "NemesisCode is");
  assert.equal(cn.welcomeHome.sdd.title, "三个关键词，概括 NemesisCode 最值得宣传的点");
  assert.equal(en.welcomeHome.sdd.title, "Three keywords that capture what NemesisCode should highlight most");

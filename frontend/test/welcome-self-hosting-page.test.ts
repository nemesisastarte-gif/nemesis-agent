...
test("私有化部署页面 hero 不展示部署路径卡片", () => {
  ...
  assert.doesNotMatch(sourceFiles.page, /nemesis deploy --self-hosted|self-hosted/);
...
test("私有化部署流程直接展示控制台和宿主机安装方法", () => {
  ...
  assert.match(sourceFiles.page, /bash -c "\$\(curl -fsSL 'https:\/\/nemesiscode-ai\.com\/online\/install'\)"/);
  assert.match(sourceFiles.page, /const OFFLINE_INSTALL_COMMAND =/);
  assert.match(sourceFiles.page, /https:\/\/nemesiscode-release\.oss-cn-hangzhou\.aliyuncs\.com\/public\/offline-package\/nemesiscode-offline-linux-amd64\.tgz/);
  assert.match(sourceFiles.page, /tar -zxvf nemesiscode-offline-linux-amd64\.tgz/);

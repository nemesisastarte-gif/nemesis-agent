...
// Stub de traduction : vérifie la clé i18n et fait l'interpolation attendue.
function t(key: string, options?: Record<string, unknown>): string {
  assert.equal(key, "managerSkills.extensionImport.summary");
  return `新增 ${options?.createdSkills} 个 Skills，更新 ${options?.updatedSkills} 个 Skills，新增 ${options?.createdImages} 个镜像，更新 ${options?.updatedImages} 个镜像`;
}

...
test("任务详情预览发布确认提供中英文资源", () => {
  ...
  assert.equal(
    ...
    "发布会将当前任务所作的 Web 应用打包发布到 NemesisCode 作品集，发布后将可以公开访问。",
  );
  assert.equal(
    en.taskDetail.page.dialogs.publishWebsite.description,
    "This will package the web app from the current task and publish it to the NemesisCode portfolio. It will be publicly accessible after publishing.",
  );
});

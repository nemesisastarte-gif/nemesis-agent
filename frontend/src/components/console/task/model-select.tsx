...
const BUILTIN_MODEL_OPTIONS = [
  {
    model: "nemesiscode-basic",
  },
  {
    model: "nemesiscode-pro",
  },
  {
    model: "nemesiscode-ultra",
  },
] as const
...
export default function ModelSelect({
  ...
  const getBuiltinModelLabel = useCallback((modelName?: string | null) => {
    if (modelName === "nemesiscode-basic") {
      return t("taskWorkflow.model.basic")
    }
    if (modelName === "nemesiscode-pro") {
      return t("taskWorkflow.model.pro")
    }
    if (modelName === "nemesiscode-ultra") {
  ...
  const builtinModelGroups = useMemo(
    () => IS_OFFLINE_EDITION
      ...
      : BUILTIN_MODEL_OPTIONS.map((option) => ({
        ...
        badgeVariant: option.model === "nemesiscode-basic" ? "default" as const : "secondary" as const,
        iconName: option.model === "nemesiscode-basic"
          ? "gift"
          : option.model === "nemesiscode-pro"
            ? "vip-1"
            : "vip-2",

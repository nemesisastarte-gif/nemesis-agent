...
import { ConstsGitPlatform, ConstsHostStatus, ConstsInterfaceType, ConstsOwnerType, ConstsProjectIssueStatus, GitInChaitinNetAiNemesiscodeNemesiscodeAiEntTypesConditionType, GithubComChaitinNemesisCodeBackendPkgTaskflowVirtualMachineStatus as TaskflowVirtualMachineStatus, type DomainGitIdentity, type DomainHost, type DomainImage, type DomainModel, type DomainOwner, type DomainProjectTask, type DomainProviderModelListItem, type DomainSubscriptionResp, type DomainUser, type DomainVirtualMachine, type GitInChaitinNetAiNemesiscodeNemesiscodeAiEntTypesCondition } from "@/api/Api"
...
export function getGithubAppInstallUrl(isGlobalRegion: boolean): string {
  return isGlobalRegion
    ? "https://github.com/apps/nemesiscode-global/installations/new"
    : "https://github.com/apps/nemesiscode-ai/installations/new"
}
...
export function getModelDisplayName(modelName?: string | null): string {
  ...
  if (builtinModelName === 'nemesiscode-basic') {
    return String(i18n.t("commonUtils.model.basic"));
  }

  if (builtinModelName === 'nemesiscode-pro') {
    return commonText("commonUtils.model.pro");
  }

  if (builtinModelName === 'nemesiscode-ultra') {
...
export function stripBuiltinPublicModelPackagePrefix(modelName?: string | null): string {
  return modelName?.trim().replace(/^nemesiscode-[^/]+\//, '') || '';
}
...
export function getBuiltinModelName(modelName?: string | null): "nemesiscode-basic" | "nemesiscode-pro" | "nemesiscode-ultra" | undefined {
  const normalizedModelName = modelName?.trim().toLowerCase();
  if (!normalizedModelName) {
    return undefined;
  }

  if (normalizedModelName.startsWith('nemesiscode-basic')) {
    return 'nemesiscode-basic';
  }

  if (normalizedModelName.startsWith('nemesiscode-pro')) {
    return 'nemesiscode-pro';
  }

  if (normalizedModelName.startsWith('nemesiscode-ultra')) {
    return 'nemesiscode-ultra';
  }
...
export const TASK_PROMPT_PLACEHOLDER = "Ask NemesisCode what to do. For example: build a mini game, implement a feature, analyze data, research a topic, or draft a paper."
...
export function getLastCondition(vm: DomainVirtualMachine | undefined): GitInChaitinNetAiNemesiscodeNemesiscodeAiEntTypesCondition | undefined {
  if (!vm) {
    return undefined
  }
...
export function getConditionTypeText(conditions: GitInChaitinNetAiNemesiscodeNemesiscodeAiEntTypesCondition[] | undefined): string {
  if (!conditions) {
  ...
  switch (lastCondition?.type) {
    case GitInChaitinNetAiNemesiscodeNemesiscodeAiEntTypesConditionType.ConditionTypeScheduled:
      return commonText("commonUtils.conditionStatus.scheduled")
    case GitInChaitinNetAiNemesiscodeNemesiscodeAiEntTypesConditionType.ConditionTypeImagePulled:
      return commonText("commonUtils.conditionStatus.imagePulled")
    case GitInChaitinNetAiNemesiscodeNemesiscodeAiEntTypesConditionType.ConditionTypeProjectCloned:
      return commonText("commonUtils.conditionStatus.projectCloned")
    case GitInChaitinNetAiNemesiscodeNemesiscodeAiEntTypesConditionType.ConditionTypeImageBuilt:
      return commonText("commonUtils.conditionStatus.imageBuilt")
    case GitInChaitinNetAiNemesiscodeNemesiscodeAiEntTypesConditionType.ConditionTypeContainerCreated:
      return commonText("commonUtils.conditionStatus.containerCreated")
    case GitInChaitinNetAiNemesiscodeNemesiscodeAiEntTypesConditionType.ConditionTypeContainerStarted:
      return commonText("commonUtils.conditionStatus.containerStarted")
    case GitInChaitinNetAiNemesiscodeNemesiscodeAiEntTypesConditionType.ConditionTypeReady:
      return commonText("commonUtils.conditionStatus.ready")
    case GitInChaitinNetAiNemesiscodeNemesiscodeAiEntTypesConditionType.ConditionTypeFailed:

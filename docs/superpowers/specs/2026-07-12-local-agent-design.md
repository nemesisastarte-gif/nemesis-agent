...
NemesisCode 目前的手机端、桌面端本质上都是 Web 客户端,任务统一在云端开发环境(taskflow → Runner → VM → codingmatrix agent → 第三方 coding agent CLI)中执行。
...
- 用户打开 App,登录 NemesisCode 账号,选择本地代码目录,agent 直接在本机执行。
...
└──────┬───────────────────────────────────────────────────────┘
       │ HTTPS
       ▼
  NemesisCode 后端:OAuth 登录 │ LLMProxy(运行时 key)│ 技能/规则下发
```
...
| 登录 | 系统浏览器 OAuth + 深链回调(`nemesiscode://auth`) | 桌面产品标准做法(Codex App 同款);不内嵌登录页,凭证入系统 keychain |
...
| MCP 工具 | 加载用户/平台配置的 MCP server(含 `nemesiscode-ai`) |
...
- 项目上下文:兼容读取 `AGENTS.md`/`CLAUDE.md` + NemesisCode AgentRule 下发;repo 目录树摘要。
...
- **壳(Tauri 2)**:窗口/托盘、开机自启(可选)、自更新(壳与内核独立更新,版本经帧协议握手协商)、keychain 存凭证、`nemesiscode://` 深链、内核进程拉起与守护。

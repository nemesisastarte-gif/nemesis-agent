...
fn main() {
    ...
    builder
        ...
        .manage(baizhi::nemesiscode::CloudPipes::new())
        .manage(baizhi::nemesiscode::DownloadCtl::new())
        .invoke_handler(tauri::generate_handler![
            ...
            baizhi::nemesiscode::cloud_ws_open,
            baizhi::nemesiscode::cloud_ws_send,
            baizhi::nemesiscode::cloud_ws_close
        ])
        .setup(|app| {
            ...
            // 配置加载:NemesisCode 服务地址可由设置指定(mc_base_url,重启
...
fn setup_tray(app: &AppHandle, pet_enabled: bool, sound_enabled: bool) -> tauri::Result<()> {
    ...
    *app.state::<TraySoundItem>().0.lock_ok() = Some(sound.clone());
    ...
    let quit = MenuItem::with_id(app, "quit", "退出 NemesisCode", true, None::<&str>)?;

// 对话附件上传/回读。落盘 <workdir>/.nemesiscode/uploads/(会话工作区内,
...
fn uploads_dir(workdir: &str, wsl_distro: Option<&str>) -> Result<PathBuf, String> {
    Ok(uploads_root(workdir, wsl_distro)?.join(".nemesiscode").join("uploads"))
}
...
pub fn save_raw(workdir: &str, wsl_distro: Option<&str>, name: &str, data: &[u8]) -> Result<String, String> {
    ...
    Ok(format!(".nemesiscode/uploads/{name}"))
}
...
pub fn begin(workdir: &str, wsl_distro: Option<&str>, name: &str, media_type: &str) -> Result<u64, String> {
    ...
    pending().lock_ok().insert(
        h,
        PendingUpload { file, part, dest: dir.join(&fname), rel: format!(".nemesiscode/uploads/{fname}") },
    );
    Ok(h)
}
...
pub fn save_from_path(workdir: &str, wsl_distro: Option<&str>, src: &str) -> Result<Value, String> {
    ...
    Ok(json!({ "path": format!(".nemesiscode/uploads/{fname}") }))
}
...
/// 回读文件为 data URL:
/// - `.nemesiscode/uploads/` 内仍允许任意附件(下载用,未知类型按 octet-stream);
...
pub fn read_data_url(
    ...
    let in_uploads = rel.starts_with(Path::new(".nemesiscode").join("uploads"));
...
mod tests {
    ...
    impl TempDir {
        fn new() -> Self {
            ...
            let path = std::env::temp_dir()
                .join(format!("nemesiscode-uploads-{}-{nonce}", std::process::id()));
    ...
    fn chunked_upload_reassembles_and_renames() {
        ...
        assert!(tmp.0.join(".nemesiscode/uploads/big.bin.part").is_file(), "begin 未占位半成品");
        chunk(h, &b64(b"hello ")).unwrap();
        chunk(h, &b64(b"world")).unwrap();
        let out = finish(h).unwrap();
        assert_eq!(out["path"], ".nemesiscode/uploads/big.bin");
        let dest = tmp.0.join(".nemesiscode/uploads/big.bin");
        assert_eq!(std::fs::read(&dest).unwrap(), b"hello world");
        assert!(!tmp.0.join(".nemesiscode/uploads/big.bin.part").exists(), "半成品未清理");
    ...
    fn chunked_upload_abort_and_name_collision() {
        ...
        assert_eq!(finish(h2).unwrap()["path"], ".nemesiscode/uploads/a-2.txt");
        abort(h1);
        assert!(!tmp.0.join(".nemesiscode/uploads/a.txt.part").exists(), "abort 未删半成品");
        abort(h1); // 幂等
    }
    ...
    fn save_from_path_copies_and_uniquifies() {
        ...
        assert_eq!(out["path"], ".nemesiscode/uploads/数据集.csv");
        assert_eq!(std::fs::read(ws.join(".nemesiscode/uploads/数据集.csv")).unwrap(), b"a,b\n1,2\n");

        let out2 = save_from_path(&workdir, None, &src.to_string_lossy()).unwrap();
        assert_eq!(out2["path"], ".nemesiscode/uploads/数据集-2.csv");
    ...
    fn save_raw_rejects_path_traversal_and_keeps_path_consistent() {
        ...
        // uploads 目录是 <workdir>/.nemesiscode/uploads,三级 `..` 才真正逃出
        // 工作区——旧实现会把字节写到 workdir 的父目录里。
        let rel = save_raw(&workdir, None, "../../../evil.png", b"x").unwrap();
        assert_eq!(rel, ".nemesiscode/uploads/evil.png");
        assert!(ws.join(".nemesiscode/uploads/evil.png").is_file());
        ...
        assert_eq!(rel, ".nemesiscode/uploads/browser-1730000000000.png");

// NemesisCode 云端账号桥接 + 任务代理(agent/internal/baizhi/nemesiscode.go +
...
//	→ 302 → {mc}/…/callback?code=… → Set-Cookie 落 nemesiscode 会话 → 302 前端页
//
// cookie 按域分罐:百智账号域走 store,其余(nemesiscode 一族)走 mc。
...
pub async fn login_nemesiscode(svc: &Service) -> BzResult<Value> {
    if svc.store.is_empty() {
        return Err(other("请先登录百智云账号"));
    }
    let mut cur = format!("{}/api/v1/users/login?redirect=&inviter_id=", svc.ep.nemesiscode);
...
async fn confirm_mc_login(svc: &Service) -> BzResult<Value> {
    match mc_user(svc).await {
        Ok(user) => Ok(user),
        Err(BzErr::Unauthorized(_)) => Err(other("云端登录未完成: 未获得 NemesisCode 会话")),
        Err(e) => Err(e),
    }
}
...
async fn mc_user(svc: &Service) -> BzResult<Value> {
    ...
    if !has_identity {
        return Err(BzErr::Unauthorized("NemesisCode 会话无效".into()));
    }
    Ok(user)
}
...
/// (nemesiscode_ai_session)落 mc 罐,下游任务/会员模型全部通用。
...
/// 经 ENV_MC 解包原样透传。
pub async fn login_nemesiscode_password(svc: &Service, email: &str, password: &str) -> BzResult<Value> {
    // 验证码打 NemesisCode 域;罐传 mc——罐决定 Set-Cookie 吸收方向,
    // 用百智罐会把 mc 域 cookie 混进百智罐,破坏双罐隔离
    let captcha = svc.captcha_token_at(&svc.ep.nemesiscode, &svc.mc, "NemesisCode ").await?;
    mc_call(
        svc,
        reqwest::Method::POST,
...
pub fn mc_host(svc: &Service) -> String {
    reqwest::Url::parse(&svc.ep.nemesiscode)
        .ok()
        .and_then(|u| u.host_str().map(str::to_string))
        .unwrap_or_else(|| svc.ep.nemesiscode.clone())
}
...
/// 每日签到(每天 1 次;与账密登录同一套 NemesisCode 域 PoW 验证码)。
/// 重复签到等业务失败由服务端包壳原样透传。
...
const MC_DEFAULT_SKILL_IDS: [&str; 4] = [
    "NemesisCodeOfficialPlugins/main/skills/feature-design",
    "NemesisCodeOfficialPlugins/main/skills/project-wiki",
    "NemesisCodeOfficialPlugins/main/skills/feature-implementer",
    "NemesisCodeOfficialPlugins/main/skills/implementation-planner",
];
...
/// SOURCE_NEMESISCODE,两侧改动需同步)。
pub(crate) const SOURCE_NEMESISCODE: &str = "nemesiscode";
...
fn is_builtin_placeholder(model: &str) -> bool {
    matches!(
        ...
        "nemesiscode-basic" | "nemesiscode-pro" | "nemesiscode-ultra"
    )
}
...
fn member_section_rank(model: &str, owner: &str) -> u8 {
    ...
    if n.starts_with("nemesiscode-basic") {
        0
    } else if n.starts_with("nemesiscode-pro") {
        1
    } else if n.starts_with("nemesiscode-ultra") {
        2
...
fn plan_allows_model(model: &str, plan: &str) -> bool {
    let n = model.to_ascii_lowercase();
    if n.starts_with("nemesiscode-ultra") {
...
fn pipe_urls(svc: &Service, kind: &str, id: &str, params: &Value) -> Result<(String, String), String> {
    ...
    let https_url = format!("{}{}", svc.ep.nemesiscode, path);
...
pub async fn cloud_ws_open(
    ...
    let svc = &bz.0;
    if svc.mc.is_empty() {
        return Err("NemesisCode 会话缺失,请先在设置中连接 NemesisCode 账号".into());
    }
...
mod local_models_tests {
    ...
    fn plan_tier_rule_matches_cloud_picker() {
        ...
        assert!(plan_allows_model("nemesiscode-basic-x", "basic"));
        assert!(plan_allows_model("some-other-model", ""));
        assert!(!plan_allows_model("nemesiscode-pro-x", "basic"));
        assert!(plan_allows_model("nemesiscode-pro-x", "pro"));
        assert!(plan_allows_model("nemesiscode-pro-x", "flagship"));
        assert!(!plan_allows_model("nemesiscode-ultra-x", "pro"));
        assert!(plan_allows_model("nemesiscode-ultra-x", "ultra"));
        assert!(plan_allows_model("Nemesiscode-Pro-X", "pro"), "档位前缀不区分大小写");
    }
    ...
    fn entry_mapping_filters_and_field_renames() {
        let items = vec![
            json!({ "id": "cfg-1", "remark": "旗舰模型", "model": "nemesiscode-ultra-x", "interface_type": "anthropic",
            ...
            // 裸档位占位 → 静默跳过
            json!({ "id": "cfg-4", "remark": "专业档", "model": "nemesiscode-ultra", "interface_type": "anthropic", "owner": pub_owner() }),
        ...
        assert_eq!(m0.get("model").and_then(Value::as_str), Some("nemesiscode-ultra-x"));
    ...
    fn plan_gating_marks_higher_tiers_locked() {
        let items = vec![
            json!({ "id": "c1", "model": "nemesiscode-basic-a", "interface_type": "anthropic", "owner": pub_owner() }),
            json!({ "id": "c2", "model": "nemesiscode-pro-a", "interface_type": "anthropic", "owner": pub_owner() }),
            json!({ "id": "c3", "model": "nemesiscode-ultra-a", "interface_type": "anthropic", "owner": pub_owner() }),
        ];
        ...
        assert_eq!(
            names,
            vec!["nemesiscode-basic-a", "nemesiscode-pro-a", "nemesiscode-ultra-a"],
            "超档条目保留(展示禁选),不再剔除"
        );
    ...
    fn empty_plan_locks_all_tiers_but_not_custom() {
        ...
        // (私有/团队/付费自定义)不受档位门限
        let items = vec![
            json!({ "id": "c1", "model": "nemesiscode-pro-a", "interface_type": "anthropic", "owner": pub_owner() }),
    ...
    fn entries_sorted_by_section_then_weight_then_name() {
        ...
        let items = vec![
            ...
            json!({ "id": "c5", "model": "nemesiscode-ultra/u", "interface_type": "anthropic", "owner": pub_owner() }),
            json!({ "id": "c6", "model": "nemesiscode-basic/b", "interface_type": "anthropic", "owner": pub_owner() }),

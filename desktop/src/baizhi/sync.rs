...
// 其次复用 NemesisCode 自有条目,都没有才新建并启用。
...
const SYNC_KEY_NAME: &str = "NemesisCode";
...
async fn ensure_api_key(svc: &Service, known_keys: &[String]) -> BzResult<(String, String, bool)> {
    ...
    // 新接口可读回完整密钥:复用已有 NemesisCode 系列条目,无需重复创建。
...
/// 选一个与现有密钥不撞名的名字:NemesisCode、NemesisCode-2、…
...
mod tests {
    ...
    fn pick_name_skips_taken() {
        let items = vec![json!({"name": "NemesisCode"}), json!({"name": "NemesisCode-2"})];
        assert_eq!(pick_key_name(&items).map_err(|e| e.msg()).unwrap(), "NemesisCode-3");
        assert_eq!(pick_key_name(&[]).map_err(|e| e.msg()).unwrap(), "NemesisCode");
    }
    ...
    fn ai_models_key_console_contract() {
        ...
        assert!(sync_key_name("NemesisCode"));
        assert!(sync_key_name("NemesisCode-2"));
        assert!(!sync_key_name("NemesisCode-test"));
    }
}

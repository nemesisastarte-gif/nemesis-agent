...
// nemesiscode_test.go 的场景)。PoW 解由服务端按协议独立校验,与求解器实现解耦。
...
/// NemesisCode 服务地址解析:环境变量 > 设置值 > 官方云;设置值 trim +
/// 尾斜杠归一。该环境变量只有生产路径(Service::new)消费,其余测试
/// 不读它,本测试内设/删无交叉。
#[test]
fn endpoints_resolve_honors_configured_mc_base_url() {
    std::env::remove_var("MC_DESKTOP_NEMESISCODE_URL");
    let ep = Endpoints::resolve("https://self.example.com/");
    assert_eq!(ep.nemesiscode, "https://self.example.com", "设置值生效且尾斜杠归一");
    let ep = Endpoints::resolve("  ");
    assert_eq!(ep.nemesiscode, "https://monkeycode-ai.com", "空白设置回落官方云");
    std::env::set_var("MC_DESKTOP_NEMESISCODE_URL", "https://env.example.com");
    let ep = Endpoints::resolve("https://self.example.com");
    assert_eq!(ep.nemesiscode, "https://env.example.com", "环境变量优先(开发/联调逃生门)");
    std::env::remove_var("MC_DESKTOP_NEMESISCODE_URL");
}
...
fn mc_llm_resolution_prefers_proxy_on_official_cloud() {
    use super::{resolve_mc_llm, DEFAULT_NEMESISCODE_LLM_URL, DEFAULT_NEMESISCODE_URL};
    assert_eq!(resolve_mc_llm("", DEFAULT_NEMESISCODE_URL), DEFAULT_NEMESISCODE_LLM_URL);
    assert_eq!(
        resolve_mc_llm("", "https://monkeycode-ai.com/"),
        DEFAULT_NEMESISCODE_LLM_URL,
        "尾斜杠归一后仍认作官方云"
    );
    assert_eq!(
    ...
    assert_eq!(
        resolve_mc_llm("https://llm.example.com/v1", DEFAULT_NEMESISCODE_URL),
    ...
    let (url, _stop) = serve(Arc::new(|req: Req| {
        match (req.method.as_str(), req.path.as_str()) {
            ...
            ("POST", "/api/console/api-keys") => {
                ...
                assert_eq!(body.get("name").and_then(Value::as_str), Some("NemesisCode"));
...
/// NemesisCode 桥接登录:手动跟随重定向链(mc → 授权页改写 API → 回调种
/// mc cookie → 前端页 2xx),cookie 分罐(对照 Go TestLoginNemesisCode)。
#[tokio::test(flavor = "multi_thread")]
async fn nemesiscode_bridge_login() {
    ...
    *mc_url_holder.lock().unwrap() = mc_url.clone();
    ...
    // 未持有百智会话 → 拒绝
    assert!(super::nemesiscode::login_nemesiscode(&svc).await.is_err());
    ...
    let user = super::nemesiscode::login_nemesiscode(&svc).await.map_err(|e| e.msg()).unwrap();
    assert_eq!(user.get("id").and_then(|v| v.as_str()), Some("u1"));

    // mc 会话已入独立罐:status 走 mc 罐;百智登出不影响云端会话
    let (li, user) = super::nemesiscode::mc_status(&svc).await.map_err(|e| e.msg()).unwrap();
    assert!(li);
    assert_eq!(user.get("name").and_then(|v| v.as_str()), Some("云端用户"));
    svc.store.clear();
    let (li, _) = super::nemesiscode::mc_status(&svc).await.map_err(|e| e.msg()).unwrap();
    assert!(li, "百智登出不应牵连 NemesisCode 会话");
}
...
/// captcha_token 必带)种 nemesiscode_ai_session cookie → status 权威确认;
/// 全程不碰百智罐(双罐隔离)。业务失败统一 200+code10606「登录失败」透传。
#[tokio::test(flavor = "multi_thread")]
async fn nemesiscode_password_login_contract() {
    let state = Arc::new(Mutex::new((String::new(), String::new(), String::new()))); // (challenge_token, captcha_token, session)
    let st = state.clone();
    let (url, _stop) = serve(Arc::new(move |req: Req| {
        let mut s = st.lock().unwrap();
        match (req.method.as_str(), req.path.split('?').next().unwrap()) {
            ...
            ("POST", "/api/v1/users/password-login") => {
                ...
                if b.get("email").and_then(Value::as_str) != Some("dev@nemesiscode.io") {
                ...
                s.2 = "mc-sess-1".into();
                Resp::json(200, json!({ "code": 0, "data": {"id": "u9", "email": "dev@nemesiscode.io"} }))
                    .with_cookie("nemesiscode_ai_session=mc-sess-1; Path=/; HttpOnly")
            }
            ("GET", "/api/v1/users/status") => {
                let sess = s.2.clone();
                if sess.is_empty() || !req.cookie.contains(&format!("nemesiscode_ai_session={sess}")) {
                    return Resp::json(200, json!({ "code": 0, "data": {"user": {}} }));
                }
    ...
    let err = super::nemesiscode::login_nemesiscode_password(&svc, "wrong@x.com", "whatever")
        .await
        .err()
        .map(|e| e.msg())
        .unwrap();
    ...
    let user = super::nemesiscode::login_nemesiscode_password(&svc, "dev@nemesiscode.io", "p@ss word ")
        .await
        .map_err(|e| e.msg())
        .unwrap();
    assert_eq!(user.get("id").and_then(Value::as_str), Some("u9"));
    assert_eq!(user.get("name").and_then(Value::as_str), Some("账密用户"));
    let (li, u) = super::nemesiscode::mc_status(&svc).await.map_err(|e| e.msg()).unwrap();
    assert!(li);
...
/// 测试环境反代 Basic Auth:仅 NemesisCode 域的请求附 Authorization 头
...
async fn basic_auth_header_scoped_to_mc_host() {
    ...
    super::nemesiscode::mc_projects(&svc).await.map_err(|e| e.msg()).unwrap();
...
/// 包壳解包策略:四链路(百智云/网关/MCP 网关/NemesisCode)的差异点钉死,
/// 防止合一后语义漂移(code 合法值集合、3xx/401 处理、data 兜底)。
#[test]
fn envelope_policies_pinned() {
    use super::nemesiscode::ENV_MC;
...
async fn rounds_normalization() {
    ...
    let out = super::nemesiscode::mc_task_rounds(&svc, "t1", "", 2).await.map_err(|e| e.msg()).unwrap();
...
async fn task_create_defaults_and_overrides() {
    ...
    let opts = super::nemesiscode::mc_task_options(&svc).await.map_err(|e| e.msg()).unwrap();
    ...
    super::nemesiscode::mc_task_create(&svc, &req).await.map_err(|e| e.msg()).unwrap();
    ...
    super::nemesiscode::mc_task_create(&svc, &req2).await.map_err(|e| e.msg()).unwrap();
    let p = captured.lock().unwrap().last().cloned().unwrap();
...
async fn cloud_sidebar_and_task_actions_contract() {
    ...
    let tasks = super::nemesiscode::mc_tasks(&svc, 1, 50, "pending,processing", "p/1", Some(true))
        .await
        .map_err(|e| e.msg())
        .unwrap();
    assert_eq!(tasks.pointer("/tasks/0/id").and_then(Value::as_str), Some("t1"));
    let projects = super::nemesiscode::mc_projects(&svc).await.map_err(|e| e.msg()).unwrap();
    assert_eq!(projects.pointer("/projects/0/id").and_then(Value::as_str), Some("p1"));
    // 提问索引(云端大纲):{items, next_cursor, has_more} 原样透传
    let inputs = super::nemesiscode::mc_task_user_inputs(&svc, "t1", "c1", 100).await.map_err(|e| e.msg()).unwrap();
    ...
    super::nemesiscode::mc_task_stop(&svc, "t1").await.map_err(|e| e.msg()).unwrap();
    super::nemesiscode::mc_task_delete(&svc, "t1").await.map_err(|e| e.msg()).unwrap();
...
async fn mc_usage_contract() {
    ...
    let usage = super::nemesiscode::mc_usage(&svc).await.map_err(|e| e.msg()).unwrap();
    ...
    assert!(super::nemesiscode::mc_usage(&svc).await.is_err());
}

/// 签到契约:壳内先取 NemesisCode 域 PoW 验证码,再带 captcha_token POST
/// 签到端点(与账密登录同一套验证码,全程不碰百智罐)。PoW 求解本身由
/// nemesiscode_password_login_contract 按协议校验,这里只盯签到这一跳。
#[tokio::test(flavor = "multi_thread")]
async fn mc_checkin_contract() {
    ...
    super::nemesiscode::mc_checkin(&svc).await.map_err(|e| e.msg()).unwrap();
...
async fn mc_member_models_sync_and_revoke_contract() {
    ...
    let tmp = TempDir(std::env::temp_dir().join(format!("nemesiscode-omk-{}-{nonce}", std::process::id())));
        ...
        match (req.method.as_str(), req.path.split('?').next().unwrap()) {
            ...
            ("GET", "/api/v1/users/models") => Resp::json(
                ...
                json!({ "code": 0, "data": { "models": [
                    { "id": "cfg-1", "remark": "专业模型", "model": "nemesiscode-pro-claude", "interface_type": "anthropic",
                    ...
                    { "id": "cfg-3", "remark": "旗舰", "model": "nemesiscode-ultra-x", "interface_type": "anthropic",
                      "owner": { "type": "public" } },
    ...
    // 种 mc 会话 cookie:创建/删除/模型清单都走既有 NemesisCode 会话
    svc.mc.update(
    ...
    assert_eq!(m0.get("source").and_then(Value::as_str), Some("nemesiscode"));

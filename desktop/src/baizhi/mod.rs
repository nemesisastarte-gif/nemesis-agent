// 百智云账号 + NemesisCode 云端(agent/internal/baizhi 的 Rust 移植)。
...
/// NemesisCode 官方云地址(config.rs 的 Basic Auth 作用域判定也用它)。
pub(crate) const DEFAULT_NEMESISCODE_URL: &str = "https://monkeycode-ai.com";
/// 官方云的模型请求地址:llmproxy 走独立子域,不与主服务同域。
pub(crate) const DEFAULT_NEMESISCODE_LLM_URL: &str = "https://proxy.monkeycode-ai.com/v1";
...
///
/// `nemesiscode` 传已解析的服务地址(Endpoints::resolve 之后,含环境变量覆盖):
/// 环境变量指向联调环境时按自建处理,不会误发官方代理域。
pub(crate) fn resolve_mc_llm(mc_llm_base_url: &str, nemesiscode: &str) -> String {
    let explicit = mc_llm_base_url.trim().trim_end_matches('/');
    if !explicit.is_empty() {
        return explicit.to_string();
    }
    let server = nemesiscode.trim().trim_end_matches('/');
    if server == DEFAULT_NEMESISCODE_URL {
        return DEFAULT_NEMESISCODE_LLM_URL.to_string();
    }
    format!("{server}/v1")
}

/// 百智云服务地址。模型与 MCP 服务固定走官方云;账号和 NemesisCode 地址可覆盖。
pub struct Endpoints {
    ...
    /// NemesisCode 云端(账号桥接登录 + 云端任务)
    pub nemesiscode: String,
}
...
impl Endpoints {
    ...
    pub fn resolve(mc_base_url: &str) -> Self {
        ...
        let mc_default = if mc_default.is_empty() { DEFAULT_NEMESISCODE_URL } else { mc_default };
        Self {
            ...
            nemesiscode: env_or("MC_DESKTOP_NEMESISCODE_URL", mc_default),
        }
    }
}
...
/// 百智云账号服务。cookie 分双罐:百智会话(store)与 NemesisCode 会话(mc),
/// 凭证语义不同,一方登出不牵连另一方。
pub struct Service {
    pub ep: Endpoints,
    ...
    /// 未配置)。仅对 NemesisCode 域的请求附加,见 mc_basic_header。
...
impl Service {
    ...
    pub fn test_service(ep: Endpoints) -> Self {
        ...
        let mc_llm = resolve_mc_llm("", &ep.nemesiscode);
        Self {
            ep,
            http: Some(mk(10)),
            lp: Some(mk(10)),
    ...
    pub fn new(config_dir: std::path::PathBuf, cfg: &crate::config::DesktopConfig) -> Self {
        ...
        let mc_llm = resolve_mc_llm(&cfg.mc_llm_base_url, &ep.nemesiscode);
        Self {
            ep,
            http: mk(30),
            lp: mk(40),
            store: CookieStore::new(Some(config_dir.join("baizhi-cookies.json"))),
            mc: CookieStore::new(Some(config_dir.join("nemesiscode-cookies.json"))),
    ...
    /// 测试环境反向代理的 Basic Auth 头(仅当 url 落在 NemesisCode 域时返回;
    ...
    pub(crate) fn mc_basic_header(&self, url: &reqwest::Url) -> Option<&str> {
        ...
        let mc = reqwest::Url::parse(&self.ep.nemesiscode).ok()?;
    ...
    /// NemesisCode 域必须传 mc 罐,用百智罐会把 mc 域 cookie 混进百智罐,
    /// 破坏双罐隔离。
    async fn raw_at(
        &self,
        store: &CookieStore,
    ...
    /// NemesisCode 服务端用同一套 go-cap 协议(challenge 201 裸结构 → 本地
...
/// 包壳解包策略。四个后端(百智云账号域/模型网关/MCP 网关/NemesisCode)的
...
pub(crate) struct Envelope {
    ...
    /// (NemesisCode 链路的 401 恢复动作是"到设置中重新连接"——桥接或账密皆可)
...
pub async fn mc_status(bz: State<'_, BaizhiState>) -> Result<Value, String> {
    let (logged_in, user) = nemesiscode::mc_status(&bz.0).await.map_err(BzErr::msg)?;
    let mut resp = json!({ "logged_in": logged_in, "host": nemesiscode::mc_host(&bz.0) });
    if !user.is_null() {
        resp["user"] = user;
    }
    Ok(resp)
...
pub async fn mc_login(bz: State<'_, BaizhiState>) -> Result<Value, String> {
    let user = nemesiscode::login_nemesiscode(&bz.0).await.map_err(BzErr::msg)?;
...
/// NemesisCode 账号密码直连登录(不经百智云;壳内自动完成 PoW 验证码)。
...
pub async fn mc_password_login(bz: State<'_, BaizhiState>, email: String, password: String) -> Result<Value, String> {
    ...
    let user = nemesiscode::login_nemesiscode_password(&bz.0, email, &password)
        .await
        .map_err(BzErr::msg)?;
    let mut resp = json!({ "ok": true });
    if !user.is_null() {
...
pub async fn mc_usage(bz: State<'_, BaizhiState>) -> Result<Value, String> {
    nemesiscode::mc_usage(&bz.0).await.map_err(BzErr::msg)
}

/// 每日签到(壳内自动完成 PoW 验证码)。成功后 UI 重拉 mc_usage 刷新余额。
#[tauri::command]
pub async fn mc_checkin(bz: State<'_, BaizhiState>) -> Result<Value, String> {
    nemesiscode::mc_checkin(&bz.0).await.map_err(BzErr::msg)?;
    Ok(json!({ "ok": true }))
}

// ==================== 会员模型本地同步 ====================

pub(crate) const OHMYAGENT_KEY_FILE: &str = "nemesiscode-ohmyagent-key.json";
...
async fn ensure_ohmyagent_key(svc: &Service, cfg_dir: &std::path::Path) -> BzResult<Value> {
    let server = svc.ep.nemesiscode.as_str();
    ...
    let mut k = nemesiscode::mc_ohmyagent_key_create(svc).await?;
    k["server"] = json!(server);
    k["base_url"] = json!(llm);
    persist(&k)?;
    Ok(k)
...
pub(crate) async fn sync_member_models(svc: &Service, cfg_dir: &std::path::Path) -> BzResult<Value> {
    ...
    nemesiscode::mc_member_models_sync(svc).await
}
...
pub(crate) async fn revoke_member_models(svc: &Service, cfg_dir: &std::path::Path) -> BzResult<()> {
    ...
    nemesiscode::mc_ohmyagent_key_delete(svc, id).await?;
    let _ = std::fs::remove_file(cfg_dir.join(OHMYAGENT_KEY_FILE));
    Ok(())
}

/// 同步 NemesisCode 会员内置模型为本地条目(source="nemesiscode")。与
...
/// 断开 NemesisCode 账号时调用(从未同步过直接成功)。须在清除 mc 会话
/// 之前调用——请求走 mc 会话认证。
#[tauri::command]
...
pub async fn mc_projects(bz: State<'_, BaizhiState>) -> Result<Value, String> {
    nemesiscode::mc_projects(&bz.0).await.map_err(BzErr::msg)
}

#[tauri::command]
pub async fn mc_task_info(bz: State<'_, BaizhiState>, id: String) -> Result<Value, String> {
    nemesiscode::mc_task_info(&bz.0, &id).await.map_err(BzErr::msg)
}
...
pub async fn mc_task_rounds(
    ...
    nemesiscode::mc_task_rounds(&bz.0, &id, cursor.as_deref().unwrap_or(""), limit)
        .await
        .map_err(BzErr::msg)
}
...
pub async fn mc_task_user_inputs(
    ...
    nemesiscode::mc_task_user_inputs(&bz.0, &id, cursor.as_deref().unwrap_or(""), limit)
        .await
        .map_err(BzErr::msg)
}
...
pub async fn mc_upload(bz: State<'_, BaizhiState>, filename: String, data: String) -> Result<Value, String> {
    ...
    let access_url = nemesiscode::mc_upload(&bz.0, &filename, bytes).await.map_err(BzErr::msg)?;
...
pub async fn mc_terminal_list(bz: State<'_, BaizhiState>, vm_id: String) -> Result<Value, String> {
    nemesiscode::mc_terminal_list(&bz.0, &vm_id).await.map_err(BzErr::msg)
}
...
pub async fn mc_file_download(
    app: tauri::AppHandle,
    bz: State<'_, BaizhiState>,
    ctl: State<'_, nemesiscode::DownloadCtl>,
    dl_id: String,
    vm_id: String,
    path: String,
    filename: String,
    ...
    let bytes = nemesiscode::mc_file_download(&app, &ctl, &bz.0, &dl_id, &vm_id, &path, &filename, &dest)
        .await
        .map_err(BzErr::msg)?;
...
pub async fn mc_file_upload(
    ...
    nemesiscode::mc_file_upload(&bz.0, &vm_id, &path, bytes).await.map_err(BzErr::msg)?;
    Ok(json!({ "ok": true }))
}

...
/// 旧版 ~/.nemesiscode/chat-workspace 不匹配，删除旧对话不会误删共享历史。
...
mod source_suffix_tests {
    ...
    fn strips_source_marker_with_optional_config_id() {
        assert_eq!(strip_source_suffix("深度求索@nemesiscode#cfg-9"), "深度求索");
        assert_eq!(strip_source_suffix("深度求索@nemesiscode"), "深度求索");
        ...
        assert_eq!(strip_source_suffix("x@nemesiscode-plus"), "x@nemesiscode-plus");
...
mod chat_workdir_tests {
    ...
    fn test_root(label: &str) -> PathBuf {
        ...
        let suffix = random.iter().map(|b| format!("{b:02x}")).collect::<String>();
        std::env::temp_dir()
            .join(format!("nemesiscode-chat-workdir-{label}-{suffix}"))
    ...
    fn host_view_of_a_chat_workdir_under_wsl_is_an_unusable_unc_path() {
        let guest = "/mnt/c/Users/phxa1/AppData/Local/com.nemesiscode.app\

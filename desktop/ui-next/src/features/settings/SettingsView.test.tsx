...
describe("同步自动保存(旧 UI autoSaveDecision 随迁)", () => {
  ...
  const syncExtra = () => ({
    ...
    mc_models_sync: () => ({
      models: [{ name: "member-m", base_url: "https://m", api_key: "k", model: "mm", source: "nemesiscode" }],
    }),
  });
  ...
  it("保存在途时第二路同步落地:补存一轮把它写进去,不留给用户手点", async () => {
    ...
    const model = (name: string) => ({ name, base_url: "https://m", api_key: "k", model: name, source: "nemesiscode" });
    let round = 0;
    const { calls } = stubShell({
      extra: {
        mc_status: () => ({ logged_in: true, user: { name: "李四" } }),

...
import { SOURCE_BAIZHI, SOURCE_NEMESISCODE } from "@/lib/models/modelMenu";
import {
  buildPayload,
  draftFromConfig,
  mergeSyncedMcps,
 ...
 * 品牌主题(nemesiscode/nemesiscode-dark)恒居列表头两位(THEMES 序)。 */
function ThemePicker({ theme, custom, onPick }: { theme: Theme; custom: CustomTheme; onPick: (v: Theme) => void }) {
  ...
  // 名称一律用主题原名(含品牌两项 nemesiscode/nemesiscode-dark,用户定案
...
export function SettingsView({
  ...
  const applySync = (r: BaizhiSyncResult | McModelsSyncResult): SyncApplied | undefined => {
    ...
    const source = r.models[0]?.source || (fromBaizhi ? SOURCE_BAIZHI : SOURCE_NEMESISCODE);
    let skipped: string[] = [];
  ...
  /** 断开 NemesisCode 后清掉会员模型组(旧 UI disconnectMcWithCleanup 第四步)。
  ...
  const applyMcDisconnect = (): SyncApplied | undefined => {
    ...
    const next = removeSyncedSource(cur, SOURCE_NEMESISCODE);
   ...
   * 可能又被并入新的同步结果——扫码登录顺带桥接 NemesisCode,百智云与会员

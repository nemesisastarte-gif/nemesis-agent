...
import { memberTierRank, modelSourceRank, sameModelName, stripSourceSuffix, SOURCE_BAIZHI, SOURCE_NEMESISCODE } from "@/lib/models/modelMenu";
 ...
 * 就是静默失效的。同名条目若是 source=nemesiscode 的会员行,UI 里连删除入口
...
const SOURCE_SUFFIX: Record<string, string> = {
  ...
  [SOURCE_NEMESISCODE]: `@${SOURCE_NEMESISCODE}`,
};
...
export function syncedName(name: string, source?: string, id?: string): string {
  ...
  const entryId = source === SOURCE_NEMESISCODE ? (id?.trim() ?? "") : "";
...
const autoDefaultRank = (m: HostModel): number =>
  (m.source === SOURCE_NEMESISCODE ? 10 : 0) + memberTierRank(m.model);
...
export function mergeSyncedModels(
  ...
  // 明确选择,不该被一次登录同步改掉
  const memberClaims = source === SOURCE_NEMESISCODE && !!cur?.source && cur.source !== SOURCE_NEMESISCODE;

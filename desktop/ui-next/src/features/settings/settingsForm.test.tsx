...
describe("草稿 ⇄ 载荷(全量写回)", () => {
  ...
  it("draftFromConfig 载入即收敛同名:校验放行(否则保存被 modelDup 永久拦死)", () => {
    const base = cfg({
      models: [
        ...
        model({ name: "Claude", model: "new", source: "nemesiscode", locked: true }),
      ],
    });
    const draft = draftFromConfig(base);
    expect(draft.models).toHaveLength(1);
    expect(draft.models[0]).toMatchObject({ name: "Claude", model: "new", source: "nemesiscode" });
  ...
  it("锁定/同步标记(source/locked/owner)与高级字段随保存透传", () => {
    const base = cfg({
      models: [model({ source: "nemesiscode", locked: true, owner: "team", context_window: 128000, think: "high" })],
    });
...
describe("同步并入(mergeSyncedModels / mergeSyncedMcps)", () => {
  ...
  it("落盘名加来源后缀(会员条目再缀配置 id);整组替换并按来源排序", () => {
    ...
    const r = mergeSyncedModels(
      ...
      [
        synced({ name: "glm-5" }),
        synced({ name: "旗舰", source: "nemesiscode", id: "c1" }),
      ],
      "baizhi",
    );
    // 本次按 baizhi 组并入:nemesiscode 条目也带后缀落名(以自身 source 计)
  ...
  it("跨组撞名先到先得:后来者跳过并回报展示名", () => {
    const d = draft([model({ name: "glm-5@baizhi", source: "nemesiscode" })]);
    ...
    expect(r!.draft.models[0]?.source).toBe("nemesiscode");
  });
  ...
  it("首次同步无默认模型:落会员可用最高档位的第一条(用户定案 2026-08-06)", () => {
    ...
    const r = mergeSyncedModels(
      draft([]),
      [
        synced({ name: "b1", source: "nemesiscode", id: "1", model: "nemesiscode-basic/m1" }),
        synced({ name: "b2", source: "nemesiscode", id: "2", model: "nemesiscode-basic/m2" }),
        synced({ name: "p1", source: "nemesiscode", id: "3", model: "nemesiscode-pro/m3" }),
        synced({ name: "u1", source: "nemesiscode", id: "4", model: "nemesiscode-ultra/m4", locked: true }),
      ],
      "nemesiscode",
    );
    const next = r!.draft;
    // 锁定的 ultra 不可当默认;可用最高档 = pro,取其第一条(不是列表首条 basic)
    expect(next.models[next.defaultIdx]?.name).toBe("p1@nemesiscode#3");
  });
  ...
  it("会员组同步接管默认:百智云先落地占的默认让位给会员最高档(扫码登录两路同步)", () => {
    ...
    const r = mergeSyncedModels(
      ...
      [
        synced({ name: "b1", source: "nemesiscode", id: "1", model: "nemesiscode-basic/m1" }),
        synced({ name: "p1", source: "nemesiscode", id: "2", model: "nemesiscode-pro/m3" }),
        synced({ name: "u1", source: "nemesiscode", id: "3", model: "nemesiscode-ultra/m4", locked: true }),
      ],
      "nemesiscode",
    );
    const next = r!.draft;
    expect(next.models[next.defaultIdx]?.name).toBe("p1@nemesiscode#2");
  });
  ...
  it("会员组同步不抢手工条目的默认(用户自己配的是明确选择);已是会员默认也不动", () => {
    ...
    const r1 = mergeSyncedModels(
      custom,
      [synced({ name: "u1", source: "nemesiscode", id: "1", model: "nemesiscode-ultra/m" })],
      "nemesiscode",
    );
    expect(r1!.draft.models[r1!.draft.defaultIdx]?.name).toBe("手工");

    // 默认已是会员基础档:重同步不擅自跳到 ultra(用户可能就想用便宜的)
    const onMember = draft([model({ name: "b1@nemesiscode#1", source: "nemesiscode", model: "nemesiscode-basic/m1" })], 0);
    const r2 = mergeSyncedModels(
      onMember,
      [
        synced({ name: "b1", source: "nemesiscode", id: "1", model: "nemesiscode-basic/m1" }),
        synced({ name: "u1", source: "nemesiscode", id: "2", model: "nemesiscode-ultra/m4" }),
      ],
      "nemesiscode",
    );
    expect(r2!.draft.models[r2!.draft.defaultIdx]?.name).toBe("b1@nemesiscode#1");
  });

  it("百智云组同步不接管默认(接管只在会员组落地时发生)", () => {
    const onMember = draft([model({ name: "u1@nemesiscode#1", source: "nemesiscode", model: "nemesiscode-ultra/m" })], 0);
    const r = mergeSyncedModels(onMember, [synced({ name: "glm-5" })], "baizhi");
    expect(r!.draft.models[r!.draft.defaultIdx]?.name).toBe("u1@nemesiscode#1");
  });

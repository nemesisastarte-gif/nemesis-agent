...
describe("useComposer:附件上传的纪元守卫", () => {
  it("上传落地时人已切走:附件归原会话留档,不落进当前 composer", async () => {
    ...
    await act(async () => {
      finish({ path: ".nemesiscode/uploads/图.png" });
      await done;
    });
    // 落进当前 composer 的话,path 是**旧工作区**的相对路径,发出去读不到
    expect(result.current.atts).toHaveLength(0);
    expect(stashGet("a")?.atts.map((a) => a.path)).toEqual([".nemesiscode/uploads/图.png"]);

    rerender({ id: "a" });
    expect(result.current.atts.map((a) => a.path)).toEqual([".nemesiscode/uploads/图.png"]);
  });
});

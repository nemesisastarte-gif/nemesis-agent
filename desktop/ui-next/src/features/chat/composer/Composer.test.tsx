  ...
  (window as unknown as { __TAURI__?: unknown }).__TAURI__ = {
    core: {
      invoke: (cmd: string, args?: Record<string, unknown>) => {
        ...
        if (cmd === "upload_finish") return Promise.resolve({ path: ".nemesiscode/uploads/shot.png" });
...
describe("模型 / 思考深度 / 权限模式", () => {
  ...
  it("≥2 来源出 tabs(会员→百智云→自定义序,默认跟随当前模型来源);切 tab 换列表,选中发原名", async () => {
    const SOURCED: ModelInfo[] = [
      ...
      { name: "nemesiscode-basic/glm@nemesiscode#c1", model: "nemesiscode-basic/glm", source: "nemesiscode", owner: "public", default: false },
    ];
  ...
  it("会员 tab 分节:节头 + 资格徽标;locked 条目灰态禁选(title 说明);选中发原名", async () => {
    const MEMBER: ModelInfo[] = [
      { name: "m", default: true },
      { name: "nemesiscode-basic/glm@nemesiscode#c1", model: "nemesiscode-basic/glm", source: "nemesiscode", owner: "public", default: false },
      { name: "nemesiscode-ultra/claude@nemesiscode#c2", model: "nemesiscode-ultra/claude", source: "nemesiscode", owner: "public", locked: true, default: false },
      { name: "团队甲", model: "team-x", source: "nemesiscode", owner: "team", default: false },
    ];
    ...
    expect(calls(ops, "session_set_model").map((o) => o.args?.payload)).toEqual([
      { model: "nemesiscode-basic/glm@nemesiscode#c1" },
    ]);
  });

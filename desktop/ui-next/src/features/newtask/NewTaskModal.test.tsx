...
describe("新建任务", () => {
  it("默认本地模式:目录预填 ~/NemesisCode,模型取默认且锁定项禁选", async () => {
    stubShell();
    ...
    expect(input.value).toBe("~/NemesisCode");
  ...
  it("本地 + 默认目录:createDir=true、think 缺省空串;创建成功回调并记忆模型", async () => {
    ...
    expect(create?.args).toEqual({ workdir: "~/NemesisCode", model: "gpt-5", createDir: true, kind: "local", think: "" });
  ...
  // ~/NemesisCode,`~` 交给壳按内核环境展开(WSL → guest 家目录)。前端拼 UNC
  // 落点完全一样,却要等引擎起来才拿得到基座、拿不到时又退回 ~/NemesisCode,
  // 同一个"默认目录"两种形态;旧 UI 从头到尾就是 ~/NemesisCode。
  it("WSL 运行环境:默认目录仍是 ~/NemesisCode(不拼 UNC),且享受静默创建", async () => {
    const calls = stubShell({
    ...
    render(<NewTaskModal open onClose={() => {}} onCreated={onCreated} />);
    const input = await openDirMenu();
    await waitFor(() => expect(input.value).toBe("~/NemesisCode"));
    await userEvent.click(screen.getByRole("button", { name: "创建" }));
    await waitFor(() => expect(onCreated).toHaveBeenCalled());
...
describe("首条消息附件", () => {
  it("粘贴文件进暂存 chips;创建后先上传再把附件行并进首条消息", async () => {
    ...
    const calls = stubShell({
      ...
      upload_finish: () => Promise.resolve({ path: ".nemesiscode/uploads/shot.png" }),
    });
    render(<NewTaskModal open onClose={() => {}} onCreated={() => {}} />);
    ...
    expect((sent?.args?.payload as { content: string }).content).toBe(
      b64encode("看这张图\n[图片] .nemesiscode/uploads/shot.png"),
    );
  });

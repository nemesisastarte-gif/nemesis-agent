...
// - 本地会话:workdir 必填;默认目录(本机 ~/NemesisCode / WSL 家目录基座下的
//   NemesisCode)允许静默创建(createDir);其他目录不存在时壳报「…目录不存在…」
...
export function NewTaskModal({
  ...
  /** 云端页签未连接 NemesisCode 时的出口(空态里的「去设置连接」)。不传的话
  ...
  useEffect(() => {
    ...
    // WSL 下就是 guest 家目录下的 NemesisCode,见 lib/util/workdir 头注)
    void (async () => {

...
describe("账号分区:门与登录面板", () => {
  ...
  it("扫码 ok:刷新登录态、顺带桥接 NemesisCode,且两路自动同步(登录即同步,不用手点)", async () => {
    let statusCalls = 0;
    ...
    const { calls } = stubShell({
      ...
      mc_models_sync: () => ({ models: [{ name: "m", base_url: "https://m", api_key: "k", model: "m", source: "nemesiscode" }] }),
    });
    render(<AccountSection />);
...
describe("短信验证码登录", () => {
  ...
  it("登录成功:baizhi_login 携带手机号与验证码,并顺带桥接 NemesisCode", async () => {
    let loggedIn = false;
    let mcConnected = false;
    const { calls } = stubShell({
      ...smsHandlers(),
...
describe("NemesisCode 账号密码登录入口", () => {
  it("全未登录:NemesisCode 组照出,入口归本卡;不摆「连接」死钮(桥接要百智云会话)", async () => {
    stubShell({ baizhi_status: bzOut, mc_status: mcOut, baizhi_wechat_start: never });
    render(<AccountSection />);
    expect(await screen.findByText("NemesisCode 云端")).toBeDefined();
    expect(screen.queryByRole("button", { name: "连接 NemesisCode 云端" })).toBeNull();
    // 展开 → 收起:表单在 NemesisCode 卡内,不再挂在百智云登录卡下方
    await userEvent.click(screen.getByRole("button", { name: "使用 NemesisCode 账号密码登录" }));
  ...
  it("百智云已登录、MC 未连:出「连接」主钮,账密入口仍在同一张卡", async () => {
    ...
    render(<AccountSection />);
    expect(await screen.findByRole("button", { name: "连接 NemesisCode 云端" })).toBeDefined();
    expect(screen.getByRole("button", { name: "使用 NemesisCode 账号密码登录" })).toBeDefined();
  });
  ...
  it("空提交拦截;正确提交 mc_password_login 原样携带 email/password", async () => {
    ...
    render(<AccountSection />);
    await userEvent.click(await screen.findByRole("button", { name: "使用 NemesisCode 账号密码登录" }));
    ...
    expect(screen.queryByRole("button", { name: "使用 NemesisCode 账号密码登录" })).toBeNull();
...
describe("自建部署配置(彩蛋解锁)", () => {
  it("默认隐藏;连点 NemesisCode 卡图标 6 次解锁并记住", async () => {
    ...
    await screen.findByText("NemesisCode 云端");

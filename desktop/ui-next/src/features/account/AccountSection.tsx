...
// NemesisCode 云账号(桥接/账密登录、用量/签到/邀请、会员模型同步)。
//
// 状态语义:
// - 两路登录态挂载时并发自取(baizhi_status / mc_status),不进全局轮询;
// - 百智云登录成功顺带桥接 NemesisCode(mc_login 走同一账号的 OAuth),
//   桥接失败不阻断——NemesisCode 卡保留手动「连接」入口;
// - 断开 NemesisCode 必须先吊销会员模型密钥再清会话(disconnectMc 收口,
//   顺序由 lib 与本组件测试双重钉住);
...
function BaizhiCard({
  ...
  const sync = async () => {
    ...
    try {
      ...
      const keyLine = r.key_created ? ` ${t("account.baizhi.keyCreated", { name: r.key_name || "NemesisCode" })}` : "";
      setMsg({
        text:
...
/** NemesisCode 云账号卡:未连=连接入口;已连=账号信息 + 用量面板 +
...
function McCard({
   ...
   *  NemesisCode 会话,未登录时点它必失败,不摆这个死钮) */
  ...
  const connect = async () => {
    ...
    try {
      ...
      // 已删掉本机 nemesiscode-ohmyagent-key.json,而**重建它的唯一路径**是
    ...
    return (
      ...
      >
        ...
        {/* 账密登录:不经百智云的手动路径(桥接失败/私有化/换账号)。入口与
            表单都在本卡内——它登的是 NemesisCode 账号,挂在百智云登录卡下方
...
/** 自建/私有化部署地址(账号分区末尾的高级块)。默认隐藏:连点 NemesisCode
...
export function AccountSection({
  ...
  /** 断开 NemesisCode 成功后清掉会员模型组(SettingsView.applyMcDisconnect);
   ...
   *  NemesisCode(同一账号的 OAuth,登录一次两边都通),桥接成功顺带起
   *  会员模型同步。 */
  const onBaizhiLoggedIn = useCallback(async () => {
    ...
    // (B 账号)只是想拿模型与 MCP —— 无条件桥接会拿 B 的会话对 NemesisCode
    // 重走一遍 OAuth(壳侧 login_nemesiscode 会带着现有 cookie 罐走完整条链),
    ...
    <section aria-label={t("settings.nav.account")} className="flex max-w-xl flex-col gap-3">
      ...
      {loaded && (
        <>
          ...
          {/* NemesisCode 组恒在(2026-08-06 用户定案):两个账号 = 两块,
              NemesisCode 的连接/账密登录入口都在本块内。未登录时卡里不摆

...
describe("NewCloudTask", () => {
  ...
  it("未连接 NemesisCode:不拉选项,给「请先连接」空态与去设置入口", async () => {
    const calls: string[] = [];
    (window as unknown as { __TAURI__?: unknown }).__TAURI__ = {
      core: {
        invoke: (cmd: string) => {
    ...
    expect(await screen.findByText("请先连接 NemesisCode")).toBeTruthy();
  ...
  it("mc_status 自己抛错:不敢断言未连接,原因照常外显(不把一切故障粉饰成未连接)", async () => {
    ...
    expect(screen.queryByText("请先连接 NemesisCode")).toBeNull();
  });

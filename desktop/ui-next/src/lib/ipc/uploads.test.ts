    ...
    __TAURI__: {
      core: {
        invoke: (cmd: string, args?: Record<string, unknown>) => {
          ...
          if (cmd === "upload_finish") return Promise.resolve({ path: ".nemesiscode/uploads/a.bin" });
...
describe("uploadFileStream 分块边界", () => {
  it("10 字节按 4 字节分块 → 3 块字节精确重组;begin/finish 契约与进度回调", async () => {
    ...
    expect(r).toEqual({ path: ".nemesiscode/uploads/a.bin" });
...
describe("路径直拷与对话框选择", () => {
  it("uploadFilePath:{id, src} 契约透传", async () => {
    const calls = stubShell((cmd) => (cmd === "upload_file_path" ? { path: ".nemesiscode/uploads/数据.csv" } : undefined));
    const r = await uploadFilePath("s1", "/home/u/数据.csv");
    expect(r.path).toBe(".nemesiscode/uploads/数据.csv");
  ...
  it("uploadFileURL:{id, path} 契约透传,返回 data URL 原样", async () => {
    ...
    const r = await uploadFileURL("s1", ".nemesiscode/uploads/截图.png");
    expect(r).toBe("data:image/png;base64,AAA");
    expect(calls[0]).toEqual({ cmd: "upload_read", args: { id: "s1", path: ".nemesiscode/uploads/截图.png" } });
  });
...
describe("path-backed File 与图片判定", () => {
  ...
  it("isImagePath 与壳侧 image_mime 同口径", () => {
    expect(isImagePath(".nemesiscode/uploads/a.PNG")).toBe(true);

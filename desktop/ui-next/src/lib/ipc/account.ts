// 账号域 IPC:百智云登录(短信/微信扫码/登出/同步)与 NemesisCode 云账号
// (桥接登录/账密登录/用量/签到/会员模型同步)。契约对表壳侧
// desktop/src/baizhi/{mod.rs,nemesiscode.rs}:类型字段名 = 壳响应实形。
//
...
export interface BaizhiSyncedModel {
  ...
  /** nemesiscode 同步:服务端模型配置 id(区分同批重名) */
  id?: string;
  /** nemesiscode 同步:超会员档,展示禁选 */
  locked?: boolean;
  owner?: string;
}
...
/** mc_models_sync 返回:会员内置模型 → 本地条目(source="nemesiscode")。 */
...
/** 断开 NemesisCode:先吊销会员模型密钥、再清会话,顺序不可倒置(见

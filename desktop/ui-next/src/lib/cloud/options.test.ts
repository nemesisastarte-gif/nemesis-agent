...
const models: McCloudModel[] = [
  { id: "m-basic", model: "nemesiscode-basic-x", owner: { type: "public" }, weight: 1 },
  { id: "m-pro", model: "nemesiscode-pro-x", owner: { type: "public" }, weight: 5 },
  { id: "m-ultra", model: "nemesiscode-ultra-x", owner: { type: "public" }, weight: 9 },
  { id: "m-mine", model: "my-model", owner: { type: "private" } },
  { id: "", model: "no-id" }, // 无 id:剔除
  { id: "m-meta", model: "nemesiscode-pro" }, // 裸档位占位:剔除
...
describe("usableCloudModels / pickDefaultCloudModel", () => {
  ...
  it("展示名:remark 优先,内置档位翻译", () => {
    expect(cloudModelLabel({ model: "nemesiscode-pro-x", remark: "" })).toContain("专业模型");
...
describe("usableCloudHosts / pickDefaultCloudHost", () => {
  const hosts: McCloudHost[] = [
    ...
    { id: PUBLIC_CLOUD_HOST_ID, name: "NemesisCode", status: "online" },
  ];

...
function translateBuiltinNames(text: string): string {
  return text
    .replace(/nemesiscode-ultra/gi, t("cloud.model.tier.ultra"))
    .replace(/nemesiscode-pro/gi, t("cloud.model.tier.pro"))
    .replace(/nemesiscode-basic/gi, t("cloud.model.tier.basic"))
    .replace(/\s*\/\s*/g, " / ");
}
...
function planAllowsModel(model: McCloudModel, plan?: string): boolean {
  ...
  if (b === "nemesiscode-pro") return plan === "pro" || plan === "flagship" || plan === "ultra";
  if (b === "nemesiscode-ultra") return plan === "flagship" || plan === "ultra";
  return true;
}
...
export function groupCloudModels(models: McCloudModel[], plan?: string): McCloudModelGroup[] {
  ...
  const builtin: McCloudModelGroup[] = (
    [
      { key: "nemesiscode-basic", label: t("cloud.model.tier.basic"), badge: t("cloud.badge.free") },
      { key: "nemesiscode-pro", label: t("cloud.model.tier.pro"), badge: t("cloud.badge.proFree") },
      { key: "nemesiscode-ultra", label: t("cloud.model.tier.ultra"), badge: t("cloud.badge.ultraFree") },
    ] as const
  ).map((group) => ({
    ...group,
...
export function pickDefaultCloudModel(models: McCloudModel[], plan?: string): string {
  ...
  const planBuiltin =
    plan === "pro" ? "nemesiscode-pro" : plan === "flagship" || plan === "ultra" ? "nemesiscode-ultra" : "nemesiscode-basic";
...
export function usableCloudHosts(hosts: McCloudHost[], publicModel = false): McCloudHost[] {
  ...
  const publicHost: McCloudHost = {
    ...
    name: remotePublic?.name || "NemesisCode",
    status: "online",

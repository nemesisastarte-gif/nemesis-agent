...
/** 同步条目来源标记(壳侧 baizhi/nemesiscode.rs 的 SOURCE_* 同值)。 */
export const SOURCE_BAIZHI = "baizhi";
export const SOURCE_NEMESISCODE = "nemesiscode";
...
export function builtinTierLabel(model?: string): string | undefined {
  ...
  if (n.startsWith("nemesiscode-basic")) return t("model.tier.basic");
  if (n.startsWith("nemesiscode-pro")) return t("model.tier.pro");
  if (n.startsWith("nemesiscode-ultra")) return t("model.tier.ultra");
  return undefined;
}
...
export function memberTierRank(model?: string): number {
  const n = (model || "").toLowerCase();
  if (n.startsWith("nemesiscode-ultra")) return 3;
  if (n.startsWith("nemesiscode-pro")) return 2;
  if (n.startsWith("nemesiscode-basic")) return 1;
  return 0;
}

/** 剥会员档位前缀(nemesiscode-xxx/)得短名,剥空回落原名。Web
 * stripBuiltinPublicModelPackagePrefix 同款正则;remark 里写了前缀也一并剥。 */
export function stripTierPrefix(name: string): string {
  return name.replace(/^nemesiscode-[^/]+\//i, "") || name;
}
...
// 展示同款口径,几处必须一致)
const SOURCE_SUFFIX_RE = new RegExp(`@(?:${SOURCE_BAIZHI}|${SOURCE_NEMESISCODE})(?:#.*)?$`, "i");
 ...
 * 只对会员来源生效(手工条目取名 nemesiscode-pro-x 不该被误打会员档);
...
export function modelDisplay(m: Pick<ModelInfo, "name" | "model" | "source">): {
  ...
  if (m.source !== SOURCE_NEMESISCODE) return { label: short };
 ...
 * (剥来源后缀),都没中原样返回。
 *
 * 为什么必须有这一步:`@nemesiscode#<配置 id>` 后缀是后加的,升级后第一次同步
...
export const modelSourceRank = (source?: string): number =>
  source === SOURCE_NEMESISCODE ? 0 : source === SOURCE_BAIZHI ? 1 : source ? 2 : 3;
...
export function filterModels(items: ModelInfo[], filter: string): ModelInfo[] {
  ...
  if (!q) return items;
  // 按展示名匹配:落盘名尾巴上的 @来源#id 是寻址用的,搜「nemesiscode」
  // 不该把整组会员模型都捞出来
  return items.filter(

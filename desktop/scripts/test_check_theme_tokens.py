...
def index_html(light: str = LIGHT_BG, dark: str = DARK_BG, dark_selector: str | None = None) -> str:
    selector = dark_selector if dark_selector is not None else (
        ...
        '      html[data-theme="nemesiscode-dark"], html[data-theme="nemesiscode-dark"] body'
    )
...
<html>
  <head>
    ...
    <script>
      try {{
        document.documentElement.dataset.theme = localStorage.getItem("mc.theme") || "nemesiscode";
...
class ThemeBlockParsingTest(unittest.TestCase):
    ...
    def test_blocks_and_flags(self) -> None:
        ...
        self.assertEqual([b["name"] for b in blocks], ['"nemesiscode"', '"nemesiscode-dark"'])
        self.assertEqual(prefersdark_name(app_css()), "nemesiscode-dark")

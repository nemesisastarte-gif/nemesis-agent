...
if platform == "macos":
    ...
    src = bundle / "macos/NemesisCode.app.tar.gz"
    name = f"NemesisCode_{short}_universal.app.tar.gz"
    manifest_name = "latest.json"
    # universal 包同时服务两种架构,两个 target key 指向同一 URL
    targets = ["darwin-aarch64", "darwin-x86_64"]
elif platform == "windows":
    bundle = ROOT / "target/release/bundle"
    src = bundle / f"nsis/NemesisCode_{version}_x64-setup.exe"
    name = f"NemesisCode_{short}_x64-setup.exe"
    manifest_name = "latest-windows.json"
    targets = ["windows-x86_64"]
elif platform == "linux":
    bundle = ROOT / "target/release/bundle"
    # AppImage 文件名用 Tauri 的 x86_64 命名(非 deb 的 amd64)
    src = bundle / f"appimage/NemesisCode_{version}_amd64.AppImage"
    name = f"NemesisCode_{short}_amd64.AppImage"

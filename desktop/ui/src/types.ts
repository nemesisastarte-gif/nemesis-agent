...
/** NemesisCode 会员模型同步条目的 source 值(壳侧赋值在
 * desktop/src/baizhi/nemesiscode.rs 的 SOURCE_NEMESISCODE,两侧改动需同步)。 */
...
export function modelSourceLabel(source?: string): string {
  ...
  if (source === SOURCE_NEMESISCODE) return "NemesisCode 会员";
  return source;
}
...
export interface ModelInfo {
  ...
  source?: string;
  /** 底层模型串。name 可能是 remark 别名,判会员档位(nemesiscode-…)靠它 */
  model?: string;
...
export interface HostConfig {
  ...
  kernel_env?: string;
  /** NemesisCode 服务地址(自建/私有化部署;空/缺省 = 官方云)。环境变量
   * MC_DESKTOP_NEMESISCODE_URL 优先;修改保存后需重启应用生效 */
  mc_base_url?: string;
  /** NemesisCode 测试环境反向代理的 HTTP Basic Auth("user:pass",空 = 无;
   * 对齐 mobile 的 mc.basicAuth)。仅 NemesisCode 域的请求附头;重启应用生效 */
...
export interface BaizhiSyncedModel {
  ...
  context_window?: number;
  max_output?: number;
  /** 思考深度档;nemesiscode 同步在服务端标注不支持思考时下发 "off"
   * (压掉产品默认「低」,免得首个请求带思考参数被上游拒) */
  think?: string;
  vision?: boolean;
  source: string; // "baizhi" | "nemesiscode"
  /** nemesiscode 同步:服务端模型配置 id。落盘名靠它区分同批重名的条目
   * (见 settingsConfig.syncedName),不进 HostModel、不落 config.json */
  id?: string;
  /** nemesiscode 同步:超出会员档的条目(展示禁选,升级后重同步解锁) */
  locked?: boolean;
  /** nemesiscode 同步:服务端归属(public/private/team) */
  owner?: string;
}
...
export interface BaizhiSyncResult {
  ...
  key_name?: string; // 使用的密钥在网关里的名字(撞名时是 NemesisCode-N)
  notes?: string[];
}

/** mc_models_sync 返回:会员内置模型 → 本地条目(已带 source="nemesiscode",
 * 条目 model 字段是服务端模型名)。 */

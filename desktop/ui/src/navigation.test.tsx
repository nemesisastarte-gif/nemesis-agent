...
describe("侧栏新建任务入口", () => {
  ...
  it("云端按快速任务、历史任务和项目分组，并与本地会话使用同款单行", () => {
    ...
    const html = renderToStaticMarkup(
      <Sidebar
        ...
        cloudProjects={[{
          id: "project-1",
          name: "NemesisCode",
          repo_url: "https://github.com/example/NemesisCode",
    ...
    expect(html).toContain("NemesisCode");
  ...
  it("云端入口的预填会直接打开云端模式", () => {
    ...
    expect(html).toContain("云端任务需要先连接 NemesisCode");
    expect(html).toContain('title="请先连接 NemesisCode 后再创建云端任务"');
  ...
  it("从云端项目新建任务时会直接预选该项目", () => {
    const html = renderToStaticMarkup(
      <NewTaskView
        ...
        lastDir=""
        recentDirs={[]}
        prefill={{ mode: "cloud", cloudProject: { id: "p1", name: "NemesisCode", repo_url: "https://github.com/example/NemesisCode" } }}
        cloudReady={false}
        onCreated={() => {}}
        onCloudCreated={() => {}}
      />,
    ...
    expect(html).toContain("NemesisCode");
    expect(html).toContain(">NemesisCode</span>");

...
describe("ModelMenuItem", () => {
  ...
  it("会员条目:tag 档位药丸 + title 用完整原名覆盖短名", () => {
    const html = renderToStaticMarkup(
      <ModelMenuItem label="deepseek-pro" tag="专业" title="nemesiscode-pro/deepseek-pro" selected={false} onClick={vi.fn()} />,
    );

    expect(html).toContain("专业");
    expect(html).toContain('title="nemesiscode-pro/deepseek-pro"');
...
describe("ModelPicker 关闭态(静态渲染,不触 localStorage)", () => {
  ...
  it("会员长名:trigger 显示剥前缀短名,title 保留完整原名(仅剥寻址后缀)", () => {
    const full = "nemesiscode-pro/deepseek-pro";
    const html = renderToStaticMarkup(
      <ModelPicker
        models={[{ name: full, model: full, source: "nemesiscode", default: true }]}
        current={full}
        onPick={vi.fn()}
      />,
    );
  ...
  it("落盘名的来源后缀不出现在任何展示面(短名与 title 都剥掉)", () => {
    const stored = "nemesiscode-pro/deepseek-pro@nemesiscode#cfg-9";
    const html = renderToStaticMarkup(
      <ModelPicker
        models={[{ name: stored, model: "nemesiscode-pro/deepseek-pro", source: "nemesiscode", default: true }]}
        current={stored}
        onPick={vi.fn()}
      />,
    );

    expect(html).not.toContain("@nemesiscode");
    expect(html).not.toContain("cfg-9");
    expect(html).toContain(">deepseek-pro<");
    expect(html).toContain("nemesiscode-pro/deepseek-pro · 点击切换(下一轮生效)");
  });
});

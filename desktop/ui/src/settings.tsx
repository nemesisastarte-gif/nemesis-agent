...
// + 底部脏状态保存条。账号优先:NemesisCode 是百智云旗下产品,"登录 → 同步"
// 是主路径,手工配置是高级路径。
  ...
  return (
    <div className="card card-lg" style={{ padding: "14px 16px", display: "flex", alignItems: "center", gap: 12 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 1, minWidth: 0 }}>
        <span style={{ fontWeight: 700, fontSize: 13 }}>NemesisCode</span>
    ...
    <>
      ...
      <div style={{ fontSize: 12.5, color: "var(--t4)", lineHeight: 1.8 }}>
        ...
        <div style={{ marginTop: 2 }}>
          连接后，NemesisCode 可以打开网页、点击、输入和截图，并使用 Chrome/Edge 中已有的登录状态。
        </div>
        <ol style={{ margin: "8px 0 0", paddingLeft: 20 }}>
          ...
          <li>
            <strong style={{ color: "var(--t2)" }}>连接 NemesisCode。</strong>
            点击浏览器工具栏中的「NemesisCode 浏览器助手」，选择「去设置页配对」。输入上方的一次性配对码，端口保持为空，再点击「连接并配对」。
          </li>
          <li>
            <strong style={{ color: "var(--t2)" }}>授权要操作的页面。</strong>
            上方状态显示「已连接」后，打开目标网页，再次点击扩展图标，选择「把此标签页交给 agent 操作」，即可授权 NemesisCode 操作当前页面。
          </li>
        </ol>
...
function mcIdentity(s: McConnectionState): string {
  const u = s.user;
  return u?.name || u?.username || u?.email || u?.id || "NemesisCode 用户";
}
...
/** NemesisCode 云端任务关联卡。百智云只是显式连接时的授权前提,
 * 两者状态和退出操作互不代替。已关联时可把会员内置模型同步为本地任务
 * 可用的条目。 */
function NemesisCodeAccountCard({
  connection,
  usage,
  onCheckin,
  baizhiLoggedIn,
  ...
  return (
    <div className="card card-lg" style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 1, minWidth: 0 }}>
          <span style={{ fontWeight: 700, fontSize: 13 }}>NemesisCode 账号</span>
        ...
        {connected || connection.phase === "disconnecting" ? (
          ...
          >
            {connection.phase === "connecting" ? "连接中…" : "连接 NemesisCode"}
          </button>
        )}
      </div>
      ...
      {/* 第二条登录路径:NemesisCode 账号密码(不经百智云,私有化/未绑百智账号可用) */}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <Field label="邮箱">
                  <input
                    style={input}
                    value={pwEmail}
                    placeholder="dev@nemesiscode.io"
...
export function SettingsView({
  ...
  const [mcModelsOpen, setMcModelsOpen] = useState(true); // NemesisCode 会员模型组(默认展开)
  ...
  const [mcBaseUrl, setMcBaseUrl] = useState(""); // NemesisCode 服务地址("" = 官方云;重启应用生效)
  // 自建部署配置默认隐藏(彩蛋:连点 NemesisCode 卡图标 6 次解锁,解锁态
  ...
  // 同步条目并入表单(百智云/NemesisCode 共用;整组替换 + 跨组撞名先到先得,
  ...
  const mergeSyncedModels = (
    ...
    setAdvOpen({});
    setExpanded(null);
    (source === SOURCE_NEMESISCODE ? setMcModelsOpen : setBaizhiOpen)(true);
  ...
  const syncBaizhi = async () => {
    ...
    try {
      ...
      if (r.key_created) parts.push(`已在网关新建密钥「${r.key_name || "NemesisCode"}」`);
  ...
  // ---- NemesisCode 会员模型同步流水线(拉取 → 并入表单 → 自动保存)----
  ...
  const syncMcModels = async () => {
    ...
    try {
      ...
      const applied = applySyncedModels(r.models, SOURCE_NEMESISCODE);
  ...
  // 百智云登录成功 → 顺带把 NemesisCode 也连上(桥接授权本就用百智云账号,
  // 登录一次两边都通)。只认**真实登录事件**,不看 loggedIn 快照:后者在
  // 每次打开设置读到既有登录态时也会翻转,会把用户"显式断开 NemesisCode"
  ...
  // 断开 NemesisCode:①先走 mcModelsRevoke(须在清会话前;断网失败不阻断
  ...
  const disconnectMcWithCleanup = async () => {
    ...
    const hasSavedMc = (cfg?.models ?? []).some((m) => m.source === SOURCE_NEMESISCODE);
    if (
      hasSavedMc &&
      !confirm(
        dirty
    ...
    if (models.some((m) => m.source === SOURCE_NEMESISCODE)) {
      const defaultName = models[defaultIdx]?.name?.trim() ?? "";
      const rest = models.filter((m) => m.source !== SOURCE_NEMESISCODE);
    ...
    let next = cfg.models.filter((m) => m.source !== SOURCE_NEMESISCODE);
  ...
  const validateBeforeSave = (ms: HostModel[], mcArr: McpEntry[]): boolean => {
    // NemesisCode 会员条目不校验接口地址/API Key:表单里是空占位,
    // 物化时由壳补齐
    for (const m of ms) {
      const managed = m.source === SOURCE_NEMESISCODE;
  ...
  const performSave = async (ms: HostModel[], di: number, mcArr: McpEntry[]) => {
    ...
    try {
      ...
      // (百智云登录会顺带连上 NemesisCode,两路同步先后落地)。补存到表单不
  ...
  const modelRow = (m: HostModel, i: number) => {
    // NemesisCode 会员条目只读不可展开:随同步整组更新,表单里没有
    // 可改的东西
    const managed = m.source === SOURCE_NEMESISCODE;
  ...
  const modelsSection = () => {
    ...
    const nemesiscode = entries.filter((e) => e.m.source === SOURCE_NEMESISCODE);
    const custom = entries.filter((e) => e.m.source !== SOURCE_BAIZHI && e.m.source !== SOURCE_NEMESISCODE);
    return (
      <>
        {loginHint}
        {/* 百智云组在前(主路径) */}
        ...
        {/* NemesisCode 会员组:空时不渲染(入口引导在账号页卡片,不堆空态) */}
        {nemesiscode.length > 0 && (
          <Section
            label={`${modelSourceLabel(SOURCE_NEMESISCODE)}(${nemesiscode.length})`}
            action={collapseToggle(mcModelsOpen, () => setMcModelsOpen((v) => !v))}
          >
            {mcModelsOpen && groupCard(nemesiscode, modelRow)}
          </Section>
        )}
        {/* 自定义组(高级路径) */}
  ...
  const accountSection = () => (
    <>
      ...
      <Section label="NemesisCode">
        <NemesisCodeAccountCard
        ...
        {desktop && (mcServerCfgUnlocked || !!(mcBaseUrl.trim() || mcBasicAuth.trim() || mcLlmBaseUrl.trim())) && (
          <div className="card card-lg" style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 240px", gap: 12 }}>
              ...
              <Field label="Basic Auth(可选)">
                <input
                  ...
                  title="测试环境反向代理层的 HTTP Basic Auth;仅对 NemesisCode 服务的请求附加"
            ...
            <span style={{ fontSize: 12, color: "var(--t5)", lineHeight: 1.7 }}>
              指向自建/私有化 NemesisCode 服务,保存后需<strong style={{ color: "var(--t3)" }}>重启应用</strong>生效。
              环境变量 MC_DESKTOP_NEMESISCODE_URL 优先于地址设置。切换服务地址后需重新连接账号并重新同步会员模型;

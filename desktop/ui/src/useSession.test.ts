      ...
      core: {
        invoke: (cmd: string, args?: Record<string, unknown>) => {
          ...
          if (cmd === "upload_finish") {
            ...
            uploaded.push(name);
            return Promise.resolve({ path: ".nemesiscode/uploads/" + name });
          }
          ...
          if (cmd === "upload_file_path") {
            ...
            return Promise.resolve({ path: ".nemesiscode/uploads/" + base });
          }
...
describe("本地会话核心:连接生命周期", () => {
  ...
  it("首条消息上传期间切会话:不发进新会话,转入原会话排队暂存,切回补投", async () => {
    ...
    core.open("s1");
    expect(out.queued).toBe("首条\n[图片] .nemesiscode/uploads/slow.png");

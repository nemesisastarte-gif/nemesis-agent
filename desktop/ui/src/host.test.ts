...
describe("workdir 与内核运行环境匹配", () => {
  ...
  it("WSL 模式放行所有可归一化形态(guest/UNC/盘符/~)", () => {
    ...
    expect(workdirMatchesEnv("C:\\Users\\u\\NemesisCode", wsl, true)).toBe(true);
    expect(workdirMatchesEnv("d:/dev/x", wsl, true)).toBe(true);
    expect(workdirMatchesEnv("~/NemesisCode", wsl, true)).toBe(true);
    // 归一化不了的形态仍滤掉
    expect(workdirMatchesEnv("relative/path", wsl, true)).toBe(false);
  });

  it("Windows 本机滤掉 guest 形态的遗留", () => {
    expect(workdirMatchesEnv("C:\\Users\\u\\NemesisCode", "", true)).toBe(true);
    expect(workdirMatchesEnv("~/NemesisCode", "", true)).toBe(true);
  ...
  it("macOS/Linux 本机的 / 开头是正常路径,只滤 WSL UNC", () => {
    ...
    expect(workdirMatchesEnv("~/NemesisCode", "", false)).toBe(true);

...
import { SOURCE_BAIZHI, SOURCE_NEMESISCODE, type ModelInfo } from "./types";
...
describe("会员长名的展示投影(对齐 Web 前缀口径,纯展示层)", () => {
  it("stripTierPrefix 只剥斜杠形前缀,剥空回落原名", () => {
    expect(stripTierPrefix("nemesiscode-pro/deepseek-pro")).toBe("deepseek-pro");
    expect(stripTierPrefix("Nemesiscode-Ultra/Claude-X")).toBe("Claude-X");
    // 连字符形整串可能就是真实模型 id,不剥
    expect(stripTierPrefix("nemesiscode-pro-claude")).toBe("nemesiscode-pro-claude");
    expect(stripTierPrefix("nemesiscode-pro/")).toBe("nemesiscode-pro/");
    expect(stripTierPrefix("some-other-model")).toBe("some-other-model");
  });

  it("modelDisplay:会员条目短名 + 档位(档位从底层 model 判);其余来源原样", () => {
    expect(
      modelDisplay({ name: "nemesiscode-pro/deepseek-pro", model: "nemesiscode-pro/deepseek-pro", source: SOURCE_NEMESISCODE }),
    ).toEqual({ label: "deepseek-pro", tier: "专业" });
    // remark 别名:短名 = remark,档位仍能从 model 判出
    expect(
      modelDisplay({ name: "深度求索", model: "nemesiscode-ultra/ds", source: SOURCE_NEMESISCODE }),
    ).toEqual({ label: "深度求索", tier: "旗舰" });
    // 会员组里的服务端自定义模型:无前缀 → 原样、无档位
    expect(
      modelDisplay({ name: "some-other-model", model: "some-other-model", source: SOURCE_NEMESISCODE }),
    ).toEqual({ label: "some-other-model", tier: undefined });
    // 手工条目取了会员形状的名字:不投影(name 是引擎键,别误打会员档)
    expect(modelDisplay({ name: "nemesiscode-pro/x" })).toEqual({ label: "nemesiscode-pro/x" });
    // 下线模型兜底项没有 model 字段:容缺
    expect(modelDisplay({ name: "x", source: SOURCE_NEMESISCODE })).toEqual({ label: "x", tier: undefined });
  });

  it("modelDisplayByName:回查条目投影,查不到原样", () => {
    const models = [
      { name: "nemesiscode-basic/glm", model: "nemesiscode-basic/glm", source: SOURCE_NEMESISCODE, default: false },
    ];
    expect(modelDisplayByName(models, "nemesiscode-basic/glm")).toEqual({ label: "glm", tier: "基础" });
...
describe("modelMenuTabs(无「全部」,tab 即来源导航)", () => {
  it("来源序:会员 → 百智云 → 未知透传 → 自定义恒尾,会员缩写", () => {
    const tabs = modelMenuTabs([
      ...
      m("b", "mystery"),
      m("c", SOURCE_BAIZHI),
      m("d", SOURCE_NEMESISCODE),
    ]);
    expect(tabs).toEqual([
      { key: "nemesiscode", label: "会员" },
...
describe("filterModels(tab 内过滤)", () => {
  const items = [
    m("手工模型"),
    { ...m("深度求索", SOURCE_NEMESISCODE), model: "nemesiscode-pro/deepseek-pro" },
  ];
...
describe("同步条目落盘名的来源后缀(寻址用,展示一律剥掉)", () => {
  it("stripSourceSuffix 剥来源后缀与会员条目的 #配置 id;剥空回落原名", () => {
    ...
    expect(stripSourceSuffix("深度求索@nemesiscode#cfg-9")).toBe("深度求索");
    expect(stripSourceSuffix("深度求索@nemesiscode")).toBe("深度求索");
    // 手工条目原样;名字里本来就有 @ 的、形似后缀的都不误伤
    expect(stripSourceSuffix("my@model")).toBe("my@model");
    expect(stripSourceSuffix("@baizhi")).toBe("@baizhi");
    expect(stripSourceSuffix("x@nemesiscode-plus")).toBe("x@nemesiscode-plus");
    // id 的字符集不归我们管,# 之后放行(与壳侧 strip_source_suffix 同口径)
    expect(stripSourceSuffix("深度求索@nemesiscode#a@b")).toBe("深度求索");
  });

  it("sameModelName:带不带后缀算同一条(存量引用靠它落到新条目上)", () => {
    expect(sameModelName("深度求索", "深度求索@nemesiscode#cfg-9")).toBe(true);
  ...
  it("modelDisplay 剥后缀后再剥档位前缀;filterModels 按展示名匹配", () => {
    expect(
      modelDisplay({ name: "nemesiscode-pro/deepseek@nemesiscode#c1", model: "nemesiscode-pro/deepseek", source: SOURCE_NEMESISCODE }),
    ).toEqual({ label: "deepseek", tier: "专业" });
    const items = [{ ...m("深度求索@nemesiscode#c1", SOURCE_NEMESISCODE), model: "mc-ds" }];
    expect(filterModels(items, "深度")).toEqual(items);
    expect(filterModels(items, "nemesiscode")).toEqual([]); // 后缀不参与匹配
  });
});

describe("memberCategory(设置页药丸与分节共用的分类词汇)", () => {
  it("档位优先于 owner;公共非档位与旧同步条目都归付费", () => {
    expect(memberCategory({ model: "nemesiscode-basic/b", owner: "public" })).toBe("基础");
    expect(memberCategory({ model: "nemesiscode-pro/p", owner: "public" })).toBe("专业");
    expect(memberCategory({ model: "nemesiscode-ultra/u", owner: "private" })).toBe("旗舰");
...
describe("groupMemberSections(会员 tab 分节,对齐 Web 分类)", () => {
  const ultra = { ...m("旗舰甲", SOURCE_NEMESISCODE), model: "nemesiscode-ultra/a", owner: "public", locked: true };
  const basic = { ...m("基础乙", SOURCE_NEMESISCODE), model: "nemesiscode-basic/b", owner: "public" };
  const paid = { ...m("付费丙", SOURCE_NEMESISCODE), model: "some-model", owner: "public" };
  const legacy = { ...m("旧同步丁", SOURCE_NEMESISCODE), model: "another-model" }; // 改造前同步的条目无 owner
  const mine = { ...m("私有戊", SOURCE_NEMESISCODE), model: "my-model", owner: "private" };
  const team = { ...m("团队己", SOURCE_NEMESISCODE), model: "team-model", owner: "team" };

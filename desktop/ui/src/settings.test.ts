...
import { SOURCE_BAIZHI, SOURCE_NEMESISCODE, type HostModel } from "./types";
...
describe("syncedName(同步条目的落盘名)", () => {
  it("百智云缀来源;会员再缀服务端配置 id(同批重名靠它区分)", () => {
    ...
    expect(syncedName("深度求索", SOURCE_NEMESISCODE, "cfg-9")).toBe("深度求索@nemesiscode#cfg-9");
    // 同名两条 → 落盘名互不相同,不再有条目因重名被丢
    expect(syncedName("同名", SOURCE_NEMESISCODE, "c1")).not.toBe(syncedName("同名", SOURCE_NEMESISCODE, "c2"));
    // 服务端没给 id(不该发生)时退回只缀来源,至少跨来源不撞
    expect(syncedName("无 id", SOURCE_NEMESISCODE)).toBe("无 id@nemesiscode");
  });

  it("重同步不叠加后缀;自定义条目与空名不动", () => {
    expect(syncedName("深度求索@nemesiscode#cfg-9", SOURCE_NEMESISCODE, "cfg-9")).toBe("深度求索@nemesiscode#cfg-9");
    // 服务端换了配置 id:按新 id 重缀,不是叠加
    expect(syncedName("深度求索@nemesiscode#old", SOURCE_NEMESISCODE, "new")).toBe("深度求索@nemesiscode#new");
...
describe("replaceSourceGroup", () => {
  ...
  it("replaces only the target source group and keeps other groups intact", () => {
    const cur = [entry("手工"), entry("bz-old", SOURCE_BAIZHI), entry("mc-old", SOURCE_NEMESISCODE)];
    ...
    // 同步 nemesiscode:baizhi 组与手工条目不动
    const afterMc = replaceSourceGroup(cur, [entry("mc-new", SOURCE_NEMESISCODE)], SOURCE_NEMESISCODE);
  ...
  it("跨组撞名先到先得:同名非本组条目保留,同步条目跳过(不静默换通道)", () => {
    ...
    // 会员同步:手工条目与百智条目对 nemesiscode 组都算"非本组",同名让位
    const kept = replaceSourceGroup(
      cur,
      [entry("撞名", SOURCE_NEMESISCODE), entry("百智撞名", SOURCE_NEMESISCODE), entry("新条目", SOURCE_NEMESISCODE)],
      SOURCE_NEMESISCODE,
    );
    expect(kept.map((m) => [m.name, m.source])).toEqual([
      ["撞名", undefined],
      ["百智撞名", SOURCE_BAIZHI],
      ["新条目", SOURCE_NEMESISCODE],
    ]);
    ...
    const bz = replaceSourceGroup(
      [entry("deepseek", SOURCE_NEMESISCODE)],
      [entry("deepseek", SOURCE_BAIZHI), entry("claude", SOURCE_BAIZHI)],
      SOURCE_BAIZHI,
    );
    expect(bz.map((m) => [m.name, m.source])).toEqual([
      ["deepseek", SOURCE_NEMESISCODE],
      ["claude", SOURCE_BAIZHI],
    ]);
  });

  it("removes stale synced entries when the new batch omits them", () => {
    const cur = [entry("a", SOURCE_NEMESISCODE), entry("b", SOURCE_NEMESISCODE)];
    const next = replaceSourceGroup(cur, [entry("a", SOURCE_NEMESISCODE)], SOURCE_NEMESISCODE);
...
describe("dedupeModelsByName(载入自愈)", () => {
  ...
  it("同名条目收敛为一条:内容后者覆盖前者,落在首现位置(与引擎物化 Map 一致)", () => {
    const out = dedupeModelsByName([
      entry("deepseek-v4-flash", SOURCE_NEMESISCODE),
      entry("其他"),
...
describe("sortModelsBySource(载入/同步后的分组排序)", () => {
  ...
  it("组间按 会员→百智云→未知→自定义,组内保持原相对顺序(稳定)", () => {
    const out = sortModelsBySource([
      ...
      entry("mc-2", SOURCE_NEMESISCODE),
      entry("神秘", "mystery"),
      entry("手工甲"),
      entry("bz-1", SOURCE_BAIZHI),
      entry("mc-1", SOURCE_NEMESISCODE),
    ]);
    expect(out.map((m) => m.name)).toEqual(["mc-2", "mc-1", "bz-2", "bz-1", "神秘", "手工乙", "手工甲"]);
  });

  it("空名草稿 source 缺省 → 随自定义组留在尾部,不丢", () => {
    const out = sortModelsBySource([entry(""), entry("mc", SOURCE_NEMESISCODE)]);

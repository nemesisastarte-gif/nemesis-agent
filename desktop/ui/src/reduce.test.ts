...
describe("轮次与系统帧", () => {
  ...
  it("model_update 的系统行剥寻址后缀与会员档位前缀,状态仍存原始名", () => {
    const s = run([acp({ sessionUpdate: "model_update", model: "nemesiscode-pro/deepseek@nemesiscode#cfg-9" })]);
    expect(s.items.at(-1)).toEqual({ kind: "sys", text: "模型已切换为 deepseek" });
    expect(s.model).toBe("nemesiscode-pro/deepseek@nemesiscode#cfg-9");

    const remark = run([acp({ sessionUpdate: "model_update", model: "深度求索@nemesiscode#cfg-9" })]);

...
function patchAppDelegate(projectRoot) {
  const file = path.join(projectRoot, 'ios', 'NemesisCode', 'AppDelegate.swift');
  if (!fs.existsSync(file)) return;
  let source = fs.readFileSync(file, 'utf8');
...
function writeIOSNativeFiles(projectRoot) {
  const base = path.join(projectRoot, 'ios', 'NemesisCode');
...
module.exports = function withDouyinLogin(config, props = {}) {
  ...
  const packageName = config.android?.package || 'com.nemesiscode.app';

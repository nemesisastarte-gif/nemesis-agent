...
import { BAIZHI_BASE_URL, getBaizhiOAuthLoginUrl, getNemesisCodeBaizhiLoginUrl } from '@/api/baizhi';
...
const GITHUB_CALLBACK_URL = 'nemesiscode://oauth/github';
...
function isGithubCallback(url: string): boolean {
  return url.startsWith(GITHUB_CALLBACK_URL) || url.startsWith('nemesiscode:///oauth/github');
}
...
export default function LoginScreen() {
  ...
  const startBaizhiBridge = useCallback((title: string, targetBaseUrl: string, phoneToSave?: string, authorizeViaFetch = false) => {
    ...
    setWebOAuthUrl(getNemesisCodeBaizhiLoginUrl(cleanTarget));
  ...
  const webOAuthNeedsNemesisCodeAuth = webOAuthMode === 'baizhiBridge' && hostOf(webOAuthUrl) !== BAIZHI_HOST;
  const webOAuthCanUseNemesisCodeAuth = webOAuthMode === 'baizhiBridge';
  const webOAuthSource = webOAuthNeedsNemesisCodeAuth
      ...
      <View style={{ flex: 1, backgroundColor: t.bg }}>
        ...
        <View style={{ flex: 1 }}>
          <WebView
            ...
            originWhitelist={['https://*', 'http://*', 'nemesiscode://*']}
            basicAuthCredential={webOAuthCanUseNemesisCodeAuth ? getBasicAuthCred() : undefined}
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: pageBg }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1, paddingHorizontal: 28, paddingBottom: 34, paddingTop: insets.top + 36 }} keyboardShouldPersistTaps="handled">
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14 }}>
          ...
          <View>
            <Text style={{ fontSize: 24, fontWeight: '800', color: '#FFFFFF', letterSpacing: 0 }}>NemesisCode</Text>
        ...
        <View style={[{ marginTop: 30, backgroundColor: sheetBg, borderRadius: 26, paddingTop: 22, paddingHorizontal: 22, paddingBottom: 24, gap: 15 }, loginShadow]}>
          {view === 'phone' && cloud ? (
            ...
            <>
              ...
              <View style={fieldFrameStyle('email')}>
                ...
                <TextInput value={email} onChangeText={setEmail} placeholder="dev@nemesiscode.io" placeholderTextColor="#B4B9B0"
          ...
          {showServer ? (
            <View style={{ marginTop: 16, paddingTop: 16, borderTopWidth: 1, borderColor: t.line }}>
              ...
              <TextInput value={serverUrl} onChangeText={setServerUrl} placeholder="http://<ip-du-serveur>:8888" placeholderTextColor={t.tx3}

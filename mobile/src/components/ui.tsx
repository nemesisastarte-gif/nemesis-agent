...
// NemesisCode 鹰徽 logo（随主题：浅色主题用深色鹰徽，深色主题用亮色鹰徽）
const NEMESIS_LIGHT = require('../../assets/logo-light.png'); // 深色鹰徽(透明底) → 浅色背景
const NEMESIS_DARK = require('../../assets/logo-dark.png');   // 亮色鹰徽(透明底) → 深色背景
export function NemesisLogo({ size = 40, style }: { size?: number; style?: StyleProp<ImageStyle> }) {
  const t = useTheme();
  return <Image source={t.dark ? NEMESIS_DARK : NEMESIS_LIGHT} style={[{ width: size, height: size }, style]} resizeMode="contain" />;
}
  ...
  return (
    <Centered>
      {isError ? (
        ...
        <NemesisLogo size={76} style={{ marginBottom: 6, opacity: 0.92 }} />
      )}

...
const BUILTIN_META = new Set(['nemesiscode-basic', 'nemesiscode-pro', 'nemesiscode-ultra']);

function builtinName(model?: string): 'nemesiscode-basic' | 'nemesiscode-pro' | 'nemesiscode-ultra' | undefined {
  const n = (model || '').toLowerCase();
  if (n.startsWith('nemesiscode-basic')) return 'nemesiscode-basic';
  if (n.startsWith('nemesiscode-pro')) return 'nemesiscode-pro';
  if (n.startsWith('nemesiscode-ultra')) return 'nemesiscode-ultra';
  return undefined;
}
...
export function translateBuiltinNames(text: string): string {
  return text
    .replace(/nemesiscode-ultra/gi, '旗舰模型')
    .replace(/nemesiscode-pro/gi, '专业模型')
    .replace(/nemesiscode-basic/gi, '基础模型')
    .replace(/\s*\/\s*/g, ' / ');
}
...
export function modelLabel(model?: { model?: string; remark?: string } | null): string {
  ...
  if (name.startsWith('nemesiscode-basic')) {
    const nested = (model.model || '').slice('nemesiscode-basic'.length).replace(/^\/+/, '');
    return nested ? `基础模型 / ${nested}` : '基础模型';
  }
  if (name.startsWith('nemesiscode-pro')) {
    const nested = (model.model || '').slice('nemesiscode-pro'.length).replace(/^\/+/, '');
    return nested ? `专业模型 / ${nested}` : '专业模型';
  }
  if (name.startsWith('nemesiscode-ultra')) {
    const nested = (model.model || '').slice('nemesiscode-ultra'.length).replace(/^\/+/, '');
...
function planAllowsBuiltin(builtin: ReturnType<typeof builtinName>, plan?: string): boolean {
  if (builtin === 'nemesiscode-pro') return plan === 'pro' || plan === 'flagship' || plan === 'ultra';
  if (builtin === 'nemesiscode-ultra') return plan === 'flagship' || plan === 'ultra';
  return true;
}
...
export function groupModels(models: Model[], plan?: string): ModelGroup[] {
  ...
  const builtin: ModelGroup[] = ([
    { key: 'nemesiscode-basic', label: '基础模型', badge: '免费使用' },
    { key: 'nemesiscode-pro', label: '专业模型', badge: '专业会员免费' },
    { key: 'nemesiscode-ultra', label: '旗舰模型', badge: '旗舰会员免费' },
  ] as const)

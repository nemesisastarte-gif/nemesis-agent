...
#[ignore = "PTY e2e; run with cargo test -p xai-nemesis-pager --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared NEMESIS_HOME so every spawn elects/attaches to the
    // same leader (mirrors `LeaderCluster`).
    let nemesis_home = content.home().join(".nemesis");
    std::fs::create_dir_all(&nemesis_home).expect("create NEMESIS_HOME");
    std::fs::write(
        nemesis_home.join("config.toml"),
        format!("[models]\ndefault = \"{CONFIG_MODEL}\"\n"),
    )
    .expect("write config.toml");
    let socket = nemesis_home.join("leader-e2e.sock");
    ...
    let state_path = nemesis_home.join("campaigns_state.json");
    ...
    let config = std::fs::read_to_string(nemesis_home.join("config.toml")).expect("read config.toml");
    assert!(

...
//!
//! Uses `xai-nemesis-pager-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.nemesis/auth.json` from `GROK_AUTH_JSON` (path) or the
//! developer's `~/.nemesis/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p xai-nemesis-pager --test pty_auto_mode -- --ignored --nocapture`

use std::path::{Path, PathBuf};
use std::time::Duration;

use xai_nemesis_pager_pty_harness::{PtyHarness, pager_binary};
...
/// Prefer explicit path, else the user's real `~/.nemesis/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    ...
    dirs_next_home()
        .map(|h| h.join(".nemesis/auth.json"))
        .filter(|p| p.is_file())
}
...
fn prepare_sandbox(home: &Path, gate_on: bool) -> Vec<(String, String)> {
    ...
    if let Some(src) = auth_json_source() {
        ...
        eprintln!("pty_auto_mode: no ~/.nemesis/auth.json — may hit device login");
    }

    let home_s = home.display().to_string();
    let mut env = vec![
        ("HOME".into(), home_s.clone()),
        ("NEMESIS_HOME".into(), grok.display().to_string()),
    ...
    // layer below requirements; "1"/"0" parse to on/off (xai_nemesis_config::
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness = PtyHarness::new(&binary, ROWS, COLS, &[], &env_refs)
        .expect("spawn pager in PTY (xai-nemesis-pager-pty-harness)");

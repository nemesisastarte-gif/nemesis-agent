...
pub(crate) use xai_nemesis_pager_pty_harness::{
...
// `xai_nemesis_pager_pty_harness::flows` (re-exported above).
...
/// else `0` (an explicit opt-out that overrides the now-on default). HOME/NEMESIS_HOME
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".nemesis")
        .join(xai_nemesis_workspace::trust::TRUST_FILE_NAME);
    let store = xai_nemesis_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&xai_nemesis_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `xai_nemesis_pager_pty_harness::flows`
// (re-exported above).
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    let nemesis_home = content.home().join(".nemesis");
    std::fs::create_dir_all(&nemesis_home).expect("create fake NEMESIS_HOME");
    let config = format!(
        "[mcp_servers.{MCP_TEST_SERVER}]\ncommand = \"{command}\"\nargs = []\nstartup_timeout_sec = 2\n"
    );
    std::fs::write(nemesis_home.join("config.toml"), config).expect("write config.toml");
}
...
// unit tests in xai-nemesis-hooks::discovery::tests. The PTY E2E test requires
...
/// Seed `~/.nemesis/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{NEMESIS_HOME|HOME}/.nemesis/config.toml` location
...
pub(crate) fn seed_ui_config(content: &ContentController, ui_body: &str) {
    let nemesis_home = content.home().join(".nemesis");
    std::fs::create_dir_all(&nemesis_home).expect("create .grok");
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let nemesis_home = content.home().join(".nemesis");
    std::fs::create_dir_all(&nemesis_home).expect("create .grok");
    // Minimal opt-in only — matches load_config's `{NEMESIS_HOME|HOME}/.nemesis/config.toml`.
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    ...
    env.push((
        ...
        xai_nemesis_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string(),
    ));
    let env_refs: Vec<(&str, &str)> = env.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
    PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &[], &env_refs).expect("spawn pager")
}
...
/// `NEMESIS_HOME`, returning the exit code (`None` if it never exited within
...
pub(crate) fn run_wrap_driving(
    ...
    let mut env: Vec<(&str, &str)> = vec![("NEMESIS_HOME", &home_str), ("NO_COLOR", "1")];
...
pub(crate) use xai_nemesis_pager_pty_harness::host_clipboard::{
...
pub(crate) use xai_nemesis_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;

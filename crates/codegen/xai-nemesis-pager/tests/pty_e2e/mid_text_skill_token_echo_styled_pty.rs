...
use xai_nemesis_pager_pty_harness::StyledLine;
...
/// Seed a user-invocable skill under `dir/.nemesis/skills`. The workspace must
...
fn seed_test_skill(dir: &Path) {
    let skill_dir = dir.join(".nemesis").join("skills").join("test-skill");

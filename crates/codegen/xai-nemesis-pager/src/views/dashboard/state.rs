...
/// `~/.nemesis/config.toml`. Lenient — corrupted fields fall back to
...
pub struct PendingDispatchModel {
    ...
    pub effort: Option<xai_nemesis_shell::sampling::types::ReasoningEffort>,
    pub display: String,
}
    ...
    fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> InputOutcome {
        ...
        if matches!(mouse.kind, MouseEventKind::Down(MouseButton::Left)) {
            ...
            if self.upgrade_cta_hit.contains(mouse.column, mouse.row) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    xai_nemesis_telemetry::events::AnnouncementCtaSurface::Dashboard,
                ));
            }
...
/// Load the full persisted dashboard from `~/.nemesis/config.toml`.
///
...
/// `~/.nemesis/config.toml` (`[ui]`, `[hints]`, `[mcpServers]`, …).
///
...
fn config_path() -> Option<PathBuf> {
    let home = xai_nemesis_shell::util::nemesis_home::nemesis_home();
...
mod tests {
    ...
    /// per-key serial lock. The `NEMESIS_AGENT_DASHBOARD` key means this
    /// test runs serially with any other test that decorates itself
    /// with `#[serial_test::serial(NEMESIS_AGENT_DASHBOARD)]` — see the
    ...
    fn env_var_force_disables() {
        ...
        // same `NEMESIS_AGENT_DASHBOARD` key reads it concurrently.

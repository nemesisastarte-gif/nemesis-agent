...
use xai_nemesis_tools::implementations::skills::types::SkillInfo;
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> xai_nemesis_telemetry::events::ExtensionsModalTab {
        use xai_nemesis_telemetry::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    AddMcpServer {
        name: String,
        config: Box<xai_nemesis_shell::util::config::McpServerConfig>,
    },
...
fn parse_mcp_add_fields(name: &str, url_or_cmd: &str) -> Option<ButtonAction> {
    use xai_nemesis_shell::util::config::{McpServerConfig, McpServerTransportConfig};
...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    let grok = xai_nemesis_config::nemesis_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user grok home (NEMESIS_HOME-aware)
    // or a project-scoped `{cwd}/.nemesis/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User grok home (NEMESIS_HOME-aware).
        if let Ok(rest) = source_path.strip_prefix(grok.join(subdir))
            && let Some(name) = first_comp(rest)
        {
            return Some(name);
        }
        // Project-scoped `.nemesis/<subdir>/<name>` anywhere in the path.
    ...
    // Project hooks
    if source_dir.ends_with("/.nemesis/hooks") || source_dir.contains("/.nemesis/hooks/") {
        return ("Project hooks".into(), false);
    }
    // Custom directory — removable
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&grok) {
            let prefix = crate::util::display_nemesis_home_prefix();
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            xai_nemesis_tools::types::config_source::ConfigSource::User { path } => {
                if crate::util::is_under_user_nemesis_home(path) {
            ...
            xai_nemesis_tools::types::config_source::ConfigSource::Project { path } => {
                let s = path.display().to_string();
                if s.contains("/.nemesis/") {
                    ".nemesis/skills".into()
            ...
            xai_nemesis_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.nemesis/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // grok-home branch is NEMESIS_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.nemesis/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.nemesis/installed-plugins/vendor-abc123/skills");
    ...
    fn make_skill(
        ...
        xai_nemesis_tools::implementations::skills::types::SkillInfo {
            ...
            metadata: None,
            path: "test".to_string(),
            scope: xai_nemesis_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = xai_nemesis_tools::implementations::skills::types::SkillScope::Plugin;
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    xai_nemesis_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    xai_nemesis_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn local_plugins_source() -> xai_hooks_plugins_types::MarketplaceScanResult {
        xai_hooks_plugins_types::MarketplaceScanResult {
            ...
            source_url_or_path: "/home/user/.nemesis/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),

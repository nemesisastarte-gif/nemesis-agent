...
use xai_nemesis_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use xai_nemesis_shell::agent::config::AgentSelectionConfig;
use xai_nemesis_tools::implementations::skills::discovery::extract_first_paragraph;
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as shell: `[agent]`, `NEMESIS_AGENT`,
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = xai_nemesis_agent::discovery::discover(cwd);
...
/// Union bundled personas with local `~/.nemesis/personas` and `{cwd}/.nemesis/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let bundled_dir = nemesis_home.join("bundled").join("personas");
    ...
    let dirs = [
        (ConfigFileScope::Project, cwd.join(".nemesis").join("personas")),
        (ConfigFileScope::User, nemesis_home.join("personas")),
    ];
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match xai_nemesis_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => xai_nemesis_config::nemesis_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".nemesis").join("personas"),
    }
}
...
/// True when `path` is a deletable local persona file (user or project `.nemesis/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.nemesis/{subdir}` or `{cwd}/.nemesis/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let nemesis_home = xai_nemesis_config::nemesis_home();
    let in_user = dunce::canonicalize(nemesis_home.join(subdir))
        .ok()
        .is_some_and(|d| canonical.starts_with(&d));
    let project_suffix = std::path::Path::new(".nemesis").join(subdir);
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    xai_nemesis_shell::config::load_effective_config()
        .ok()
        .and_then(|root| xai_nemesis_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in xai-nemesis-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    xai_nemesis_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        xai_nemesis_agent::config::PromptMode::Extend => "extend",
        xai_nemesis_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use xai_nemesis_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".nemesis").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".nemesis").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.nemesis/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".nemesis").join("personas").join("gone.toml");

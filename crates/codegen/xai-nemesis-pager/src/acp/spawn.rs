...
pub async fn spawn_grok_shell(
    ...
    memory_config: Option<xai_nemesis_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    // skills to ~/.nemesis/skills/), and model catalog construction.
    let (agent_config, models_manager) =
        xai_nemesis_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)

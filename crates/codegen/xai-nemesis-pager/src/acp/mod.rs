...
use xai_nemesis_shell::sampling::types::ReasoningEffort;
...
pub struct AcpConnection {
    ...
    // When available, add field: startup_announcements: Option<Vec<xai_nemesis_announcements::RemoteAnnouncement>>
    ...
    pub auth_manager: std::sync::Arc<xai_nemesis_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<xai_nemesis_shell::util::config::RemoteSettings>,
...
pub async fn connect_via_leader(
    ...
    xai_nemesis_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = xai_nemesis_shell::util::nemesis_home::nemesis_home().join("config.toml");
...
mod tests {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.nemesis/config.toml` with a
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use xai_nemesis_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};
    ...
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use xai_nemesis_shell::agent::auth_method::{GROK_COM_METHOD_ID, XAI_API_KEY_METHOD_ID};

        let methods = vec![
            make_auth_method(GROK_COM_METHOD_ID, "Nemesis", None),

...
/// firehose (`NEMESIS_DEBUG_LOG` / `NEMESIS_LOG_FILE`).
pub use xai_nemesis_telemetry::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
/// Target for the always-on compact ACP update summary line (kind, ids,
/// status, payload sizes). Cheap to format at streaming rate.
///
/// Defined in `xai-nemesis-telemetry` so the firehose directives and the pager
/// filter share one constant (re-exported here for callsites).
pub use xai_nemesis_telemetry::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "xai_nemesis_shell=info,xai_nemesis_pager=trace,xai_nemesis_tools=info,xai_acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let otel_layer = xai_nemesis_telemetry::otel_layer::build_otel_layer(
    ...
    let instrumentation_layer = xai_nemesis_telemetry::instrumentation::layer();
    let sampling_log_layer = xai_nemesis_telemetry::sampling_log::layer();
    let hooks_log_layer = xai_nemesis_telemetry::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        ...
        .with(otel_layer);
    xai_nemesis_telemetry::debug_log::install_firehose(registry, "tui");

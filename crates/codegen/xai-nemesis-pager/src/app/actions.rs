...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: xai_nemesis_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    OpenExtensionsModal {
        ...
        trigger: xai_nemesis_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    AnnouncementsOpenCta(xai_nemesis_telemetry::events::AnnouncementCtaSurface),
...
///
/// Both variants write to `~/.nemesis/config.toml` and route ACP
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: xai_nemesis_workspace::foreign_sessions::EnabledForeignSessionSources,
        nemesis_home: std::path::PathBuf,
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.nemesis/config.toml`.
    ...
    /// Persist a typed setting to `~/.nemesis/config.toml`. On failure,
    ...
    /// registry (`~/.nemesis/active_sessions.json`).
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<xai_nemesis_workspace::session::git::RestoreDegree>,
    },
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<xai_nemesis_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<xai_nemesis_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<xai_nemesis_shell::util::changelog::ChangelogEntry>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<xai_nemesis_tools::implementations::skills::types::SkillInfo>, String>,
    },
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<xai_nemesis_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    DeepSearchResults {
        results: Vec<xai_nemesis_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
    ...
    GateRefreshed {
        settings: Option<xai_nemesis_shell::util::config::RemoteSettings>,
    },

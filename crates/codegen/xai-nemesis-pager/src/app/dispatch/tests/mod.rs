...
/// Plant a Build session under the process `nemesis_home()` (OnceLock-cached;
/// do not rely on setting `NEMESIS_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    ...
    let encoded = xai_nemesis_shell::util::nemesis_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: xai_nemesis_tools::implementations::grok_build::ask_user_question::AskUserQuestionMode::Default,
        questions: vec![
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    xai_nemesis_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str =
    xai_nemesis_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;

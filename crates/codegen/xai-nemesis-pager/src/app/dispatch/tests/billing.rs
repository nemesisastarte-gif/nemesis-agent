...
use super::*;
use xai_nemesis_shell::sampling::error::is_free_usage_exhausted_error;
...
fn is_credit_limit_error_matches_legacy_403_and_pool_402() {
    ...
    assert!(is_credit_limit_error(
        None,
        "API error (status 402 Payment Required): Nemesis Agent usage balance exhausted"
    ));
    assert!(is_credit_limit_error(
        None,
        "status 403: run out of credits"
...
fn free_usage_upsell_shows_two_options_with_exact_labels() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: xai_nemesis_telemetry::events::SuperGrokUpsell::FreeUsagePaywall,
            }
        )
    ));
    ...
    let expected = [
        ...
        (
            ...
            "Get the most out of Nemesis Agent. Highest usage limits.",
            Some(UPSELL_URL_UPGRADE),
        ),
    ];
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use xai_nemesis_shell::extensions::notification::{RetryState, SessionUpdate};
...
fn restricted_command_submit_opens_two_option_upsell() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: xai_nemesis_telemetry::events::SuperGrokUpsell::RestrictedCommand,
            }
        )
    ));
    let q = &qv.questions[0];

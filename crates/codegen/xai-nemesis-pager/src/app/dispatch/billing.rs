...
use xai_nemesis_telemetry::events::{SuperGrokUpsell, SuperGrokUpsellClicked};
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        xai_nemesis_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        CreditLimitUpsellMode::UnifiedCredits => (
            ...
            "Buy more credits",
            "Purchase credits to keep using Nemesis Agent",
            CreditLimitCardAction::PurchaseCredits,
            xai_nemesis_telemetry::events::CreditLimitChoice::PurchaseCredits,
            false,
        ),
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            xai_nemesis_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(xai_nemesis_telemetry::events::CreditLimitUpsellShown {
            surface: xai_nemesis_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
    ...
    log_event(xai_nemesis_telemetry::events::CreditLimitUpsellShown {
        surface: xai_nemesis_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
        max_tier: false,
        pay_as_you_go: payg_telemetry,
        unified_billing,
    });
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            xai_nemesis_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
fn open_supergrok_upsell(
    ...
    log_event(xai_nemesis_telemetry::events::SuperGrokUpsellShown {
        source,
        auth_method,
    });
    ...
    let options = vec![
        ...
        QuestionOption {
            ...
            description: "Get the most out of Nemesis Agent. Highest usage limits.".into(),
            preview: None,
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<xai_nemesis_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<xai_nemesis_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }

...
pub(super) fn set_voice_stt_language_inner(app: &mut AppView, canonical: &str) {
    ...
    if language_changed && let Some(tx) = app.voice_cmd_tx.take() {
        app.voice_reset();
        let _ = tx.try_send(xai_nemesis_voice::VoiceCommand::Shutdown);
    }
}
...
pub(in crate::app::dispatch) fn set_voice_stt_language(
    ...
    refresh_open_settings_modals(app);
    let effective = xai_nemesis_voice::language_for_api(canonical);
    tracing::info!(
        target: "settings",
    ...
    let toast = if canonical == xai_nemesis_voice::STT_LANGUAGE_AUTO {
        format!("\u{2713} Voice language: System ({effective})")
    } else {
        let name = xai_nemesis_voice::stt_language_by_code(canonical).map_or(canonical, |l| l.name);
...
pub(super) fn set_contextual_hint_inner(
    app: &mut AppView,
    write: fn(&mut xai_nemesis_shell::agent::config::ContextualHints, Option<bool>),
    new: bool,
) {
    write(&mut app.current_ui.contextual_hints, Some(new));
    let resolved = xai_nemesis_shell::util::config::resolve_contextual_hints(
...
pub(in crate::app::dispatch) fn set_default_model(
    ...
    // so persisting the human-readable name (e.g. "Nemesis Agent")
    // would silently fail to resolve on the next startup.
    //
    // Chat (`--chat` / NEMESIS_CHAT_MODE) catalogs use opaque `/rest/modes`
...
pub(in crate::app::dispatch) fn clear_fork_secondary_model(app: &mut AppView) -> Vec<Effect> {
    let baseline = xai_nemesis_shell::models::default_model().to_string();
    let prev_id_str = app.current_ui.fork_secondary_model.clone();

...
/// [`decide`](xai_nemesis_workspace::folder_trust::decide) precedence.
///
...
pub(crate) async fn run(
    ...
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<xai_nemesis_update::auto_update::UpdateAvailable>>,
    >,
    ...
    let launch_auto = xai_nemesis_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
    ...
    let launch_effective_ui = xai_nemesis_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(xai_nemesis_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = xai_nemesis_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match xai_nemesis_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.sharing_enabled = remote_settings
        ...
        .unwrap_or(false);
    app.plugin_cta_enabled = xai_nemesis_config::env_bool("GROK_PLUGIN_CTA")
    ...
    app.session_picker_grouped = std::env::var("GROK_SESSION_PICKER_GROUPED")
        ...
        .or_else(|| {
            xai_nemesis_shell::config::load_effective_config()
                .ok()
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    xai_nemesis_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<xai_nemesis_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == xai_nemesis_shell::agent::auth_method::XAI_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = xai_nemesis_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = xai_nemesis_shell::config::load_merged_requirements();
    let user_config = xai_nemesis_shell::config::load_from_disk().ok();
    let managed_config = xai_nemesis_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match xai_nemesis_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = xai_nemesis_shell::agent::config::resolve_compat_sessions_from_raw(
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            ...
            let endpoints_base =
                xai_nemesis_shell::agent::config::EndpointsConfig::from_config_value(raw)
                    .xai_api_base_url;
            app.voice_config =
                xai_nemesis_voice::VoiceConfig::from_config_table(table, Some(&endpoints_base));
        }
    }
    ...
    app.zdr_access_enabled = xai_nemesis_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        xai_nemesis_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        xai_nemesis_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        xai_nemesis_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    {
        use xai_nemesis_shell::util::config::{resolve_announcements, resolve_tips};
        ...
        app.active_announcements = xai_nemesis_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            let nemesis_home = xai_nemesis_tools::util::nemesis_home::nemesis_home();
            app.tip = xai_nemesis_shell::util::tips::pick_and_advance(&app.tips, &nemesis_home);
        }
    }

    let hints = xai_nemesis_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.nemesis/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        xai_nemesis_shell::util::config::clamped_display_permission_mode(
            xai_nemesis_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        xai_nemesis_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = xai_nemesis_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = xai_nemesis_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<xai_nemesis_voice::VoiceEvent>>;
    ...
    // CLI subcommand wrote a `NEMESIS_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("NEMESIS_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(xai_nemesis_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.nemesis/config.toml [ui]`
                // rather than `~/.nemesis/pager.toml`. The watcher only knows
                        ...
                        let join_handle = tokio::spawn(async move {
                            ...
                            let ok = tokio::time::timeout(timeout, async {
                                ...
                                for (agent_id, plan) in load_plans {
                                    ...
                                    let mcp_servers = xai_nemesis_shell::util::config::load_mcp_servers(
                                        &plan.cwd,
...
pub(crate) fn load_initial_ui_config() -> xai_nemesis_shell::agent::config::UiConfig {
    use xai_nemesis_shell::agent::config::UiConfig;
    let Ok(root) = xai_nemesis_shell::config::load_effective_config() else {
...
async fn drain_and_process(
    ...
    let mut handle_one = |routed: &RoutedInputEvent| -> bool {
        ...
        if let Event::Key(ke) = ev
            ...
            && xai_nemesis_voice::AUDIO_SUPPORTED
            && is_voice_chord(ke)

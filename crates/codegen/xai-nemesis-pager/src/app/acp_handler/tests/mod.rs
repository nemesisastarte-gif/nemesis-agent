...
use xai_nemesis_shell::extensions::notification::RetryState;
use xai_nemesis_shell::extensions::notification::SessionUpdate as XaiSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> xai_nemesis_shell::extensions::notification::ImageCompressedEntry {
    xai_nemesis_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn xai_hook_execution_notif_for_prompt(
    ...
    use xai_nemesis_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
...
pub(super) fn xai_hook_execution_notif_with_runs(
    ...
    prompt_id: Option<&str>,
    is_replay: bool,
    runs: Vec<xai_nemesis_shell::extensions::notification::HookRunEntryDto>,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
/// Shared temp `NEMESIS_HOME` for disk-replay tests. `nemesis_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `NEMESIS_HOME`
...
/// depend on process-wide `nemesis_home()` cache order when the full suite runs.
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    let home = replay_disk_test_home();
    crate::app::subagent::set_replay_nemesis_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_nemesis_home_for_tests(None);
    out
}
...
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = xai_nemesis_workspace::session::git::GitHeadChanged {
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: xai_nemesis_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use xai_nemesis_shell::extensions::mcp::{
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = xai_nemesis_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),

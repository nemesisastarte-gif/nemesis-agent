use super::*;
use xai_nemesis_shell::sampling::error::format_rate_limited_user_message;
    ...
    let changed = match session_notif.update {
            ...
            let hook_entries: Vec<HookRunEntry> = runs
                ...
                .map(|r| {
                    let status = match r.status {
                        xai_nemesis_shell::extensions::notification::HookRunStatusDto::Success {
                            elapsed_ms,
                        } => HookRunStatus::Success {
                            elapsed: std::time::Duration::from_millis(elapsed_ms),
                        },
                        xai_nemesis_shell::extensions::notification::HookRunStatusDto::Skipped => {
                            HookRunStatus::Skipped
                        }
                        xai_nemesis_shell::extensions::notification::HookRunStatusDto::Failed {
                            error,
                            elapsed_ms,
                            blocked: true,
        ...
        XaiSessionUpdate::ModelChanged {
            ...
            if !agent.session.models.available.contains_key(&new_model_id) {
                if xai_nemesis_shell::agent::chat_modes::process_chat_mode_enabled() {
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[xai_nemesis_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
...
pub(super) fn apply_retry_state(
    retry: &xai_nemesis_shell::extensions::notification::RetryState,
    session: &mut AgentSession,
    scrollback: &mut crate::scrollback::state::ScrollbackState,
    is_api_key_auth: bool,
) {
        ...
        RetryState::Exhausted {
            ...
            if *rate_limited {
                xai_nemesis_telemetry::session_ctx::log_event(
                    xai_nemesis_telemetry::events::RateLimitHit {
                        model_id: session
                            .models
                            .current
                            .as_ref()
            ...
            let is_free_usage = *rate_limited
                && xai_nemesis_shell::sampling::error::is_free_usage_exhausted_error(reason);
            if is_credit_limit {
    ...
    if is_credit_limit {
        xai_nemesis_telemetry::session_ctx::log_event(xai_nemesis_telemetry::events::CreditLimitHit {
            model_id: session
                .models
                .current
                .as_ref()
...
pub(super) fn detect_plan_mode_change(update: &acp::SessionUpdate, agent: &mut AgentView) -> bool {
    use xai_nemesis_tools::types::SessionMode;

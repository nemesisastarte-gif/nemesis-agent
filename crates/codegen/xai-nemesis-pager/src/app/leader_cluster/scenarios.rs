...
#[test]
#[ignore = "leader-cluster: needs single-process isolation (process-global env + nemesis_home OnceLock in the shared lib test binary); run: cargo test -p xai-nemesis-pager --lib -- app::leader_cluster --ignored --test-threads=1"]
...
/// Locate `<nemesis_home>/sessions/<enc-cwd>/<sid>/updates.jsonl` without the
...
fn find_session_updates_file(sid: &str) -> Option<PathBuf> {
    let sessions = effective_nemesis_home().join("sessions");

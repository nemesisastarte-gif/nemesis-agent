...
//!
//! Env sandboxing follows this crate's `serial(NEMESIS_HOME)` idiom; note
//! `nemesis_home()` is process-cached (OnceLock), so disk assertions always go
//! through [`effective_nemesis_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary: the harness
//! mutates process-global env (proxy URLs, `XAI_API_KEY`,
//! `GROK_LEADER_SOCKET`, `NEMESIS_HOME`) for a real agent's whole lifetime, and
//! in a several-thousand-test process that poisons concurrently-running tests
//! (and `nemesis_home()`'s OnceLock is usually already pinned). Run on demand:
//!
//! ```bash
//! cargo test -p xai-nemesis-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
use xai_nemesis_shell::agent::mvp_agent::MvpAgent;
...
/// The grok home the agent actually persisted under: `nemesis_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_nemesis_home() -> PathBuf {
    xai_nemesis_config::nemesis_home()
}
...
struct PagerLeaderCluster {
    ...
    /// agent on the same NEMESIS_HOME (two agents on one updates.jsonl is the
    ...
    _nemesis_home: TempDir,
}

impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(NEMESIS_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let sock_path = nemesis_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("NEMESIS_HOME", nemesis_home.path()),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (xai_nemesis_version), not
            // this crate's version: a reconnecting client evicts strictly-older
            // leaders, and "evict" here would signal THIS test process.
            leader_binary_version: xai_nemesis_version::VERSION.to_string(),
        });
        ...
        generation_tasks.push(tokio::task::spawn_local(async move {
            let _ = run_leader_server(
                ...
                xai_nemesis_shell::agent::activity::AgentActivity::default(),
                tokio::sync::watch::channel(true).1,
                tokio::sync::watch::channel(false).0,
                tokio::sync::watch::channel(xai_nemesis_shell::leader::ShutdownReason::Manual).0,
                None,
                control_state,
            )
            .await;
        ...
        // xai-nemesis-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: xai-nemesis-shell/tests/test_leader_soak.rs ("Real agent
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same NEMESIS_HOME when the next generation's

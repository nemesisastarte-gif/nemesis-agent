...
//! inside [`xai_nemesis_mermaid::render_checked`] is a no-op there: a panic in the
...
//!
//! 1. The pager re-execs itself as `xai-nemesis-pager __mermaid-render` (see
...
//!    via [`xai_nemesis_mermaid::run_with_timeout`], which **kills and reaps** the
...
//!    ([`xai_nemesis_mermaid::RenderLimits`]) and the raster megapixel/height caps
//!    (see `xai-nemesis-mermaid`). The parent also rejects obviously-oversized
...
fn render_source_to_png(
    ...
    target_width_px: u32,
    quality: MermaidRenderQuality,
) -> Result<RenderedDiagram, xai_nemesis_mermaid::MermaidError> {
...
thread_local! {
    ...
    /// `NEMESIS_HOME` (whose `nemesis_home()` value is cached first-write-wins, an
    ...
    /// `subagent::REPLAY_NEMESIS_HOME` test seam. Production never sets this.
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test seam: a hermetic per-test cache dir (no `NEMESIS_HOME` mutation).
        #[cfg(test)]
...
mod tests {
    ...
    fn is_render_subcommand_matches_only_argv1() {
        ...
        assert!(!is_render_subcommand(&argv(&["nemesis"])));
        assert!(!is_render_subcommand(&argv(&["nemesis", "chat"])));
        assert!(!is_render_subcommand(&argv(&[])));
    ...
    /// hermetic, with no process-global `NEMESIS_HOME` mutation. The `TempDir` lives
    ...
    /// so there is no cross-test contamination and no `nemesis_home()` cache race.

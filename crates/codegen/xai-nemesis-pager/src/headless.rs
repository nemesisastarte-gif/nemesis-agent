...
use xai_nemesis_shell::agent::auth_method::AuthMethodKind;
use xai_nemesis_shell::agent::config::Config as AgentConfig;
use xai_nemesis_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
...
use xai_nemesis_shell::util::config as cli_config;
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<xai_nemesis_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use xai_nemesis_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<xai_nemesis_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = xai_nemesis_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut xai_nemesis_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    xai_nemesis_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `nemesis login` to authenticate \
         (or `nemesis login --device-code` if no browser is available)."
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         nemesis login --device-code\n\n\
         Alternatively, set the XAI_API_KEY environment variable \
         or run `nemesis login` on a machine with a browser."
            .to_string()
    }
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !xai_nemesis_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !xai_nemesis_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &xai_nemesis_tools::types::compat::CompatConfig::default());
...
pub async fn run_single_turn(
    ...
    xai_nemesis_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = xai_nemesis_shell::agent::config::AgentMode::Headless;
    ...
    agent_config.cli_agent_overrides = xai_nemesis_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        xai_nemesis_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }
    ...
    if track_active {
        let _ = xai_nemesis_shell::active_sessions::register(
            xai_nemesis_shell::active_sessions::ActiveSession {
    ...
    if options.self_verify {
        prompt_blocks.push(acp::ContentBlock::Text(acp::TextContent::new(
            skill_body(xai_nemesis_shell::builtin::CHECK_SKILL_MD).to_string(),
        )));
    }
        ...
        let n = n.clamp(2, 10);
        {
            prompt_blocks.insert(
                ...
                acp::ContentBlock::Text(acp::TextContent::new(format!(
                    ...
                    skill_body(xai_nemesis_shell::builtin::BEST_OF_N_SKILL_MD)
                ))),
            );
        }
    }
    ...
    if track_active {
        ...
        let _ = xai_nemesis_shell::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = xai_nemesis_shell::sampling::error::prompt_usage_from_error(&err)
...
mod tests {
    ...
    use super::*;
    use xai_nemesis_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            xai_nemesis_workspace::permission::types::PatternMode::Domain
        );

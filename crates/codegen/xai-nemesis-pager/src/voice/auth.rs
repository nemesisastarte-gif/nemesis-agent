...
use xai_nemesis_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
pub fn build_voice_auth(auth_manager: Arc<xai_nemesis_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        xai_nemesis_shell::auth::shared_api_key_provider(auth_manager),
    ))
}

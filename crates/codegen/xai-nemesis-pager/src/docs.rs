...
/// Extract user-guide docs to `<nemesis_home>/docs/user-guide/`.
///
/// Called from the pager binary startup so the model can read them from disk.
pub fn extract_user_guide_docs(nemesis_home: &std::path::Path) {
    let docs_dir = nemesis_home.join("docs").join("user-guide");

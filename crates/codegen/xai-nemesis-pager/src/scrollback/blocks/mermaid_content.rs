...
//! the generic [`CodeBlockSpan`](xai_nemesis_markdown::CodeBlockSpan) API) and
...
use xai_nemesis_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](xai_nemesis_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](xai_nemesis_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a xai_nemesis_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
/// dark. The render worker maps this to `xai_nemesis_mermaid::MermaidTheme`; it

...
pub fn run(shell: Shell) {
    // Ensure the script always uses the public "nemesis" name (matches historical
    // behavior and what the installers + docs expect).
    let mut cmd = PagerArgs::command().name("nemesis");
    if shell != Shell::Zsh {
        generate(shell, &mut cmd, "nemesis", &mut std::io::stdout());
        return;
    }
    // zsh needs post-processing (see fix_zsh_root_prompt_positional).
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "nemesis", &mut buf);
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("nemesis");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "nemesis", &mut buf);

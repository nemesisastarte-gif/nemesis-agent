...
//!
//! Business logic lives in `xai_nemesis_shell::plugin` (shared orchestration)
//! and lower crates (`xai-nemesis-agent`, `xai-nemesis-plugin-marketplace`). This
...
use xai_nemesis_agent::plugins::install_registry::{InstallKind, InstallRegistry};
use xai_nemesis_agent::plugins::manifest::{ManifestLoadResult, PluginManifest, load_manifest};
use xai_nemesis_plugin_marketplace::SourceKind;
use xai_nemesis_plugin_marketplace::git::SourceCacheLease;
use xai_nemesis_shell::plugin::{self, RepoUpdateOutcome, UninstallError};
...
fn installed_plugins(
    repos: &[(
        &str,
        &xai_nemesis_agent::plugins::install_registry::InstalledRepo,
    )],
) -> Vec<PluginEntry> {
    repos
        .iter()
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    let config = xai_nemesis_shell::config::load_effective_config()
        .ok()
        .unwrap_or(toml::Value::Table(toml::map::Map::new()));
    let mut sources = xai_nemesis_plugin_marketplace::load_sources(&config);
    sources.extend(xai_nemesis_plugin_marketplace::load_extra_sources_from_settings(&sources));
    ...
    for source in &sources {
        ...
        for plugin in xai_nemesis_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                xai_nemesis_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &xai_nemesis_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = xai_nemesis_plugin_marketplace::git::default_cache_root();
            xai_nemesis_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
...
fn install_kind(is_git: bool) -> xai_nemesis_telemetry::events::InstallKind {
    if is_git {
        xai_nemesis_telemetry::events::InstallKind::Git
    } else {
        xai_nemesis_telemetry::events::InstallKind::Local
    }
}

fn log_plugin_installed(
    install_kind: xai_nemesis_telemetry::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    xai_nemesis_telemetry::session_ctx::log_event(xai_nemesis_telemetry::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    if let Some(mref) = xai_nemesis_plugin_marketplace::install_resolve::parse_marketplace_ref(source)
    {
    ...
    if !trust {
        use xai_nemesis_agent::plugins::git_install::{self, InstallSource};
...
fn cmd_install_marketplace(
    source: &str,
    mref: &xai_nemesis_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
    ...
    match plugin::uninstall_plugin(name, confirm, keep_data) {
        Ok(outcome) => {
            xai_nemesis_telemetry::session_ctx::log_event(
                xai_nemesis_telemetry::events::PluginUninstalled {
                    confirmed: true,
                    success: true,
                },
            );
...
fn cmd_enable(name: &str) -> Result<()> {
    ...
    if let Err(e) = xai_nemesis_shell::config::remove_disabled_plugin(name) {
        tracing::warn!("failed to remove from disabled list: {e}");
    }
    xai_nemesis_shell::config::add_enabled_plugin(name)
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    if let Err(e) = xai_nemesis_shell::config::remove_enabled_plugin(name) {
        tracing::warn!("failed to remove from enabled list: {e}");
    }
    xai_nemesis_shell::config::add_disabled_plugin(name)
...
fn marketplace_list(
    sources: &[xai_nemesis_plugin_marketplace::MarketplaceSource],
    json: bool,
) -> Result<()> {
    if json {
...
fn marketplace_add(
    ...
    use xai_nemesis_shell::plugin::MarketplaceAddInput;
...
fn marketplace_remove(
    ...
    let local_input = match plugin::classify_marketplace_add_input(url, &cwd) {
        xai_nemesis_shell::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &xai_nemesis_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match xai_nemesis_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    use super::*;
    use xai_nemesis_plugin_marketplace::MarketplaceSource;
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir = xai_nemesis_plugin_marketplace::git::sync_source_cache(
            &url,
            Some("main"),
            cache_root.path(),
        )
    ...
    fn current_head(repo: &Path) -> String {
        let output = xai_nemesis_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();

...
use xai_nemesis_shell::session::repo_changes::UploadMethod;
...
pub struct TraceArgs {
    ...
    #[arg(long)]
    pub local: bool,
    /// Output path (default: $NEMESIS_HOME/trace-exports/<session-id>.tar.gz)
    #[arg(short, long)]
...
pub(crate) fn find_session_dir(session_id: &str) -> Result<PathBuf> {
    xai_nemesis_shell::session::persistence::find_session_dir_by_id(session_id).with_context(|| {
        format!(
...
pub fn trace_exports_dir() -> PathBuf {
    nemesis_home().join("trace-exports")
}
...
async fn run_upload(
    ...
    let upload_method = match upload_method {
        ...
        None => {
            ...
            anyhow::bail!(
                "No upload credentials. Run `nemesis login` or set a deployment key. \
    ...
    if bucket_url.is_none()
        && matches!(
            upload_method,
            xai_nemesis_shell::session::repo_changes::UploadMethod::Direct { .. }
        )
    {
        anyhow::bail!(
    ...
    let upload_config = xai_nemesis_shell::session::repo_changes::TraceExportConfig {
...
async fn upload_with_retries(
    config: &xai_nemesis_shell::session::repo_changes::TraceExportConfig,
    object_path: &str,
    archive: &[u8],
...
pub async fn resolve_upload_method(agent_config: &AgentConfig) -> Option<UploadMethod> {
    ...
    let auth_token = xai_nemesis_shell::auth::ensure_authenticated_or_noninteractive(

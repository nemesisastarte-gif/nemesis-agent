...
impl Default for PagerLocalSnapshot {
    fn default() -> Self {
        Self {
            ...
            voice_stt_language: xai_nemesis_voice::STT_LANGUAGE_DEFAULT.to_string(),
        }
    }
}
...
///
/// Delegates to [`xai_nemesis_voice::canonicalize_stt_language`] so the pager and
...
pub fn canonical_voice_stt_language(value: Option<&str>) -> &'static str {
    xai_nemesis_voice::canonicalize_stt_language(value)
}
...
pub fn current_value_for(
    ...
    match key {
        ...
        "fork_secondary_model" => Some(SettingValue::String({
            let baseline = xai_nemesis_shell::models::default_model();
...
mod tests {
            ...
            match (meta.key, &meta.kind) {
                    ...
                    assert_eq!(
                        *default,
                        ...
                        "toolset.ask_user_question.timeout_enabled default drifts from the \
                         shared resolver const in xai-nemesis-tools"
                    );
                }
                ...
                ("fork_secondary_model", SettingKind::DynamicEnum { default, .. }) => {
                    ...
                    assert_eq!(
                        ...
                        xai_nemesis_shell::models::default_model(),
    ...
    /// Spot-check the delegation to `xai_nemesis_voice::canonicalize_stt_language`
    ...
    fn voice_stt_language_settings_match_voice_crate_catalog() {
        ...
        for c in choices.iter() {
            ...
            let lang = xai_nemesis_voice::stt_language_by_code(c.canonical)
        ...
        let crate_codes: HashSet<&str> = xai_nemesis_voice::STT_LANGUAGES
            .iter()
            .map(|l| l.code)
            .collect();
        assert_eq!(
            setting_codes, crate_codes,
            "settings concrete languages must match xai_nemesis_voice::STT_LANGUAGES exactly"
        );
    }
    ...
    fn contextual_hints_group_and_children_registered() {
        ...
        let ui = UiConfig {
            contextual_hints: xai_nemesis_shell::agent::config::ContextualHints {
                undo: Some(false),
                ..Default::default()
            },
            ..UiConfig::default()

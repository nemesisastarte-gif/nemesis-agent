...
impl TitleManager {
    ...
    pub fn update(&mut self, state: &TitleState<'_>) -> Option<String> {
        ...
        if !has_parts {
            self.composed.clear();
            self.composed.push_str("nemesis");
        }
    ...
    pub fn reset(&mut self) -> String {
        let esc = build_title_escape("nemesis");
        self.last_title.clear();
        self.last_title.push_str("nemesis");
...
mod tests {
    ...
    fn grok_only_produces_just_grok() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "nemesis");
    }
    ...
    fn reset_clears_state_and_emits_grok() {
        ...
        mgr.update(&state);
        assert_ne!(mgr.last_title, "nemesis");
    ...
    fn default_config_active_turn_with_permissions() {
        ...
        for t in [&t1, &t2] {
            assert!(t.contains("nemesis"), "title missing 'grok': {t}");

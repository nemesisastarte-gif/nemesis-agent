...
//! render mode.
//!
//! `xai-nemesis-pager` cannot depend on `xai-nemesis-pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`xai-nemesis-pager-bin`) wires it up once at
//! startup by calling `xai_nemesis_pager_minimal::install()`. When nothing has
...
/// (`xai_nemesis_pager_minimal::draw`).

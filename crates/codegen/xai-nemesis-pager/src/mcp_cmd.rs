...
Examples:
  ...
  # Add to the project config (./.nemesis/config.toml) instead of ~/.nemesis/config.toml
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.nemesis/config.toml`, available in all your projects
    User,
    /// `./.nemesis/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// Config to write to: user (~/.nemesis/config.toml) or project (./.nemesis/config.toml)
...
fn run_list(json: bool) -> Result<()> {
    ...
    let servers = xai_nemesis_shell::util::config::load_mcp_server_configs_with_project(&cwd);
...
async fn run_add(args: AddArgs) -> Result<()> {
    ...
    xai_nemesis_shell::util::config::save_mcp_server_config_at(&path, name, &config).await?;
...
fn scope_target(scope: McpScope) -> PathBuf {
    match scope {
        McpScope::User => xai_nemesis_shell::util::config::user_config_path(),
        McpScope::Project => {
            xai_nemesis_shell::util::config::project_config_path(&current_dir_or_exit())
        }
    }
}
...
fn select_remove_site(
    ...
    use xai_nemesis_shell::util::config::user_config_path;
    ...
    project_site
        ...
        .or_else(|| {
            user_defined.then(|| {
                (
                    McpScope::User,
                    xai_nemesis_shell::util::config::user_config_path(),
                )
            })
        })
}
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let find_project_site = || {
        xai_nemesis_shell::config::find_project_configs(&cwd)
            .into_iter()
            .rev()
    ...
    // ancestor .nemesis/config.toml, where it still resolves for sessions.
...
async fn run_doctor(json: bool, name: Option<String>) -> Result<()> {
    let cwd = current_dir_or_exit();
    let report = xai_nemesis_shell::mcp_doctor::run_doctor(&cwd, name.as_deref()).await;
    ...
    if json {
        ...
        xai_nemesis_shell::mcp_doctor::print_report(&report);
    }
...
mod tests {
    ...
    fn add_hyphen_flag_without_double_dash_is_rejected() {
        let err = PagerArgs::try_parse_from(["nemesis", "mcp", "add", "fs", "npx", "-y"])
    ...
    fn add_default_transport_warns_on_url_looking_command() {
        let add = parse_add(&["nemesis", "mcp", "add", "api", "https://mcp.example.com/mcp"]);
        ...
        let add = parse_add(&["nemesis", "mcp", "add", "local", "localhost:3000"]);
    ...
    fn add_scope_project_parses_and_invalid_scope_is_rejected() {
        let add = parse_add(&[
            "nemesis", "mcp", "add", "-s", "project", "fs", "--", "npx", "pkg",
        ]);
        assert_eq!(add.scope, McpScope::Project);

        let err = PagerArgs::try_parse_from([
            "nemesis", "mcp", "add", "-s", "local", "fs", "--", "npx", "pkg",
        ])
    ...
    fn add_legacy_multi_value_env_is_rejected() {
        ...
        let add = parse_add(&[
            "nemesis", "mcp", "add", "pg", "--env", "A=1", "B=2", "--", "npx", "-y", "server",
        ]);
    ...
    fn add_validates_name_env_and_headers() {
        let add = parse_add(&["nemesis", "mcp", "add", "fs", "-e", "NOT_A_PAIR", "--", "npx"]);
        ...
        let add = parse_add(&["nemesis", "mcp", "add", "bad name!", "--", "npx"]);
    ...
    fn add_rejects_mismatched_options_per_transport() {
        let add = parse_add(&["nemesis", "mcp", "add", "fs", "-H", "X: y", "--", "npx"]);
    ...
    fn add_requires_a_source_for_each_transport() {
        let add = parse_add(&["nemesis", "mcp", "add", "fs"]);
        ...
        let add = parse_add(&["nemesis", "mcp", "add", "--transport", "http", "api"]);
    ...
    fn remove_accepts_optional_scope() {
        let args = PagerArgs::try_parse_from(["nemesis", "mcp", "remove", "fs", "-s", "project"])
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        let user = xai_nemesis_shell::util::config::user_config_path();
        let project = PathBuf::from("/repo/.nemesis/config.toml");

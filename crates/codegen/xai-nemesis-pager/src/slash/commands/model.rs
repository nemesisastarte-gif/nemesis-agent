...
use xai_nemesis_shell::sampling::types::supports_reasoning_effort_meta;
...
impl SlashCommand for ModelCommand {
    ...
    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        // first, a shorter catalog entry ("Nemesis") would steal the prefix and
...
mod tests {
    ...
    fn run_prefers_full_multi_word_model_name_over_prefix_plus_effort() {
        // Catalog has both "Nemesis" (reasoning) and "Grok 4.5". `/model Grok 4.5`
        // must select the full name, not treat "4.5" as an effort on "Nemesis".
        let mut state = ModelState::default();
        let (short_id, short_info) = model_with_reasoning("nemesis", "Nemesis");

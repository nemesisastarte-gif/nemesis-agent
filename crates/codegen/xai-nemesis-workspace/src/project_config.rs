...
//! `.nemesis/config.toml` files by walking from `cwd` up to the git root.
//!
...
use xai_nemesis_agent::repo::RepoDirChain;
...
/// True when `config_path` is `$NEMESIS_HOME/config.toml` (user tier, not project).
fn is_user_grok_config_file(config_path: &Path) -> bool {
    let Some(user_home) = xai_nemesis_config::user_nemesis_home() else {
        return false;
    };
...
/// Find all `.nemesis/config.toml` files from `cwd` upward to the git repo root.
...
///
/// If no git repo is found, only checks `cwd/.nemesis/config.toml`. Excludes the
/// user-global config so `cwd == $HOME` does not treat `~/.nemesis/config.toml` as
/// a project overlay.
...
/// `cwd == $HOME` does not treat `~/.nemesis/config.toml` as a project overlay.
/// `pub(crate)` — the gate (`repo_configs_present`) reaches it within this crate.
pub(crate) fn find_project_configs_in(chain_dirs: &[PathBuf]) -> Vec<PathBuf> {
    ...
    chain_dirs
        .iter()
        .rev()
        .map(|dir| dir.join(".nemesis").join("config.toml"))
...
mod tests {
    ...
    fn find_project_configs_excludes_user_grok_config_file() {
        ...
        std::fs::create_dir_all(project.join(".nemesis")).unwrap();
        std::fs::write(project.join(".nemesis/config.toml"), "# project\n").unwrap();

...
//!
//! Reads OIDC credentials from `~/.nemesis/auth.json`, connects to a
//! server, exposes workspace tools, and refreshes tokens
//! automatically.
use clap::Parser;
use std::path::PathBuf;
use url::Url;
use xai_nemesis_workspace::config::WorkspaceServerMetadata;
use xai_nemesis_workspace::daemonize;
use xai_nemesis_workspace::diag_server;
use xai_nemesis_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
struct Args {
    ...
    /// `NEMESIS_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "NEMESIS_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Trust project-scoped LSP servers from `<repo>/.nemesis/lsp.json`.
    /// Defaults off; sandbox opts in only after workspace trust is established.
    #[arg(
        long,
        env = "NEMESIS_WORKSPACE_PROJECT_LSP_TRUSTED",
    ...
    /// Override with `NEMESIS_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "NEMESIS_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
/// `/usr/local/bin/xai-nemesis-preview-proxy` child (see `cli.rs` for the proxy's
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        ...
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("NEMESIS_WORKSPACE_OTLP_ENDPOINT") {
    ...
    {
        use xai_nemesis_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("GROK_SANDBOX_PROFILE").ok() {
            ...
            None if xai_nemesis_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            ...
            sandbox.install();
            let active = xai_nemesis_sandbox::is_active();
            ...
            tracing::info!(
                ...
                xai_nemesis_sandbox::should_restrict_child_network(), "{status_msg}"
            );
        }
    }
    let auth_provider = xai_nemesis_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    ...
    let status_config = xai_nemesis_workspace::StatusConfig::from_env();
    ...
    xai_nemesis_workspace::init_metrics();
    let ws_handle = xai_nemesis_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = xai_nemesis_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            xai_nemesis_workspace::handle::DrainReason::Sigterm,
        )
        .await;
...
mod tests {
    ...
    fn project_lsp_trust_defaults_off_and_is_opt_in() {
        unsafe { std::env::remove_var("NEMESIS_WORKSPACE_PROJECT_LSP_TRUSTED") };
        let args = Args::try_parse_from(["xai-workspace-server"]).unwrap();

//! Hub [`AuthProvider`] from `~/.nemesis/auth.json` for the standalone
...
fn default_auth_path() -> anyhow::Result<PathBuf> {
    let grok = xai_nemesis_config::user_nemesis_home()
        .ok_or_else(|| anyhow::anyhow!("no user grok home (set $NEMESIS_HOME or $HOME)"))?;
...
fn read_auth_entry(path: &Path) -> anyhow::Result<(String, AuthEntry)> {
    if !path.exists() {
        anyhow::bail!(
            "No auth credentials found at {}. Run `nemesis login` first.",
            path.display()
        );
    }
    ...
    entries
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "no OIDC auth entry found in {}. Run `nemesis login` first.",
                path.display()
            )
        })
}
...
/// the default credential path (`~/.nemesis/auth.json`).
pub fn provider(
    hub_url: &Url,

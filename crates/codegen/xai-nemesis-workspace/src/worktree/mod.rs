...
//!
//! Moved from `xai-nemesis-shell/src/session/worktree.rs` into the workspace
...
// Canonical in xai-nemesis-workspace-types; re-exported for existing paths.
...
/// `worktrees.db` (`xai_fast_worktree::resolve_nemesis_home`), so checkout dirs and
/// the metadata DB always live under the same `.grok` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `xai_nemesis_config::nemesis_home()`,
/// so worktree paths also agree with trust/hooks and other grok-home paths.
fn nemesis_home() -> std::path::PathBuf {
    xai_fast_worktree::resolve_nemesis_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
/// Returns `~/.nemesis/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    nemesis_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.nemesis/worktrees/<repo_name>`)
/// for a given source path, correctly handling grok-managed worktrees.
///
/// When `source_path` is already under `~/.nemesis/worktrees/<repo>/...`, the
...
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = nemesis_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.nemesis/worktrees/`) to its registered
/// worktree record.
///
...
/// returns `None` (without DB I/O) for paths outside `~/.nemesis/worktrees/`.
...
///
/// When the source path is already inside `~/.nemesis/worktrees/<repo>/`, the
...
use xai_fast_worktree::{
    ...
    gc_worktrees as fw_gc_worktrees, rebuild_worktree_db, resolve_nemesis_home,
};
...
/// Scan `~/.nemesis/worktrees/<repo_name>/` for subdirectories not tracked
...
mod tests {
    ...
    /// Point `NEMESIS_HOME` at an isolated tempdir (`resolve_nemesis_home` re-reads
    ...
    /// and restores `NEMESIS_HOME` on drop (before releasing the lock), so the

...
//! [`crate::trust::TrustStore`] (`~/.nemesis/trusted_folders.toml`). The
//! consume/gating half (the `DECISIONS` cache, `resolve_and_record`,
//! `project_scope_allowed`, the loader filters) lives in `xai-nemesis-shell`.
//!
...
//! provisional and re-checked rather than cached — is a `xai-nemesis-shell`
...
use xai_nemesis_config_types::{BoolFlag, RemoteSettings};
...
///
/// Kept local (not in `xai-nemesis-version`) on purpose: adding a symbol to that
...
fn is_local_build() -> bool {
    ...
    if std::env::var(xai_nemesis_version::TEST_VERSION_ENV).is_ok() {
        return false;
    }
...
fn feature_enabled_for_build(remote: Option<&RemoteSettings>, is_local_build: bool) -> bool {
    ...
    let user = xai_nemesis_config::load_from_disk().ok();
    let managed = xai_nemesis_config::load_managed_config().ok();
...
/// same `~/.nemesis/trusted_folders.toml`. Best-effort; a write failure is logged,
/// not fatal.
...
/// `[plugins].paths` / `.claude` / `.nemesis/agents` / subdir-launch gaps. NOT
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    let chain = xai_nemesis_agent::repo::RepoDirChain::resolve(cwd);
    ...
    // Project `.nemesis/config.toml` declaring repo-controlled code-exec: a
    ...
    for path in crate::project_config::find_project_configs_in(&chain.dirs) {
        let Ok(root) = xai_nemesis_config::load_config_file(&path) else {
            continue;
        };
    ...
    // Project `.nemesis/lsp.json`.
    if cwd.join(".nemesis").join("lsp.json").is_file() {
        hit!("lsp");
    }
    ...
    // must be gated — else a hooks-only clone (e.g. `.nemesis/hooks/evil.json`) would
    ...
    if path_present_or_uncertain(&hook_root.join(".nemesis").join("hooks"))
    ...
    // that must be gated — else a plugin clone (e.g. `.nemesis/plugins/evil/`, even
    ...
    if !xai_nemesis_agent::plugins::project_plugin_dirs_in(&chain.dirs).is_empty() {
        hit!("plugins");
    }
    // Project AGENT dirs (`.nemesis/agents` / `.claude/agents`): a project agent
    ...
    if !xai_nemesis_agent::discovery::project_agent_dirs_in(&chain.dirs).is_empty() {
        hit!("agents");
    }
    // Presence matches exact-cwd discovery without parsing repository content.
    let grok = cwd.join(".nemesis");
...
pub fn prompt_for_trust(key: &Path) -> bool {
    ...
    let _ = writeln!(
        err,
        "This folder contains repo-local config (.mcp.json / .nemesis/lsp.json / hooks) \
...
mod tests {
    ...
    fn repo_configs_present_detects_project_agents() {
        // A `.nemesis/agents`-only clone must be gated: a project agent definition
        // can carry an inline `hooks:` block (code-exec) and can shadow a built-in
        // subagent by name.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".nemesis").join("agents")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_roles() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".nemesis").join("roles")).unwrap();
    ...
    fn repo_configs_present_detects_project_personas() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".nemesis").join("personas")).unwrap();
    ...
    fn project_subagent_marker_at_repo_root_is_absent_from_subdir() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".nemesis/roles")).unwrap();
    ...
    fn repo_configs_present_detects_project_plugins() {
        ...
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".nemesis").join("plugins").join("x")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_plugins_in_subdir() {
        ...
        std::fs::create_dir_all(subdir.join(".nemesis").join("plugins").join("evil")).unwrap();
        assert!(repo_configs_present(&subdir));
    }
    ...
    fn repo_config_kinds_matches_gate_and_reports_all_kinds() {
        ...
        // `.nemesis/agents` — even when launched from a SUBDIR (the cwd→git-root walk
    ...
    // NEMESIS_HOME-isolation idiom mirrored from this crate's `permission::claude_compat`
    // tests (the workspace crate has no `serial_test`/`xai-nemesis-test-support`
    ...
    fn simulate_release_build() -> EnvVarGuard {
        EnvVarGuard::set(xai_nemesis_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"))
    }
    ...
    fn local_build_ignores_remote_rollout() {
        ...
        let _home = EnvVarGuard::set("NEMESIS_HOME", home.path());
    ...
    fn release_build_keeps_gate_when_enabled() {
        ...
        // config nor an ambient env flag can override it: empty NEMESIS_HOME (no
        // config.toml/managed_config.toml) + GROK_FOLDER_TRUST unset. nextest's
        // process-per-test makes nemesis_home()'s OnceLock pick up the temp dir.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("NEMESIS_HOME", home.path());
        let _flag = EnvVarGuard::unset("GROK_FOLDER_TRUST");
    ...
    fn local_build_ignores_explicit_env_optin() {
        ...
        // never prompts). NEMESIS_HOME isolated so on-disk config can't influence it.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("NEMESIS_HOME", home.path());
    ...
    fn release_build_defaults_on() {
        ...
        // the feature ON. Empty NEMESIS_HOME (no config.toml/managed config) +
    ...
    fn is_local_build_honors_test_version_override() {
        ...
        {
            let _sim = EnvVarGuard::set(xai_nemesis_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"));
        ...
        let _unset = EnvVarGuard::unset(xai_nemesis_version::TEST_VERSION_ENV);
    ...
    fn store_io_is_noop_on_local_build() {
        ...
        // (mirrors `is_local_build_honors_test_version_override`); NEMESIS_HOME-isolated
    ...
    fn revoke_folder_trust_store_persists_untrust_for_trusted_folder() {
        ...
        // NEMESIS_HOME-isolated so the seed/deny hit a temp store, not the real file.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("NEMESIS_HOME", home.path());
        let _sim = simulate_release_build();
        let tmp = repo_tmp();
        let key = workspace_key(tmp.path());
    ...
    fn revoke_folder_trust_store_writes_no_deny_for_never_trusted_folder() {
        ...
        // NEMESIS_HOME-isolated so the grant writes to a temp store.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("NEMESIS_HOME", home.path());
        let _sim = simulate_release_build();

...
//!
//! Persists per-folder trust decisions to `~/.nemesis/trusted_folders.toml`.
...
//!
//! The store is rooted at [`xai_nemesis_config::user_nemesis_home`] — the **Option**
//! home that resolves to `None` (rather than a cwd-relative `./.grok`) when
//! neither `$NEMESIS_HOME` nor a home directory is set (e.g. a minimal container /
...
//! a `./.nemesis/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.nemesis/`.
...
impl TrustStore {
    /// Load the trust store from `<user_nemesis_home>/trusted_folders.toml`.
    ///
    ...
    /// Default on-disk path: `<user_nemesis_home>/trusted_folders.toml`, or `None`
    /// when no user home resolves.
    ///
    /// Resolves via [`xai_nemesis_config::user_nemesis_home`], never
    /// [`xai_nemesis_config::nemesis_home`], so it never falls back to a cwd-relative
    ...
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(xai_nemesis_config::user_nemesis_home())
    }
    ...
    fn default_path_in(user_nemesis_home: Option<PathBuf>) -> Option<PathBuf> {
        Some(user_nemesis_home?.join(TRUST_FILE_NAME))
    }
...
/// ROOT (via the `~/.nemesis/worktrees.db` registry), so every `grok -w` worktree
...
/// sharing `~/.nemesis/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.nemesis/trusted-hook-projects` file listed one canonical project
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = xai_nemesis_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match xai_nemesis_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        // cwd-relative `./.grok` that nemesis_home() would invent, which is exactly
        // how a cloned repo's own `<repo>/.nemesis/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
        assert_eq!(TrustStore::default_path_in(None), None);
    }

    #[test]
    fn default_path_sources_from_user_nemesis_home() {
        // Thin source-pin: the production accessor reads user_nemesis_home()
        // (Option, no cwd fallback), not nemesis_home(). The real regression guard
        // is the seam test above (default_path_in(None) == None).
        assert_eq!(
            TrustStore::default_path(),
            xai_nemesis_config::user_nemesis_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
    ...
    /// Point `NEMESIS_HOME` at an isolated tempdir and register one grok-managed
    ...
    /// dir)`; the [`LockedTestEnv`] holds the lock and restores `NEMESIS_HOME` on
    ...
    fn register_grok_worktree(
        ...
        let env = LockedTestEnv::lock().set("NEMESIS_HOME", &home);
    ...
    fn workspace_key_ignores_registry_for_cwd_outside_worktrees_dir() {
        ...
        // `<nemesis_home>/worktrees`: `worktree_record_for_cwd` skips the registry
        ...
        assert_eq!(
            ...
            "a cwd outside <nemesis_home>/worktrees keys on its own repo root"
        );
        assert_ne!(
            key,

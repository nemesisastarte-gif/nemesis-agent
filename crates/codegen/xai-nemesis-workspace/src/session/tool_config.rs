...
use std::sync::Arc;
use xai_nemesis_tools::registry::types::{
...
pub(crate) fn resolve_session_toolset(
    ...
    notification_handle: Option<xai_nemesis_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<xai_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<xai_nemesis_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn xai_nemesis_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            xai_nemesis_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// enabled (`NEMESIS_WORKSPACE_TOOL_STATE_ENABLED=true`; any other value keeps
/// legacy behavior).
pub fn tool_state_enabled() -> bool {
    std::env::var("NEMESIS_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
/// `NEMESIS_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
/// [`LocalTerminalBackend`]: xai_nemesis_tools::computer::local::LocalTerminalBackend
pub struct WorkspaceSessionContextFactory {
    auth: Option<xai_computer_hub_sdk::SharedAuthProvider>,
    api_base_url: Option<String>,
    /// Resolved `$NEMESIS_WORKSPACE_HOME` when tool-state persistence is enabled;
...
impl WorkspaceSessionContextFactory {
    ...
    /// (`$NEMESIS_WORKSPACE_HOME`). Callers should only invoke this when
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(xai_nemesis_tools::computer::local::LocalFs)
            as Arc<dyn xai_nemesis_tools::computer::types::AsyncFileSystem>;
        let notification_handle = xai_nemesis_tools::notification::ToolNotificationHandle::noop();
    ...
    fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
        crate::config::SessionTerminalBackend::local(
            xai_nemesis_tools::computer::local::LocalTerminalBackend::new(),
        )
    }
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    let version = xai_nemesis_version::VERSION;
    headers.insert(
        "user-agent".to_string(),
        format!("xai-nemesis-workspace/{version}"),
    );
    ...
    if base_url.contains("cli-chat-proxy") || base_url.contains("chat-proxy") {
        headers.insert("X-XAI-Token-Auth".to_string(), "xai-nemesis-cli".to_string());
        headers.insert(
...
/// `NEMESIS_DISABLE_WEB_FETCH=1` is set.
fn build_web_fetch_config() -> xai_nemesis_tools::implementations::grok_build::web_fetch::WebFetchConfig
{
    use xai_nemesis_tools::implementations::grok_build::web_fetch::{WebFetchConfig, WebFetchParams};
    if std::env::var("NEMESIS_DISABLE_WEB_FETCH").is_ok_and(|v| v == "1" || v == "true") {
    ...
    if xai_nemesis_config::env_bool("GROK_WEB_FETCH_ALLOW_LOCAL") == Some(true) {
...
mod tests {
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "NEMESIS_WORKSPACE_TOOL_STATE_ENABLED";
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use xai_nemesis_tools::types::resources::{State, WebCitationCounter};

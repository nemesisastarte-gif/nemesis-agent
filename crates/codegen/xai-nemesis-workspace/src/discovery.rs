...
//! `xai-nemesis-agent` and `xai-nemesis-tools` rather than duplicating them.
...
//!
//! Re-exports [`AgentsMdTracker`] from `xai-nemesis-tools` for external
...
// workspace crate without a direct xai-nemesis-tools dependency.
pub use xai_nemesis_tools::types::agents_md_tracker::AgentsMdTracker;

// Re-export the config types that callers pass into WorkspaceConfig.
pub use xai_nemesis_agent::plugins::discovery::DiscoveryConfig as PluginDiscoveryConfig;
pub use xai_nemesis_agent::plugins::trust::TrustStore as PluginTrustStore;
pub use xai_nemesis_agent::prompt::skills::SkillsConfig;
...
///
/// Delegates to [`xai_nemesis_agent::prompt::skills::list_skills`] with
...
pub async fn discover_skills(root_cwd: &Path, config: &SkillsConfig) -> Vec<Value> {
    ...
    let skills = xai_nemesis_agent::prompt::skills::list_skills(
        Some(&cwd_str),
        config,
...
pub async fn discover_agents_md(root_cwd: &Path) -> Vec<Value> {
    let cwd_str = root_cwd.to_string_lossy();
    let files = xai_nemesis_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd_str,
...
///
/// Delegates to [`xai_nemesis_agent::plugins::discover_plugins`] with
...
pub fn discover_plugins(
    ...
    let discovered = xai_nemesis_agent::plugins::discover_plugins(
        Some(root_cwd),
        config,
        trust_store,
        project_trusted,
...
/// Load the project config from `<root_cwd>/.nemesis/config.toml`.
///
...
pub fn load_project_config(root_cwd: &Path) -> Value {
    let config_path = root_cwd.join(".nemesis").join("config.toml");
    match xai_nemesis_config::load_config_file(&config_path) {
...
mod tests {
    ...
    // `~/.nemesis/skills/`, so on a developer machine the result may be
    ...
    async fn discover_skills_finds_skill_md() {
        ...
        let skills_dir = tmp.path().join(".nemesis").join("skills").join("my-skill");
    ...
    async fn discover_skills_respects_ignore_config() {
        let tmp = tempfile::tempdir().unwrap();
        let skills_dir = tmp.path().join(".nemesis").join("skills").join("ignored");
        fs::create_dir_all(&skills_dir).unwrap();
        fs::write(
            skills_dir.join("SKILL.md"),
            "---\nname: ignored\ndescription: Ignored\n---\n",
    ...
    fn agent_config_file_wire_matches_workspace_types_mirror() {
        ...
        // break discovery.
        let src = xai_nemesis_agent::prompt::agents_md::AgentConfigFile {
            file_name: "AGENTS.md".to_string(),
            file_path: "/repo/AGENTS.md".to_string(),
            content: "# Instructions\n".to_string(),
        };
        let json = serde_json::to_value(&src).unwrap();
        let mirror: xai_nemesis_workspace_types::rpc::agents_md::AgentConfigFile =
    ...
    // from `~/.nemesis/plugins/`, so tests check for specific plugins.

    #[test]
    fn discover_plugins_finds_manifest_plugin() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join(".nemesis").join("plugins").join("test-plugin");
    ...
    fn discover_plugins_json_has_expected_fields() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join(".nemesis").join("plugins").join("field-test");
        fs::create_dir_all(plugins_dir.join("skills")).unwrap();
        fs::write(
            plugins_dir.join("plugin.json"),

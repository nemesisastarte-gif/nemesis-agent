...
use xai_nemesis_tools::implementations::grok_build::web_fetch::domain_from_url;
...
///        `~/.nemesis/config.toml` via the `Effect::PersistPermissionMode` effect
...
/// Canonical definition lives in `xai_nemesis_workspace_types` (so both the
/// permission-validation layer and the MCP transport in `xai-nemesis-mcp` can
...
/// reached `xai_nemesis_workspace::permission::MCP_TOOL_NAME_DELIMITER`.

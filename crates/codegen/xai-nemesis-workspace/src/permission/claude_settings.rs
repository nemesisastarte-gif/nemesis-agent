...
// permissions), consider extracting to a shared helper (e.g., in xai-nemesis-hooks
...
// =============================================================================
//
// `xai-nemesis-shell::claude_import` writes the marker. We re-implement a small
...
pub fn is_claude_import_marked() -> bool {
    ...
    // lives in xai-nemesis-shell (inaccessible from here at runtime). They also
    ...
    let Some(config_path) = xai_nemesis_config::user_nemesis_home().map(|g| g.join("config.toml")) else {
        return false;
    };

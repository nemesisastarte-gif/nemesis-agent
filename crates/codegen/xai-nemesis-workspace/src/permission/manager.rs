...
use xai_nemesis_mcp::servers::parse_mcp_qualified_name;
use xai_nemesis_paths::AbsPathBuf;
use xai_nemesis_tools::implementations::grok_build::web_fetch::{
    DomainMatcher, domain::normalize_domain,
};
use xai_nemesis_tools::types::resources::resolve_model_path;
...
fn permission_mode_artifact_str(mode: xai_nemesis_telemetry::enums::PermissionMode) -> &'static str {
    use xai_nemesis_telemetry::enums::PermissionMode;
    match mode {
...
fn spawn_permission_manager_with_pin(
    ...
    let _task = tokio::task::spawn_local(async move {
        ...
        // persist it to ~/.nemesis/sessions/<cwd>/permission.toml. This caused
                ...
                PermissionCommand::Request {
                    ...
                    if matches!(&access, AccessKind::Bash(_))
                        && sandbox_may_auto_allow_bash(
                            bash_evaluation.as_ref(),
                            xai_nemesis_sandbox::should_auto_allow_bash(),
                        )
                        && !policy_forced_prompt
                        && !auto_forced_prompt
                    {

//! Permission resolution engine: merges native `.nemesis/config.toml`,
...
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    xai_nemesis_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Find every `<dir>/.nemesis/config.toml` from `cwd` upward to the git repo
/// root (or just `<cwd>/.nemesis/config.toml` when there is no git repo).
///
/// Returned paths are ordered from repo root (lowest priority) to `cwd`
/// (highest priority), matching `xai-nemesis-shell::config::find_project_configs`.
fn find_project_grok_configs(cwd: &Path) -> Vec<PathBuf> {
    ...
    if let Some(ref root) = git_root {
        ...
        while let Some(dir) = current {
            let p = dir.join(".nemesis").join("config.toml");
            if p.is_file() {
                configs.push(p);
            }
            if dir == *root {
        ...
        let p = cwd.join(".nemesis").join("config.toml");
        if p.is_file() {
            configs.push(p);
        }
    }
...
///
///   * `~/.nemesis/config.toml` (lowest priority)
///   * Each `.nemesis/config.toml` from the git repo root down to `cwd`
...
fn load_config_toml_permissions(cwd: &Path) -> Vec<Sourced<PermissionRule>> {
    ...
    // Global `~/.nemesis/config.toml` first (lowest priority within this layer).
    // Gated on user_nemesis_home() so a project's .nemesis/config.toml is never read as
    // global permissions when neither NEMESIS_HOME nor a home dir resolves.
    if let Some(global_path) = xai_nemesis_config::user_nemesis_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match xai_nemesis_config::load_config_file(&global_path) {
    ...
    for path in find_project_grok_configs(cwd) {
        match xai_nemesis_config::load_config_file(&path) {
            Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[xai_nemesis_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
    ...
    fn live() -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(
                &xai_nemesis_config::managed_config_layers(),
            ),
        }
    }
}
...
/// [`xai_nemesis_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = xai_nemesis_config::claude_managed_settings_path() else {
...
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = xai_nemesis_config::requirements_layers();
...
mod tests {
    ...
    // Shared so `NEMESIS_HOME`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // NEMESIS_HOME-isolate so the claude-import marker reads clean (an imported
        ...
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("NEMESIS_HOME", home.path());
    ...
    fn load_claude_env_empty_when_no_settings() {
        // Isolate NEMESIS_HOME (claude-import marker) AND HOME (global `~/.claude`)
    ...
    fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
        ...
        // drop it. Isolate NEMESIS_HOME so the claude-import marker reads clean (an
...
if name == "internal-only"),
    ...
    fn admin_source_trusts_only_root_owned_tiers() {
        ...
        // the user-writable `~/.nemesis/requirements.toml` is not, despite its path.
        ...
        assert!(!is_admin_source(&RequirementSource::Requirements {
            path: "/home/u/.nemesis/requirements.toml".into(),
        }));
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers =
            xai_nemesis_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
        ...
        .unwrap();
        assert_eq!(
            xai_nemesis_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );

...
//!
//! All values are read once at startup from `NEMESIS_WORKSPACE_*` environment
...
// documented fallbacks for each `NEMESIS_WORKSPACE_*` env var.
...
pub struct StatusConfig {
    ...
    /// Legacy single-phase drain timeout (`NEMESIS_WORKSPACE_DRAIN_TIMEOUT_SECS`),
    /// retained for compatibility; the SIGTERM and server-evict paths now use the
    /// two-phase drain bounded by `NEMESIS_WORKSPACE_TERMINATION_GRACE_MS`.
    ...
    /// Opt-in foreground-only idle (`NEMESIS_WORKSPACE_IDLE_IGNORE_BACKGROUND_TASKS`);
    ...
    /// (`NEMESIS_WORKSPACE_PREVIEW_ACTIVITY_WINDOW_MS`).
    pub preview_activity_window: Duration,
    /// Cadence at which the preview-activity scraper polls the proxy
    /// (`NEMESIS_WORKSPACE_PREVIEW_ACTIVITY_SCRAPE_INTERVAL_MS`); kept strictly
...
impl StatusConfig {
    /// Populate from `NEMESIS_WORKSPACE_*`. Unset or unparseable vars fall
    /// back to the default with a `warn!`. Never fails.
    pub fn from_env() -> Self {
        let defaults = Self::default();
        let (agent_rpc, agent_connect) = Self::agent_timeouts_from_env();
        let mut cfg = Self {
            heartbeat: secs_or("NEMESIS_WORKSPACE_HEARTBEAT_SECS", defaults.heartbeat),
            keepalive: secs_or("NEMESIS_WORKSPACE_KEEPALIVE_SECS", defaults.keepalive),
            ws_ping: secs_or("NEMESIS_WORKSPACE_WS_PING_SECS", defaults.ws_ping),
            ws_reconnect_backoff: backoff_schedule_from_env(
                "NEMESIS_WORKSPACE_WS_RECONNECT_BACKOFF_MS",
            ),
            hub_warn_threshold: parse_or(
                "NEMESIS_WORKSPACE_HUB_WARN_THRESHOLD",
                defaults.hub_warn_threshold,
            ),
            hub_backoff_base: ms_or(
                "NEMESIS_WORKSPACE_HUB_BACKOFF_BASE_MS",
                defaults.hub_backoff_base,
            ),
            session_idle_prune: secs_or(
                "NEMESIS_WORKSPACE_SESSION_IDLE_PRUNE_SECS",
                defaults.session_idle_prune,
            ),
            drain_timeout: secs_or("NEMESIS_WORKSPACE_DRAIN_TIMEOUT_SECS", defaults.drain_timeout),
            agent_rpc_timeout: agent_rpc,
            agent_connect_timeout: agent_connect,
            idle_ignores_background: parse_or(
                "NEMESIS_WORKSPACE_IDLE_IGNORE_BACKGROUND_TASKS",
            ...
            preview_activity_scrape_interval: ms_or(
                "NEMESIS_WORKSPACE_PREVIEW_ACTIVITY_SCRAPE_INTERVAL_MS",
    ...
    pub fn agent_timeouts_from_env() -> (Duration, Duration) {
        let defaults = Self::default();
        const RPC_VAR: &str = "NEMESIS_WORKSPACE_AGENT_RPC_TIMEOUT_SECS";
        const CONNECT_VAR: &str = "NEMESIS_WORKSPACE_AGENT_CONNECT_TIMEOUT_SECS";
        (
            nonzero_secs_or(
                RPC_VAR,
    ...
    fn validate(&mut self) {
        if self.keepalive <= self.heartbeat {
            tracing::warn!(
                ...
                "NEMESIS_WORKSPACE keepalive <= heartbeat; transport may time out between heartbeats"
            );
        }
        ...
        if window != self.preview_activity_window || scrape != self.preview_activity_scrape_interval
        {
            tracing::warn!(
                ...
                "NEMESIS_WORKSPACE preview scrape interval/window out of range; clamped to 1ms <= scrape < window"
            );
...
fn parse_or<T: FromStr>(var: &str, default: T) -> T {
    match std::env::var(var) {
        ...
        Ok(raw) => match raw.parse::<T>() {
            Ok(value) => value,
            Err(_) => {
                tracing::warn!(var, value = %raw, "Unparseable NEMESIS_WORKSPACE value; using default");
                default
            }
        },
    }
...
fn nonzero_secs_or(var: &str, secs: u64, default: Duration) -> Duration {
    if secs == 0 {
        tracing::warn!(
            var,
            default = ?default,
            "NEMESIS_WORKSPACE agent timeout of 0s is invalid; using default"
        );
        return default;
    }
        ...
        match part.parse::<u64>() {
            ...
            Err(_) => {
                tracing::warn!(
                    var,
                    value = %raw,
                    "Unparseable NEMESIS_WORKSPACE backoff schedule; using SDK default"
                );
                return None;
            }
        }
...
mod tests {
    ...
    fn parse_or_unset_returns_default() {
        ...
        let var = "NEMESIS_WORKSPACE_TEST_PARSE_OR_UNSET";
    ...
    fn parse_or_valid_parses() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "NEMESIS_WORKSPACE_TEST_PARSE_OR_VALID";
    ...
    fn parse_or_invalid_falls_back_without_panic() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "NEMESIS_WORKSPACE_TEST_PARSE_OR_INVALID";
    ...
    fn secs_or_parses_into_duration() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "NEMESIS_WORKSPACE_TEST_SECS_OR_VALID";
    ...
    fn secs_or_unset_returns_default() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "NEMESIS_WORKSPACE_TEST_SECS_OR_UNSET";
        unsafe { std::env::remove_var(var) };
        assert_eq!(
            secs_or(var, Duration::from_secs(30)),
    ...
    fn secs_or_invalid_falls_back_without_panic() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "NEMESIS_WORKSPACE_TEST_SECS_OR_INVALID";
    ...
    fn ms_or_parses_into_duration() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "NEMESIS_WORKSPACE_TEST_MS_OR_VALID";
    ...
    fn ms_or_invalid_falls_back_without_panic() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "NEMESIS_WORKSPACE_TEST_MS_OR_INVALID";
    ...
    /// With none of the `NEMESIS_WORKSPACE_*` vars set, `from_env` reproduces
    ...
    fn from_env_clean_matches_default() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        for var in [
            "NEMESIS_WORKSPACE_HEARTBEAT_SECS",
            "NEMESIS_WORKSPACE_KEEPALIVE_SECS",
            "NEMESIS_WORKSPACE_WS_PING_SECS",
            "NEMESIS_WORKSPACE_WS_RECONNECT_BACKOFF_MS",
            "NEMESIS_WORKSPACE_HUB_WARN_THRESHOLD",
            "NEMESIS_WORKSPACE_HUB_BACKOFF_BASE_MS",
            "NEMESIS_WORKSPACE_SESSION_IDLE_PRUNE_SECS",
            "NEMESIS_WORKSPACE_DRAIN_TIMEOUT_SECS",
            "NEMESIS_WORKSPACE_AGENT_RPC_TIMEOUT_SECS",
            "NEMESIS_WORKSPACE_AGENT_CONNECT_TIMEOUT_SECS",
            "NEMESIS_WORKSPACE_IDLE_IGNORE_BACKGROUND_TASKS",
            "NEMESIS_WORKSPACE_PREVIEW_ACTIVITY_WINDOW_MS",
            "NEMESIS_WORKSPACE_PREVIEW_ACTIVITY_SCRAPE_INTERVAL_MS",
    ...
    fn from_env_reads_idle_ignore_background_true() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        unsafe { std::env::set_var("NEMESIS_WORKSPACE_IDLE_IGNORE_BACKGROUND_TASKS", "true") };
        let cfg = StatusConfig::from_env();
        unsafe { std::env::remove_var("NEMESIS_WORKSPACE_IDLE_IGNORE_BACKGROUND_TASKS") };
        assert!(cfg.idle_ignores_background);
    }
    ...
    fn from_env_reads_preview_activity_window() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        unsafe { std::env::set_var("NEMESIS_WORKSPACE_PREVIEW_ACTIVITY_WINDOW_MS", "120000") };
        let cfg = StatusConfig::from_env();
        unsafe { std::env::remove_var("NEMESIS_WORKSPACE_PREVIEW_ACTIVITY_WINDOW_MS") };
    ...
    fn from_env_reads_preview_activity_scrape_interval() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        unsafe { std::env::set_var("NEMESIS_WORKSPACE_PREVIEW_ACTIVITY_SCRAPE_INTERVAL_MS", "5000") };
        let cfg = StatusConfig::from_env();
        unsafe { std::env::remove_var("NEMESIS_WORKSPACE_PREVIEW_ACTIVITY_SCRAPE_INTERVAL_MS") };
        assert_eq!(
            cfg.preview_activity_scrape_interval,
    ...
    fn backoff_schedule_unset_returns_none() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "NEMESIS_WORKSPACE_TEST_BACKOFF_UNSET";
        unsafe { std::env::remove_var(var) };
    ...
    fn backoff_schedule_valid_list_parses_in_order() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "NEMESIS_WORKSPACE_TEST_BACKOFF_VALID";
    ...
    fn backoff_schedule_malformed_returns_none() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "NEMESIS_WORKSPACE_TEST_BACKOFF_MALFORMED";

...
/// `NEMESIS_WORKSPACE_TERMINATION_GRACE_MS`. 45s fits under the K8s grace period.
const DEFAULT_TERMINATION_GRACE_MS: u64 = 45_000;
/// preStop-hook drain marker; override via `NEMESIS_WORKSPACE_DRAINING_FILE`.
...
impl WorkspaceHandle {
    ...
    /// Construct a handle with an explicit `$NEMESIS_WORKSPACE_HOME` and a
    ...
    fn build(
        ...
        let (hook_registry, hook_load_errors) = {
            use xai_nemesis_hooks::discovery::{HookSource, load_hooks_from_sources};
        ...
        let lsp: Option<Arc<dyn xai_nemesis_tools::implementations::lsp::LspBackend>> = {
            let sourced =
                xai_nemesis_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
                    &config.root_cwd,
                    &[],
                    &[],
                    &[],
            ...
            let servers =
                xai_nemesis_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
                    sourced,
            ...
            if servers.is_empty() {
                ...
                use xai_nemesis_tools::implementations::lsp::{
                ...
                let mgr = Arc::new(tokio::sync::Mutex::new(LspManager::new(
                    ...
                    true,
                    xai_nemesis_tools::notification::ToolNotificationHandle::noop(),
                )));
    ...
    pub fn create_session_with_config(
        ...
        session_id: impl Into<String>,
        cwd: Option<std::path::PathBuf>,
        tool_config: Option<xai_nemesis_tools::registry::types::ToolServerConfig>,
    ...
    fn create_session_with_tracker_inner(
        ...
        let system_notify_channel = system_notifications
            .then(xai_nemesis_tools::notification::types::ToolNotificationHandle::channel);
    ...
    pub(crate) async fn update_tool_config(
        ...
        session_id: &str,
        new_config: xai_nemesis_tools::registry::types::ToolServerConfig,
    ...
    async fn resolve_and_swap_session_toolset_locked(
        ...
        let _ = self
            .shared
            .events
            .send(xai_nemesis_workspace_types::WorkspaceEvent::ToolsChanged {
    ...
    pub(crate) async fn rebind_existing_hub_session(
        &self,
        session_id: &str,
        explicit_cfg: Option<xai_nemesis_tools::registry::types::ToolServerConfig>,
    ...
    /// `NEMESIS_WORKSPACE_TOOL_STATE_ENABLED` is off, opted out,
    ...
    ///
    /// No-op when the `NEMESIS_WORKSPACE_TOOL_DEFS_ENABLED` flag is off, no upload
    ...
    pub fn hook_registry(&self) -> xai_nemesis_hooks::discovery::HookRegistry {
    ...
    pub fn hook_load_errors(&self) -> &[xai_nemesis_hooks::error::HookError] {
    ...
    fn spawn_codebase_index_event_forwarder(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(xai_nemesis_workspace_types::WorkspaceEvent::FsChanged { ref path, kind }) => {
                    ...
                    Ok(xai_nemesis_workspace_types::WorkspaceEvent::GitHeadChanged { .. }) => {
    ...
    fn spawn_tool_definitions_event_forwarder(&self) -> Option<tokio::task::JoinHandle<()>> {
        ...
        Some(tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(xai_nemesis_workspace_types::WorkspaceEvent::ToolsChanged { session_id }) => {
    ...
    pub async fn start_session_mcp_servers(
        ...
        use xai_nemesis_mcp::servers::MCP_TOOL_NAME_DELIMITER;
        ...
        let mcp_results: Vec<
            Result<xai_nemesis_mcp::servers::McpClient, xai_nemesis_mcp::servers::McpError>,
        > = tokio::task::spawn_blocking(move || {
            use std::collections::HashMap;
            use xai_nemesis_mcp::oauth_config::McpOAuthConfigMap;
            use xai_nemesis_mcp::servers::{McpClientTimeoutOverrides, McpMetaConfigMap};
            ...
            rt_handle.block_on(xai_nemesis_mcp::servers::start_mcp_servers(
                configs,
                ...
                &oauth_config_map,
                &event_writer,
                xai_nemesis_mcp::servers::OauthInteractivity::Interactive,
            ))
        })
        .await
    ...
    pub fn on_mcp_snapshot_changed(
        &self,
        new_snapshot: Vec<xai_nemesis_tools::registry::types::ToolConfig>,
    ) -> usize {
    ...
    pub fn on_hub_tools_changed(
        &self,
        new_hub_tools: Vec<xai_nemesis_tools::registry::types::ToolConfig>,
    ) -> usize {
        self.shared
            .hub_tools_snapshot
            ...
            move |sid: xai_tool_protocol::SessionId, params: Option<serde_json::Value>| {
                ...
                async move {
                    ...
                    let bind_config = params
                        ...
                        .unwrap_or_default();
                    let empty_toolset = || xai_nemesis_tools::registry::types::ToolServerConfig {
                        tools: vec![],
    ...
    pub async fn connect_hub(&self) -> WorkspaceResult<()> {
        ...
        let (activity_notify_handle, activity_notify_rx) =
            xai_nemesis_tools::notification::types::ToolNotificationHandle::channel();
...
fn build_session_routed_handlers(
    toolset: &xai_nemesis_tools::registry::types::FinalizedToolset,
    ws: &WorkspaceHandle,
...
pub(crate) fn apply_background_task_notification(
    ...
    notification: &xai_nemesis_tools::notification::types::ToolNotification,
) {
    use xai_nemesis_tools::notification::types::ToolNotification;
    match notification {
...
pub(crate) async fn run_activity_feed(
    ...
    mut rx: tokio::sync::mpsc::UnboundedReceiver<
        xai_nemesis_tools::notification::types::ToolNotification,
    >,
) {
...
/// The SIGTERM drain budget from `NEMESIS_WORKSPACE_TERMINATION_GRACE_MS`
...
pub fn termination_grace_from_env() -> std::time::Duration {
    grace_budget_from_raw(std::env::var("NEMESIS_WORKSPACE_TERMINATION_GRACE_MS").ok())
}
...
/// Path of the preStop drain marker (`NEMESIS_WORKSPACE_DRAINING_FILE` or
/// [`DEFAULT_DRAINING_FILE`]).
fn draining_file_path() -> std::path::PathBuf {
    std::env::var("NEMESIS_WORKSPACE_DRAINING_FILE")
        .map(std::path::PathBuf::from)
...
/// sandbox; override via `NEMESIS_WORKSPACE_CONFINE_FS_TO_ROOT`); the CLI leader
/// passes `false`.
///
...
pub async fn connect_local_workspace(
    ...
    // NEMESIS: Create identity with default values (auth conversion not available for external type)
    ...
    let data_collection_disabled =
        std::env::var("NEMESIS_WORKSPACE_DATA_COLLECTION_DISABLED").as_deref() != Ok("false");
    ...
    let tool_config = xai_nemesis_agent::workspace_grok_build_toolset();
    ...
    if let Ok(dir) = std::env::var("NEMESIS_WORKSPACE_SERVER_SKILLS_DIR")
        && !dir.is_empty()
    {
        ws_config.skills_config.server_skill_dirs = vec![dir];
    }
    if let Ok(dir) = std::env::var("NEMESIS_WORKSPACE_BUNDLED_SKILLS_DIR")
        && !dir.is_empty()
    {
        let allowlist = std::env::var("NEMESIS_WORKSPACE_BUNDLED_SKILLS_ALLOWLIST").ok();
        ws_config
            .skills_config
            .ignore
    ...
    // NEMESIS: Create trace source and cast to trait object
    let trace_source: Arc<dyn xai_file_utils::queue::TraceExportSource> = 
        Arc::new(crate::upload::WorkspaceTraceExportSource::new(proxy_storage.clone()));
...
/// Resolve `$NEMESIS_WORKSPACE_HOME` — the workspace-owned on-disk state root.
///
/// Precedence:
/// 1. `$NEMESIS_WORKSPACE_HOME` (operator override).
/// 2. `<nemesis_home>/workspace`, where `<nemesis_home>` honours `$NEMESIS_HOME` and
///    otherwise falls back to `~/.grok` (see [`xai_nemesis_config::nemesis_home`]).
pub fn resolve_workspace_home() -> std::path::PathBuf {
    if let Ok(p) = std::env::var("NEMESIS_WORKSPACE_HOME")
        && !p.trim().is_empty()
    {
        return std::path::PathBuf::from(p);
    }
    xai_nemesis_config::nemesis_home().join("workspace")
}
...
/// (`NEMESIS_WORKSPACE_EVENTS_ENABLED=true`). Any other value — including unset —
...
fn events_enabled() -> bool {
    std::env::var("NEMESIS_WORKSPACE_EVENTS_ENABLED").as_deref() == Ok("true")
}
...
/// `NEMESIS_WORKSPACE_AFTER_TURN_WATCHDOG_MS` (malformed values fall back).
fn after_turn_watchdog() -> std::time::Duration {
    const DEFAULT_MS: u64 = 8_000;
    let ms = std::env::var("NEMESIS_WORKSPACE_AFTER_TURN_WATCHDOG_MS")
        .ok()
...
/// (`NEMESIS_WORKSPACE_TOOL_DEFS_ENABLED=true`; any other value keeps legacy
/// behaviour).
fn tool_defs_enabled() -> bool {
    std::env::var("NEMESIS_WORKSPACE_TOOL_DEFS_ENABLED").as_deref() == Ok("true")
}
...
/// only [`connect_local_workspace`] resolves `$NEMESIS_WORKSPACE_HOME` — so the
...
/// Resolve `workspace_rewind_all_outcomes` from `NEMESIS_WORKSPACE_REWIND_ALL_OUTCOMES` (default off).
fn rewind_all_outcomes_from_env() -> bool {
    xai_nemesis_config::env_bool("NEMESIS_WORKSPACE_REWIND_ALL_OUTCOMES").unwrap_or(false)
}
...
impl WorkspaceHandle {
    ...
    pub fn new_minimal(
        ...
        let config = WorkspaceConfig {
            root_cwd: cwd,
            default_tool_config: xai_nemesis_tools::registry::types::ToolServerConfig {
                tools: vec![],
                behavior_preset: None,
            },
...
pub(crate) mod tests {
    ...
    use std::sync::Arc;
    use xai_nemesis_tools::registry::types::ToolServerConfig;
    use xai_nemesis_tools::types::tool::ToolKind;
    use xai_nemesis_workspace_types::WorkspaceEvent;
    ...
    impl xai_nemesis_tools::types::tool_metadata::ToolMetadata for BashCcoStub {
        fn kind(&self) -> ToolKind {
            ToolKind::Execute
        }
        fn tool_namespace(&self) -> xai_nemesis_tools::types::tool::ToolNamespace {
            xai_nemesis_tools::types::tool::ToolNamespace::MCP
        }
    ...
    impl xai_tool_runtime::Tool for BashCcoStub {
        ...
        type Output = xai_nemesis_tools::types::output::ToolOutput;
        fn id(&self) -> xai_tool_protocol::ToolId {
        ...
        ) -> Result<xai_nemesis_tools::types::output::ToolOutput, xai_tool_runtime::ToolError>
        {
            let output = BASH_CCO_STUB_STDOUT.as_bytes();
            Ok(xai_nemesis_tools::types::output::ToolOutput::Bash(
                xai_nemesis_tools::types::output::BashOutput {
                    output: output.to_vec(),
                    output_for_prompt:
                        xai_nemesis_tools::types::output::BashOutput::make_output_for_prompt(
    ...
    async fn toolset_terminal(
        toolset: &Arc<xai_nemesis_tools::registry::types::FinalizedToolset>,
    ) -> Arc<dyn xai_nemesis_tools::computer::types::TerminalBackend> {
        let res = toolset.resources.lock().await;
        res.get::<xai_nemesis_tools::types::resources::Terminal>()
            .map(|t| t.0.clone())
    ...
    ///
    /// [`TerminalRunRequest`]: xai_nemesis_tools::computer::types::TerminalRunRequest
    pub(crate) fn terminal_run_request(
        command: &str,
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> xai_nemesis_tools::computer::types::TerminalRunRequest {
        xai_nemesis_tools::computer::types::TerminalRunRequest {
            ...
            notification_handle: xai_nemesis_tools::notification::ToolNotificationHandle::noop(),
            tool_call_id: tool_call_id.to_string(),
            display_command: None,
            auto_background_on_timeout: false,
            foreground_block_budget: None,
            kind: xai_nemesis_tools::computer::types::TaskKind::Bash,
    ...
    pub(crate) async fn start_background_sleep(
        ...
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> xai_nemesis_tools::computer::types::BackgroundHandle {
        session
            .terminal_backend()
    ...
    impl crate::config::SessionContextFactory for PersistentShellFactory {
        fn build_session_context(
            ...
            backend: Arc<dyn xai_nemesis_tools::computer::types::TerminalBackend>,
        ) -> xai_nemesis_tools::registry::types::SessionContext {
            self.inner
        ...
        fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
            crate::config::SessionTerminalBackend::local(
                xai_nemesis_tools::computer::local::LocalTerminalBackend::with_persistent_shell(),
            )
        }
        fn registry_builder(&self) -> xai_nemesis_tools::registry::types::ToolRegistryBuilder {
    ...
    async fn restarted_workspace_recreates_session_and_reports_lost_task() {
        ...
        let xai_nemesis_tools::types::output::ToolOutput::TaskOutput(
    ...
    /// `$NEMESIS_WORKSPACE_HOME` — must NOT configure an upload queue, and must leave
    ...
    async fn new_defaults_to_ephemeral_home_and_inert_legacy_upload() {
        ...
        assert_ne!(
            ...
            "default construction must NOT use the real $NEMESIS_WORKSPACE_HOME"
        );
        assert!(
    ...
    async fn tool_state_upload_is_noop_when_flag_off() {
        ...
        unsafe { std::env::remove_var("NEMESIS_WORKSPACE_TOOL_STATE_ENABLED") };
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let queue_home = tempfile::TempDir::new().unwrap();
        let queue = spawn_test_queue(queue_home.path());
    ...
    async fn tool_state_upload_is_noop_when_data_collection_disabled() {
        ...
        let _env = crate::session::tool_config::TOOL_STATE_ENV_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        unsafe { std::env::set_var("NEMESIS_WORKSPACE_TOOL_STATE_ENABLED", "true") };
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let queue_home = tempfile::TempDir::new().unwrap();
        let queue = spawn_test_queue(queue_home.path());
    ...
    async fn fork_session_cwd_override_used_when_set() {
        let handle = make_handle();
        let alt = std::env::temp_dir().join("xai-nemesis-workspace-test-cwd-override");
    ...
    async fn hook_registry_snapshot_is_disconnected() {
        ...
        {
            let spec = xai_nemesis_hooks::config::HookSpec {
                name: "injected".into(),
                event: xai_nemesis_hooks::event::HookEventName::SessionStart,
                handler_type: xai_nemesis_hooks::config::HandlerType::Command,
    ...
    async fn strict_bind_without_explicit_toolset_fails_closed_end_to_end() {
        ...
        assert!(
            reason.contains(xai_nemesis_version::VERSION),

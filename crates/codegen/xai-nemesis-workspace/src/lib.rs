...
pub use xai_nemesis_workspace_client::WorkspaceClient;
pub use xai_nemesis_workspace_types::WorkspaceEvent;
...
/// environment (`NEMESIS_HOME`, `HOME`, …). nextest isolates each test in its own
...
/// can clobber `NEMESIS_HOME` mid-test. A single shared lock (used by every

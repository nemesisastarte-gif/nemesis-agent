...
//! spawns the unchanged `/usr/local/bin/xai-nemesis-preview-proxy` binary as a
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/xai-nemesis-preview-proxy";
...
impl PreviewArgs {
    ...
    /// `xai-nemesis-preview-proxy/src/cli.rs`). Absent options and a false
...
/// `xai-nemesis-preview-proxy`'s `/__control/activity` route).
...
/// the default in `xai-nemesis-preview-proxy/src/cli.rs`).
...
mod tests {
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match xai-nemesis-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",

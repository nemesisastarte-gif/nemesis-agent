...
pub struct RequirementsLayer {
    ...
    /// not re-derive from the source (`NEMESIS_HOME`-influenced, could carry `..`).
    pub is_system: bool,
}
...
pub fn requirements_layers() -> Vec<RequirementsLayer> {
    let mut out = Vec::new();
    if let Some(user_path) = user_nemesis_home().map(|g| g.join("requirements.toml"))
...
pub(crate) fn load_requirements() -> Option<toml::Value> {
    load_user_requirements(user_nemesis_home().as_deref())
}
...
fn validate_requirements_value(
    ...
    let Ok(version) = xai_nemesis_version::installed_semver() else {
        return Ok(());
    };
...
pub fn validate_requirements() -> Result<(), RequirementsError> {
    validate_user_requirements(user_nemesis_home().as_deref())?;
    if let Some(dir) = system_config_dir() {
...
/// no-op (no cwd-relative `.nemesis/requirements.toml` is read or enforced).

...
static NEMESIS_HOME: OnceLock<PathBuf> = OnceLock::new();
...
/// `NEMESIS_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`nemesis_home()`] is the default without duplicating the computation.
///
...
/// `~/.nemesis/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `xai_fast_worktree::db::resolve_nemesis_home` (deliberately standalone crate).
pub fn default_nemesis_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".nemesis")
}

/// Per-user config directory: `$NEMESIS_HOME` or `~/.grok`. Created if needed.
pub fn nemesis_home() -> PathBuf {
    NEMESIS_HOME
        .get_or_init(|| {
            let nemesis_home = if let Ok(v) = std::env::var("NEMESIS_HOME") {
                PathBuf::from(v)
            } else {
                default_nemesis_home()
            };
...
/// `$NEMESIS_HOME` is set or a home directory is found, `None` otherwise. Unlike
/// [`nemesis_home()`], this never falls back to a cwd-relative `.grok`, so callers
...
pub fn user_nemesis_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("NEMESIS_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(nemesis_home)
}

/// Canonical grok application path: `$NEMESIS_HOME/bin/grok` (Unix) or `grok.exe` (Windows).
pub fn grok_application() -> PathBuf {
    grok_application_in(&nemesis_home())
}

/// [`grok_application`] under an explicit home instead of `$NEMESIS_HOME`.
pub fn grok_application_in(home: &std::path::Path) -> PathBuf {
    let name = if cfg!(windows) { "grok.exe" } else { "nemesis" };
...
/// `nemesis_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    nemesis_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = nemesis_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_nemesis_home_has_no_verbatim_prefix() {
        ...
        let home = default_nemesis_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".nemesis"));
    }

...
use crate::paths::{system_config_dir, user_nemesis_home};
...
pub fn load_from_disk() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_nemesis_home().as_deref(), "config.toml")
}
...
pub fn load_managed_config() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_nemesis_home().as_deref(), MANAGED_CONFIG_FILENAME)
}
...
/// `.nemesis/<filename>` (the cwd-fallback would silently promote an untrusted
...
pub fn managed_config_layers() -> Vec<ManagedConfigLayer> {
    managed_config_layers_at(system_config_dir().as_deref(), user_nemesis_home().as_deref())
}
...
/// (`xai_nemesis_shell::util::config::load_effective_config`) when remote campaigns
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match xai_nemesis_version::installed_semver() {

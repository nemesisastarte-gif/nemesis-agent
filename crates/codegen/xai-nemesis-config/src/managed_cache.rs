...
use crate::paths::user_nemesis_home;
...
pub fn is_managed_config_stale_for(identity: &ServingIdentity) -> bool {
    managed_config_stale_at(user_nemesis_home().as_deref(), identity)
}
...
pub fn mark_managed_config_synced(marker: SyncMarker<'_>) {
    if let Some(home) = user_nemesis_home() {
...
pub fn managed_deployment_id(key_fingerprint: &str) -> Option<String> {
    managed_deployment_id_at(user_nemesis_home()?.as_path(), key_fingerprint)
}
...
pub fn confirmed_team_switch(new_team_id: &str) -> Option<String> {
    user_nemesis_home().and_then(|home| confirmed_team_switch_at(&home, new_team_id))
}
...
pub fn managed_policy_compromised_for(identity: &ServingIdentity) -> bool {
    user_nemesis_home().is_some_and(|home| managed_policy_compromised_for_at(&home, identity))
}

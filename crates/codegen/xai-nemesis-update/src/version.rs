...
use xai_nemesis_shell::env::GrokBuildEnvironment;
...
pub struct UpdateConfig {
    ...
    /// Auth scope key for `~/.nemesis/auth.json`.
    pub auth_scope: String,
    /// Enterprise deployment key (NEMESIS_API_KEY).
...
impl UpdateConfig {
    pub fn from_environment(env: &GrokBuildEnvironment) -> Self {
        Self {
            ...
            auth_scope: xai_nemesis_shell::auth::GrokComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
pub async fn write_version_cache(version: &str, stable_version: Option<&str>) {
    let version_path = nemesis_home().join("version.json");
...
pub use xai_nemesis_version::installed as get_installed_grok_version;

/// Version of the managed grok binary currently on disk, read from the
/// `~/.nemesis/bin/grok` symlink target (`../downloads/grok-<version>-<platform>`)
...
/// link whose target binary was deleted (e.g. manual `~/.nemesis/downloads`
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = xai_nemesis_shell::util::nemesis_home::grok_application();
        ...
        version_from_versioned_binary_name(target.file_name()?.to_str()?, "nemesis")
    }
    #[cfg(not(unix))]
    {
        None
...
/// Read the cached stable version from `~/.nemesis/version.json` (sync, for display).
///
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(xai_nemesis_version::VERSION, &stable)
    })
}
...
/// `~/.nemesis/version.json` (written by the auto-updater):
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(xai_nemesis_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
...
mod tests {
    ...
    fn test_version_from_versioned_binary_name() {
        let cases: &[(&str, Option<&str>)] = &[
            ...
            ("nemesis", None),                                    // bare name
            ("", None),
        ];
        for (name, expected) in cases {
            assert_eq!(
                version_from_versioned_binary_name(name, "nemesis").as_deref(),
                *expected,
        ...
        // prefix but not under "nemesis".
        assert_eq!(
            version_from_versioned_binary_name("grok-pager-0.1.5-darwin-arm64", "nemesis-agent")
                .as_deref(),
            Some("0.1.5")
        );
    }

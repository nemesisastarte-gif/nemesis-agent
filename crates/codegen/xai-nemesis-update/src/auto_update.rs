...
use xai_nemesis_shell::util::config;
use xai_nemesis_shell::util::nemesis_home::{grok_application, nemesis_home};
...
pub fn print_update_status(status: &UpdateStatus, json: bool) -> anyhow::Result<()> {
    ...
    if status.update_available {
        if let Some(latest_version) = status.latest_version.as_deref() {
            println!(
                "A new version of Nemesis Agent is available: {} -> {}{}",
                status.current_version, latest_version, channel_label
            );
        } else {
            println!("A new version of Nemesis Agent is available.");
        }
        return Ok(());
    }
    ...
    if let Some(latest_version) = status.latest_version.as_deref() {
        println!(
            "Nemesis Agent - v{} (latest: {}){}",
            status.current_version, latest_version, channel_label
        );
        return Ok(());
    }

    println!("Nemesis Agent - v{}{}", status.current_version, channel_label);
    Ok(())
}
...
/// managed `~/.nemesis/bin/grok` symlink.
///
...
async fn run_update_subcommand(run_mode: UpdateRunMode) -> Result<Option<tokio::process::Child>> {
    ...
    match run_mode {
        ...
        UpdateRunMode::NonBlocking => {
            ...
            xai_nemesis_tools::util::detach_command(&mut cmd);
            let child = cmd.spawn()?;
            Ok(Some(child))
        }
    }
...
/// Prefer `~/.nemesis/bin/grok` which always points to the latest version.
...
/// Delete `~/.nemesis/models_cache.json` after a successful update.
///
...
async fn remove_stale_models_cache() {
    let cache = nemesis_home().join("models_cache.json");
...
/// Remove the stale `grok-pager` symlink/binary from `~/.nemesis/bin/` left by
...
/// config and writes to `~/.nemesis/bin/`, so callers must isolate
/// `NEMESIS_HOME`.
#[doc(hidden)]
...
/// A downloaded and smoke-tested binary in `~/.nemesis/downloads/`, not yet
...
async fn download_verified_from_base(
    ...
    let nemesis_home = nemesis_home();
    let download_dir = nemesis_home.join("downloads");
...
async fn activate_verified_download(download: &VerifiedDownload) -> Result<()> {
    let nemesis_home = nemesis_home();
    let download_dir = nemesis_home.join("downloads");
    let bin_dir = nemesis_home.join("bin");
    tokio::fs::create_dir_all(&bin_dir).await?;

    // Atomic swap of ~/.nemesis/bin/{grok,agent} -> downloaded binary.
    ...
    cleanup_old_downloads(&download_dir, "nemesis", &download.version).await;
    cleanup_old_downloads(&download_dir, "nemesis-agent", &download.version).await;
    ...
    regenerate_completions(&link_path, &nemesis_home).await;
...
async fn regenerate_completions(binary: &std::path::Path, nemesis_home: &std::path::Path) {
    // Derive $HOME independently — nemesis_home may be overridden via NEMESIS_HOME
    // env var, so nemesis_home.parent() isn't necessarily the user's home dir.
    #[allow(deprecated)]
    let user_home = std::env::home_dir().unwrap_or_default();

    let completions: &[(&str, std::path::PathBuf)] = &[
        ("bash", nemesis_home.join("completions/bash/grok.bash")),
        ("zsh", nemesis_home.join("completions/zsh/_grok")),
...
///
/// When both paths share a grandparent (e.g. `~/.nemesis/bin/grok` and
/// `~/.nemesis/downloads/grok-0.1.203-linux-x86_64`), returns a relative path
...
///
/// Relative symlinks survive Docker bind-mounts where `~/.nemesis/` is mapped
...
/// Swap `~/.nemesis/bin/{grok,agent}` to point at `binary_path`. Returns the
...
/// bind-mounts of `~/.nemesis/`). Windows: [`windows_replace_exe`].
///
...
async fn swap_managed_bin_links(
    ...
    let grok_name = if cfg!(windows) { "grok.exe" } else { "nemesis" };
...
///
/// `bin_prefix` is the binary name prefix, e.g. `"nemesis"` or `"nemesis-agent"`.
...
async fn reconcile_agent_to_grok(bin_dir: &std::path::Path) {
    let grok_link = bin_dir.join("nemesis");
...
async fn gh_release_download(tag: &str, pattern: &str, dest: &std::path::Path) -> Result<()> {
    ...
    .stderr(Stdio::piped());
    xai_nemesis_tools::util::detach_command(&mut cmd);
    cmd.envs(xai_nemesis_tools::util::pager_env());
...
async fn install_gh_release(target: Option<&str>) -> Result<()> {
    ...
    // through it (e.g. /usr/local/bin/grok -> ~/.nemesis/downloads/grok-latest)
    ...
    // ~/.nemesis/downloads/ (legacy layout — skips the grok-latest indirection).
    // Permission errors ignored.
    #[cfg(unix)]
    for name in ["nemesis", "agent"] {
        ...
        if let Ok(existing_target) = tokio::fs::read_link(&system_link).await {
            ...
            if target_str.contains(".nemesis/downloads/") && !target_str.ends_with("grok-latest") {
    ...
    // Clean up old versioned binaries (keeps current + 1 previous).
    cleanup_old_downloads(&download_dir, "nemesis", &version).await;
    cleanup_old_downloads(&download_dir, "nemesis-agent", &version).await;
...
/// inode is unlinked.
///
/// While our postinstall.js now uses versioned binaries under ~/.nemesis/bin/
...
fn warn_if_other_grok_processes_running() {
    ...
    cmd.args(["-f", "nemesis"])
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
    xai_nemesis_tools::util::detach_std_command(&mut cmd);
...
async fn refresh_deployment_config() {
    if !xai_nemesis_shell::managed_config::has_principal() {
        return;
    }
    if !xai_nemesis_shell::managed_config::is_fetch_enabled() {
        return;
    }
    // Clear a logged-out team's files before deciding to fetch (mirrors the loop).
    xai_nemesis_shell::managed_config::clear_orphan();
    if !xai_nemesis_shell::config::is_managed_config_stale_for(
        &xai_nemesis_shell::managed_config::current_serving_identity(),
    ) {
        return;
    }
    match xai_nemesis_shell::managed_config::sync().await {
        Ok(true) => eprintln!("  Applied managed configuration."),
        Ok(false) => tracing::debug!("no managed configuration to apply"),
        // Auth issues aren't actionable mid-update: quiet here, loud on `nemesis setup`.
        Err(e) if e.is_auth_rejection() => tracing::debug!("managed config not applied: {e}"),
        Err(e) if e.is_retryable() => {
            tracing::debug!("managed config refresh failed: {e}");
            eprintln!("  Couldn't apply managed configuration. Run `nemesis setup` to retry.");
        }
...
mod tests {
    ...
    fn test_tmp_download_path_is_unique_per_version_and_per_attempt() {
        ...
        let dest_181 = std::path::Path::new("/home/u/.nemesis/downloads/grok-0.1.181-linux-x86_64");
        let dest_182 = std::path::Path::new("/home/u/.nemesis/downloads/grok-0.1.182-linux-x86_64");
        ...
        assert_eq!(
            a.parent(),
            std::path::Path::new("/home/u/.nemesis/downloads").into(),
    ...
    async fn test_atomic_symlink_swap_creates_new_symlink() {
        ...
        let link = dir.path().join("nemesis");
    ...
    async fn test_reconcile_agent_repoints_diverged_agent() {
        ...
        std::os::unix::fs::symlink("../downloads/grok-0.2.101-macos-aarch64", bin.join("nemesis"))
            .unwrap();
    ...
    async fn test_reconcile_agent_noop_when_consistent() {
        ...
        std::os::unix::fs::symlink(target, bin.join("nemesis")).unwrap();
    ...
    async fn test_reconcile_agent_skips_when_grok_not_symlink() {
        let (_dir, bin, downloads) = managed_layout();
        std::fs::write(bin.join("nemesis"), "copy-binary").unwrap();
        std::fs::write(downloads.join("grok-0.1.199-macos-aarch64"), "old").unwrap();
        std::os::unix::fs::symlink("../downloads/grok-0.1.199-macos-aarch64", bin.join("agent"))
            .unwrap();
    ...
    fn test_relative_symlink_target_sibling_dirs() {
        ...
        let target = std::path::Path::new("/home/alice/.nemesis/downloads/grok-0.1.203");
        let link = std::path::Path::new("/home/alice/.nemesis/bin/grok");
    ...
    fn test_relative_symlink_target_same_dir() {
        ...
        let target = std::path::Path::new("/home/alice/.nemesis/downloads/grok-0.1.203");
        let link = std::path::Path::new("/home/alice/.nemesis/downloads/grok-latest");
        let result = relative_symlink_target(target, link);
    ...
    fn test_relative_symlink_target_cross_tree_stays_absolute() {
        // /usr/local/bin/grok -> /home/alice/.nemesis/downloads/grok-0.1.203
        ...
        assert_eq!(
            result,
            std::path::PathBuf::from("/home/alice/.nemesis/downloads/grok-0.1.203")
        );
    }
    ...
    async fn test_relative_symlink_survives_directory_move() {
        // Simulates Docker bind-mount: create ~/.nemesis/ layout at path A,
        // then move it to path B and verify the symlink still resolves.
        let dir = tempfile::tempdir().unwrap();

        // Create alice's layout
        let alice = dir.path().join("alice").join(".nemesis");
        ...
        let rel_target = std::path::Path::new("../downloads/grok-0.1.203");
        let link = alice_bin.join("nemesis");
        atomic_symlink_swap(rel_target, &link).await.unwrap();
        ...
        let bob_link = bob.join("bin").join("nemesis");
    ...
    async fn test_cleanup_old_downloads_keeps_current_plus_one() {
        ...
        cleanup_old_downloads(d, "nemesis", "0.1.145").await;
    ...
    async fn test_cleanup_old_downloads_does_not_touch_other_binaries() {
        ...
        cleanup_old_downloads(d, "nemesis", "0.1.141").await;
    ...
    async fn test_cleanup_old_downloads_empty_dir() {
        ...
        cleanup_old_downloads(dir.path(), "nemesis", "0.1.141").await;
    }
    ...
    async fn test_cleanup_old_downloads_version_prefix_collision() {
        ...
        cleanup_old_downloads(d, "nemesis", "0.1.14").await;
    ...
    async fn test_cleanup_old_downloads_pager_multi_version() {
        ...
        cleanup_old_downloads(d, "nemesis-agent", "0.1.151").await;
    ...
    async fn test_cleanup_old_downloads_alpha_versions() {
        ...
        cleanup_old_downloads(d, "nemesis", "0.1.150-alpha.1").await;
    ...
    async fn test_cleanup_old_downloads_mixed_stable_and_alpha() {
        ...
        cleanup_old_downloads(d, "nemesis", "0.1.150").await;
    ...
    async fn test_cleanup_old_downloads_invalid_current_version_is_no_op() {
        ...
        cleanup_old_downloads(d, "nemesis", "not-a-version").await;
    ...
    async fn test_cleanup_old_downloads_missing_dir_no_panic() {
        ...
        cleanup_old_downloads(&missing, "nemesis", "0.1.141").await;
    }

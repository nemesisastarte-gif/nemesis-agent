...
//!
//! These tests touch global process state — `NEMESIS_HOME` (a `OnceLock` in
//! `xai-nemesis-config`), `GROK_TEST_VERSION`, and `NPM_TOKEN` — so they
//! must run serially. Once `NEMESIS_HOME` is initialized for a process, it can't
//! be changed; we set it from a single shared `OnceLock` and reset the
//! contents of the directory between tests.
//!
//! The patterns here mirror the NEMESIS_HOME isolation used in other
//! integration tests.
...
use xai_nemesis_update::write_version_cache;
...
#[tokio::test]
#[serial]
async fn write_version_cache_creates_file_at_nemesis_home() {
    let _ = test_home();
    reset();
...
async fn cache_is_fresh() -> bool {
    // Mirror the implementation: look at version.json under NEMESIS_HOME,
...
async fn get_installed_version_uses_env_var_override() {
    ...
    let v = xai_nemesis_update::version::get_installed_grok_version();
    assert_eq!(v, "9.9.9");
    unsafe {
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    let real = {
        ...
        xai_nemesis_update::version::get_installed_grok_version()
    };

    unsafe {
        std::env::set_var("GROK_TEST_VERSION", "0.0.0-test");
    }
    let overridden = xai_nemesis_update::version::get_installed_grok_version();

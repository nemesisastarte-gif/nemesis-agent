...
//!
//! > `~/.nemesis/bin/grok` resolves to a binary that passes the smoke-test, OR it
...
use xai_nemesis_update::auto_update::install_internal_from_base;
...
/// `cargo nextest run -p xai-nemesis-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]

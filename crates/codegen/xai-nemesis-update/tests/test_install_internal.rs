...
//!
//! Wires together a wiremock-mocked GCS bucket + an isolated `NEMESIS_HOME`
...
//!
//! The function reads `nemesis_home()` (a process-wide `OnceLock`), so all
//! tests in this binary share a single `NEMESIS_HOME` and run serially via
//! `#[serial]`.
...
use xai_nemesis_update::UpdateConfig;
use xai_nemesis_update::auto_update::{install_internal_from_base, install_internal_from_bases};
...
async fn install_internal_pinned_version_writes_binary_and_symlink() {
    ...
    let symlink = home.join("bin").join("nemesis");
...
async fn install_internal_updates_stale_agent_symlink_to_new_version() {
    ...
    std::os::unix::fs::symlink(&rel_old, bin_dir.join("nemesis")).unwrap();
...
async fn install_internal_rolls_back_grok_when_agent_swap_fails() {
    ...
    let grok_target = std::fs::read_link(bin_dir.join("nemesis")).unwrap();
    assert_eq!(
...
async fn install_internal_rollback_removes_absent_prior_grok_link() {
    ...
    std::fs::write(agent_dir.join("blocker"), b"x").unwrap();
    assert!(
        !bin_dir.join("nemesis").exists() && !bin_dir.join("nemesis").is_symlink(),
    ...
    let grok_path = bin_dir.join("nemesis");
    assert!(
...
async fn install_internal_cleans_up_stale_pager_symlink() {
    ...
    // update removes the stale symlink from ~/.nemesis/bin/.
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    let bin_dir = home.join("bin");
    std::fs::create_dir_all(&bin_dir).unwrap();
    let pager_link = bin_dir.join("nemesis-agent");
...
async fn install_internal_cleans_up_old_versions_keeping_n_minus_one() {
    ...
    let target = std::fs::read_link(home.join("bin").join("nemesis")).unwrap();
    assert!(
        target
            .file_name()
            .unwrap()
...
async fn install_internal_idempotent_for_same_version() {
    ...
    let target = std::fs::read_link(test_home().join("bin").join("nemesis")).unwrap();
    assert!(target.to_string_lossy().contains("0.1.181"));
}

#[tokio::test]
#[serial]
async fn install_internal_creates_nemesis_home_subdirs_if_missing() {
    let _ = test_home();
    reset_home();

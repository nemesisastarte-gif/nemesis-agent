...
use xai_nemesis_update::auto_update::{download_silent, download_with_progress};
use xai_nemesis_update::version::fetch_gcs_version_from_base;
...
async fn download_silent_writes_body_to_dest() {
    ...
    let dest = tmp.path().join("nemesis");
...
async fn download_silent_to_nonexistent_parent_dir_fails() {
    ...
    let dest = tmp.path().join("missing-subdir").join("nemesis");
    let err = download_silent(&format!("{}/x", server.uri()), &dest)
        .await
        .unwrap_err();
    let msg = format!("{err:#}").to_lowercase();

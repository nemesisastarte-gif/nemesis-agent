...
//!
//! The installer lives in the sibling `xai-nemesis-pager` crate; it is resolved by
...
fn script_path(name: &str) -> Option<PathBuf> {
    dunce::canonicalize(
        Path::new(env!("CARGO_MANIFEST_DIR")).join(format!("../xai-nemesis-pager/scripts/{name}")),
    )
    .ok()
    .filter(|p| p.exists())
}
...
fn seed_previous_good(home: &Path, platform: &str) -> PathBuf {
    let downloads = home.join(".nemesis").join("downloads");
    let bin = home.join(".nemesis").join("bin");
...
fn assert_active_grok_runs(home: &Path) {
    let link = home.join(".nemesis").join("bin").join("nemesis");
...
fn run_installer(install_sh: &Path, home: &Path, fakebin: &Path, mode: &str, shell: &str) -> bool {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", path_env)
        .env("SHELL", shell)
        .env("GROK_BIN_DIR", home.join(".nemesis").join("bin"))

...
use xai_nemesis_update::auto_update::{ensure_latest_on_disk, install_internal_from_base, run_update};
use xai_nemesis_update::version::installed_on_disk_version;

/// Assert the active `~/.nemesis/bin/grok` resolves to the expected versioned
...
fn assert_active_binary(home: &Path, version: &str, platform: &str, expected_content: &[u8]) {
    let link = home.join("bin").join("nemesis");
...
/// Lay down a managed-install layout in the test NEMESIS_HOME:
...
// installers that actually maintain the managed `~/.nemesis/bin/grok` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.nemesis/downloads cleanup), the probe must report None — otherwise
...
async fn concurrent_different_version_installs_do_not_corrupt_each_other() {
    ...
    let resolved = dunce::canonicalize(home.join("bin").join("nemesis")).unwrap();

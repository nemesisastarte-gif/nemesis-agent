...
//! its own `OnceLock<NEMESIS_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Mirrors the NEMESIS_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes NEMESIS_HOME once per binary
...
// ─────────────────────────────────────────────────────────────────────────────
// NEMESIS_HOME isolation
// ─────────────────────────────────────────────────────────────────────────────

/// Returns a process-wide test `NEMESIS_HOME`, initialized exactly once per test
/// binary. Once initialized, `xai_nemesis_config::nemesis_home()` will resolve to
...
/// Wipe state in `NEMESIS_HOME` between tests so each test sees a clean home.
...
/// Minimal [`xai_nemesis_update::UpdateConfig`] for install tests.
pub fn make_update_config(channel: &str) -> xai_nemesis_update::UpdateConfig {
    xai_nemesis_update::UpdateConfig {
...
/// Backdate every file in `NEMESIS_HOME/downloads` by ~2 hours.
///

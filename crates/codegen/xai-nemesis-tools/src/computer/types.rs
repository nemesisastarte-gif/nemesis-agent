...
/// (`xai-nemesis-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (xai-nemesis-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in xai-nemesis-tools, spawns processes)
/// - `AcpTerminalBackend` (in xai-nemesis-shell, calls ACP protocol)
#[async_trait::async_trait]

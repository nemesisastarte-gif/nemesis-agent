...
//! config helper `xai-nemesis-shell::util::config::resolve_search_tools_enabled`
...
//! (release builds, self-extracted to `~/.nemesis/vendor/<name>-<ver>-<target>`) →
//! `~/.nemesis/vendor/{name}` if a regular file → `which` on the agent `$PATH`.
...
// (release pipeline). Self-extracted to `~/.nemesis/vendor` on first use, mirroring
...
/// Write embedded `bytes` to `~/.nemesis/vendor/<versioned_name>` (chmod 755) on
...
fn extract_bundled(versioned_name: &str, bytes: &[u8]) -> std::io::Result<PathBuf> {
    ...
    let dir = crate::util::nemesis_home().join("vendor");
...
fn resolve_tool(bin_name: &str, env_override: &str, bundled: Option<PathBuf>) -> Option<PathBuf> {
    resolve_tool_from(
        ...
        bundled,
        crate::util::nemesis_home().join("vendor").join(bin_name),
        bin_name,
    )
}

/// Resolution order: explicit env path → bundled (self-extracted) →
/// `~/.nemesis/vendor/<bin>` → `which`. Env and vendor only require `is_file()` here
...
mod tests {
    ...
    /// bytes self-extract under `~/.nemesis/vendor` and the extracted `bfs` runs.
    #[cfg(all(bundle_bfs, bundle_ugrep))]
    #[test]
    fn bundled_binaries_extract_and_run() {
        let vendor = crate::util::nemesis_home().join("vendor");

...
pub fn shell_env_overrides() -> HashMap<String, String> {
    HashMap::from([
        ...
        // env cannot clear it.
        (
            crate::util::NEMESIS_AGENT_ENV.to_string(),
            crate::util::NEMESIS_AGENT_ENV_VALUE.to_string(),
        ),
    ])
}
...
dump_bash_state() {
  ...
  local env_vars
  env_vars=$(builtin export -p 2>/dev/null | command grep -viE '_proxy=|GROK_SANDBOX|NEMESIS_AGENT=|SUDO_ASKPASS|GROK_ASKPASS|ELECTRON_RUN_AS_NODE|SSH_AUTH_SOCK|DBUS_SESSION_BUS_ADDRESS|XDG_RUNTIME_DIR|WAYLAND_DISPLAY|GPG_TTY' || true)
...
function dump_zsh_state() {
  ...
  local env_vars
  env_vars=$(builtin typeset -xp 2>/dev/null | command grep -viE '_proxy=|GROK_SANDBOX|NEMESIS_AGENT=|SUDO_ASKPASS|GROK_ASKPASS|ELECTRON_RUN_AS_NODE|SSH_AUTH_SOCK|DBUS_SESSION_BUS_ADDRESS|XDG_RUNTIME_DIR|WAYLAND_DISPLAY|GPG_TTY' || true)
  _emit_encoded "$env_vars" "ENV_VARS_B64"
...
impl ShellKind {
    ...
    pub fn detect() -> Self {
        match xai_nemesis_config::shell::detect_unix_shell_kind() {
            xai_nemesis_config::shell::UnixShellKind::Bash => Self::Bash,
            xai_nemesis_config::shell::UnixShellKind::Zsh => Self::Zsh,
        }
    }

    /// Resolved absolute path to the shell binary. Falls back from `$SHELL` →
    /// `which` → common dirs → `/bin/<name>`. Result is cached process-wide
    /// in `xai_nemesis_config::shell::unix_shell_path`. See that function for
    /// the full cascade. Returns `&'static str`.
    pub fn binary_path(&self) -> &'static str {
        let kind = match self {
            Self::Bash => xai_nemesis_config::shell::UnixShellKind::Bash,
            Self::Zsh => xai_nemesis_config::shell::UnixShellKind::Zsh,
        };
        xai_nemesis_config::shell::unix_shell_path(kind)
    }
    ...
    pub fn prepare_command(
        ...
        let wrapper = match self.shell {
            ShellKind::Bash => format!(
                ...
                // Re-export NEMESIS_AGENT=1 after snapshot eval so agent-definition
                ...
                "{dump_script} \
                 ...
                 builtin export NEMESIS_AGENT=1; \
...
mod tests {
    ...
    fn shell_env_overrides_marks_agent_terminal() {
        ...
        assert_eq!(
            env.get(crate::util::NEMESIS_AGENT_ENV).map(String::as_str),
            Some(crate::util::NEMESIS_AGENT_ENV_VALUE)
        );

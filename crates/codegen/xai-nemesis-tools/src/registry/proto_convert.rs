//! Conversion from the gRPC wire config types (`xai-nemesis-tools-api`) to the
//! runtime registry types ([`ToolConfig`] / [`ToolServerConfig`]).
//!
//! The `params_json` parse/validation contract lives in
//! [`xai_nemesis_tools_api::config_validation`] so every consumer (this
...
pub use xai_nemesis_tools_api::config_validation::{ToolConfigEntryError, ToolConfigEntryErrorKind};

/// Convert one wire [`xai_nemesis_tools_api::ToolConfigEntry`] to a runtime
/// [`ToolConfig`].
///
...
pub fn tool_config_from_entry(
    index: usize,
    entry: xai_nemesis_tools_api::ToolConfigEntry,
) -> Result<ToolConfig, ToolConfigEntryError> {
    let xai_nemesis_tools_api::ToolConfigEntry {
        id,
        params_json,
        name_override,
    ...
    let params = xai_nemesis_tools_api::config_validation::parse_params_json(
        index,
        &id,
        params_json.as_deref(),
    )?;
    xai_nemesis_tools_api::config_validation::validate_name_override(
        index,
        &id,
...
pub fn tool_server_config_from_entries(
    entries: Vec<xai_nemesis_tools_api::ToolConfigEntry>,
...
mod tests {
    ...
    fn entry(id: &str) -> xai_nemesis_tools_api::ToolConfigEntry {
        xai_nemesis_tools_api::ToolConfigEntry {
            id: id.to_owned(),
            params_json: None,
            name_override: None,

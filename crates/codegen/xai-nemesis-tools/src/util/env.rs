...
//! heavyweight `xai-nemesis-tools` dependency. This module re-exports the public
...
/// Note: the CLI also uses `NEMESIS_AGENT` as an
/// optional agent-definition selector for launching `grok` itself; child terminal
/// processes only need the sentinel value `"1"`.
pub const NEMESIS_AGENT_ENV: &str = "NEMESIS_AGENT";

/// Sentinel value for [`NEMESIS_AGENT_ENV`] on agent tool terminals.
pub const NEMESIS_AGENT_ENV_VALUE: &str = "1";

/// Force `NEMESIS_AGENT=1` on an agent terminal child so request/login env cannot
/// clear the agent marker.
pub fn apply_grok_agent_marker(cmd: &mut tokio::process::Command) {
    cmd.env(NEMESIS_AGENT_ENV, NEMESIS_AGENT_ENV_VALUE);
}
...
#[cfg(test)]
mod tests {
    use super::{NEMESIS_AGENT_ENV, NEMESIS_AGENT_ENV_VALUE, substitute_plugin_tokens};
    ...
    fn agent_marker_constants_match_cursor_parity() {
        assert_eq!(NEMESIS_AGENT_ENV, "NEMESIS_AGENT");
        assert_eq!(NEMESIS_AGENT_ENV_VALUE, "1");
    }
}

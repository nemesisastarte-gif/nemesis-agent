...
//! after every tool call to check for SKILL.md files in `.nemesis/skills/`,
...
/// Directories that contain skill definitions (`.nemesis/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".nemesis", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.nemesis/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.nemesis/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.nemesis/) are outside the git root so the walk breaks early.

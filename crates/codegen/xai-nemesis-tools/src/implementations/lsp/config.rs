//! LSP server configuration from `.nemesis/lsp.json`.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::nemesis_home::nemesis_home().join("lsp.json");
    let project_path = cwd.join(".nemesis").join("lsp.json");
...
/// Load LSP server configs from `~/.nemesis/lsp.json` and `<cwd>/.nemesis/lsp.json`.

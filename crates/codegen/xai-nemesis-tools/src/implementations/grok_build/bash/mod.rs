...
use regex::Regex;
use xai_nemesis_config::shell::AmpersandSemantics;
...
pub struct BashParams {
    ...
    /// grok-build opts up to 10h via xai-nemesis-shell's `BashToolConfig`.
    ///
    /// When set:
...
// particular production grok-build sets it to 10h in xai-nemesis-shell's
...
/// [`xai_nemesis_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = xai_nemesis_config::shell::chain_separator();
...
impl xai_tool_runtime::Tool for BashTool {
    ...
    async fn run(
        ...
        let ampersand = xai_nemesis_config::shell::ampersand_semantics();

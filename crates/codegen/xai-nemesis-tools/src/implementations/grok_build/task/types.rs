...
//! (in `xai-nemesis-tools`) and the subagent coordinator (in `xai-nemesis-shell`).
//!
//! ## Resource types
//!
...
/// defense-in-depth check in `xai-nemesis-shell`'s subagent coordinator.
...
/// the cross-crate drain loop in `xai-nemesis-shell` relies on
...
/// xai-nemesis-shell produces it, the channel protocol carries it, and the
...
///
/// Set by xai-nemesis-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by xai-nemesis-shell at turn start.

...
///
/// `skills` is the standard layout (`.nemesis/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into Nemesis Agent sessions.
...
/// name (e.g. `~/.nemesis/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Nemesis Agent.
...
/// `cwd` (exclusive). At each directory, checks for `.nemesis/skills/`,
...
pub fn discover_skills_for_paths(
    ...
    let mut config_dir_names: Vec<&str> = vec![".nemesis", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        ...
        assert!(!is_vendor_default_skill(
            "/home/u/.nemesis/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.nemesis/ → kept (user content).
        let grok_shell = tmp.path().join(".nemesis").join("skills").join("shell");
        ...
        assert!(skills[0].path.contains("/.nemesis/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        .unwrap();
        let grok_skill = sub.join(".nemesis").join("skills").join("grok-dyn");

...
//!
//! `ToolSearchIndex` is defined in `xai-nemesis-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `xai-nemesis-shell`
...
/// in `xai-nemesis-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `xai-nemesis-shell` after MCP initialization.
#[derive(Clone)]

...
//!
//! Historically the agent hard-coded the dir lists `[".nemesis", ".agents",
...
impl CompatConfig {
    ...
    ///
    /// Replaces the hard-coded `[".nemesis", ".agents", ".claude", ".cursor"]`
    ...
    pub fn skill_config_dirs(&self) -> Vec<&'static str> {
        let mut dirs = vec![".nemesis", ".agents"];
    ...
    /// Subdirectories scanned for `*.md` rules files. `.nemesis/rules` is always
    ...
    pub fn rules_dirs(&self) -> Vec<&'static str> {
        let mut dirs = vec![".nemesis/rules"];
...
mod tests {
    ...
    fn skill_config_dirs_all_on_matches_legacy_constant() {
        // Historical constant was `[".nemesis", ".agents", ".claude", ".cursor"]`.
        assert_eq!(
            CompatConfig::default().skill_config_dirs(),
            vec![".nemesis", ".agents", ".claude", ".cursor"]
        );
    }
    ...
    fn skill_config_dirs_gates_each_vendor() {
        ...
        assert_eq!(c.skill_config_dirs(), vec![".nemesis", ".agents", ".claude"]);

        c.claude.skills = false;
        assert_eq!(c.skill_config_dirs(), vec![".nemesis", ".agents"]);

        // Only the `cursor` cell on (`claude` off): `cursor` still appended last.
        let mut c2 = CompatConfig::default();
        c2.claude.skills = false;
        assert_eq!(c2.skill_config_dirs(), vec![".nemesis", ".agents", ".cursor"]);
    }

    #[test]
    fn rules_dirs_all_on_matches_legacy_constant() {
        // Historical `RULES_DIRS` was `[".nemesis/rules", ".claude/rules", ".cursor/rules"]`.
        assert_eq!(
            CompatConfig::default().rules_dirs(),
            vec![".nemesis/rules", ".claude/rules", ".cursor/rules"]
        );
    }
    ...
    fn rules_dirs_gates_each_vendor() {
        ...
        assert_eq!(c.rules_dirs(), vec![".nemesis/rules", ".claude/rules"]);
        c.claude.rules = false;
        assert_eq!(c.rules_dirs(), vec![".nemesis/rules"]);
    }

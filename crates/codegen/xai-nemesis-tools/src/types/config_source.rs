...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.nemesis/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.nemesis/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.nemesis/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.nemesis/ or ~/.claude/.
    User { path: PathBuf },

...
//!
//! `MemoryBackend` is defined in `xai-nemesis-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `xai-nemesis-tools`.
/// Mirrors `xai_nemesis_shell::session::memory_log::TARGET`.

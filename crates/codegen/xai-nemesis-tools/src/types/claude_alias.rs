...
//!
//! Two consumers read it independently. The hook matcher (`xai-nemesis-hooks`) needs the
//! Grok tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`xai-nemesis-agent`) needs the [`ToolKind`] a `tools:`
...
/// Every distinct Grok name the table references, for the `xai-nemesis-agent` drift-check

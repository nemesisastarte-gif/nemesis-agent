...
impl LeaderStdioClient {
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            ...
            .env("GROK_LEADER_SOCKET", home.join(".nemesis").join("leader.sock"))
            ...
            // ~/.nemesis/leader.log — keep it chatty for diagnosis.
            .env("RUST_LOG", "xai_nemesis_shell=debug");
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".nemesis").join("leader.lock")
}
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".nemesis").join("leader.log")).unwrap_or_default()
}

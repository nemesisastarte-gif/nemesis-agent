...
fn local_grok_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("xai-nemesis-pager{}", std::env::consts::EXE_SUFFIX))
}
...
fn ensure_local_grok_binary(binary: &Path) {
    ...
    let output = Command::new(&cargo)
        ...
        .args([
            "build",
            "-p",
            "xai-nemesis-pager-bin",
            "--bin",
            "xai-nemesis-pager",
        ])
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build xai-nemesis-pager: {e}"));

    assert!(
        output.status.success(),
        "failed to build xai-nemesis-pager for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "xai-nemesis-pager build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve grok binary: `GROK_BINARY` env (CI) or a locally built `xai-nemesis-pager` binary.
pub fn grok_binary() -> PathBuf {
    ...
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_xai-nemesis-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
...
pub fn test_env_cmd_tokio(
    ...
    cmd.env("HOME", home)
        ...
        // without an explicit NEMESIS_HOME every spawned child shares the real
        ...
        .env("NEMESIS_HOME", home.join(".nemesis"))
        ...
        .env("NEMESIS_DISABLE_AUTOUPDATER", "1");
}

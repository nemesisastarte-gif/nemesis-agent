...
//! lookup must be cheaper than re-loading from `~/.nemesis/config.toml`.
...
//! `xai_nemesis_shell::util::config::set_theme()` (and friends), invoked
...
///
/// On the first call, reads from `~/.nemesis/config.toml` (via the shell's
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    xai_nemesis_markdown::set_color_level_cap(if locked {
        xai_nemesis_markdown::ColorLevel::Basic
    } else {
        xai_nemesis_markdown::ColorLevel::TrueColor
    });
    xai_nemesis_markdown::set_polarity_safe_syntax(locked);
}
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `xai_nemesis_shell::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = xai_nemesis_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = xai_nemesis_config::load_effective_config_disk_only() else {
...
mod tests {
    ...
    fn terminal_native_lock_enables_polarity_safe_syntax() {
        with_test_env(|| {
            assert!(!xai_nemesis_markdown::polarity_safe_syntax());
            set_terminal_native_lock(true);
            assert!(
                xai_nemesis_markdown::polarity_safe_syntax(),

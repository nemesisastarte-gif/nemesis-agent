...
/// Documented in `xai-nemesis-pager/docs/internal/22-environment-variables.md`.
...
fn is_remote() -> bool {
    ...
    *REMOTE.get_or_init(xai_nemesis_shared::clipboard::is_remote_session)
}
...
fn is_container_no_display() -> bool {
    ...
    *CONTAINER.get_or_init(xai_nemesis_shared::clipboard::is_containerized_without_display)
}
...
/// clipboard (see `xai-nemesis-pager`'s `pty_wrap` module). It advertises this to
...
pub fn wayland_data_control_label() -> &'static str {
    match crate::host::DisplayServer::current() {
        crate::host::DisplayServer::Wayland => {
            if xai_nemesis_shared::clipboard::wayland_data_control_supported() {
                "yes"
            } else {
                "no"
            }
...
fn write_tmux_buffer(text: &str) -> bool {
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stdin = xai_nemesis_shared::clipboard::spool_for_stdin(text.as_bytes())?;
        ...
        let status = xai_nemesis_shared::clipboard::wait_with_deadline(
            &mut child,
...
///
/// Delegates to [`xai_nemesis_shared::clipboard`] which uses
...
impl ClipboardProvider for SystemClipboard {
    fn get(&mut self) -> Option<String> {
        xai_nemesis_shared::clipboard::get_text().ok().flatten()
    }
...
fn clipboard_write_with_route(text: &str, route: &ClipboardRoute) -> ClipboardWriteLegs {
    ...
    if route.native {
        let outcome = xai_nemesis_shared::clipboard::set_text_with_outcome(text);
    ...
    if route.osc52 {
        match xai_nemesis_shared::clipboard::set_text_osc52(text, route.osc52_tmux_passthrough) {
...
/// `~/.nemesis/last-copy.txt` (grok's per-user home — short, stable, and
...
pub fn default_copy_fallback_path() -> Option<std::path::PathBuf> {
    ...
    xai_nemesis_config::user_nemesis_home().map(|nemesis_home| nemesis_home.join("last-copy.txt"))
}
...
/// prefix collapses to `~/.grok` (or `$NEMESIS_HOME` when overridden), and a
...
fn log_clipboard_copy_event(
    ...
    if !xai_nemesis_telemetry::client::is_enabled() {
        return;
    }
    xai_nemesis_telemetry::session_ctx::log_event(xai_nemesis_telemetry::events::ClipboardCopy {
...
pub fn system_clipboard_read_text() -> Result<Option<String>, ClipboardTextReadError> {
    ...
    xai_nemesis_shared::clipboard::get_text().map_err(|error| {
...
pub fn system_primary_selection_get() -> Option<String> {
    ...
    if !xai_nemesis_shared::clipboard::x11_display_env_present() {
        return None;
    }
    xai_nemesis_shared::clipboard::get_primary_text()
        .ok()
        .flatten()
        .filter(|text| !text.is_empty())
}
...
pub fn x11_primary_guidance_available() -> bool {
    #[cfg(target_os = "linux")]
    {
        is_native_x11(crate::host::DisplayServer::current())
            && xai_nemesis_shared::clipboard::x11_display_env_present()
    }
...
pub fn log_paste_key_empty_host_clipboard(surface: &str) {
    ...
    xai_nemesis_telemetry::session_ctx::log_event(
        xai_nemesis_telemetry::events::PasteKeyEmptyHostClipboard {
            terminal,
...
fn log_clipboard_paste_event(
    ...
    xai_nemesis_telemetry::session_ctx::log_event(xai_nemesis_telemetry::events::ClipboardImagePaste {
        terminal: crate::terminal::terminal_context().telemetry_snapshot(),
...
fn system_clipboard_get_attachments() -> Result<AttachmentsProbeResult, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match xai_nemesis_shared::clipboard::get_attachments() {
        Ok(att) => {
...
pub use xai_nemesis_shared::clipboard::ImageData;
...
pub fn clipboard_image_snapshot() -> (Option<u64>, bool) {
    ...
    xai_nemesis_shared::clipboard::clipboard_image_snapshot()
}
...
pub fn clipboard_change_count() -> Option<u64> {
    ...
    xai_nemesis_shared::clipboard::clipboard_change_count()
}
...
pub fn clipboard_image_probe_supported() -> bool {
    ...
    xai_nemesis_shared::clipboard::clipboard_image_probe_supported()
}
...
pub fn prewarm_image_probe() {
    ...
    WARMED.call_once(|| {
        std::thread::spawn(xai_nemesis_shared::clipboard::clipboard_prewarm);
    });
}
...
fn system_clipboard_get_image_result() -> Result<Option<ImageData>, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match xai_nemesis_shared::clipboard::get_image() {
        Ok(img) => {
...
mod tests {
    ...
    /// Without `GROK_COPY_FILE`, the default is `~/.nemesis/last-copy.txt`
    /// (grok home) — short and toast-friendly, unlike macOS's temp dir.
    #[test]
    #[serial_test::serial(grok_copy_file)]
    fn default_copy_fallback_path_is_nemesis_home() {
        unsafe {
            std::env::remove_var(GROK_COPY_FILE_ENV);
        }
        let path = default_copy_fallback_path();
        // Test envs always resolve a home (or set NEMESIS_HOME).
        let expected = xai_nemesis_config::user_nemesis_home()
    ...
    /// the `NEMESIS_HOME`-override integration test in `xai-nemesis-pager`).
    #[test]
    fn display_copy_path_abbreviates_home() {
        if std::env::var_os("NEMESIS_HOME").is_none() {
            let home = dirs::home_dir().expect("home resolves in tests");
            assert_eq!(
                display_copy_path(&home.join(".nemesis").join("last-copy.txt")),
                "~/.nemesis/last-copy.txt"
            );
        }

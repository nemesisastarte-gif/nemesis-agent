...
pub use xai_nemesis_config::nemesis_home;

/// Path to `$NEMESIS_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    nemesis_home().join("pager.toml")
}

/// User-facing label for the user grok directory (``~/.grok`` or ``$NEMESIS_HOME``).
///
/// Derived from resolved [`nemesis_home()`] vs `xai_nemesis_config::default_nemesis_home()`,
/// not from whether `NEMESIS_HOME` is set in the environment.
pub fn display_nemesis_home_prefix() -> String {
    if nemesis_home() == xai_nemesis_config::default_nemesis_home() {
        "~/.grok".to_string()
    } else {
        "$NEMESIS_HOME".to_string()
    }
}

/// User-facing path under [`nemesis_home()`], e.g. ``~/.nemesis/config.toml``.
pub fn display_user_grok_path(relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_nemesis_home_prefix();
...
/// Abbreviate an absolute path for display: prefer [`nemesis_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let grok = nemesis_home();
...
/// True when `path` is under user [`nemesis_home()`] (not project `{cwd}/.grok`).
pub fn is_under_user_nemesis_home(path: &Path) -> bool {
    path.starts_with(nemesis_home())
}
...
mod tests {
    ...
    #[test]
    fn display_nemesis_home_prefix_default_install() {
        if std::env::var("NEMESIS_HOME").is_ok() {
            return;
        }
        assert_eq!(display_nemesis_home_prefix(), "~/.grok");
    }
    ...
    fn display_user_grok_path_joins_relative() {
        ...
        assert!(path.contains(".nemesis") || path.contains("$NEMESIS_HOME"));
    }
    ...
    fn abbreviate_path_uses_home_when_under_default_grok() {
        if let Ok(home) = std::env::var("HOME") {
            ...
            let full = format!("{home}/.nemesis/memory/MEMORY.md");

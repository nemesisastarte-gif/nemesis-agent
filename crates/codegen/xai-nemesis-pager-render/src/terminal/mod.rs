...
impl TerminalContext {
    ...
    pub fn telemetry_snapshot(&self) -> xai_nemesis_telemetry::events::TerminalTelemetry {
        ...
        xai_nemesis_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: xai_nemesis_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> xai_nemesis_shared::session::FeedbackTerminalInfo {
        use xai_nemesis_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(xai_nemesis_shared::clipboard::native_tool_name().to_owned()),
...
/// `xai-nemesis-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
///
/// Parsed from `[terminal] alt_screen` in `~/.nemesis/pager.toml` and

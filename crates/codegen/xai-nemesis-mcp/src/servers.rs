...
pub struct McpState {
    ...
    /// Persisted to `~/.nemesis/config.toml` under `[mcp_servers.<name>].disabled_tools`.
...
/// MCP initialization strategy. Defined in `xai-nemesis-telemetry`; re-exported
/// here so existing call sites continue to work.
pub use xai_nemesis_telemetry::enums::McpInitStrategy;
...
impl xai_tool_runtime::Tool for McpErasedTool {
    ...
    async fn run(
        ...
        xai_nemesis_telemetry::session_ctx::log_event(xai_nemesis_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
/// External-config overrides for an MCP server, surfaced to xai-nemesis-mcp
...
impl McpClient {
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: xai_nemesis_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    fn make_client_info(server_name: &str) -> ClientInfo {
        ...
        ClientInfo::new(
            ...
            Implementation::new(
                ...
                xai_nemesis_version::VERSION.to_string(),
            ),
        )
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer =
            xai_nemesis_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = xai_nemesis_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
/// Copy an MCP server's stderr to `~/.nemesis/logs/mcp/<server>.stderr.log`
/// in a background task. Truncated per spawn.
fn drain_mcp_stderr_to_log(server_name: &str, mut stderr: tokio::process::ChildStderr) {
    let log_dir = xai_nemesis_config::nemesis_home().join("logs").join("mcp");
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = xai_nemesis_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = xai_nemesis_telemetry::instrumentation::timer("mcp_stdio_spawn");
                ...
                SafeTokioChildProcess::spawn(cmd, name.clone(), event_writer.clone()).map_err(
                    |e| {
                        ...
                        xai_nemesis_telemetry::session_ctx::log_event(
                            xai_nemesis_telemetry::events::McpServerFailed {
                                server_name: name.clone(),
                                error_type: xai_nemesis_telemetry::events::McpErrorType::SpawnFailed,
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    xai_nemesis_telemetry::instrumentation::timer("mcp_http_auth_discovery");
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = xai_nemesis_telemetry::instrumentation::timer("mcp_start_servers");

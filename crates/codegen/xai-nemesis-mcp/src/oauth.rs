...
const MCP_OAUTH_CLIENT_NAME: &str = "Nemesis";
...
//
// Layer 1 (cross-process): filesystem lock at $NEMESIS_HOME/mcp_auth_{safe_name}.lock
...
fn auth_lock_path(server_name: &str) -> std::path::PathBuf {
    let safe: String = server_name
        ...
        .collect();
    xai_nemesis_config::nemesis_home().join(format!("mcp_auth_{safe}.lock"))
}

...
//!
//! Credentials are stored in `$NEMESIS_HOME/mcp_credentials.json`, keyed by a
...
///
/// Local helper (not shell-base): `xai-nemesis-mcp` sits below `config-types` in the
...
/// File name for the credential store inside `$NEMESIS_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$NEMESIS_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$NEMESIS_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user grok home (set $NEMESIS_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$NEMESIS_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(xai_nemesis_config::user_nemesis_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$NEMESIS_HOME/mcp_credentials.json`. Existing credential files must keep

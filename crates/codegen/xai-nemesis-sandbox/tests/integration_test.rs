//! Integration tests for xai-nemesis-sandbox.
//!
...
fn test_profile_capability_set_construction() {
    use xai_nemesis_sandbox::ProfileName;
...
fn test_sandbox_logger() {
    use xai_nemesis_sandbox::{SandboxEvent, SandboxLogger};
...
fn test_should_restrict_child_network_default() {
    ...
    assert!(!xai_nemesis_sandbox::should_restrict_child_network());
}

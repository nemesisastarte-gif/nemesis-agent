...
//! OS-level sandboxing for Nemesis Agent via [nono](https://crates.io/crates/nono).
//!
...
//!
//! ```rust,no_run
//! use xai_nemesis_sandbox::{SandboxManager, ProfileName};
//! use std::path::Path;
//!
...
/// Zero-permission placeholder (file or dir) under `nemesis_home` used by bwrap bind-over.
///
...
fn bwrap_blocked_placeholder(name: &str, want_dir: bool) -> Option<PathBuf> {
    ...
    let path = paths::nemesis_home().join(format!("{name}.{}", std::process::id()));
...
mod tests {
    ...
    /// Create a temp workspace whose `.nemesis/sandbox.toml` contains `toml_body`.
    ...
    fn bwrap_reexec_uses_dir_placeholder_for_directories() {
        ...
        let args: Vec<String> = cmd
            ...
            .map(|a| a.to_string_lossy().to_string())
            .collect();
        let blocked_dir = paths::nemesis_home()

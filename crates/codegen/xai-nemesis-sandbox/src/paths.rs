...
/// Grok state directory — always writable (`$NEMESIS_HOME` or `~/.grok`).
pub(crate) fn nemesis_home() -> PathBuf {
...
pub(crate) fn essential_writable_paths(workspace: &Path) -> Vec<PathBuf> {
    let mut paths = vec![workspace.to_path_buf(), nemesis_home()];
...
pub(crate) fn essential_writable_paths_minimal() -> Vec<PathBuf> {
    let mut paths = vec![nemesis_home()];
    paths.extend(temp_writable_paths());
    paths
}

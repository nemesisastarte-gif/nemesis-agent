...
impl ChatStateActor {
    ...
    pub(super) fn truncate_to_prompt_index(&mut self, target_prompt_index: usize) {
        ...
        for (i, item) in self.state.conversation.iter().enumerate() {
            if matches!(item, xai_nemesis_sampling_types::ConversationItem::User(_)) {
    ...
    pub(super) fn get_last_model_metadata(&self) -> crate::commands::ModelMetadata {
        self.state
            ...
            .rev()
            .find_map(|item| {
                if let xai_nemesis_sampling_types::ConversationItem::Assistant(a) = item {
    ...
    pub(super) fn has_dangling_tool_calls(&self) -> bool {
        xai_nemesis_sampling_types::has_dangling_tool_calls(&self.state.conversation)
    }
    ...
    ///
    /// [`SyntheticReason::starts_prompt_turn`]: xai_nemesis_sampling_types::SyntheticReason::starts_prompt_turn
    pub(super) fn get_last_assistant_text_in_turn(&self) -> Option<String> {
        for item in self.state.conversation.iter().rev() {
            match item {
                xai_nemesis_sampling_types::ConversationItem::Assistant(a)
                    if !a.content.trim().is_empty() =>
                {
                    return Some(a.content.as_ref().to_owned());
                }
                xai_nemesis_sampling_types::ConversationItem::User(u)
    ...
    pub(super) fn get_first_user_text(&self) -> Option<String> {
        self.state.conversation.iter().find_map(|item| {
            if let xai_nemesis_sampling_types::ConversationItem::User(u) = item {
                ...
                u.content.first().and_then(|part| {
                    if let xai_nemesis_sampling_types::ContentPart::Text { text } = part {
    ...
    pub(super) fn get_conversation_item_at(
        &self,
        index: usize,
    ) -> Option<xai_nemesis_sampling_types::ConversationItem> {
    ...
    pub(super) fn get_conversation_counts(&self) -> ConversationCounts {
        ...
        for item in &self.state.conversation {
            match item {
                xai_nemesis_sampling_types::ConversationItem::User(_) => counts.user += 1,
                xai_nemesis_sampling_types::ConversationItem::Assistant(_) => {
                    counts.assistant += 1;
                }
                xai_nemesis_sampling_types::ConversationItem::ToolResult(_) => {
                    counts.tool_result += 1;
                }
                xai_nemesis_sampling_types::ConversationItem::System(_) => {}
                xai_nemesis_sampling_types::ConversationItem::BackendToolCall(_) => {}
                xai_nemesis_sampling_types::ConversationItem::Reasoning(_) => {}
            }
        }
        counts
    }
    ...
    pub(super) fn get_system_message(&self) -> Option<xai_nemesis_sampling_types::ConversationItem> {
        self.state
            .conversation
            .iter()
            .find(|item| matches!(item, xai_nemesis_sampling_types::ConversationItem::System(_)))
            .cloned()
    }
}

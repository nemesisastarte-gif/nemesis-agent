//! Memory-system configuration value types, extracted from xai-nemesis-shell
...
//! `xai-nemesis-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.nemesis/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.nemesis/memory/` are scanned:

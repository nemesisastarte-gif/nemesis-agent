...
pub struct RemoteSettings {
    ...
    /// `xai_nemesis_telemetry::external::apply_remote_policy`.
    #[serde(default)]
    ...
    /// Whether the user is allowed to use Nemesis Agent. Set by remote settings
    ...
    /// `0` disables. Env (`NEMESIS_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`NEMESIS_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`NEMESIS_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings verbatim-input flag; env (`NEMESIS_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]

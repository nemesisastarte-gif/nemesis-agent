//! Parse marketplace sources from `~/.nemesis/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
...
pub fn env_require_sha() -> bool {
    xai_nemesis_config::env_bool("GROK_MARKETPLACE_REQUIRE_SHA").unwrap_or(false)
}
...
/// and `known_marketplaces.json` files under `~/.nemesis/` and `~/.claude/`.
pub fn load_extra_sources_from_settings(existing: &[MarketplaceSource]) -> Vec<MarketplaceSource> {
    let roots: Vec<PathBuf> = [
        xai_nemesis_config::user_nemesis_home(),

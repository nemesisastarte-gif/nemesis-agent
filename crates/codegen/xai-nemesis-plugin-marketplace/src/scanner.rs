...
fn scan_single_plugin(plugin_dir: &Path, relative_path: &str) -> MarketplaceEntry {
    ...
    let manifest_result = xai_nemesis_agent::plugins::manifest::load_manifest(plugin_dir);
    let manifest = match &manifest_result {
        Ok(xai_nemesis_agent::plugins::manifest::ManifestLoadResult::Found(m)) => Some(m.as_ref()),
        _ => None,
    };
...
mod tests {
    ...
    fn grok_plugin_dir_index_drives_scan_end_to_end() {
        ...
        assert_eq!(plugins[0].tags, vec!["nemesis"]);
        assert!(plugins[0].keywords.is_empty());
    }

...
mod tests {
    ...
    fn load_index_valid_grok_dir() {
        ...
        std::fs::write(
            grok_dir.join("marketplace.json"),
            r#"{"name": "nemesis", "plugins": []}"#,
        )
        .unwrap();
        let result = load_index(dir.path()).unwrap();
        assert!(result.is_some());
        assert_eq!(result.unwrap().name, "nemesis");
    }

    #[test]
    fn load_index_grok_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".grok-plugin", "nemesis"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "nemesis");
    }

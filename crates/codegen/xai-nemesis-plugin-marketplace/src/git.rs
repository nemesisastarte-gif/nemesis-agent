...
//! Cache root: `~/.nemesis/marketplace-cache/<url-hash>/`
...
pub fn sync_source_cache_with_mode(
    ...
    let url = xai_nemesis_agent::plugins::git_install::validate_git_url(url)?;
    let branch = branch
        .map(xai_nemesis_agent::plugins::git_install::validate_git_ref)
        .transpose()?;
...
pub fn default_cache_root() -> PathBuf {
    xai_nemesis_config::nemesis_home().join("marketplace-cache")
}

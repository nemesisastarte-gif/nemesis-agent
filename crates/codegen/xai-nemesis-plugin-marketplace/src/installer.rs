...
use xai_nemesis_agent::plugins::manifest::{ManifestLoadResult, load_manifest, name_from_dirname};
...
mod tests {
    ...
    fn with_test_registry<T>(f: impl FnOnce(&mut InstallRegistry) -> T) -> T {
        ...
        // process-global `nemesis_home()` `OnceLock` (first-write-wins). A parallel

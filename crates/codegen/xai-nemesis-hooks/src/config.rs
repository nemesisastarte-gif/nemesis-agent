...
pub struct HookSpec {
    ...
    ///    `GROK_SESSION_ID`, `NEMESIS_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are
    ///    stripped at load time with a tracing warning.
    /// 2. Plugin-injected vars merged by the plugin adapter
    ///    (`xai-nemesis-agent::plugins::hooks_adapter`): `GROK_PLUGIN_ROOT`,
...
mod tests {
    ...
    fn source_dir_from_file_path() {
        ...
        let (specs, _) = parse_hook_file(json, Path::new("/home/user/.nemesis/hooks/safety.json"));
        assert_eq!(specs[0].source_dir, PathBuf::from("/home/user/.nemesis/hooks"));
    }

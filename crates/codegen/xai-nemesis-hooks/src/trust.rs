...
// (`~/.nemesis/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_nemesis_home>/trusted-hook-projects`), or `None` when no user grok home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(xai_nemesis_config::user_nemesis_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user grok home (set $NEMESIS_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$NEMESIS_HOME/disabled-hooks`, or `None` when no user grok
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(xai_nemesis_config::user_nemesis_home()?.join("disabled-hooks"))
}

...
pub async fn run_command_hook(
    ...
    let mut cmd = if is_shell_command {
        ...
        #[cfg(not(unix))]
        {
            let inv = xai_nemesis_config::shell::shell_command_argv(&command_str);
    ...
    // `NEMESIS_WORKSPACE_ROOT`, or `CLAUDE_PROJECT_DIR`, which are the
    ...
    let mut child = match cmd
        ...
        .env("NEMESIS_WORKSPACE_ROOT", ctx.workspace_root)
        // Compatibility alias for external hooks that read this env name.
        // Same value as `NEMESIS_WORKSPACE_ROOT`; native `.grok` hooks should use
        // `NEMESIS_WORKSPACE_ROOT`.
    ...
    fn resolve_command_path_variants() {
        ...
        assert_eq!(
            resolve_command_path(&spec(
                HandlerType::Command,
                Some("bin/check.sh"),
                "/project/.nemesis/hooks"
            )),
            Some(std::path::PathBuf::from(
                "/project/.nemesis/hooks/bin/check.sh"
            ))
        );
        assert_eq!(

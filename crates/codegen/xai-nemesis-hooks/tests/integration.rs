//! Integration tests for xai-nemesis-hooks.
//!
...
use xai_nemesis_hooks::discovery::load_hooks;
use xai_nemesis_hooks::dispatcher;
use xai_nemesis_hooks::event::*;
use xai_nemesis_hooks::result::HookDecision;
use xai_nemesis_hooks::runner::RunContext;
    ...
    let cases = vec![
        ...
        Case {
            ...
            payload: HookPayload::StopFailure {
                error: xai_nemesis_hooks::event::StopFailureKind::RateLimit,
...
/// `GROK_SESSION_ID`, `NEMESIS_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$GROK_HOOK_EVENT" > {f}; echo "NAME=$GROK_HOOK_NAME" >> {f}; echo "SESSION=$GROK_SESSION_ID" >> {f}; echo "ROOT=$NEMESIS_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                ...
                                "NEMESIS_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "NEMESIS_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed NEMESIS_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(

...
//!
//! ```bash
//! LEADER_SOAK_SECS=1200 cargo test -p xai-nemesis-shell --test test_leader_soak -- --ignored --nocapture
//! ```
...
async fn leader_soak_churning_clients_no_leaks_no_zombies() {
    ...
    let sock_path = nemesis_home.path().join("leader-soak.sock");
    ...
    local
        .run_until(async {
            ...
            // the same wiring: xai-nemesis-pager/src/app/leader_cluster/mod.rs

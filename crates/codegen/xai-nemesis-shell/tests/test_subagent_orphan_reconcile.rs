...
//! ```bash
//! cargo test -p xai-nemesis-shell --test test_subagent_orphan_reconcile -- --ignored
//! ```
...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        // GrokStdioClient sets HOME=<temp>; the binary uses <HOME>/.grok as NEMESIS_HOME.
        let nemesis_home = shared_home.path().join(".nemesis");
        let session_dir = locate_session_dir(&nemesis_home, session_id.0.as_ref());

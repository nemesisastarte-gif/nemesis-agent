...
//! ```bash
//! cargo test -p xai-nemesis-shell --test test_auth_provider_e2e -- --ignored
//! ```
//!
...
async fn provider_backed_model_sends_minted_token_on_the_wire() {
    ...
    let counter = nemesis_home.join("mint-count");
    let helper = nemesis_home.join("mock-auth.sh");
    std::fs::write(
        &helper,
        format!(
...
async fn provider_with_args_and_json_output_sends_minted_token() {
    ...
    let seen_args = nemesis_home.join("seen-args");
    let helper = nemesis_home.join("mock-auth-json.sh");
    std::fs::write(
        &helper,
        format!(
            "#!/bin/sh\nprintf '%s' \"$*\" > {}\n\

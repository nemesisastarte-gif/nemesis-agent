...
//!   cargo test -p xai-nemesis-shell --test test_leader_version_skew -- --ignored --nocapture
//! ```
...
use xai_nemesis_test_support::leader::{
...
fn sandbox_unified_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".nemesis").join("logs").join("unified.jsonl"))
        .unwrap_or_default()
}
...
/// `signal_leaders_to_relaunch` in `xai-nemesis-pager-bin/src/main.rs` (which is
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let control = LeaderClient::connect(
                home.path().join(".nemesis").join("leader.sock"),
                "grok-pager-update",
                ClientMode::Stdio,
...
async fn eviction_leaves_single_leader_and_single_auth_owner() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let auth_path = home.path().join(".nemesis").join("auth.json");

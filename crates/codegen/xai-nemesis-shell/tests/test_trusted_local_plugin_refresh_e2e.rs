...
//! 4. Smoke-validate session JSON under `$NEMESIS_HOME/sessions/` after exit.
//!
//! Requires a built `grok` binary (`GROK_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p xai-nemesis-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p xai-nemesis-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use xai_nemesis_agent::plugins::SharedPluginRegistryHandle;
use xai_nemesis_agent::plugins::discovery::DiscoveryConfig;
use xai_nemesis_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _grok_guard = EnvVarGuard::set("NEMESIS_HOME", &nemesis_home);
    ...
    let mut registry = InstallRegistry::empty(nemesis_home.join("installed-plugins"));
    ...
    // nemesis_home() caches the first NEMESIS_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        xai_nemesis_agent::plugins::PluginScope::CliOverride
    );
...
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    let home = dunce::canonicalize(home_tmp.path()).unwrap();
    let nemesis_home = home.join(".nemesis");
    std::fs::create_dir_all(&nemesis_home).unwrap();
    ...
    // The spawned binary gets HOME/NEMESIS_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via nemesis_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _grok_guard = EnvVarGuard::set("NEMESIS_HOME", &nemesis_home);
    ...
    cmd.env("HOME", &home);
    cmd.env("NEMESIS_HOME", &nemesis_home);
    ...
    // Smoke: session storage under NEMESIS_HOME/sessions has JSON artifacts.

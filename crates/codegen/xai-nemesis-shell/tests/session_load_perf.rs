...
//!
//! Run:
//!   cargo test -p xai-nemesis-shell --test session_load_perf -- --nocapture
//!   cargo test -p xai-nemesis-shell --test session_load_perf full_session_load_e2e -- --ignored --nocapture
...
use xai_nemesis_shell::session::info::Info;
use xai_nemesis_shell::session::storage::{
    JsonlStorageAdapter, StorageAdapter, load_updates_for_replay_at,
};
use xai_nemesis_workspace::session::file_state::{FileSnapshot, FlexiblePath, RewindPoint};
...
async fn full_session_load_e2e() {
    ...
    let server = xai_nemesis_test_support::MockInferenceServer::start()
        .await
        .unwrap();
    ...
    let cwd = TempDir::new().unwrap();
    let opts = GenOpts::from_env();
    let instr_log = nemesis_home.path().join("instr.jsonl");

    // SAFETY: single-threaded current-thread runtime; set before any agent code
    // reads these process-globals (nemesis_home()/instrumentation mode are OnceLock).
    unsafe {
        std::env::set_var("NEMESIS_HOME", nemesis_home.path());
    ...
    let _ = tracing_subscriber::registry()
        .with(xai_nemesis_shell::instrumentation::layer::<Registry>())
        .try_init();

    let (info, dir) = prepare_session(nemesis_home.path(), cwd.path(), &opts).await;
    let updates_path = dir.join("updates.jsonl");
    let rewind_path = dir.join("rewind_points.jsonl");
    eprintln!(
    ...
    local
        .run_until(async move {
            ...
            let _ = xai_nemesis_shell::instrumentation::finalize();

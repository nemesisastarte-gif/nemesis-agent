//! Built-binary end-to-end tests for the grok (xai-nemesis-pager) binary.
//!
...
//! ```bash
//! cargo test -p xai-nemesis-shell --test test_built_binary_e2e -- --ignored
//! ```
//!
...
async fn test_headless_free_usage_exhausted_prints_paywall_message() {
    ...
    assert!(
        combined.contains("reached your free Nemesis Agent usage limit"),
...
async fn test_headless_json_reports_server_cost() {
    use xai_nemesis_test_support::scripted::SseEvent;
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        xai_nemesis_test_support::scripted::ScriptedResponse::sse(vec![
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        xai_nemesis_test_support::scripted::ScriptedResponse::sse(
            xai_nemesis_test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
...
/// Isolated headless run with a custom `~/.nemesis/`. Clean env (no leaked
...
impl ConfigTestHarness {
    fn new(server: &MockInferenceServer) -> Self {
        ...
        std::fs::create_dir_all(home.path().join(".nemesis")).unwrap();
        Self {
            home,
            workdir: git_workdir(),
            env: vec![
                ...
                ("NEMESIS_DISABLE_AUTOUPDATER".into(), "1".into()),
            ],
        }
    }

    fn grok_dir(&self) -> std::path::PathBuf {
        self.home.path().join(".nemesis")
    }
    ...
    async fn run(&self) -> HeadlessResult {
        ...
        cmd.args(["-p", "say hello", "--yolo"])
            ...
            // `test_env_cmd_tokio`).
            .env("NEMESIS_HOME", self.grok_dir())

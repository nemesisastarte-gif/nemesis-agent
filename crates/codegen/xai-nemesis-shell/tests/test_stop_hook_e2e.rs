...
//! ```bash
//! cargo test -p xai-nemesis-shell --test test_stop_hook_e2e -- --ignored
//! ```

use xai_nemesis_test_support::env::test_env_cmd_tokio;
...
async fn run_with_stop_hook(respond: &str) -> StopHookRun {
    ...
    let hooks_dir = home.path().join(".nemesis").join("hooks");
...
async fn stop_block_loop_ends_at_continuation_cap() {
    ...
    assert_eq!(
        run.invocations(),
        xai_nemesis_shell::session::MAX_STOP_HOOK_CONTINUATIONS_PER_TURN,

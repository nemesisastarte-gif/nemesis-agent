...
use xai_nemesis_sampler::RetryPolicy;
use xai_nemesis_sampling_types::doom_loop::{DoomLoopSignalKind, SAMPLE_CHECK_EVENT_DATA_CUMULATIVE};
use xai_nemesis_shell::sampling::{
    ApiBackend, Client, ConversationItem, ConversationRequest, RequestId, SamplerActor,
    SamplerHandle,
};
use xai_nemesis_test_support::sse::{
...
use xai_nemesis_test_support::{MockInferenceServer, MockModelEntry, ScriptedResponse};
...
/// ```bash
/// cargo test -p xai-nemesis-shell --test test_doom_loop_recovery -- --ignored
/// ```
#[tokio::test]
...
async fn headless_config_enables_doom_loop_check_header() {
    ...
    let workdir = xai_nemesis_test_support::git_workdir();
    let home = tempfile::TempDir::new().unwrap();

    let nemesis_home = home.path().join(".nemesis");
    std::fs::create_dir_all(&nemesis_home).expect("create .grok home");
    std::fs::write(
    ...
    let mut cmd = tokio::process::Command::new(xai_nemesis_test_support::grok_binary());
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    xai_nemesis_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home.path());
    ...
    let result = xai_nemesis_test_support::run_headless_with_cmd(cmd).await;
    xai_nemesis_test_support::assert_headless_success(&result, "doom-loop header e2e", Some(&server));

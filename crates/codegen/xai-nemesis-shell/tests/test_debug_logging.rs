...
//! caller-owned `$NEMESIS_HOME`, then inspects `~/.nemesis/debug/`:
...
//!   `~/.nemesis/debug/<sessionId>.txt` with first-party content, and does NOT enable
...
//! - a live `agent` session (explicit `NEMESIS_DEBUG_LOG=1`) writes a per-session
//!   `~/.nemesis/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.nemesis/debug/` files).
//! - `NEMESIS_LOG_FILE=<path>` writes that explicit file (back-compat single file).
//!
...
//! ```bash
//! cargo test -p xai-nemesis-shell --test test_debug_logging -- --ignored
//! ```
...
/// The per-session firehose directory under a pinned `$NEMESIS_HOME`.
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".nemesis").join("debug")
}

/// List firehose `*.txt` files under `~/.nemesis/debug` (excluding the `latest.txt`
...
/// Build a headless `grok -p` command with a pinned `$NEMESIS_HOME` so the firehose
/// lands under `<home>/.nemesis/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    xai_nemesis_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home);
    // Pin the home location and drop inherited firehose toggles for determinism.
    cmd.env("NEMESIS_HOME", home.join(".nemesis"));
    cmd.env_remove("NEMESIS_DEBUG_LOG");
    cmd.env_remove("NEMESIS_LOG_FILE");
    cmd.env_remove("GROK_LOG_SAMPLING");
    cmd.env_remove("NEMESIS_HOOKS_LOG");
    cmd
}
...
/// A live `agent` session writes `~/.nemesis/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        let workdir = git_workdir();
        let home = TempDir::new().expect("create temp home");
        let nemesis_home = home.path().join(".nemesis");
        let nemesis_home_str = nemesis_home.to_string_lossy().into_owned();

        let client = GrokStdioClient::spawn_with_home_and_env(
            &server,
            workdir.path(),
            home,
            &[("NEMESIS_DEBUG_LOG", "1"), ("NEMESIS_HOME", &nemesis_home_str)],
        )
        .await;
        ...
        let session_file = nemesis_home.join("debug").join(format!("{sid}.txt"));
        ...
        // file, so `tail -f ~/.nemesis/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
            let link = nemesis_home.join("debug").join("latest.txt");
...
/// The `--debug` FLAG (not `NEMESIS_DEBUG_LOG` directly) drives the firehose end to
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // `~/.nemesis/logs/sampling.jsonl` is never written — the `--debug`
        ...
        let sampling = nemesis_home.join("logs").join("sampling.jsonl");
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.nemesis/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
...
/// `NEMESIS_LOG_FILE=<path>` (no `--debug`) writes that exact file (back-compat).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn grok_log_file_explicit_path_is_written() {
    ...
    cmd.env("NEMESIS_LOG_FILE", &custom);
    let result = run_headless_with_cmd(cmd).await;

    assert_headless_success(&result, "grok NEMESIS_LOG_FILE=path", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        custom.exists(),
        "explicit NEMESIS_LOG_FILE path not written: {custom:?}\nstderr tail:\n{}",
        stderr_tail(&result.stderr, 800)
    );
    // Single-file mode bypasses per-session routing.
    assert!(
        firehose_txt_files(home.path()).is_empty(),
        "NEMESIS_LOG_FILE must bypass per-session routing, found: {:?}",
        firehose_txt_files(home.path())
    );
}

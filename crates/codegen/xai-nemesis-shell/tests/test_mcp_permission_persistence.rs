...
/// Shared `NEMESIS_HOME` for the entire test binary. The `OnceLock` in
/// `xai-nemesis-config` only allows one value per process, so all tests share
...
async fn dont_ask_policy_denies_without_prompting() {
    use xai_nemesis_workspace::permission::types::{PermissionConfig, PromptPolicy};

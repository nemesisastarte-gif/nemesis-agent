...
//! Run locally:
//! ```bash
//! cargo test -p xai-nemesis-shell --test test_agent_type_invariant -- --ignored
//! ```
...
fn invalidate_models_cache(home: &std::path::Path) {
    let cache = home.join(".nemesis").join("models_cache.json");
    if cache.exists() {
...
async fn test_default_model_uses_grok_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("Nemesis") || sys_prompt.contains("nemesis"),
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        let original_has_grok =
            original_sys_prompt.contains("Nemesis") || original_sys_prompt.contains("nemesis");
        let resumed_has_grok =
            resumed_sys_prompt.contains("Nemesis") || resumed_sys_prompt.contains("nemesis");
        assert_eq!(
            original_has_grok,
            resumed_has_grok,
...
/// The `NEMESIS_AGENT` escape hatch should override the model's agent_type.
/// Setting `NEMESIS_AGENT=grok-build` with an alternate-agent model should use
/// grok-build harness.
#[tokio::test]
#[ignore]
async fn test_grok_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.args(["agent", "stdio"])
                ...
                .kill_on_drop(true);
            xai_nemesis_test_support::env::test_env_cmd_tokio(
                &mut cmd,
                &server.url(),
                home.path(),
            );
            cmd.env("NEMESIS_AGENT", "grok-build");
            ...
            assert!(
                sys_prompt.contains("Nemesis") || sys_prompt.contains("nemesis"),
                "NEMESIS_AGENT=grok-build should override cursor model's agent_type\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;

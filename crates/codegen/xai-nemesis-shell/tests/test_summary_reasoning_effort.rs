...
//! Run locally:
//! ```bash
//! cargo test -p xai-nemesis-shell --test test_summary_reasoning_effort -- --ignored
//! ```
...
/// Find `summary.json` for `session_id` under `<home>/.nemesis/sessions/` and
...
fn read_summary(home: &std::path::Path, session_id: &str) -> serde_json::Value {
    let sessions_root = home.join(".nemesis").join("sessions");
...
async fn test_fresh_session_persists_reasoning_effort() {
    with_local_set(|| async {
        ...
        let grok_dir = home.path().join(".nemesis");

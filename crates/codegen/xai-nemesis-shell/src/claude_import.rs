// claude_import.rs
// Scans Claude settings and generates TOML patches for .nemesis/config.toml.
//
...
use xai_nemesis_workspace::permission::claude_settings::{
    find_claude_settings_paths, load_claude_settings,
};
use xai_nemesis_workspace::permission::rules::parse_permission_rule;
use xai_nemesis_workspace::permission::types::{PatternMode, PermissionRule, RuleAction, ToolFilter};
...
pub enum ImportScope {
    /// User-level: writes to `~/.nemesis/config.toml`.
    Global,
    /// Project-level: writes to `<repo>/.nemesis/config.toml`.
    Project,
}
...
pub struct ImportPlan {
    /// Items to write to `~/.nemesis/config.toml`.
    pub global_items: Vec<ImportableItem>,
    /// Items to write to `<repo>/.nemesis/config.toml`.
...
//
// The marker `[claude_compat] imported = true` in `~/.nemesis/config.toml` is
...
///
/// Reads `[claude_compat] imported = true` from `~/.nemesis/config.toml` once
...
/// Write `[claude_compat] imported = true` to `~/.nemesis/config.toml`.
///
...
pub fn mark_claude_imported() -> anyhow::Result<()> {
    let path = crate::util::nemesis_home::nemesis_home().join("config.toml");
...
/// overwritten).
///
/// Project items are written to `<repo_root>/.nemesis/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.nemesis/config.toml`, to avoid
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.global_items.is_empty() {
        let global_path = crate::util::nemesis_home::nemesis_home().join("config.toml");
        ...
        // Hooks are written separately to ~/.nemesis/hooks/imported-from-claude.json.
        let hooks_dir = crate::util::nemesis_home::nemesis_home().join("hooks");
    ...
    if !plan.project_items.is_empty() {
        let project_root = find_project_root(cwd);
        let project_path = project_root.join(".nemesis").join("config.toml");
        ...
        let hooks_dir = project_root.join(".nemesis").join("hooks");
...
fn apply_items_to_config(config_path: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    for item in items {
        match item {
            ...
            // Hooks are written to .nemesis/hooks/ JSON files in apply_hooks_to_dir,
...
///
/// The output JSON is the same shape that `xai-nemesis-hooks` natively understands
/// (Claude-compatible). The native hooks loader scans `.nemesis/hooks/*.json`
...
fn apply_hooks_to_dir(hooks_dir: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    if count > 0 || dirty {
        ...
        info!(
            path = %target.display(),
            count,
            "Wrote imported hooks to .nemesis/hooks/imported-from-claude.json"
        );
    }
...
mod tests {
    ...
    fn gate_load_claude_env_returns_empty_when_marker_set() {
        ...
        refresh_marker_cache(true);
        let dir = tempfile::tempdir().unwrap();
        let env = xai_nemesis_workspace::permission::claude_settings::load_claude_env_with_project(
            dir.path(),
            true,
        );
        assert!(
    ...
    fn discover_hook_source_paths_skips_cursor_when_disabled() {
        ...
        refresh_marker_cache(false);
        let dir = tempfile::tempdir().unwrap();
        let mut compat = xai_nemesis_tools::types::compat::CompatConfig::default();
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        let has_probe = |reg: &xai_nemesis_hooks::discovery::HookRegistry| {
...
"#;
    ...
    fn scan_claude_path_dirs_dedupes_global_and_project_when_same() {
        ...
        // requires `NEMESIS_HOME` plumbing which is intentionally out of scope.
    ...
    async fn gate_resolve_permissions_with_provenance_skips_claude_when_marker_set() {
        ...
        // managed settings, and the developer's real `~/.nemesis/config.toml`.
        // We can't isolate `nemesis_home()` because it's `OnceLock`-cached.
        ...
        let resolved =
            xai_nemesis_workspace::permission::resolution::resolve_permissions_with_provenance(
                dir.path(),
            )
            .await;
        if let Some(r) = resolved {
            let tempdir_claude = claude_dir.join("settings.json");
            use xai_nemesis_workspace::permission::types::RequirementSource;
...
if path == &tempdir_claude)
    ...
    fn gate_marker_cache_unset_means_uses_disk() {
        ...
        // dev's real `~/.nemesis/config.toml` may legitimately have the marker
        // set during local testing, and we can't override `nemesis_home()`

...
async fn subagent_inherits_session_cli_overrides() {
    use xai_nemesis_agent::config::{AgentDefinition, PermissionMode};
...
fn subagent_bypass_permission_mode_gated_by_policy_pin() {
    ...
    const PIN: &str = xai_nemesis_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    assert_eq!(
...
fn subagent_inherits_parent_lsp_via_context() {
    let parent: std::sync::Arc<dyn xai_nemesis_tools::implementations::lsp::LspBackend> = Arc::new(
        DummyLspDispatch,
    );
...
fn inject_subagent_completed_prompt_sends_prompt_and_marks_delivered() {
    ...
    let reservations = xai_nemesis_tools::reminders::task_completion::TaskCompletionReservations::default();
...
fn dummy_tracker(
    ...
    let cwd = xai_nemesis_paths::AbsPathBuf::new(PathBuf::from("/tmp")).unwrap();
    let fs: Arc<dyn xai_nemesis_workspace::file_system::AsyncFileSystem> = Arc::new(
        xai_nemesis_workspace::file_system::LocalFs::new(PathBuf::from("/tmp")),
    );
...
fn explicit_override_takes_precedence_over_role() {
    ...
    let role = xai_nemesis_subagent_resolution::config::SubagentRole {
...
fn persona_resolved_from_config() {
    ...
    personas
        .insert(
            ...
            xai_nemesis_subagent_resolution::config::SubagentPersona {
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = xai_nemesis_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = xai_nemesis_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
    ...
    if let ConversationItem::User(ref u) = ctx.conversation[1] {
        let text: String = u
            ...
            .filter_map(|p| match p {
                xai_nemesis_sampling_types::conversation::ContentPart::Text { text } => {
                    Some(text.as_ref())
                }
                _ => None,
            })
...
fn forked_initial_context_inherits_parent_across_reasoning() {
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(xai_nemesis_sampling_types::synthesized_reasoning_item("deliberating",)),
...
fn verbatim_fork_keeps_items_byte_for_byte_when_small() {
    use xai_nemesis_sampling_types::conversation::{
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(xai_nemesis_sampling_types::synthesized_reasoning_item("thinking",)),
        ConversationItem::assistant("ack"),
    ];
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use xai_nemesis_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
        ConversationItem::system("parent system"),
                    ...
                    ConversationItem::User(u) => {
                        Some(
                            u
                                ...
                                .iter()
                                .filter_map(|p| match p {
                                    xai_nemesis_sampling_types::conversation::ContentPart::Text {
                                        text,
                                    } => Some(text.as_ref()),
                                    _ => None,
                                })
...
fn subagent_keeps_default_flavor_when_parent_model_is_non_strict() {
    use xai_nemesis_agent::config::BuiltinAgentName;
    let mut ctx = ctx_with_toggle(HashMap::new());
...
fn normalize_forked_context_empty_parent() {
    ...
    let (conv, prefix_len) = xai_nemesis_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn test_sampling_config(model_slug: &str) -> xai_nemesis_sampling_types::SamplingConfig {

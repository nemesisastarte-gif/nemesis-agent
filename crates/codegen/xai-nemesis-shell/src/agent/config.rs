...
use std::sync::Arc;
use xai_nemesis_agent::prompt::skills::SkillsConfig;
use xai_nemesis_sampler::{AuthScheme, SamplerConfig};
...
use xai_nemesis_tools::types::compat::{
...
pub struct EndpointsConfig {
    ...
    /// Env: `NEMESIS_API_KEY`. Management API key for enterprise deployments.
...
impl EndpointsConfig {
    ...
    /// Managed deployment-config URL (`nemesis setup`): explicit `managed_config_url`,
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            ...
            deployment_key: env_string("NEMESIS_API_KEY"),
...
pub use xai_nemesis_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
pub use xai_nemesis_config::env_bool;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(xai_nemesis_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: xai_nemesis_tools::types::compat::CompatCell,
...
pub(crate) use xai_nemesis_telemetry::config::env_telemetry_mode;
pub use xai_nemesis_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    /// Native `.nemesis/config.toml` entries already present take precedence:
    ...
    pub fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                xai_nemesis_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub fn to_discovery_config(&self) -> xai_nemesis_agent::plugins::discovery::DiscoveryConfig {
        xai_nemesis_agent::plugins::discovery::DiscoveryConfig {
...
///
/// These supplement the built-in scan locations (`.nemesis/skills/`,
/// `.agents/skills/`, `~/.nemesis/skills/`). They're written by `/import-claude`
...
pub struct Config {
    ...
    #[serde(default, skip_serializing)]
    pub announcements: Vec<xai_nemesis_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]` — also read by `xai_nemesis_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<xai_nemesis_agent::config::AgentDefinition>,
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, xai_nemesis_subagent_resolution::config::SubagentRole>,
    /// Trust-independent personas from inline, user, and bundled sources.
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, xai_nemesis_subagent_resolution::config::SubagentPersona>,
    ...
    pub zdr_video_output_s3:
        Option<xai_nemesis_tools::implementations::grok_build::video_gen::ZdrVideoOutputS3Config>,
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<xai_nemesis_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<xai_nemesis_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub fn apply_to_definition(&self, def: &mut xai_nemesis_agent::config::AgentDefinition) {
    ...
    pub fn apply_to_subagent_definition(&self, def: &mut xai_nemesis_agent::config::AgentDefinition) {
...
pub use xai_nemesis_agent::config::AgentDefinition;
pub use xai_nemesis_agent::config::Effort;
pub use xai_nemesis_agent::config::PermissionMode;
pub use xai_nemesis_shared::ui_config::{ContextualHints, UiConfig};
...
/// ```toml
/// [agent]
/// # Use a named agent (looked up via discovery: .nemesis/agents/, ~/.nemesis/agents/, built-ins)
...
pub struct AgentSelectionConfig {
    ...
    /// Supports environment variable expansion (e.g., `$HOME/.nemesis/agents/my-agent.md`).
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            client_version: Some(xai_nemesis_version::VERSION.to_string()),
...
impl Config {
    ...
    pub fn create_auth_manager(&self) -> AuthManager {
        AuthManager::new(
            &crate::util::nemesis_home::nemesis_home(),
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<xai_nemesis_sampling_types::DoomLoopRecoveryPolicy> {
        use xai_nemesis_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use xai_nemesis_tools::implementations::grok_build::IMAGE_EDIT_TOOL_NAME;
    ...
    /// Resolve the mode (env `NEMESIS_COMPACTION_MODE` > config > remote settings >
    ...
    pub(crate) fn resolve_compaction_mode(&self) -> xai_chat_state::CompactionMode {
        resolve_compaction_mode_from(
            env_string("NEMESIS_COMPACTION_MODE").as_deref(),
    ...
    /// Resolve verbatim-input flag: env `NEMESIS_COMPACTION_VERBATIM_INPUT` > config > remote settings > default `true`.
    pub(crate) fn resolve_compaction_verbatim_input(&self) -> bool {
        BoolFlag::env("NEMESIS_COMPACTION_VERBATIM_INPUT")
    ...
    /// Precedence: env `NEMESIS_COMPACTION_DETAIL`, then config
    ...
    fn resolve_compaction_detail(&self) -> xai_chat_state::CompactionDetail {
        resolve_compaction_detail_from(
            env_string("NEMESIS_COMPACTION_DETAIL").as_deref(),
    ...
    /// narrow non-recursive watches for `<cwd>/` and `<cwd>/.nemesis/`.
    ///
    /// Thin delegate to the canonical
...
/// Load `~/.nemesis/requirements.toml` standalone so the admin pin can beat
/// env vars. The merged config layer can't express that — last-merge-wins
/// loses provenance.
pub(crate) fn read_requirements_toml() -> Option<toml::Value> {
    let path = crate::util::nemesis_home::nemesis_home().join("requirements.toml");
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        xai_nemesis_config::load_merged_requirements().as_ref(),
...
/// by `nemesis setup`) sit under the env vars, requirements pins are applied on
...
pub fn resolve_external_otel_config(
    client: xai_nemesis_telemetry::external::config::ExternalClientInfo,
) -> Option<xai_nemesis_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<xai_nemesis_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| xai_nemesis_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            xai_nemesis_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = xai_nemesis_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub fn apply_external_otel_remote_policy(settings: Option<&crate::util::config::RemoteSettings>) {
    ...
    let policy = xai_nemesis_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        xai_nemesis_telemetry::external::apply_remote_policy(policy);
    }
}
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = xai_nemesis_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    xai_nemesis_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<xai_nemesis_workspace::permission::ClassifierPromptType>,
...
pub struct Features {
    ...
    /// env (`NEMESIS_COMPACTION_MODE`). Parsed via `CompactionMode::parse`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_mode: Option<String>,
    /// `none` | `minimal` | `balanced` | `verbose` (default). `None` = defer to
    /// env (`NEMESIS_COMPACTION_DETAIL`). The `segments` verbatim detail level.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// `~/.nemesis/config.toml` will NOT disable the pager's
    ...
    /// non-recursive watches for `<cwd>/` and `<cwd>/.nemesis/`.
    ///
    /// When `true` (default), edits to `<cwd>/.mcp.json`,
    /// `<cwd>/.nemesis/config.toml`, or `<cwd>/.claude.json` flow
...
pub fn enforce_disable_api_key_auth(
        ...
        && crate::util::is_xai_api_url(&creds.base_url)
    {
        ...
        xai_nemesis_telemetry::unified_log::debug(
...
pub use xai_nemesis_telemetry::config::deployment_id_from_key;
...
pub fn inject_url_derived_headers(
    ...
    if crate::util::is_cli_chat_proxy_url(base_url) {
        headers
            ...
            .or_insert_with(|| "xai-nemesis-cli".to_string());
        headers
...
mod tests {
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use xai_nemesis_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
...
"#;
    ...
    fn session_resolver_is_not_stamped_onto_third_party_samplers() {
        ...
        impl xai_nemesis_sampler::BearerResolver for SessionResolver {
...
if field.as_deref() == Some("auth_provider"))
    ...
    fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
        ...
        assert_eq!(config.api_backend, ApiBackend::Messages);
        let client = xai_nemesis_sampler::SamplingClient::new(config).expect("client should build");
    ...
    fn user_override_parses_compaction_at_tokens_from_toml() {
        use xai_nemesis_sampling_types::CompactionAtTokens;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn user_override_parses_compactions_remaining_from_toml() {
        use xai_nemesis_sampling_types::CompactionsRemaining;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    /// xai-nemesis-shell/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
...
"#,
    ...
    fn ext_client() -> xai_nemesis_telemetry::external::config::ExternalClientInfo {
        xai_nemesis_telemetry::external::config::ExternalClientInfo::default()
    }
    #[test]
...
"#,
        ...
        xai_nemesis_config::apply_version_overrides(&mut value, &v).unwrap();

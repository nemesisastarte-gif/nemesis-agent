...
use xai_nemesis_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new(&nemesis_home, GrokComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    /// Hot-reload the catalog from `~/.nemesis/models_cache.json` after an
    ...
    /// `ModelsCacheManager` path is fixed to `nemesis_home()`, a process-wide
    /// `OnceLock`).
...
impl ModelsCacheManager {
    fn new() -> Self {
        Self {
            path: crate::util::nemesis_home::nemesis_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    fn load_fresh(
        ...
        if cache.grok_version.as_deref() != Some(xai_nemesis_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            grok_version: Some(xai_nemesis_version::VERSION.to_string()),
...
/// routing slug (e.g. "Auto" and "Nemesis Agent" both route to `grok-build`)
...
fn resolve_prefetch_env(grok_com_config: Option<GrokComConfig>) -> Option<PrefetchEnv> {
    let nemesis_home = crate::util::nemesis_home::nemesis_home();
    let auth_manager = AuthManager::new(&nemesis_home, grok_com_config.unwrap_or_default());
...
mod tests {
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        use indexmap::IndexMap;
        use xai_nemesis_sampling_types::ReasoningEffortOption;
    ...
    fn build_prefetched_map_distinct_ids_same_slug() {
        let entries = vec![
            ...
            make_entry_config_with_id(Some("grok-build"), "grok-build", Some("Nemesis Agent")),

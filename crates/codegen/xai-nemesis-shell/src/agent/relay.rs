...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `nemesis login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(xai_nemesis_version::VERSION),
    );
    req.headers_mut().insert(
    ...
    let read_from_ws = async move {
        loop {
            tokio::select! {
                ...
                xai_nemesis_telemetry::unified_log::warn("relay: read liveness timeout, reconnecting",

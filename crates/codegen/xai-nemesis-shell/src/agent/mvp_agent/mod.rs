...
use xai_nemesis_telemetry::id::{agent_id, agent_instance_id};
use xai_nemesis_telemetry::session_ctx::log_event;
use xai_nemesis_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use xai_nemesis_workspace::permission::{ClientType, PermissionEvent};
...
use xai_nemesis_workspace::session::git::GitDiscoveryResult;
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a xai_nemesis_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[xai_nemesis_announcements::RemoteAnnouncement]>,
    last_emitted: &[xai_nemesis_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<xai_nemesis_announcements::RemoteAnnouncement>> {
    let current = xai_nemesis_announcements::filter_expired_at(
...
pub struct MvpAgent {
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.nemesis/`).
    ///
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        xai_nemesis_tools::implementations::grok_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: xai_nemesis_tools::implementations::grok_build::task::types::MonitorEventBuffer,
    ...
    pub(crate) plugin_registry_handle: xai_nemesis_agent::plugins::SharedPluginRegistryHandle,
    ...
    workspace_ops: RefCell<Option<xai_nemesis_workspace::WorkspaceOps>>,
    ...
    last_emitted_announcements: RefCell<Vec<xai_nemesis_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &xai_nemesis_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<xai_nemesis_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !xai_nemesis_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = xai_nemesis_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use xai_nemesis_agent::config::is_strict_harness_agent_type;
    match (
...
impl MvpAgent {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = xai_nemesis_tools::types::TaskSnapshot {
    ...
    ///    against `~/.nemesis/bundled/` and the manifest.

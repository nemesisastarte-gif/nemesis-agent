...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        xai_nemesis_telemetry::unified_log::info("agent initialized", None, None);
        ...
        {
            let root = crate::util::nemesis_home::nemesis_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    xai_nemesis_workspace::permission::cleanup_stale_permission_state(
        ...
        xai_nemesis_workspace::trust::migrate_legacy_hook_trust();
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        xai_nemesis_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!(
                    { "nemesis_home" : crate ::util::nemesis_home::nemesis_home().display()
        ...
        Ok(
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    let metadata = parse_json_object_env("NEMESIS_AGENT_METADATA");
                    serde_json::json!(
                        { "grokShell" : true, "defaultAuthMethodId" :
                        default_auth_method_id_wire, (xai_nemesis_mcp::wire::MCP_SDK) :
                        ...
                        xai_nemesis_version::VERSION, "agentId" : agent_id(),
    ...
    async fn authenticate(
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::XAI_API_KEY_METHOD_ID => {
                ...
                log_event(xai_nemesis_telemetry::events::Login {
    ...
    async fn new_session(
        ...
        xai_nemesis_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        if product_analytics || xai_nemesis_telemetry::external::is_active() {
            ...
            tokio::spawn(async move {
                let git = xai_nemesis_telemetry::context::collect_git_context(&cwd_str);
                let ev = xai_nemesis_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                xai_nemesis_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) = match xai_nemesis_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
    ...
    async fn load_session(
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        xai_nemesis_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        xai_nemesis_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        xai_nemesis_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => xai_nemesis_telemetry::events::PlanModeState::Inactive,
        };
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            xai_nemesis_workspace::session::git::warn_registry_disabled_restore(
                session_id.0.as_ref(),
            );
        }
        let restore_checkout_allowed = xai_nemesis_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target : xai_nemesis_workspace::session::git::RESTORE_CODE_LOG, session_id =
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use xai_nemesis_workspace::session::git::RestoreKind;
            let outcome = xai_nemesis_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let preloaded_envrc = xai_nemesis_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    xai_nemesis_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = xai_nemesis_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = xai_nemesis_workspace::session::git::detect_head_divergence(
        ...
        if self.product_analytics_enabled() {
            log_event(xai_nemesis_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
            ...
            Ok(turn_ok) => {
                ...
                if let Some(ctx) = trace_context {
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = xai_nemesis_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = xai_nemesis_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
        ...
        let result = match method.as_ref() {
            ...
            "x.ai/cloud/terminate" => {
                crate::extensions::auth_gate::require_xai_auth(
                    ...
                    "Run `nemesis login` to authenticate.",
                )?;
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "x.ai/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                xai_nemesis_telemetry::session_ctx::log_event(xai_nemesis_telemetry::events::NonGitDecisionEvent {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_followup" {
            ...
            >(args.params.get()) {
                ...
                xai_nemesis_telemetry::session_ctx::log_event(xai_nemesis_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| xai_nemesis_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_apply" {
            ...
            >(args.params.get()) {
                ...
                xai_nemesis_telemetry::session_ctx::log_event(xai_nemesis_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_discard" {
            ...
            >(args.params.get()) {
                ...
                xai_nemesis_telemetry::session_ctx::log_event(xai_nemesis_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| xai_nemesis_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == xai_nemesis_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                xai_nemesis_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            xai_nemesis_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }

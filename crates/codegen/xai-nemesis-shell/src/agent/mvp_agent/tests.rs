...
fn harness_pair(id: &str) -> Vec<xai_nemesis_sampling_types::conversation::ConversationItem> {
...
fn resolve_agent_definition_defaults_to_grok_build() {
    let prev = std::env::var("NEMESIS_AGENT").ok();
    unsafe {
        std::env::remove_var("NEMESIS_AGENT");
    }
    ...
    if let Some(v) = prev {
        unsafe { std::env::set_var("NEMESIS_AGENT", v) }
    }
}
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = xai_nemesis_agent::AgentDefinition::from_json(&serde_json::json!(
...
fn inherited_harness_template_skips_nonstrict_model() {
    use xai_nemesis_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let agents_dir = tmp.path().join(".nemesis").join("agents");
    ...
    assert_eq!(
        def.model,
        xai_nemesis_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("NEMESIS_AGENT", v) },
        None => unsafe { std::env::remove_var("NEMESIS_AGENT") },
    }
}
#[test]
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use xai_nemesis_tools::computer::local::{LocalFs, LocalTerminalBackend};
    ...
    let builder = xai_nemesis_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: xai_nemesis_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: xai_nemesis_tools::implementations::grok_build::image_gen::ImageGenConfig::default(),
        video_gen_config: xai_nemesis_tools::implementations::grok_build::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: xai_nemesis_tools::implementations::grok_build::deploy_app::AppBuilderDeployerConfig::default(),
...
fn make_test_handle(
    ...
    crate::session::SessionHandle {
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            xai_nemesis_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(xai_nemesis_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
        ...
        permission_handle: xai_nemesis_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        ...
        hook_registry: None,
        workspace_ops: xai_nemesis_workspace::WorkspaceOps::for_test(),
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use xai_nemesis_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn session_config_options_resolves_routing_slug_to_catalog_model() {
    use crate::agent::config::{EndpointsConfig, ModelEntry};
    use xai_nemesis_sampling_types::ReasoningEffort;
    let agent = build_minimal_agent_for_tests();
...
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    ...
    use xai_nemesis_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("NEMESIS_DISABLE_API_KEY_AUTH");
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use xai_nemesis_tools::implementations::grok_build::video_gen::{
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = xai_nemesis_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = xai_nemesis_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(xai_nemesis_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<xai_nemesis_workspace::permission::PermissionEvent>();
    agent
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::NEMESIS_DISABLE_CUSTOM_BRIDGE_ENV);
...
fn write_project_subagent_definitions(cwd: &std::path::Path) {
    let roles = cwd.join(".nemesis/roles");
    let personas = cwd.join(".nemesis/personas");
    ...
    run_local_for_bridge_test(|| async {
        ...
        agent.sessions.borrow_mut().insert(sid.clone(), handle);
        {
            ...
            cfg.subagent_roles.insert(
                "refreshed".into(),
                xai_nemesis_subagent_resolution::config::SubagentRole {
...
/// End-to-end gate wiring: project `.nemesis/roles` / `personas` alone must drive
...
fn project_roles_personas_gated_via_resolve_and_record_chain() {
    ...
    let _sim = EnvGuard::set(xai_nemesis_version::TEST_VERSION_ENV, "0.0-sim");
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use xai_nemesis_test_support::EnvGuard;
    use xai_nemesis_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("NEMESIS_HOME", home.path());
    let _sim = EnvGuard::set(xai_nemesis_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
...
fn ann(id: &str) -> xai_nemesis_announcements::RemoteAnnouncement {
    xai_nemesis_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<xai_nemesis_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = xai_nemesis_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = xai_nemesis_announcements::RemoteAnnouncement {

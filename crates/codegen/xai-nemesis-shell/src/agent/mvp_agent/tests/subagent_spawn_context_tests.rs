...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    use xai_nemesis_workspace::permission::types::{
    ...
    local
        .run_until(async {
            ...
            let cwd = xai_nemesis_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                xai_nemesis_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    xai_nemesis_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                xai_nemesis_workspace::permission::AccessKind::Read(Some(".env".into())),
                xai_nemesis_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        xai_nemesis_workspace::permission::Decision::PolicyDeny(_)
                    ),

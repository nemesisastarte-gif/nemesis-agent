...
impl MvpAgent {
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            ...
            xai_nemesis_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
                ...
                xai_nemesis_version::VERSION.to_owned(),
                subscription_tier,
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &xai_nemesis_agent::plugins::SharedPluginRegistryHandle {
    ...
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<xai_nemesis_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    xai_nemesis_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    xai_nemesis_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match xai_nemesis_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => xai_nemesis_workspace::WorkspaceOps::local(handle),
            Err(e) => {
    ...
    pub(super) async fn refresh_remote_settings(&self, auth: &crate::auth::GrokAuth) {
        ...
        xai_nemesis_telemetry::client::init(
            telemetry_config,
            telemetry_mode,
            grok_user_id,
            grok_team_id,
    ...
    pub(super) fn apply_polled_announcements(
        ...
        pre_fetch: Option<Vec<xai_nemesis_announcements::RemoteAnnouncement>>,
    ) {
        let mut cfg = self.cfg.borrow_mut();
    ...
    pub(super) fn prepare_image_gen_config(
        &self,
    ) -> xai_nemesis_tools::implementations::grok_build::image_gen::ImageGenConfig {
        use xai_nemesis_tools::implementations::grok_build::image_gen::ImageGenConfig;
        ...
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| xai_nemesis_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
        let mut headers = indexmap::IndexMap::new();
        headers.insert("user-agent".to_string(), format!("xai-nemesis-build/{version}"));
    ...
    pub(super) fn prepare_app_builder_deployer_config(
        ...
        use xai_nemesis_tools::implementations::grok_build::deploy_app::AppBuilderDeployerConfig;
    ...
    pub(super) fn prepare_video_gen_config(
        &self,
    ) -> xai_nemesis_tools::implementations::grok_build::video_gen::VideoGenConfig {
        use xai_nemesis_tools::implementations::grok_build::video_gen::VideoGenConfig;
    ...
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> xai_nemesis_tools::implementations::grok_build::web_fetch::WebFetchConfig {
    ...
    pub fn with_models(
        ...
        if relay_sync_enabled {
            ...
            tracing::info!(
                "[grok] Relay sync: DISABLED (no auth - run 'nemesis login' first)"
            );
        ...
        let instance = Self {
            ...
            plugin_registry_handle: xai_nemesis_agent::plugins::SharedPluginRegistryHandle::new(
                None,
            ...
            monitor_event_buffer: xai_nemesis_tools::implementations::grok_build::task::types::MonitorEventBuffer::default(),
    ...
    pub async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
    ) -> Result<xai_nemesis_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> xai_nemesis_tools::implementations::grok_build::task::types::SubagentCancelOutcome {
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<xai_nemesis_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub async fn execute_hooks_action(
            ...
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&xai_nemesis_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
    ...
    pub fn plugin_registry_snapshot(
        &self,
    ) -> Option<std::sync::Arc<xai_nemesis_agent::plugins::PluginRegistry>> {
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<xai_nemesis_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let metadata = PromptMetadata {
                ...
                agent_type: None,
                shell_version: Some(xai_nemesis_version::VERSION.to_string()),
                workspace_type: None,
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            Some(method) => method,
            None => {
                xai_nemesis_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
        ...
        let queue = session_handle
            ...
            .get_or_init(|| {
                ...
                let queue = crate::upload::trace::spawn_upload_queue(
                    &nemesis_home,
                    &gcs_config,
                    Some(xai_nemesis_version::VERSION),
    ...
    /// 5. `NEMESIS_AGENT` env var.
    /// 6. Built-in default agent.
    ///
    /// `NEMESIS_AGENT` and an explicit `[agent] name` bypass step 1.
    /// Strict-harness classification is structural — see
    /// [`xai_nemesis_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub fn resolve_agent_definition(
        ...
        acp_agent_profile: Option<xai_nemesis_agent::AgentDefinition>,
        model_agent_type: Option<&str>,
    ) -> xai_nemesis_agent::AgentDefinition {
        use xai_nemesis_agent::AgentDefinition;
        let grok_agent_env_set = std::env::var("NEMESIS_AGENT")
            .ok()
            .is_some_and(|s| !s.trim().is_empty());
        let config_agent_explicitly_set = agent_config.name.is_some();
        let model_requires_strict_harness = model_agent_type
            .is_some_and(xai_nemesis_agent::config::is_strict_harness_agent_type);
        if !grok_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && let Some(def) = xai_nemesis_agent::discovery::by_name_in_cwd(required, cwd)
        {
            tracing::info!(
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = xai_nemesis_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = % name,
        ...
        let agent_name = std::env::var("NEMESIS_AGENT").ok();
        let resolved = match agent_name.as_deref() {
            ...
            Some(name) => {
                xai_nemesis_agent::discovery::by_name_in_cwd(name, cwd)
            ...
            && resolved.name != required
        {
            ...
            if let Some(def) = xai_nemesis_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let fs: Arc<dyn xai_nemesis_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let mut session_env = xai_nemesis_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        let envrc = match preloaded_envrc {
            Some(env) => env,
            None => {
                xai_nemesis_workspace::envrc::load_envrc_or_empty_when_trusted(
                    cwd.as_path(),
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            xai_nemesis_agent::config::ModelOverride::Override(id) => {
            ...
            xai_nemesis_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = xai_nemesis_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = xai_nemesis_tools::util::nemesis_home::nemesis_home()
                    .join("lsp.json");
                let project_path = tool_ctx.cwd.as_path().join(".nemesis").join("lsp.json");
                tracing::warn!(
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let attribution_callback: Option<
                xai_nemesis_sampler::SharedAttributionCallback,
            > = Some(
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = xai_nemesis_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
                    ...
                    let git_root = xai_nemesis_workspace::session::git::find_git_root_from_path(
                            cwd,
                        )
                        .ok();
            ...
            spawn_session_on_thread(
                    ...
                    subagent_toggle,
                    Vec::new(),
                    xai_nemesis_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,

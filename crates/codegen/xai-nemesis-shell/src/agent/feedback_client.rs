...
// ============================================================================
// Turn delta wire types (local to xai-nemesis-shell until cli-chat-proxy catches up)
// ============================================================================
...
impl FeedbackClient {
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(xai_nemesis_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }
    ...
    fn make_auth_provider(
        ...
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(xai_nemesis_auth::StaticAuthCredentialProvider::new(
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        let builder = builder
            .header(CLIENT_VERSION_HEADER, xai_nemesis_version::VERSION)
            .header(
        ...
        if self.credentials.deployment_key.is_none() {
            builder.header("X-XAI-Token-Auth", "xai-nemesis-cli")
        } else {
            builder
        }
    }

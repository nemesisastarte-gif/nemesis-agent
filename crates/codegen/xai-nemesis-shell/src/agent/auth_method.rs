...
/// `NEMESIS_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `nemesis login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `nemesis login`, set XAI_API_KEY, or add api_key to ~/.nemesis/config.toml.";
...
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `nemesis login` to authenticate.";
...
pub fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.nemesis/auth.json".to_string())),
    )
}
...
pub fn grok_com_auth_method(
    ...
    let name = label.unwrap_or("Nemesis");
...
mod tests {
    ...
    /// `~/.nemesis/config.toml` skeleton from the bug report, walks it through
    ...
    // -- nemesis login --legacy regression coverage ------------------------
    //
    // `nemesis login --legacy` produces a GrokAuth with `auth_mode: WebLogin`,
    ...
    fn grok_login_legacy_token_does_not_require_login() {
        ...
        // Construct a legacy-style token exactly as `nemesis login --legacy`

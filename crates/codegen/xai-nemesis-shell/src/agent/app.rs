    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = xai_nemesis_agent::prompt::workspace_user::optional_workspace_user_dir();
...
pub async fn run_stdio_agent(
    ...
    xai_nemesis_telemetry::unified_log::set_version(xai_nemesis_version::VERSION);
...
async fn run_headless_inner(
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a grok.com session. \
        Run `nemesis login` to sign in, or use `grok agent stdio` for API-key access.";
    ...
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!("Session expired. Please run 'nemesis login' to re-authenticate.")
            }
            None => anyhow::bail!("No cached credentials found. Run `nemesis login`."),
        }
    } else if reauthenticate {
        let auth_manager = Arc::new(AuthManager::new(&nemesis_home::nemesis_home(), ctx.clone()));
        run_auth_flow(
            &auth_manager,
            ctx,
            true,
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow && !no_browser {
            ...
            eprintln!();
            eprintln!(
                "Open Nemesis Agent: {} (press Enter to open in browser)",
                grok_code_url
            );
            eprintln!();
...
pub async fn run_leader(
    ...
    local_set
        .run_until(async move {
            ...
            // Gated on user_nemesis_home() so a cwd-relative .nemesis/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = xai_nemesis_config::user_nemesis_home()
                ...
                while let Some(update) = config_update_rx.recv().await {
                    match update {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.nemesis/models_cache.json

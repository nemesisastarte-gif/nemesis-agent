...
//! inside a cloned repository (`.mcp.json`, project `.nemesis/config.toml`,
//! `~/.claude.json` `projects.<cwd>`, project `.nemesis/lsp.json`). Those configs
...
//! interactive prompt, and the durable [`xai_nemesis_workspace::trust::TrustStore`]
//! reads/writes — lives in `xai-nemesis-workspace` (client-side); this module keeps
...
//! on [`xai_nemesis_workspace::folder_trust::decide`]; the consume-side nuance is
...
use xai_nemesis_workspace::trust::{TrustStore, is_unsafe_trust_root, workspace_key};

// Decision-side (scan/decide/prompt/store) relocated to `xai-nemesis-workspace`
// (client crate). `grant_folder_trust` is the ONLY moved item referenced from
// OUTSIDE this module (shell call sites + the pager's
// `xai_nemesis_shell::agent::folder_trust::grant_folder_trust`), so only it is
...
pub use xai_nemesis_workspace::folder_trust::grant_folder_trust;
use xai_nemesis_workspace::folder_trust::{
...
// NOTE: this folder-trust store (`~/.nemesis/trusted_folders.toml`) is SEPARATE
// from the pre-existing per-plugin trust store
// (`xai_nemesis_agent::plugins::TrustStore` at `~/.nemesis/trusted-plugins`, plus the
...
/// [`xai_nemesis_workspace::folder_trust::revoke_folder_trust_store`].
///
...
pub fn revoke_folder_trust(cwd: &Path) -> bool {
    ...
    let was_trusted = xai_nemesis_workspace::folder_trust::revoke_folder_trust_store(cwd);
...
/// ([`xai_nemesis_workspace::folder_trust::repo_config_kinds`] /
...
/// folder is gated (so an `.nemesis/lsp.json`-only repo still has a non-empty reason
...
pub(crate) fn detected_config_kinds(cwd: &Path) -> Vec<String> {
    xai_nemesis_workspace::folder_trust::repo_config_kinds(cwd)
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
pub(crate) fn agent_inline_hooks_allowed(
    scope: xai_nemesis_agent::config::AgentScope,
    trusted: impl FnOnce() -> bool,
) -> bool {
    scope != xai_nemesis_agent::config::AgentScope::Project || trusted()
}
...
///
/// Sources: project `.nemesis/config.toml [mcp_servers]` (NOT the user-tier global
...
/// `~/.nemesis/config.toml` is dropped when untrusted. This is intended — untrusted
...
pub fn project_scoped_mcp_names(cwd: &Path) -> HashSet<String> {
    ...
    // `.nemesis/config.toml [mcp_servers]` entries tagged project (the loader's key
...
pub fn filter_untrusted_project_lsp(
    ...
    sourced: std::collections::BTreeMap<
        String,
        (
            xai_nemesis_tools::implementations::lsp::config::LspServerConfig,
            xai_nemesis_tools::types::config_source::ConfigSource,
        ),
    >,
) -> std::collections::BTreeMap<String, xai_nemesis_tools::implementations::lsp::config::LspServerConfig>
{
    xai_nemesis_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
        sourced,
...
mod tests {
    ...
    use xai_nemesis_workspace::folder_trust::repo_configs_present;
    ...
    fn simulate_release_build() -> EnvGuard {
        EnvGuard::set(xai_nemesis_version::TEST_VERSION_ENV, "0.0.0-sim")
    }
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; NEMESIS_HOME-isolated so the seed can't touch the real user file and
        // `#[serial]` because NEMESIS_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("NEMESIS_HOME", home.path());
        let tmp = repo_tmp();
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. NEMESIS_HOME-isolated so the grant writes to a temp
        // store; `#[serial]` because NEMESIS_HOME is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("NEMESIS_HOME", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. NEMESIS_HOME-isolated so the grant writes to a temp store;
        // `#[serial]` because NEMESIS_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("NEMESIS_HOME", home.path());
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // home; NEMESIS_HOME-isolated store; GROK_FOLDER_TRUST unset so the
        // default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _home = EnvGuard::set("HOME", home.path());
        let nemesis_home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("NEMESIS_HOME", nemesis_home.path());
        ...
        std::fs::create_dir_all(home.path().join(".nemesis").join("hooks")).unwrap();
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // actually reads `.envrc`. NEMESIS_HOME-isolated so the trust store is empty;
        ...
        let env = xai_nemesis_workspace::envrc::load_envrc_or_empty(tmp.path());
        assert_eq!(
            env.get("GATED_ENVRC"),
            Some(&"1".to_string()),
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // folder merges it. NEMESIS_HOME-isolated so the trust store is empty;
        // GROK_FOLDER_TRUST unset so the default-on feature flag applies.
        use xai_nemesis_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("NEMESIS_HOME", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). NEMESIS_HOME-isolated so the trust store is empty.
        use xai_nemesis_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("NEMESIS_HOME", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // predicate used at mvp_agent/subagent. NEMESIS_HOME-isolated (empty store).
        use xai_nemesis_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("NEMESIS_HOME", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
        let agents = tmp.path().join(".nemesis").join("agents");
        ...
        .unwrap();
        let def = xai_nemesis_agent::discovery::by_name_in_cwd("explore", tmp.path())
    ...
    use xai_nemesis_test_support::EnvGuard;
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // code-exec config (here `.nemesis/hooks`), and no store grant must be
        // DENIED — even though no verdict was recorded first (the gate re-resolves
        // fail-closed rather than defaulting open). NEMESIS_HOME-isolated (empty
        ...
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".nemesis").join("hooks")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + NEMESIS_HOME-isolated so the
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // configs present. NEMESIS_HOME-isolated so the seeded store is the temp one;
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        let _unset_ver = EnvGuard::unset(xai_nemesis_version::TEST_VERSION_ENV);
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.nemesis/plugins/<x>/`, no
        ...
        // discover_plugins/build_for_cwd/reload. NEMESIS_HOME-isolated (empty store);
        // GROK_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("NEMESIS_HOME", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".nemesis").join("plugins").join("evil")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // NEMESIS_HOME-isolated (empty store); GROK_FOLDER_TRUST unset so the kill-switch
        // is the only signal.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("NEMESIS_HOME", home.path());
        ...
        let cold = repo_tmp();
        std::fs::create_dir_all(cold.path().join(".nemesis").join("hooks")).unwrap();
        assert!(
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // un-gating here.
        //
        // NEMESIS_HOME-isolated so both the folder-trust store and the plugin trust
        ...
        use xai_nemesis_agent::plugins::discovery::DiscoveryConfig;
        use xai_nemesis_agent::plugins::{PluginRegistry, SharedPluginRegistryHandle};
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("NEMESIS_HOME", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let plugin = tmp.path().join(".nemesis").join("plugins").join("trustgate");
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // NEMESIS_HOME-isolated so the grant writes to a temp store; GROK_FOLDER_TRUST unset
        // so the default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("NEMESIS_HOME", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
        let hooks_dir = tmp.path().join(".nemesis").join("hooks");
        ...
        let has_project_hook = |reg: &xai_nemesis_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
        ...
        let git_root = xai_nemesis_workspace::session::git::find_git_root_from_path(tmp.path()).ok();
        let (reg, _errs) = crate::util::hooks::discover_hooks(
            git_root.as_deref(),
            &xai_nemesis_tools::types::compat::CompatConfig::default(),
            untrusted,
        );
        assert!(
    ...
    fn filter_untrusted_project_lsp_drops_only_project() {
        ...
        use xai_nemesis_tools::implementations::lsp::config::LspServerConfig;
        use xai_nemesis_tools::types::config_source::ConfigSource;
            ...
            m.insert(
                ...
                (
                    ...
                    ConfigSource::Project {
                        path: PathBuf::from("/repo/.nemesis/lsp.json"),
                    },
                ),
            );
            m.insert(
                ...
                (
                    LspServerConfig::default(),
                    ConfigSource::User {
                        path: PathBuf::from("/home/.nemesis/lsp.json"),
                    },
                ),
            );
            m
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        use xai_nemesis_tools::implementations::lsp::config::load_servers_with_plugins_sourced;
        use xai_nemesis_tools::types::config_source::ConfigSource;

        // A `<cwd>/.nemesis/lsp.json` server must be tagged `Project` so the gate
        // can distinguish it from user/plugin servers. Asserts on the specific
        // key, so any real `~/.nemesis/lsp.json` on the test host is irrelevant.
        let tmp = repo_tmp();
        let grok = tmp.path().join(".nemesis");
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // from `<cwd>/.nemesis/lsp.json` is dropped once the workspace is untrusted.
        let tmp = repo_tmp();
        let grok = tmp.path().join(".nemesis");
        std::fs::create_dir_all(&grok).unwrap();
        std::fs::write(grok.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    /// (`.mcp.json`) and `projtoml` (`.nemesis/config.toml [mcp_servers]`).
    ...
    /// `.nemesis/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in
    ...
    fn explicit_child_untrust_survives_reload_despite_ancestor_trust() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join(xai_nemesis_workspace::trust::TRUST_FILE_NAME);
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        unsafe { std::env::set_var(xai_nemesis_version::TEST_VERSION_ENV, "0.0.0-sim") };
        let tmp = repo_tmp();
        ...
        unsafe { std::env::remove_var(xai_nemesis_version::TEST_VERSION_ENV) };
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // NEMESIS_HOME so the store is empty/seeded in temp. `#[serial]` because both
        ...
        let untrusted = repo_tmp();
        std::fs::create_dir_all(untrusted.path().join(".nemesis").join("hooks")).unwrap();
        ...
        let trusted = repo_tmp();
        std::fs::create_dir_all(trusted.path().join(".nemesis").join("hooks")).unwrap();
        let mut store = TrustStore::load();
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // unset so `is_local_build()` is genuinely true. NEMESIS_HOME-isolated so the
        // real store is never touched.
        let _sim = EnvGuard::unset(xai_nemesis_version::TEST_VERSION_ENV);
        if option_env!("GROK_VERSION").is_some() {
            return; // a release-stamped test binary is not a local build
        }
        let home = tempfile::tempdir().unwrap();
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // => the GUI prompt is warranted. NEMESIS_HOME-isolated so the store starts
        // empty; `#[serial]` because NEMESIS_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("NEMESIS_HOME", home.path());
        let tmp = repo_tmp();
        ...
        let _sim = EnvGuard::set(xai_nemesis_version::TEST_VERSION_ENV, "0.0.0-sim");
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let remote = RemoteSettings {
    ...
    fn prompt_warranted_false_when_feature_disabled() {
        ...
        // local-build path is not what's under test; NEMESIS_HOME-isolated and
    ...
    fn detected_config_kinds_summarizes_present_markers() {
        ...
        std::fs::write(tmp.path().join(".mcp.json"), "{}").unwrap();
        std::fs::create_dir_all(tmp.path().join(".nemesis").join("hooks")).unwrap();
        std::fs::write(tmp.path().join(".nemesis").join("lsp.json"), "{}").unwrap();
    ...
    fn detected_config_kinds_reports_lsp_only_repo() {
        ...
        // `.nemesis/lsp.json` must still produce a non-empty reason list.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".nemesis")).unwrap();
        std::fs::write(tmp.path().join(".nemesis").join("lsp.json"), "{}").unwrap();
        let kinds = detected_config_kinds(tmp.path());

...
/// auth.json scope key for plain API key auth (desktop login, `nemesis login --api-key`).
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from grok-desktop login or `nemesis login --api-key`)
    ApiKey,
}
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `nemesis login --legacy`

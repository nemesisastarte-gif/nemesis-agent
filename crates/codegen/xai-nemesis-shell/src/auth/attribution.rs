...
//!
//! 1. [`xai_nemesis_telemetry::unified_log::warn`] for the local
//!    `~/.nemesis/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`xai_nemesis_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`xai_nemesis_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`xai_nemesis_sampler::SamplerConfig::attribution_callback`] at every
...
use xai_nemesis_sampler::{Auth401AttributionCallback, SamplingConsumer};
use xai_nemesis_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`xai_nemesis_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `xai_nemesis_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn xai_nemesis_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `xai_nemesis_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `xai-nemesis-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `xai-nemesis-shell` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.nemesis/logs/unified.jsonl) + scrubbed
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.nemesis/auth.json` leaks in.
    ...
    /// `ShellAttribution` implements `xai_nemesis_tools::Auth401AttributionCallback`

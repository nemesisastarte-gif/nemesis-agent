...
pub struct GrokComConfig {
    ...
    /// can't bypass the deployment's IdP login. Env: `NEMESIS_DISABLE_API_KEY_AUTH`.
...
/// auth.json scope key used by the pre-OIDC `nemesis login --legacy` flow.
...
impl GrokComConfig {
    ...
    /// `NEMESIS_DISABLE_API_KEY_AUTH` env lockdown is sticky: because the env value
    ...
    /// pin is fail-closed. Explicit `nemesis login --devbox` / `--api-key` bypass
...
impl Default for GrokComConfig {
    fn default() -> Self {
        ...
        Self {
            ...
            token_header: "xai-nemesis-cli".to_owned(),
            oidc,
            oauth2,
            ...
            disable_api_key_auth: std::env::var("NEMESIS_DISABLE_API_KEY_AUTH")
                .ok()
...
/// `no`, empty) count as disabled — so e.g. `NEMESIS_DISABLE_API_KEY_AUTH=false`
...
/// True when the admin has set `NEMESIS_DISABLE_API_KEY_AUTH` to a truthy value in
...
fn env_lockdown_forced() -> bool {
    std::env::var("NEMESIS_DISABLE_API_KEY_AUTH")
        .ok()

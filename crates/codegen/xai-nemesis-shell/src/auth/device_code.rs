...
pub enum DeviceCodeError {
    #[error(
        "Device-code login is not available for this deployment. \
         Try `nemesis login` or set XAI_API_KEY instead."
    )]
    NotEnabled,
    #[error(transparent)]
...
///
/// On success, persists credentials to `~/.nemesis/auth.json` and returns
...
pub async fn complete_device_code_login(
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!("Device code expired. Run `nemesis login --device-auth` again.");
        }
...
pub(crate) mod tests {
    ...
    fn auth_manager_with_nemesis_home(
        nemesis_home: &std::path::Path,
        proxy_base_url: &str,
    ) -> Arc<AuthManager> {
        Arc::new(
            AuthManager::new(nemesis_home, GrokComConfig::default())
    ...
    fn build_auth_persists_credentials_without_proxy_fetch() {
        ...
        let nemesis_home = temp_dir.path().join(".nemesis");
        std::fs::create_dir_all(&nemesis_home).unwrap();
        let auth_manager = auth_manager_with_nemesis_home(&nemesis_home, "http://127.0.0.1:9");
    ...
    fn assert_build_auth_rejected(cfg: GrokComConfig, token_principal: &str, expected_err: &str) {
        ...
        std::fs::create_dir_all(&nemesis_home).unwrap();
        let auth_manager =
            Arc::new(AuthManager::new(&nemesis_home, cfg).with_proxy_base_url("http://127.0.0.1:9"));
        ...
        assert!(
            !nemesis_home.join("auth.json").exists(),
    ...
    async fn run_poll(
        ...
        let temp_dir = tempfile::tempdir().unwrap();
        let auth_manager = auth_manager_with_nemesis_home(temp_dir.path(), "http://127.0.0.1:9");

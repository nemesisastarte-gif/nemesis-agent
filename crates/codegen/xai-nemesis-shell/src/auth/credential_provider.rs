...
fn api_key_id_for(auth: Option<&crate::auth::GrokAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| xai_nemesis_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub(crate) fn embedding_session_credentials(
...
) -> xai_nemesis_memory::EndpointScopedCredentials {
    ...
    xai_nemesis_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
...
///
/// - `"grok-shell"`   — classic Grok CLI / TUI (xai-nemesis-shell)
/// - `"nemesis-agent"`   — new Grok Pager / TUI (xai-nemesis-pager)
...
pub fn build_storage_client_for_proxy(
    ...
    if let Some(am) = auth_manager {
        ...
        .with_client_identity(xai_nemesis_version::VERSION, client_identifier)
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        xai_nemesis_telemetry::external::set_identity(
            xai_nemesis_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `xai-nemesis-pager` and `xai-nemesis-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> xai_nemesis_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let grok_com_config = crate::auth::GrokComConfig::default();
    let exporter = xai_nemesis_telemetry::otel_layer::OtelExporterConfig {
    ...
    let bootstrap = Arc::new(AuthManager::new(&nemesis_home, grok_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    xai_nemesis_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let api_key_provider: xai_nemesis_tools::types::SharedApiKeyProvider =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use xai_nemesis_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();

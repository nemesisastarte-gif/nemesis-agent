...
async fn run_auth_flow_inner(
    ...
    // `nemesis login --devbox` uses run_devbox_login and is not gated here.
    ...
    anyhow::bail!(
        "No OAuth2 configuration available. Run `nemesis login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
pub async fn try_ensure_fresh_auth(grok_com_config: &GrokComConfig) -> Option<GrokAuth> {
    let nemesis_home = nemesis_home::nemesis_home();
    let auth_manager = std::sync::Arc::new(AuthManager::new(&nemesis_home, grok_com_config.clone()));
...
pub(crate) async fn try_ensure_session_noninteractive(
    ...
    let nemesis_home = nemesis_home::nemesis_home();
    let auth_manager = Arc::new(AuthManager::new(&nemesis_home, grok_com_config.clone()));
...
/// Unified `nemesis login` handler for CLI entry points (tui, pager).
///
...
pub async fn run_cli_login(
    ...
    // supports the device flow. Without this guard, `nemesis login` on an
    ...
    let authenticated = if devbox {
        ...
        let nemesis_home = nemesis_home::nemesis_home();
        let auth_manager = Arc::new(AuthManager::new(&nemesis_home, config.grok_com_config.clone()));
    ...
    // config was actually applied; `nemesis setup` reports the no-config case.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        xai_nemesis_telemetry::external::set_identity(
            xai_nemesis_telemetry::external::IdentityAttrs::default(),
        );
        xai_nemesis_telemetry::external::flush();
...
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let nemesis_home = nemesis_home::nemesis_home();
    let auth_manager = AuthManager::new(&nemesis_home, config.grok_com_config.clone());
...
mod tests {
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only nemesis_home: reading a missing auth.json succeeds (empty), but
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `nemesis login` must use loopback, not device —
    ...
    /// CLI `nemesis login` passes `on_stderr=None`; stderr must be inherited so

...
use xai_nemesis_telemetry::events::ManualAuthSurface;
...
impl AuthManager {
    pub fn new(nemesis_home: &Path, grok_com_config: GrokComConfig) -> Self {
        ...
        xai_nemesis_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "nemesis_home": nemesis_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "NEMESIS_HOME": std::env::var("NEMESIS_HOME").unwrap_or_else(|_| "(unset)".into()),
        ...
        // GROK_AUTH_PATH: custom file path (overrides default $NEMESIS_HOME/auth.json).
        let path = std::env::var("GROK_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| nemesis_home.join("auth.json"));
    ...
    pub(crate) async fn update(self: &Arc<Self>, auth: GrokAuth) -> std::io::Result<GrokAuth> {
        ...
        match &write_result {
            Ok(()) => xai_nemesis_telemetry::unified_log::info(
            ...
            Err(e) => xai_nemesis_telemetry::unified_log::error(
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<GrokAuth, AuthError> {
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`nemesis login` from
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<GrokAuth, AuthError> {
        ...
        xai_nemesis_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<xai_nemesis_telemetry::events::ManualAuth> {
...
impl xai_nemesis_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
) -> xai_nemesis_tools::types::SharedApiKeyProvider {

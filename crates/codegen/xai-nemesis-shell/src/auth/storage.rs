...
pub fn read_token_by_scope(nemesis_home: &Path, scope: &str) -> anyhow::Result<String> {
    let path = nemesis_home.join("auth.json");
    let store =
        read_auth_json(&path).map_err(|_| anyhow::anyhow!("Not logged in. Run `nemesis login`."))?;
    lookup_auth(&store, scope).map(|a| a.key).ok_or_else(|| {
        anyhow::anyhow!("Your auth token is invalid. Run `nemesis login` to re-authenticate.")
    })
}

/// Read the API key from the `xai::api_key` scope in auth.json.
pub fn read_api_key(nemesis_home: &Path) -> Option<String> {
    let path = nemesis_home.join("auth.json");
...
pub fn store_api_key(nemesis_home: &Path, api_key: &str) -> std::io::Result<()> {
    let path = nemesis_home.join("auth.json");
...
pub fn clear_api_key(nemesis_home: &Path) -> std::io::Result<()> {
    let path = nemesis_home.join("auth.json");

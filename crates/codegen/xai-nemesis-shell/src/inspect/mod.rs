...
use xai_nemesis_tools::util::truncate::estimate_tokens;
...
pub struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `NEMESIS_DISABLE_API_KEY_AUTH`).
...
pub struct ConfigSources {
    ...
    /// .nemesis/config.toml files). Driven from the same resolvers used at runtime
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = xai_nemesis_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = xai_nemesis_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = xai_nemesis_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        grok_version: xai_nemesis_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if path
        .parent()
        .is_some_and(|parent| parent == nemesis_home.join("rules"))
        || has_rules_directory(file_path, ".nemesis")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = xai_nemesis_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        xai_nemesis_agent::prompt::skills::CompatConfig::default(),
    )
    .await;

    let nemesis_home = crate::util::nemesis_home::nemesis_home();
    ...
    // `xai_nemesis_agent::prompt::agents_md::read_agents_config_with_paths`
    ...
    configs
        ...
        .map(|c| {
            let file_type =
                instruction_file_type(&c.file_path, &nemesis_home, imported, &extra_rule_prefixes);
            let scope = instruction_scope(&c.file_path, &nemesis_home, &vendor_homes, &workspace_root);
...
async fn list_permissions(cwd: &Path) -> PermissionsReport {
    use xai_nemesis_workspace::permission::resolution;
...
fn list_hooks(
    ...
    discovered_plugins: &[xai_nemesis_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = xai_nemesis_tools::types::compat::CompatConfig::default();
    ...
    let (registry, _errors) =
        xai_nemesis_hooks::discovery::load_hooks_from_sources(&global_sources, &project_sources);
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .into_iter()
        .map(|h| {
            let is_user_scope = h.source_dir.starts_with(&nemesis_home)
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &xai_nemesis_agent::plugins::PluginRegistry,
    skills_config: &xai_nemesis_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = xai_nemesis_agent::prompt::skills::list_skills_with_plugins(
    ...
    skills
        .into_iter()
        .map(|s| {
            let source = skill_entry_source(&s, &nemesis_home);
...
/// `<nemesis_home>/skills/<name>/SKILL.md` and discovered as user skills, so a
...
fn skill_entry_source(
    s: &xai_nemesis_agent::prompt::skills::SkillInfo,
    nemesis_home: &Path,
) -> ConfigSource {
    use xai_nemesis_tools::implementations::skills::types::SkillScope;
    ...
    match s.scope {
        ...
        SkillScope::User => {
            if crate::builtin::is_extracted_bundled_skill(&s.name, &path, nemesis_home) {
...
fn list_agents(
    ...
    let agents = xai_nemesis_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[xai_nemesis_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                xai_nemesis_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                xai_nemesis_agent::plugins::PluginScope::Project => Scope::Project,
                xai_nemesis_agent::plugins::PluginScope::User => Scope::User,
                xai_nemesis_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: xai_nemesis_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    xai_nemesis_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = xai_nemesis_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
...
/// `.nemesis/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_nemesis_home() {
...
mod tests {
    use super::*;
    use xai_nemesis_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use xai_nemesis_tools::implementations::skills::types::SkillScope;
    ...
    fn vendor_rule_paths_select_rules_compatibility_cells() {
        ...
        for path in ["/repo/.nemesis/rules/team.md", r"C:\repo\.nemesis\rules\team.md"] {
            assert_eq!(
    ...
    #[test]
    fn nemesis_home_nested_in_workspace_keeps_direct_surfaces_global() {
        let nemesis_home = Path::new("/repo/config");
        let workspace = Path::new("/repo");
        for path in ["/repo/config/AGENTS.md", "/repo/config/rules/global.md"] {
            assert!(matches!(
                instruction_scope(path, nemesis_home, &[], workspace),
                Scope::Global
            ));
        }
        for path in [
            "/repo/config/.nemesis/rules/project.md",
    ...
    #[test]
    fn workspace_scope_wins_inside_nemesis_home() {
        let nemesis_home = Path::new("/custom/grok");
        ...
        assert!(matches!(
            instruction_scope("/custom/grok/rules/global.md", nemesis_home, &[], workspace,),
            Scope::Global
        ));
    }

    #[test]
    fn custom_nemesis_home_rules_are_classified_as_rules() {
        assert_eq!(
    ...
    fn requirements_layer_contributes_requires_non_empty_post_strip_table() {
        ...
        let path = "/home/u/.nemesis/requirements.toml";
    ...
    fn skill_entry_source_maps_scopes() {
        ...
        let s = skill_fixture("a", "/repo/.nemesis/skills/a/SKILL.md", SkillScope::Local);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("b", "/repo/.nemesis/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.nemesis/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::User { .. }
        ));
        ...
        let s = skill_fixture(
            "d",
            "/home/u/.nemesis/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture("e", "/home/u/.nemesis/bundled/e/SKILL.md", SkillScope::Bundled);
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    /// path `<nemesis_home>/skills/<name>/SKILL.md`; a same-named skill anywhere
    ...
    fn skill_entry_source_relabels_extracted_bundled_skills() {
        ...
        let s = skill_fixture(
            "help",
            "/home/u/.nemesis/skills/help/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture("help", "/repo/.nemesis/skills/help/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        // Bundled name in a user dir outside <nemesis_home>/skills: stays user.
        let s = skill_fixture(
            "help",
        ...
        // Bundled frontmatter name in a different dir under <nemesis_home>/skills:
        // not the extracted copy — stays user.
        let s = skill_fixture(
            "help",
            "/home/u/.nemesis/skills/my-tools/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        // Non-bundled name under <nemesis_home>/skills: stays user.
        let s = skill_fixture(
            "my-skill",
            "/home/u/.nemesis/skills/my-skill/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        let registry = xai_nemesis_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);

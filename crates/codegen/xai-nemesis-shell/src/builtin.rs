//! Built-in files extracted to `~/.nemesis/` on startup.
...
///
/// These directories under `~/.nemesis/skills/` will be deleted on startup
...
/// `<nemesis_home>/skills/<name>/SKILL.md`. Exact-path (not prefix) so a
/// user-authored skill that reuses a bundled name — even elsewhere under
/// `<nemesis_home>/skills/` — is never labeled bundled. Lives beside the
...
pub(crate) fn is_extracted_bundled_skill(
    ...
    BUNDLED_SKILLS.iter().any(|&(n, _)| n == name)
        && path == nemesis_home.join("skills").join(name).join("SKILL.md")
}

/// Resolve the content for a skill, applying any name-specific transforms.
fn resolve_skill_content(name: &str, raw: &str, nemesis_home: &std::path::Path) -> String {
    match name {
        // Help skill needs path substitution so absolute paths work.
        "help" => {
            let nemesis_home_str = format!("{}/", nemesis_home.to_string_lossy());
            raw.replace("~/.nemesis/", &nemesis_home_str)
        }
        _ => raw.to_string(),
    }
}

/// Extract bundled files to `~/.nemesis/` on startup.
///
...
pub fn extract_bundled_files(nemesis_home: &std::path::Path) {
    ...
    remove_legacy_bundled_skills(nemesis_home);

    let version = xai_nemesis_version::VERSION;
    let marker = nemesis_home.join(".metadata_version");
        ...
        && existing.trim() == version
    {
        ...
        extract_missing_skills(nemesis_home);
        return;
    }
    ...
    for stale in &["CHANGELOG.json", "CHANGELOG.md"] {
        let _ = std::fs::remove_file(nemesis_home.join(stale));
    }

    for &(filename, content) in BUNDLED_FILES {
        if let Err(e) = std::fs::write(nemesis_home.join(filename), content) {
    ...
    for &(name, raw) in BUNDLED_SKILLS {
        let skill_dir = nemesis_home.join("skills").join(name);
        let _ = std::fs::create_dir_all(&skill_dir);
        let content = resolve_skill_content(name, raw, nemesis_home);
...
fn extract_missing_skills(nemesis_home: &std::path::Path) {
    for &(name, raw) in BUNDLED_SKILLS {
        let skill_md = nemesis_home.join("skills").join(name).join("SKILL.md");
        if skill_md.exists() {
            continue;
        }
...
fn remove_legacy_bundled_skills(nemesis_home: &std::path::Path) {
    remove_legacy_skills(nemesis_home, LEGACY_BUNDLED_SKILL_NAMES, BUNDLED_SKILLS);
}
...
fn remove_legacy_skills(
    ...
    for name in legacy_names {
        ...
        let dir = nemesis_home.join("skills").join(name);
        if dir.exists() {
...
mod tests {
    ...
    async fn help_skill_discovered_by_skill_pipeline() {
        ...
        std::fs::create_dir_all(workspace.join(".nemesis").join("skills").join("help")).unwrap();
        std::fs::copy(
            home.join("skills/help/SKILL.md"),
            workspace.join(".nemesis/skills/help/SKILL.md"),
        )
        .unwrap();

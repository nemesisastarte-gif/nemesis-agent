//! Notification bridge: translates `xai-nemesis-tools` `ToolNotification` events
//! into `xai-nemesis-shell`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
use xai_nemesis_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use xai_nemesis_tools::types::output::{BashOutput, ToolOutput};
use xai_nemesis_workspace::session::file_state::FileStateTracker;
...
pub struct NotificationBridgeConfig {
    ...
    pub task_wake_suppressed: xai_nemesis_tools::reminders::task_completion::TaskWakeSuppressed,
...
async fn emit_current_mode_update(
    ...
    mode: xai_nemesis_tools::types::SessionMode,
) {
...
async fn handle_notification(
    ...
    match notification {
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            let is_monitor =
                task_snapshot.kind == xai_nemesis_tools::computer::types::TaskKind::Monitor;
            ...
            if task_snapshot.block_waited || task_snapshot.explicitly_killed {
                ...
                let body = if is_monitor {
                    xai_nemesis_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    xai_nemesis_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
                };
                let message = xai_nemesis_tools::reminders::wrap_reminder(&body);
        ...
        ToolNotification::PlanModeEntered(entered) => {
            ...
            if activated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
                ...
                emit_current_mode_update(config, xai_nemesis_tools::types::SessionMode::Plan).await;
            }
            tracing::info!(
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                let _ = config
                    .persistence_tx
                    .send(PersistenceMsg::PlanModeState(snapshot));
                emit_current_mode_update(config, xai_nemesis_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
...
mod tests {
    use super::*;
    use xai_nemesis_tools::computer::types::TaskKind;
    use xai_nemesis_tools::types::TaskSnapshot;
...
if task_id == "mon-timeout")
    ...
    async fn monitor_event_skipped_after_task_completed_auto_wake() {
        ...
        handle_notification(
            &config,
            ToolNotification::MonitorEvent(xai_nemesis_tools::notification::types::MonitorEvent {
    ...
    async fn scheduled_task_created_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskCreated(
            xai_nemesis_tools::notification::types::ScheduledTaskCreated {
                task_id: "loop-1".into(),
    ...
    async fn bash_output_chunk_persists_and_broadcasts_one_event_id() {
        ...
        let notification = ToolNotification::BashOutputChunk(
            xai_nemesis_tools::notification::types::BashOutputChunk {
                base: xai_nemesis_tools::notification::types::BashNotificationBase {
    ...
    async fn scheduled_task_removed_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskRemoved(
            xai_nemesis_tools::notification::types::ScheduledTaskRemoved {
                task_id: "loop-1".into(),
            },
        );
        let mut offsets = HashMap::new();
    ...
    async fn task_backgrounded_persisted_line_is_stamped() {
        ...
        let notification = ToolNotification::BashExecutionBackgrounded(
            xai_nemesis_tools::notification::types::BashExecutionBackgrounded {
                base: xai_nemesis_tools::notification::types::BashNotificationBase {
    ...
    async fn scheduled_task_fired_is_not_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskFired(
            xai_nemesis_tools::notification::types::ScheduledTaskFired {
                task_id: "loop-1".into(),
                prompt: "check deploy".into(),
                human_schedule: "every 5 minutes".into(),
                next_fire_at: Some("2026-01-01T00:00:00Z".into()),
...
if task_id == "bg-disabled")
    ...
    async fn plan_mode_exited_emits_current_mode_update_default() {
        ...
        *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
        let notification =
            ToolNotification::PlanModeExited(xai_nemesis_tools::notification::types::PlanModeExited {
    ...
    async fn plan_mode_entered_emits_current_mode_update_plan() {
        ...
        let notification = ToolNotification::PlanModeEntered(
            xai_nemesis_tools::notification::types::PlanModeEntered {

...
/// after we stop waiting. Mirrors the bound the `xai-nemesis-tools` local terminal
...
fn spawn_shell_command(
    ...
    #[cfg(not(unix))]
    {
        let inv = xai_nemesis_config::shell::shell_command_argv(command);
...
fn spawn_with_argv(
    ...
    #[cfg(unix)]
    {
        ...
        let mut cmd = CommandWrap::with_new(program, |cmd| {
            ...
            #[cfg(target_os = "linux")]
            if xai_nemesis_sandbox::should_restrict_child_network() {
                // SAFETY: single prctl syscall (async-signal-safe).
                unsafe {
                    cmd.pre_exec(|| xai_nemesis_sandbox::child_net::install_child_network_filter());
                }
            }
        });

...
/// Resolved absolute path to bash. On Unix uses the `xai_nemesis_config` shell
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        xai_nemesis_config::shell::unix_shell_path(xai_nemesis_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `xai-nemesis-tools`.
pub use xai_nemesis_tools::util::pager_env;
...
/// xai-nemesis-pager which renders its own UI and doesn't need raw ANSI codes).
///

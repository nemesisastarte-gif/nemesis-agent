...
//!
//! Run with: `cargo test -p xai-nemesis-shell -- pull_smoke --ignored --nocapture`
...
mod tests {
    ...
    fn load_prod_auth() -> Option<GrokAuth> {
        let path = crate::util::nemesis_home::nemesis_home().join("auth.json");
    ...
    async fn smoke_push_pull_round_trip() {
        ...
        let auth = load_prod_auth().expect("No auth.json — run `nemesis login`");

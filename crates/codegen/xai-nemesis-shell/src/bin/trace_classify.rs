...
use clap::Parser;
use xai_nemesis_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// shell uses (`$NEMESIS_HOME` or `~/.grok`). Exposed primarily for
    /// tests / sandboxed invocations.
    #[arg(long)]
    nemesis_home: Option<PathBuf>,
}
...
mod tests {
    ...
    fn cli_parses_minimal_args() {
        ...
        assert!(cli.nemesis_home.is_none());
    }
    ...
    #[test]
    fn cli_nemesis_home_override_parses() {
        ...
        .expect("parse");
        assert_eq!(cli.nemesis_home, Some(PathBuf::from("/tmp/scratch-grok")));
    }

...
use xai_nemesis_tools::implementations::skills::skill::format_skill_name;
...
///   matching today's Nemesis Agent flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use xai_nemesis_tools::implementations::skills::skill::{
...
/// `xai-nemesis-tools` so it stays identical to the pager's `LoopCommand` and the

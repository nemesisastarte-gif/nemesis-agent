...
impl SessionActor {
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch) = tokio::join!(
            xai_nemesis_workspace::session::git::get_worktree_info(cwd),
            xai_nemesis_workspace::session::git::get_branch(cwd),
        );
        ...
        let params = xai_nemesis_workspace::session::git::GitHeadChanged {
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<xai_nemesis_tools::implementations::grok_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) fn classify_turn_error(err: &acp::Error) -> String {
        use xai_nemesis_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn stop_failure_error_type(
        err: &acp::Error,
    ) -> xai_nemesis_hooks::event::StopFailureKind {
        use xai_nemesis_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn classify_install_error(
        err: &xai_nemesis_agent::plugins::install_registry::InstallError,
    ) -> String {

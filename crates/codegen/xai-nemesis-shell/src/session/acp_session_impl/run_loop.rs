...
pub(super) async fn run_session(
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<xai_nemesis_mcp::servers::McpClientEvent>();
        {
    ...
    loop {
        tokio::select! {
                    ...
                    xai_nemesis_telemetry::memory_log::TARGET,
                    ...
                    } else { tracing::debug!(target : xai_nemesis_telemetry::memory_log::TARGET,
        ...
        if let Some(timeout) = session.idle_flush_timeout { idle_flush_sleep
                    ...
                    xai_nemesis_telemetry::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
                    ...
                    body_bytes_after, }) => {
                    xai_nemesis_telemetry::unified_log::info("shell.image_budget", Some(session
        ...
        if let Some(tx) = respond_to { let _ =
                    ...
                    .fire_hook(xai_nemesis_hooks::event::HookEventName::SessionEnd, None,
                    xai_nemesis_hooks::event::HookPayload::SessionEnd { reason : "channel_closed"
                    ...
                    xai_nemesis_hooks::dispatcher::dispatch_non_blocking(& registry,
                    xai_nemesis_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
                    ...
                    xai_nemesis_telemetry::memory_log::TARGET, result = ? result, tool_searches =
        ...
        { let
                    ...
                    xai_nemesis_telemetry::session_ctx::log_event(xai_nemesis_telemetry::events::SessionEnded
        ...
        if let
                    ...
                    false; xai_nemesis_telemetry::unified_log::info("shell.task_wake.gate_cleared",
        ...
        if tid == & task_id) }); }
                    ...
                    .push_capped(xai_nemesis_tools::implementations::grok_build::task::types::MonitorEventNotification
                    ...
                    xai_nemesis_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
                    ...
                    .send(xai_nemesis_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
        ...
        if let
                    ...
                    session.fire_hook(xai_nemesis_hooks::event::HookEventName::SessionStart, None,
                    xai_nemesis_hooks::event::HookPayload::SessionStart { source, model_id : None,
                    agent_type : None, },); if let Some(registry) = session.hook_registry
                    .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
                    xai_nemesis_hooks::dispatcher::dispatch_non_blocking(& registry,
                    xai_nemesis_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
                    ...
                    .fire_hook(xai_nemesis_hooks::event::HookEventName::SessionEnd, None,
                    xai_nemesis_hooks::event::HookPayload::SessionEnd { reason : "shutdown"
                    .to_string(), turn_count : None, tool_call_count : None, },); if let
                    Some(registry) = session.hook_registry.borrow().clone() { let ctx = session
                    .hook_run_ctx(); let results =
                    xai_nemesis_hooks::dispatcher::dispatch_non_blocking(& registry,
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[xai_nemesis_sampling_types::ConversationItem],
    turn_number: usize,

...
fn partition_rules_by_scope(
    files: Vec<xai_nemesis_agent::prompt::agents_md::AgentConfigFile>,
    nemesis_home: &std::path::Path,
    vendor_homes: &[(std::path::PathBuf, bool)],
    workspace_root: Option<&std::path::Path>,
) -> (
    Vec<xai_nemesis_agent::prompt::user_message::RuleEntry>,
    Vec<xai_nemesis_agent::prompt::user_message::RuleEntry>,
) {
    ...
    for file in files {
        ...
        let entry = xai_nemesis_agent::prompt::user_message::RuleEntry::from(file);
        if is_user_rule {
            user.push(entry);
        } else {
...
mod partition_rules_by_scope_tests {
    ...
    use std::path::Path;
    use xai_nemesis_agent::prompt::agents_md::AgentConfigFile;
    ...
    fn paths(entries: &[xai_nemesis_agent::prompt::user_message::RuleEntry]) -> Vec<&str> {
    ...
    #[test]
    fn nemesis_home_nested_in_workspace_keeps_direct_surfaces_user_scoped() {
        let files = vec![
            file("/repo/config/AGENTS.md"),
            file("/repo/config/rules/global.md"),
            file("/repo/config/.nemesis/rules/project.md"),
    ...
    #[test]
    fn nested_nemesis_home_workspace_files_stay_workspace_scoped() {
        let files = vec![
...
impl SessionActor {
        ...
        if drop_startup_skill_reminder {
            conversation.retain(|item| {
                !matches!(
                    ...
                    Some(xai_nemesis_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        &self,
        cwd: &std::path::Path,
        template: xai_nemesis_agent::prompt::user_message::UserMessageTemplate,
    ) -> Option<String> {
        use xai_nemesis_agent::prompt::agents_md::read_agents_config_with_paths;
        use xai_nemesis_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use xai_nemesis_workspace::file_system::{git_status_short, jj_status};
        use xai_nemesis_workspace::session::git::VcsKind;
    ...
    ) -> Vec<xai_nemesis_agent::prompt::user_message::McpServerEntry> {
        use xai_nemesis_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let client = xai_nemesis_sampler::SamplingClient::new(sampler_config).map_err(|e| {

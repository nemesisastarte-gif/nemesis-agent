...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> xai_nemesis_hooks::runner::RunContext<'_> {
        xai_nemesis_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[xai_nemesis_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, xai_nemesis_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
    ...
    pub(super) async fn dispatch_hook(
        ...
        payload: xai_nemesis_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            xai_nemesis_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx)
                .await;
    ...
    pub(super) async fn emit_hook_executed_telemetry(
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                xai_nemesis_hooks::result::HookRunResult::Success {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    xai_nemesis_telemetry::events::HookOutcome::Success,
                ),
                xai_nemesis_hooks::result::HookRunResult::Blocked {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    xai_nemesis_telemetry::events::HookOutcome::Blocked,
                ),
                xai_nemesis_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    xai_nemesis_telemetry::events::HookOutcome::Error,
                ),
                xai_nemesis_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            xai_nemesis_telemetry::session_ctx::log_event(xai_nemesis_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = XaiSessionUpdate::TaskCompleted {
            task_snapshot: xai_nemesis_tools::types::TaskSnapshot {
                task_id: "task-1".into(),

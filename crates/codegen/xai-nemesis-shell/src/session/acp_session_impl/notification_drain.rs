...
impl SessionActor {
    ...
    pub(super) fn sweep_monitor_buffer_into_pending(
        ...
        for event in xai_nemesis_tools::implementations::grok_build::task::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        ) {
            Self::push_pending_notification(
    ...
    fn notification_blocks(
        ...
        use xai_nemesis_tools::implementations::grok_build::task::types::MonitorEventNotification;
        ...
        if let (Some(index), Some(batch)) = (
            monitor_section_idx,
            xai_nemesis_tools::reminders::task_completion::format_monitor_events(
                &monitor_events,

...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &xai_nemesis_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                xai_nemesis_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                xai_nemesis_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        xai_nemesis_telemetry::session_ctx::log_event(
            xai_nemesis_telemetry::memory_telemetry::MemorySessionSummary {
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: xai_nemesis_telemetry::memory_log::TARGET, "MEMORY_FLUSH: starting");
        ...
        let result = async {
            ...
            xai_nemesis_telemetry::session_ctx::log_event(
                xai_nemesis_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
        ...
        tracing::info!(target: xai_nemesis_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        xai_nemesis_telemetry::session_ctx::log_event(
            xai_nemesis_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => xai_nemesis_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => xai_nemesis_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => xai_nemesis_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => xai_nemesis_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        xai_nemesis_telemetry::session_ctx::log_event(xai_nemesis_telemetry::events::MemoryFlushed {
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    xai_nemesis_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                xai_nemesis_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = xai_nemesis_sampler::stream_responses(
                    raw,
                    meta,
                    request_id,
                    idle_timeout,
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = xai_nemesis_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                xai_nemesis_sampler::collect_response(events).await
            }
        };

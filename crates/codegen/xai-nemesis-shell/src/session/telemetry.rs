use xai_nemesis_telemetry::events::SessionHarness;
use xai_nemesis_workspace::permission::Decision;
...
pub(crate) fn skill_source_label(skill_path: &str, cwd: &str) -> &'static str {
    ...
    let user = crate::util::nemesis_home::nemesis_home()
        .parent()
...
pub(crate) fn format_hook_name(spec: &xai_nemesis_hooks::config::HookSpec) -> String {
...
/// project dirs contain `/.nemesis/`.)
fn format_hook_source(spec: &xai_nemesis_hooks::config::HookSpec) -> &'static str {
...
impl HookRegInfo {
    pub(crate) fn from_spec(spec: &xai_nemesis_hooks::config::HookSpec) -> Self {
        Self {
...
pub(crate) struct SessionHarnessMetrics {
    ...
    pub permission_mode: xai_nemesis_telemetry::enums::PermissionMode,
    pub mcp_server_names: Vec<String>,
    pub lsp_server_names: Vec<String>,
    pub memory_enabled: bool,
    pub auto_update: Option<bool>,
    pub cwd: String,
    pub skills_config: xai_nemesis_agent::prompt::skills::SkillsConfig,
    /// Resolved vendor-compat config, so recorded skill / AGENTS.md names match
    /// what the session actually discovers.
    pub compat: xai_nemesis_tools::types::compat::CompatConfig,
...
impl SessionHarnessMetrics {
    pub async fn into_event(self, hooks: Vec<HookRegInfo>) -> SessionHarness {
        ...
        let agents_md_dir_names = xai_nemesis_agent::prompt::agents_md::read_agents_config_with_paths(
            &self.cwd,
            self.compat,
        )
        .await
        ...
        .collect();
        let skill_names = xai_nemesis_agent::prompt::skills::list_skills_with_plugins(
            Some(&self.cwd),
            &self.skills_config,
        ...
        SessionHarness {
            ...
            is_git_repo: xai_nemesis_telemetry::context::collect_git_context(&self.cwd).is_git_repo,

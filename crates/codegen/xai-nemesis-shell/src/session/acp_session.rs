...
use xai_nemesis_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use xai_nemesis_agent::prompt::skills::SkillsConfig;
use xai_nemesis_sampler::SamplerConfig as SamplingConfig;
use xai_nemesis_sampling_types::truncate_bytes;
...
use xai_nemesis_tools::types::compat::CompatConfig;
use xai_nemesis_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use xai_nemesis_workspace::file_system::CodebaseIndexManager;
use xai_nemesis_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use xai_nemesis_workspace::session::file_state::{FileStateHandle, FileStateTracker};
...
impl xai_nemesis_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        xai_nemesis_tools::types::resources::ManagedGatewayToolCallResponse,
...
pub(crate) struct SessionActor {
    ...
    /// `xai-nemesis-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<xai_nemesis_sampler::SharedAttributionCallback>,
    ...
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<xai_nemesis_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<xai_nemesis_sampling_types::CompactionAtTokens>>,
    ...
    pub(crate) doom_loop_recovery: Option<xai_nemesis_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) agent: std::cell::RefCell<xai_nemesis_agent::Agent>,
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<xai_nemesis_tools::implementations::grok_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, xai_nemesis_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<xai_nemesis_hooks::discovery::HookRegistry>>>,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<xai_nemesis_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<xai_nemesis_agent::plugins::SharedPluginRegistryHandle>,
    ...
    /// Handle to the per-session `xai-nemesis-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: xai_nemesis_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`xai_nemesis_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`xai_nemesis_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: xai_nemesis_workspace::WorkspaceOps,
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == xai_nemesis_tools::implementations::grok_build::SCHEDULER_CREATE_TOOL_NAME
            }),
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<xai_nemesis_tools::bridge::ToolBridge> {
...
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &xai_nemesis_agent::PromptContext) {
...
) -> Option<xai_nemesis_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use xai_nemesis_tools::types::output::{MCPOutput, ToolOutput};
    use xai_nemesis_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl xai_nemesis_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
...
mod tool_meta_stamp_tests {
    ...
    use super::*;
    use tokio::sync::mpsc;
    use xai_nemesis_tools::registry::types::ToolConfig;
    use xai_nemesis_tools::tool_taxonomy::TOOL_META_KEY;
    use xai_nemesis_workspace::permission::PermissionCommand;
...
mod managed_gateway_tool_tests {
    ...
    use xai_nemesis_tools::types::output::{MCPOutput, ToolOutput};
    use xai_nemesis_tools::types::tool::{ToolKind, ToolNamespace};
    use xai_nemesis_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<xai_nemesis_tools::types::resources::ManagedGatewayToolCatalog>()
            .await

...
//! both matched by the agent ([`xai_nemesis_hooks::matcher::HookMatcher`], shared with
//! file hooks):
...
use xai_nemesis_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use xai_nemesis_telemetry::events::ClientHookGateOutcome;
...
fn matching_callback_ids<'a>(
    ...
    groups
        .iter()
        .filter(|group| {
            xai_nemesis_hooks::matcher::matcher_allows(group.matcher.as_ref(), match_value)
        })
...
impl SessionActor {
    ...
    fn client_gate_responses<'a>(
        ...
        let default_timeout =
            if envelope.hook_event_name.traits().gate == xai_nemesis_hooks::event::GateKind::Stop {
        ...
        groups
            .iter()
            .filter(move |group| {
                xai_nemesis_hooks::matcher::matcher_allows(group.matcher.as_ref(), tool_name)
            })
            .flat_map(move |group| {
            ...
            .map(move |(callback_id, timeout)| {
                ...
                async move {
                    ...
                    xai_nemesis_telemetry::session_ctx::log_event(
                        xai_nemesis_telemetry::events::ClientHookGate {
    ...
    pub(super) async fn run_stop_client_hooks(
        &self,
        envelope: &HookEventEnvelope,
    ) -> xai_nemesis_hooks::dispatcher::StopDispatchResult {
        use xai_nemesis_hooks::result::HookRunResult;

        let mut out = xai_nemesis_hooks::dispatcher::StopDispatchResult::default();
        ...
        for (callback_id, (response, elapsed)) in ordered {
            ...
            let detail = xai_nemesis_hooks::dispatcher::stop_detail(
                stop_reason.is_some(),
            ...
            out.absorb(
                &hook_name,
                xai_nemesis_hooks::dispatcher::StopSignals {
                    block_reason,
                    stop_reason,
...
mod tests {
    ...
    async fn hook_event_active_inert_vs_active() {
        use xai_nemesis_hooks::event::HookEventName;
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    xai_nemesis_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
    ...
    fn matching_callback_ids_filters_by_matcher() {
        use xai_nemesis_hooks::matcher::HookMatcher;

...
//!
//! Core worktree lifecycle logic lives in [`xai_nemesis_workspace::worktree`].
...
use std::path::Path;
use xai_nemesis_workspace::session::git::find_git_root_from_path;
pub use xai_nemesis_workspace::worktree::*;
...
async fn create_worktree_for_resume(
    ...
    if find_git_root_from_path(source)
        .ok()
        .is_some_and(|root| xai_nemesis_workspace::session::git::detect_vcs_kind(&root).is_jj())
    {
...
async fn cleanup_worktree_on_failure(source_cwd: &str, worktree_path: &str) {
    ...
    if is_jj {
        ...
        if let Ok(root) = find_git_root_from_path(std::path::Path::new(source_cwd)) {
            let _ = xai_nemesis_workspace::session::git::git_cli(&root, &["worktree", "prune"]).await;
        }
    }
}
...
pub(crate) async fn checkout_persisted_head_in_worktree(
    ...
    session_id: &str,
) -> xai_nemesis_workspace::session::git::CheckoutSessionOutcome {
    let sha = match head_commit {
        Some(s) if !s.is_empty() => s,
        _ => return xai_nemesis_workspace::session::git::CheckoutSessionOutcome::default(),
    };
    xai_nemesis_workspace::session::git::checkout_session_commit(
...
pub(crate) struct WorktreeRestoreDecision {
    ...
    pub restore_degree: Option<xai_nemesis_workspace::session::git::RestoreDegree>,
}
/// Thin wire-format adapter over the shared
/// [`xai_nemesis_workspace::session::git::build_restore_decision`] helper.
pub(crate) fn build_worktree_restore_outcome(
    head_commit: Option<&str>,
    outcome: &xai_nemesis_workspace::session::git::CheckoutSessionOutcome,
    kind: xai_nemesis_workspace::session::git::RestoreKind,
) -> WorktreeRestoreDecision {
    let d = xai_nemesis_workspace::session::git::build_restore_decision(head_commit, outcome, kind);
    WorktreeRestoreDecision {
...
pub async fn resume_session_in_worktree(
    ...
    ops: &xai_nemesis_workspace::WorkspaceOps,
    ...
    use xai_nemesis_workspace::session::git::effective_worktree_path;
    tracing::info!(
    ...
    let restore_degree = if codebase_ok {
        Some(xai_nemesis_workspace::session::git::RestoreDegree::Full)
    } else {
        None
    };
...
async fn resume_local_session_in_worktree(
    req: &ResumeSessionInWorktreeRequest,
    #[allow(unused_variables)] ops: &xai_nemesis_workspace::WorkspaceOps,
    ...
    if req.restore_code.unwrap_or(restore_code_default) {
        let is_jj = find_git_root_from_path(std::path::Path::new(resolved_source_cwd))
            ...
            .is_some_and(|root| xai_nemesis_workspace::session::git::detect_vcs_kind(&root).is_jj());
        if !is_jj {
            if xai_nemesis_workspace::session::git::should_warn_registry_disabled(
                is_jj,
...
pub async fn rehydrate_session_in_worktree(
    ...
    let session_summary_exists = crate::util::nemesis_home::sessions_cwd_dir(&req.source_cwd)
        .join(&req.session_id)
        .join("summary.json")
        .exists();
...
mod tests {
    ...
    fn resume_response_round_trips() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(xai_nemesis_workspace::session::git::RestoreDegree::Full),
        };
    ...
    fn resume_response_serializes_degree_when_set() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(xai_nemesis_workspace::session::git::RestoreDegree::HeadOnly),
        };
        let json = serde_json::to_string(&resp).unwrap();
        ...
        assert_eq!(
            deser.restore_degree,
            Some(xai_nemesis_workspace::session::git::RestoreDegree::HeadOnly)
        );
    ...
    fn repo_wide_resolve_finds_session_in_sibling_cwd_skipping_remote() {
        ...
        let encoded = crate::util::nemesis_home::encode_cwd_dirname(sibling_cwd);

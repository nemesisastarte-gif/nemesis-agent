...
pub(crate) use xai_nemesis_sampling_types::is_context_length_error;
...
pub(crate) async fn generate_session_compact(
    ...
    let output = match sampling_config.api_backend {
        ApiBackend::ChatCompletions => {
            ...
            message.x_grok_agent_id = Some(xai_nemesis_telemetry::id::agent_id());
            tracing::info!(
                    ...
                    Ok(chunk) => {
                        if let Some(choice) = chunk.choices.first() {
                            ...
                            if let Some(fr) = choice.finish_reason {
                                let sr = xai_nemesis_sampling_types::StopReason::from(fr);
                                truncated =
                                    matches!(sr, xai_nemesis_sampling_types::StopReason::Length);
            ...
            loop {
                ...
                match chunk_result {
                    Ok(event) => {
                        if !matches!(
                            &event,
                            xai_nemesis_sampling_types::messages::MessageStreamEvent::Ping
                        ) {
                            last_progress_at = std::time::Instant::now();
                        }
                        match event {
                            xai_nemesis_sampling_types::messages::MessageStreamEvent::ContentBlockDelta {
                                delta: xai_nemesis_sampling_types::messages::StreamDelta::TextDelta {
                                    text,
                                },
                                ..
                            } => {
                            ...
                            xai_nemesis_sampling_types::messages::MessageStreamEvent::MessageDelta {
                                delta,
                                ..
                            } => {
                                if let Some(sr) = delta.stop_reason {
                                    truncated = matches!(
                                        sr, xai_nemesis_sampling_types::messages::StopReason::MaxTokens
                                        |
                                        xai_nemesis_sampling_types::messages::StopReason::ModelContextWindowExceeded
                                    );
                                    stop_reason = Some(
                                        match sr {
                                            xai_nemesis_sampling_types::messages::StopReason::EndTurn => {
                                                "end_turn".to_string()
                                            }
                                            xai_nemesis_sampling_types::messages::StopReason::MaxTokens => {
                                                "max_tokens".to_string()
                                            }
                                            xai_nemesis_sampling_types::messages::StopReason::ToolUse => {
                                                "tool_use".to_string()
                                            }
                                            xai_nemesis_sampling_types::messages::StopReason::StopSequence => {
                                                "stop_sequence".to_string()
                                            }
                                            xai_nemesis_sampling_types::messages::StopReason::Refusal => {
                                                "refusal".to_string()
                                            }
                                            xai_nemesis_sampling_types::messages::StopReason::PauseTurn => {
                                                "pause_turn".to_string()
                                            }
                                            xai_nemesis_sampling_types::messages::StopReason::ModelContextWindowExceeded => {
                                                "model_context_window_exceeded".to_string()
                                            }
                                            xai_nemesis_sampling_types::messages::StopReason::Unknown(
                                                s,
                                            ) => s,
                                        },
                                    );

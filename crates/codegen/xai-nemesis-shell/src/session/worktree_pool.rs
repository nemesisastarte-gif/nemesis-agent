...
//!   under `~/.nemesis/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.nemesis/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.nemesis/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.nemesis/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.nemesis/worktree_pool/`, reads each `.pid` file, and checks
...
/// The base pool directory under `~/.nemesis/`.
...
/// `CleanAll` mode. Placed under `~/.nemesis/worktree_pool/<instance_id>/<uuid>/`.

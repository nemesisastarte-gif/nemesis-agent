...
//! [`xai_nemesis_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`xai_nemesis_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`xai_nemesis_agent::Agent`] owns an [`xai_nemesis_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`xai_nemesis_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use xai_nemesis_agent::config::AgentDefinition;
use xai_nemesis_agent::error::AgentBuildError;
use xai_nemesis_agent::prompt::context::PromptAudience;
use xai_nemesis_agent::prompt::skills::SkillsConfig;
use xai_nemesis_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use xai_nemesis_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use xai_nemesis_tools::implementations::grok_build::ask_user_question::types::UserQuestionRequest;
...
use xai_nemesis_tools::implementations::lsp::LspBackend;
...
use xai_nemesis_tools::types::SharedApiKeyProvider;
use xai_nemesis_tools::types::compat::CompatConfig;
use xai_nemesis_tools::types::memory_backend::MemoryBackend;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<xai_nemesis_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<xai_nemesis_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use xai_nemesis_tools::implementations::grok_build::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(xai_nemesis_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(xai_nemesis_tools::types::resources::SchedulerBackgroundLoops(
                *scheduler_background_loops,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(xai_nemesis_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use xai_nemesis_tools::implementations::grok_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            xai_nemesis_tools::computer::local::LocalTerminalBackend::new_local(
                xai_nemesis_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        system_prompt_label: xai_nemesis_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(xai_nemesis_tools::types::tool::ToolKind::Task)

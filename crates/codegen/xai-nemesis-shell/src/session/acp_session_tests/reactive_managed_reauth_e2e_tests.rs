...
//! handshake against rmcp 2.1 in `xai-nemesis-mcp/tests/repro_sse_flood.rs`): a
...
use xai_nemesis_mcp::servers::{ClientStateKind, HttpConfig, McpClient};
...
async fn entry_b_routes_auth_rejection_but_not_policy_denial() {
    use xai_nemesis_mcp::servers::{is_auth_rejection_message, parse_mcp_tool_name};

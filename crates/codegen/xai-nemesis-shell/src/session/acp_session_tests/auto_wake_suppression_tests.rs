use super::support::*;
use super::*;
use xai_nemesis_tools::reminders::task_completion::consumed_completion_ids;
use xai_nemesis_tools::types::output::{BashOutput, TextOutput, ToolOutput};
use xai_tool_types::{
...
async fn cancel_barrier_rejects_task_completion_wake_without_reporting_it() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                res.get::<xai_nemesis_tools::types::resources::State<
                    xai_nemesis_tools::reminders::task_completion::ReportedTaskCompletions,
                >>()
                .is_none(),
                "declined admission must not report before user re-engagement"
            );
            drop(res);
            let reminder = xai_nemesis_tools::reminders::TaskCompletionReminder;
            let reminders = xai_nemesis_tools::types::tool::Reminder::collect_reminders(
                &reminder,
                resources,
        ...
        .run_until(async {
            ...
            assert!(
                resources
                    .lock()
                    .await
                    .get::<xai_nemesis_tools::types::resources::State<
                        xai_nemesis_tools::reminders::task_completion::ReportedTaskCompletions,
                    >>()
                    .is_none(),
...
async fn genuine_user_start_consumes_deferred_completions_without_notification_turn() {
    ...
    local
        .run_until(async {
            ...
            let body = xai_nemesis_tools::reminders::task_completion::format_monitor_completion(
                &xai_nemesis_tools::types::TaskSnapshot {
                    ...
                    signal: None,
                    completed: true,
                    kind: xai_nemesis_tools::computer::types::TaskKind::Monitor,
                    block_waited: false,
...
async fn already_reported(actor: &SessionActor, task_id: &str) -> bool {
    use xai_nemesis_tools::reminders::task_completion::ReportedTaskCompletions;
...
struct OneTaskTerminal {
    tasks: Vec<xai_nemesis_tools::computer::types::TaskSnapshot>,
}
#[async_trait::async_trait]
impl xai_nemesis_tools::computer::types::TerminalBackend for OneTaskTerminal {
    async fn run(
        &self,
        _: xai_nemesis_tools::computer::types::TerminalRunRequest,
    ) -> Result<
        xai_nemesis_tools::computer::types::TerminalRunResult,
        xai_nemesis_tools::computer::types::ComputerError,
    > {
        unimplemented!()
    }
    ...
    async fn get_task(&self, _: &str) -> Option<xai_nemesis_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn kill_task(&self, _: &str) -> xai_nemesis_tools::computer::types::KillOutcome {
        xai_nemesis_tools::computer::types::KillOutcome::NotFound
    }
    async fn wait_for_completion(
        &self,
        _: &str,
        _: Option<std::time::Duration>,
    ) -> Option<xai_nemesis_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn list_tasks(&self) -> Vec<xai_nemesis_tools::computer::types::TaskSnapshot> {
        self.tasks.clone()
    }
}
fn completed_bash_task(id: &str) -> xai_nemesis_tools::computer::types::TaskSnapshot {
    xai_nemesis_tools::computer::types::TaskSnapshot {
        task_id: id.into(),

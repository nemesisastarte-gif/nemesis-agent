...
use xai_nemesis_tools::implementations::grok_build::update_goal::UpdateGoalInput;
...
async fn answer_summarizer(
    ...
    req: Box<xai_nemesis_tools::implementations::grok_build::task::types::SubagentRequest>,
) {
    match behaviour {
...
fn configure_goal_actor(
    ...
    actor.tool_context.cwd =
        xai_nemesis_paths::AbsPathBuf::new(tmp.path().to_path_buf()).expect("abs cwd");

...
/// attribution.
fn auth_error() -> xai_nemesis_sampler::SamplingErrorInfo {
    xai_nemesis_sampler::SamplingErrorInfo {
        kind: xai_nemesis_sampler::SamplingErrorKind::Auth,
...
fn model_not_found_error() -> xai_nemesis_sampler::SamplingErrorInfo {
    xai_nemesis_sampler::SamplingErrorInfo {
    ...
    local
        .run_until(async {
            ...
            assert!(
                ...
                "hint must mention `nemesis login`, got: {msg}"
            );
            assert!(
...
fn unauthorized_401_error() -> xai_nemesis_sampler::SamplingErrorInfo {
    xai_nemesis_sampler::SamplingErrorInfo {
            kind: xai_nemesis_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://cli-chat-proxy.grok.com/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_xai_token_auth=xai-nemesis-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            model_metadata: None,
...
async fn sampler_non_auth_kind_401_on_provider_model_still_recovers() {
    ...
    local
        .run_until(async {
            ...
            error.kind = xai_nemesis_sampler::SamplingErrorKind::Api;

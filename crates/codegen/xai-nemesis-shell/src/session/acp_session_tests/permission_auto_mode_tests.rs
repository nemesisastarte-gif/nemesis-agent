...
use xai_nemesis_workspace::permission::{AccessKind, ClientType, spawn_permission_manager};
    ...
    local
        .run_until(async {
            ...
            let d = session
                .permissions
                .request(
                    AccessKind::Bash("cargo test -p xai-nemesis-workspace".into()),
                    dummy_update,
                    None,
                    None,
                    None,
            ...
            assert!(
                matches!(d, xai_nemesis_workspace::permission::Decision::Allow),
            ...
            let d2 = session
                ...
                .await;
            assert!(
                !matches!(d2, xai_nemesis_workspace::permission::Decision::Allow),
    ...
    local
        .run_until(async {
            ...
            actor.permissions.set_classifier_transcript(vec![
                xai_nemesis_workspace::permission::ClassifierTurn::UserText("please run tests".into()),
            ]);
...
fn build_classifier_turns_captures_tool_use_excludes_text_and_results() {
    use xai_nemesis_workspace::permission::ClassifierTurn;
    let conv = vec![
        ...
        super::ConversationItem::assistant_tool_calls(vec![
            xai_nemesis_sampling_types::conversation::ToolCall {

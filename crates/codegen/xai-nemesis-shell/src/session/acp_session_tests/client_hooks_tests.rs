...
fn install_client_hook(
    actor: &SessionActor,
    event: xai_nemesis_hooks::event::HookEventName,
    callback_ids: &[&str],
) {
...
async fn client_hooks_fire_without_file_registry() {
    ...
    local
        .run_until(async {
            ...
            install_client_hook(
                &actor,
                xai_nemesis_hooks::event::HookEventName::Stop,
                &["cb_0"],
            );
            ...
            actor.fire_hook(
                xai_nemesis_hooks::event::HookEventName::Stop,
                None,
                xai_nemesis_hooks::event::HookPayload::Stop {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
                xai_nemesis_tools::registry::types::ToolConfig::for_tool::<
                    xai_nemesis_tools::implementations::use_tool::UseTool,
                >(),
            ])
            .await;
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        xai_nemesis_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
...
fn file_registry_with_stop_spec(
    event: xai_nemesis_hooks::event::HookEventName,
    script: &str,
) -> xai_nemesis_hooks::discovery::HookRegistry {
    let (mut registry, _) = xai_nemesis_hooks::discovery::load_hooks(None, None);
    registry.append_specs(vec![xai_nemesis_hooks::config::HookSpec {
...
async fn subagent_session_gates_on_subagent_stop() {
    ...
    local
        .run_until(async {
            ...
            *actor.hook_registry.borrow_mut() =
                Some(std::sync::Arc::new(file_registry_with_stop_spec(
                    xai_nemesis_hooks::event::HookEventName::SubagentStop,
...
async fn alias_envelope_serializes_canonical_event_name() {
    ...
    local
        .run_until(async {
            ...
            let envelope = actor.make_hook_envelope(
                xai_nemesis_hooks::event::HookEventName::SubagentEnd,
                None,
                xai_nemesis_hooks::event::HookPayload::SubagentStop {
                    phase: xai_nemesis_hooks::event::SubagentStopPhase::Observe,

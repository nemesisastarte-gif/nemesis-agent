...
fn test_json_round_trip() {
    let ctx = xai_nemesis_agent::PromptContext {
        ..Default::default()
    };

    let json = serde_json::to_string_pretty(&ctx).unwrap();
    let loaded: xai_nemesis_agent::PromptContext = serde_json::from_str(&json).unwrap();
...
/// depend on `nemesis_home()` and `SessionInfo` path encoding).
#[test]
fn test_json_round_trip_via_filesystem() {
    ...
    let ctx = xai_nemesis_agent::PromptContext::default();
    ...
    let loaded: xai_nemesis_agent::PromptContext = serde_json::from_str(&read_json).unwrap();
...
fn test_system_prompt_is_plain_text_not_json() {
    let prompt = "You are a Nemesis Agent subagent.";
...
fn test_canonical_artifacts_coexist() {
    ...
    let read_ctx: xai_nemesis_agent::PromptContext = serde_json::from_str(
...
fn test_system_prompt_matches_chat_history_system_message() {
    ...
    let system_prompt = "You are a Nemesis Agent subagent.\n\n<tool_calling>\n...";
...
fn test_corrupt_json_returns_error() {
    ...
    let result: Result<xai_nemesis_agent::PromptContext, _> = serde_json::from_str(&json);
...
//
// `nemesis_home()` is a process-wide `OnceLock`, so the real async method is

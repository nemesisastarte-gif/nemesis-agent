...
//! `xai_nemesis_shell_session_support::managed_mcp` and is re-exported here so
//! `crate::session::managed_mcp::…` paths keep resolving unchanged.

pub use xai_nemesis_shell_session_support::managed_mcp::*;
...
pub fn merge_managed_mcp_servers_with_policy(
    ...
    let allowlist = &xai_nemesis_workspace::permission::resolution::managed_settings().mcp_allowlist;
...
impl McpDisabledReason {
    ...
    pub fn for_blocked_server(
        policy: &xai_nemesis_workspace::permission::resolution::McpServerAllowlist,
...
fn apply_mcp_server_policy(
    ...
    allowlist: &xai_nemesis_workspace::permission::resolution::McpServerAllowlist,
) -> Vec<McpServerWithPolicy> {
    merged
        .into_iter()
        .filter_map(|server| {
...
pub fn merge_managed_mcp_servers_sourced(
    ...
    let config_source = ConfigSource::ConfigToml {
        path: xai_nemesis_tools::util::nemesis_home::nemesis_home().join("config.toml"),
    };
...
fn load_plugin_mcp_servers_from_value(
    ...
    let normalized = xai_nemesis_agent::plugins::manifest::normalize_inline_mcp_servers(root);
...
fn load_plugin_mcp_servers_from_config(
    ...
    let sub = |s: &str| -> String {
        let s = xai_nemesis_agent::plugins::manifest::substitute_env_vars(s, plugin_root, plugin_data);
...
mod tests {
    ...
    fn merge_drops_denied_server_and_classifies_as_denylist() {
        use xai_nemesis_workspace::permission::resolution::{AllowedMcpServer, McpServerAllowlist};
    ...
    fn lower_precedence_http_servers_are_blocked_by_toml_name_claims() {
        ...
        std::fs::create_dir_all(cwd.path().join(".nemesis")).unwrap();
        std::fs::write(
            cwd.path().join(".nemesis").join("config.toml"),
            r#"
[mcp_servers.github]
...
"#,
    ...
    fn load_plugin_mcp_creates_stdio_server_with_env_substitution() {
        ...
        let (servers, _) = load_plugin_mcp_servers_from_config(
            &config,
            "team-tool",
            "/home/user/.nemesis/plugins/team-tool",
            "/home/user/.nemesis/plugin-data/team-tool",
        );
        ...
        match &servers[0] {
            acp::McpServer::Stdio(acp::McpServerStdio {
                ...
                assert_eq!(
                    args.as_slice(),
                    &["/home/user/.nemesis/plugins/team-tool/mcp-echo-server.py"]
                );
            }
    ...
    fn plugin_server_deduped_across_file_and_inline() {
        use xai_nemesis_agent::plugins::PluginRegistry;
        use xai_nemesis_agent::plugins::PluginScope;
        use xai_nemesis_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use xai_nemesis_agent::plugins::manifest::{PathOrInline, PluginManifest};
        ...
        let dp = DiscoveredPlugin {
            ...
            origin: xai_nemesis_agent::plugins::PluginOrigin::UserGrok,
            trusted: true,
            skill_dirs: vec![],
            command_dirs: vec![],
            agent_dirs: vec![],
    ...
    fn collect_plugin_oauth_configs_reads_byo_client_id_from_mcp_json() {
        ...
        use xai_nemesis_agent::plugins::PluginScope;
        use xai_nemesis_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use xai_nemesis_agent::plugins::manifest::PluginManifest;

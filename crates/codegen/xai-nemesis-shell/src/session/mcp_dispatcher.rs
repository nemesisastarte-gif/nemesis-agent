...
//!
//! Receives [`xai_nemesis_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`xai_nemesis_mcp::liveness`]),
//! - the [`xai_nemesis_mcp::servers::GrokClientHandler`] (server-pushed
...
pub struct McpServerStatusPayload {
    ...
    /// or `local` (user `.nemesis/config.toml`).
...
pub enum McpServerStatus {
    /// Client is in [`xai_nemesis_mcp::servers::ClientStateKind::Ready`]
...
/// [`xai_nemesis_mcp::servers::McpClientEventKind`]). Used as the
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && xai_nemesis_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
mod tests {
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        ...
        use xai_nemesis_mcp::servers::{McpClient, McpState};

...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.nemesis/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//!
//! ```ignore
//! use xai_nemesis_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
const CLIENT_LEADER_VERSION: &str = xai_nemesis_version::VERSION;
...
pub async fn discover_leaders() -> Vec<LeaderDescriptor> {
    discover_leaders_in(&crate::util::nemesis_home::nemesis_home()).await
}
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!(
                            ...
                            "reason" : reason, "killer_ver" : xai_nemesis_version::VERSION,
                            }
                        )),
                    );
                }
...
///
/// For a **managed install** — the running binary lives under `nemesis_home`
/// (e.g. `~/.nemesis/...`) — prefer the managed `~/.nemesis/bin/grok` symlink. After an
...
/// `xai_nemesis_update::auto_update::resolve_restart_exe`.
///
/// For a **dev / out-of-tree binary** (`cargo run`, integration tests, installs
/// not under `nemesis_home`), keep `current_exe()` so the spawned leader matches the
/// calling binary.
///
/// Falls back to `~/.nemesis/bin/grok` only when `current_exe()` is unavailable.
fn resolve_exe_for_spawn() -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_with_home(&crate::util::nemesis_home::nemesis_home())
}
fn resolve_binary_with_home(nemesis_home: &Path) -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_impl(nemesis_home, std::env::current_exe().ok())
}
/// Binary file name for the managed grok install (`grok` / `grok.exe`).
fn managed_grok_bin_name() -> &'static str {
    if cfg!(windows) { "grok.exe" } else { "nemesis" }
}
...
fn resolve_binary_impl(
    ...
    let managed_bin = nemesis_home.join("bin").join(managed_grok_bin_name());
    if let Some(ref exe) = current_exe
        && path_is_under(exe, nemesis_home)
        && managed_bin.exists()
    {
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    let log_path = crate::util::nemesis_home::nemesis_home().join("leader.log");
    ...
    let leader_log = std::env::var("GROK_LEADER_LOG")
        ...
        .unwrap_or_else(|_| "xai_nemesis_shell=info,xai_acp_lib=warn,xai_nemesis_mcp=warn".into());
...
mod tests {
    ...
    fn resolve_binary_prefers_current_exe() {
        ...
        std::fs::write(bin_dir.join("nemesis"), "fake-binary").unwrap();
    ...
    fn resolve_binary_prefers_current_exe_over_symlink() {
        ...
        std::os::unix::fs::symlink(&target_v2, bin_dir.join("nemesis")).unwrap();
        let result = resolve_binary_with_home(temp.path()).unwrap();
        let current = std::env::current_exe().unwrap();
        assert_eq!(result, current);
    }
    ...
    fn resolve_binary_prefers_managed_symlink_for_managed_install() {
        ...
        let managed = bin_dir.join("nemesis");

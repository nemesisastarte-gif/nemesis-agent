...
use fs2::FileExt;
use xai_nemesis_workspace::util::is_lock_contended;
...
/// `~/.nemesis/leader.sock` — e.g. run a local branch build's leader without
...
pub fn lock_path_for_ws_url(ws_url: &str) -> PathBuf {
    resolve_lock_path(leader_socket_override(), &nemesis_home(), ws_url)
}
...
pub fn socket_path_for_ws_url(ws_url: &str) -> PathBuf {
    resolve_socket_path(leader_socket_override(), &nemesis_home(), ws_url)
}
...
mod tests {
    ...
    fn override_socket_path_wins_over_ws_url_derivation() {
        ...
        let override_sock = PathBuf::from("/home/u/.nemesis/leader-branch.sock");
        ...
        assert_eq!(
            ...
            PathBuf::from("/home/u/.nemesis/leader-branch.lock")
        );
    }

...
use xai_nemesis_workspace::WorkspaceHandle;
...
async fn handle_workspace_start(
    ...
    let upload_queue_enabled =
        std::env::var("NEMESIS_WORKSPACE_UPLOAD_QUEUE_ENABLED").as_deref() != Ok("false");
    ...
    let handle = xai_nemesis_workspace::connect_local_workspace(
        cwd_path.clone(),
        url,
        auth,
        Some(metadata),

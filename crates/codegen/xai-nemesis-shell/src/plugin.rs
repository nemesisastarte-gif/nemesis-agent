...
use xai_nemesis_agent::plugins::discovery::PluginScope;
use xai_nemesis_agent::plugins::git_install::{self, UpdateStatus};
...
use xai_nemesis_plugin_marketplace::git::{self, SourceCacheLease};
use xai_nemesis_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.nemesis/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &xai_nemesis_agent::plugins::install_registry::MarketplaceProvenance,
...
pub fn marketplace_require_sha() -> bool {
    xai_nemesis_config::load_effective_config_disk_only()
        .map(|c| xai_nemesis_plugin_marketplace::load_require_sha(&c))
        .unwrap_or_else(|_| xai_nemesis_plugin_marketplace::env_require_sha())
}
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &xai_nemesis_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.nemesis/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user grok via user_nemesis_home() (None when no home resolves) and
    // home separately, so removal still runs from $NEMESIS_HOME when no home dir
...
mod tests {
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> xai_nemesis_workspace::permission::resolution::MarketplaceAllowlist {
        xai_nemesis_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = xai_nemesis_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![

// Re-export all types from the standalone xai-nemesis-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use xai_nemesis_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `xai-nemesis-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `xai-nemesis-sampler` crate can reference them without a circular
// dep on `xai-nemesis-shell`.

// Tests for the types now live in xai-nemesis-sampling-types crate.

use xai_nemesis_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `xai-nemesis-sampling-types` or
/// `xai-nemesis-tools`) so neither of those crates needs to depend on

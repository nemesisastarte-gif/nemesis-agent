...
//!
//! The canonical error types now live in `xai_nemesis_sampling_types::error`.
...
pub use xai_nemesis_sampling_types::error::*;
...
pub const FREE_USAGE_USER_MESSAGE: &str = "You\u{2019}ve reached your free Nemesis Agent usage limit for now. Get SuperGrok for much higher limits, or try again later: https://grok.com/supergrok?referrer=grok-build";
...
/// This stays in xai-nemesis-shell because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                xai_nemesis_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: xai_nemesis_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != xai_nemesis_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == xai_nemesis_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn attach_prompt_usage_preserves_error_kind_and_round_trips() {
        ...
        ledger.record_main_loop_call(
            "m",
            &xai_nemesis_sampling_types::TokenUsage {
                prompt_tokens: 3,
                completion_tokens: 1,
                total_tokens: 4,
                reasoning_tokens: 0,

...
/// two-function shape in `xai-nemesis-tools` (F29).
...
/// `xai-nemesis-tools` — existing items keep their prior content when
...
/// binary. Wraps a `xai_nemesis_sampler::SamplingClient` and pulls the
/// text content out of the response.
pub struct SamplerClassifierClient {
    inner: xai_nemesis_sampler::SamplingClient,
}

impl SamplerClassifierClient {
    pub fn new(inner: xai_nemesis_sampler::SamplingClient) -> Self {
        Self { inner }
    }
}
...
pub fn build_classifier_request(
    ...
    ConversationRequest {
        ...
        x_grok_agent_id: Some(xai_nemesis_telemetry::id::agent_id()),
...
pub struct RunArgs {
    ...
    /// `xai_nemesis_shell::util::nemesis_home::nemesis_home()` when `None`.
    /// Exposed as a CLI flag for testability.
    pub nemesis_home: Option<PathBuf>,
}
...
/// the user can act on.
pub async fn resolve_api_key(explicit: Option<&str>, nemesis_home: &Path) -> Result<String> {
    ...
    if let Some(key) = non_interactive_auth_key(nemesis_home).await? {
        return Ok(key);
    }
    Err(anyhow!(
        "no API key: pass --api-key, set XAI_API_KEY, or run `nemesis login` to populate \
         <grok-home>/auth.json. An expired OIDC token is auto-refreshed when a refresh_token \
         is present; if not, re-login is required."
    ))
}

/// Build an `AuthManager` for `nemesis_home`, install the
...
/// explicit `nemesis_home` so `--grok-home` overrides work in test
/// fixtures (the upstream helper hardcodes the global `nemesis_home()`
/// path).
///
/// Returns:
...
async fn non_interactive_auth_key(nemesis_home: &Path) -> Result<Option<String>> {
    ...
    let manager = std::sync::Arc::new(AuthManager::new(nemesis_home, config));
    ...
    match manager.auth().await {
        ...
        Err(e) => Err(anyhow!(
            "auth.json refresh failed: {e}. Run `nemesis login` to re-authenticate, \
...
async fn build_sampler_client(
    ...
    model: String,
    api_key: Option<&str>,
    nemesis_home: Option<&Path>,
) -> Result<xai_nemesis_sampler::SamplingClient> {
    let default_home;
    let nemesis_home_path: &Path = match nemesis_home {
        Some(p) => p,
        None => {
            default_home = crate::util::nemesis_home::nemesis_home();
            default_home.as_path()
        }
    };
    let resolved = resolve_api_key(api_key, nemesis_home_path).await?;
    let config = xai_nemesis_sampler::SamplerConfig {
        api_key: Some(resolved),
        base_url,
        model,
        max_completion_tokens: Some(LAZINESS_MAX_OUTPUT_TOKENS),
        ..xai_nemesis_sampler::SamplerConfig::default()
    };
    xai_nemesis_sampler::SamplingClient::new(config).map_err(|e| anyhow!("build SamplingClient: {e}"))
}
...
mod tests {
    use super::*;
    use xai_nemesis_sampling_types::{AssistantItem, ToolCall, ToolResultItem};
    ...
    async fn classifier_request_matches_production_shape() {
        ...
        assert_eq!(
            ...
            Some(xai_nemesis_telemetry::id::agent_id().as_str())
        );
    }
    ...
    fn write_auth_json(nemesis_home: &Path, key: &str) {
        ...
        std::fs::write(nemesis_home.join("auth.json"), body.to_string()).expect("write auth.json");
    }
    ...
    fn write_empty_auth_json(nemesis_home: &Path) {
        std::fs::write(nemesis_home.join("auth.json"), "{}").expect("write auth.json");
    }
    ...
    fn write_fresh_oidc_auth_json(nemesis_home: &Path, key: &str) {
        let scope = crate::auth::GrokComConfig::default().auth_scope();
        let body = serde_json::json!({
            scope: {
                "key": key,
    ...
    fn write_expired_oidc_auth_json_no_refresh(nemesis_home: &Path, key: &str) {
        let scope = crate::auth::GrokComConfig::default().auth_scope();
        let body = serde_json::json!({
            scope: {
                "key": key,
    ...
    async fn resolve_api_key_precedence() {
        ...
        write_empty_auth_json(nemesis_home);
        ...
        assert!(
            resolve_api_key(None, nemesis_home).await.is_err(),
            "no flag, no env, no auth.json → error",
        );
        assert!(
            resolve_api_key(Some(""), nemesis_home).await.is_err(),
            "empty flag → error",
        );
        assert!(
            resolve_api_key(Some("   "), nemesis_home).await.is_err(),
        ...
        assert_eq!(
            resolve_api_key(Some("from-flag"), nemesis_home)
                .await
                .expect("ok"),
            "from-flag",
        );
        ...
        assert_eq!(
            resolve_api_key(None, nemesis_home).await.expect("ok"),
            "from-env"
        );
        assert_eq!(
            resolve_api_key(Some("from-flag"), nemesis_home)
        ...
        assert_eq!(
            resolve_api_key(Some(""), nemesis_home).await.expect("ok"),
            "from-env",
    ...
    async fn resolve_api_key_falls_back_to_auth_json() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let nemesis_home = tmp.path();
        write_auth_json(nemesis_home, "from-auth-json");
        // `with_no_env` returns sync; we re-implement the clearing
        // inline so we can `.await` the resolver.
        let key =
            with_env_isolated(async { resolve_api_key(None, nemesis_home).await.expect("ok") }).await;
    ...
    async fn resolve_api_key_skips_auth_json_when_env_provided() {
        ...
        let key = with_env_isolated(async {
            unsafe { std::env::set_var("XAI_API_KEY", "from-env") };
            let resolved = resolve_api_key(None, nemesis_home).await.expect("ok");
    ...
    async fn resolve_api_key_errors_when_all_three_sources_missing() {
        ...
        let msg = with_env_isolated(async {
            let err = resolve_api_key(None, nemesis_home)
                .await
    ...
    async fn resolve_api_key_skips_empty_auth_json_entry() {
        ...
        // the unified error.
        write_auth_json(nemesis_home, "");
        let err1 = with_env_isolated(async { resolve_api_key(None, nemesis_home).await }).await;
        assert!(err1.is_err(), "empty auth.json entry is treated as absent");

        write_auth_json(nemesis_home, "   ");
        let err2 = with_env_isolated(async { resolve_api_key(None, nemesis_home).await }).await;
        assert!(
            err2.is_err(),
    ...
    async fn resolve_api_key_returns_refreshable_oidc_token() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let nemesis_home = tmp.path();
        write_fresh_oidc_auth_json(nemesis_home, "fresh-oidc-key");
        let key =
            with_env_isolated(async { resolve_api_key(None, nemesis_home).await.expect("ok") }).await;
    ...
    async fn resolve_api_key_returns_error_when_oidc_token_expired_and_no_refresh_token() {
        ...
        let tmp = tempfile::tempdir().expect("tempdir");
        let nemesis_home = tmp.path();
        write_expired_oidc_auth_json_no_refresh(nemesis_home, "stale-oidc-key");
        let err = with_env_isolated(async { resolve_api_key(None, nemesis_home).await }).await;
        assert!(
            err.is_err(),
    ...
    ///   the operator's real `~/.nemesis/auth.json`, the test would read
    ...
    async fn run_rejects_missing_trace_path() {
        ...
        let args = RunArgs {
            ...
            min_confidence: None,
            include_reasoning: None,
            nemesis_home: Some(tmp.path().to_path_buf()),
        };

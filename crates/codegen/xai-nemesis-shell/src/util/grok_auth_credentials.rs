...
/// User token (xAI users) sends `Bearer` + `X-XAI-Token-Auth: xai-nemesis-cli`.
...
impl GrokAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your NEMESIS_API_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `nemesis login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
...
impl xai_nemesis_auth::HttpAuth for GrokAuthCredentials {

...
///
/// Reads `stable_version` from `~/.nemesis/version.json` (written by the
...
///
/// This is a lightweight duplicate of `xai_nemesis_update::channel_name()` for
/// use in `xai-nemesis-shell` which cannot depend on `xai-nemesis-update`.
pub fn channel_name_from_cache() -> Option<&'static str> {
    use std::sync::OnceLock;
    static NAME: OnceLock<Option<&'static str>> = OnceLock::new();
    *NAME.get_or_init(|| {
        let version_path = crate::util::nemesis_home::nemesis_home().join("version.json");
        let content = std::fs::read_to_string(&version_path).ok()?;
        let parsed: serde_json::Value = serde_json::from_str(&content).ok()?;
        let stable = parsed.get("stable_version")?.as_str()?;
        let current = semver::Version::parse(xai_nemesis_version::VERSION).ok()?;

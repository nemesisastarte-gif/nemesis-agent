...
/// | config       | `[features] mcp_liveness_watchers` in `~/.nemesis/config.toml`     |
...
/// | config       | `[features] mcp_auto_restart` in `~/.nemesis/config.toml`          |
...
/// | config       | `[features] mcp_push_server_status` in `~/.nemesis/config.toml`    |
...
/// | config       | `[features] mcp_recursive_config_watch` in `~/.nemesis/config.toml`    |
...
/// Kept in sync with `xai_nemesis_mcp::servers`'s standalone fallback.
...
pub const DEFAULT_MAX_MCP_OUTPUT_BYTES: usize = xai_nemesis_tools::MCP_MAX_OUTPUT_BYTES;
...
pub fn cache_remote_max_mcp_output_bytes(remote: Option<u64>) {
    xai_nemesis_tools::set_mcp_max_output_bytes(resolve_max_mcp_output_bytes(remote));
}
...
pub fn resolve_max_mcp_output_bytes(remote: Option<u64>) -> usize {
    ...
    resolve_max_mcp_output_bytes_precedence(
        requirements,
        xai_nemesis_tools::mcp_max_output_bytes_from_env(),
...
/// `.nemesis/config.toml` chain (`cwd` → git root), deepest file wins.
///
...
fn project_max_mcp_output_bytes(cwd: &std::path::Path) -> Option<usize> {
    ...
    for config_path in crate::config::find_project_configs(cwd) {
        if let Ok(toml_val) = xai_nemesis_config::load_config_file(&config_path)
...
pub fn resolve_max_mcp_output_bytes_for_cwd(cwd: &std::path::Path) -> Option<usize> {
    let requirements = crate::config::load_merged_requirements()
        .as_ref()
        .and_then(max_mcp_output_bytes_from_toml);
    if requirements.is_some() || xai_nemesis_tools::mcp_max_output_bytes_from_env().is_some() {
        return None;
    }
...
mod max_mcp_output_bytes_tests {
    ...
    fn project_chain_deepest_file_wins() {
        ...
        std::fs::create_dir_all(sub.join(".nemesis")).unwrap();
        std::fs::create_dir_all(root.join(".nemesis")).unwrap();
        std::fs::write(
            root.join(".nemesis/config.toml"),
        ...
        std::fs::write(sub.join(".nemesis/config.toml"), "[ui]\nvim_mode = true\n").unwrap();
        assert_eq!(super::project_max_mcp_output_bytes(&sub), Some(30_000));

        // No .grok files with the key anywhere → None.
        std::fs::remove_file(root.join(".nemesis/config.toml")).unwrap();

...
pub fn auto_mode_classifier_defaults(
    ...
    xai_nemesis_workspace::permission::ClassifierPromptType,
    Option<xai_nemesis_sampling_types::ReasoningEffort>,
) {
    let prompt_type = cfg
        .prompt_type
        .unwrap_or(xai_nemesis_workspace::permission::ClassifierPromptType::Full);
    let reasoning_effort = cfg.reasoning_effort.or_else(|| {
        effective_supports_reasoning_effort.then_some(xai_nemesis_sampling_types::ReasoningEffort::Low)
    });

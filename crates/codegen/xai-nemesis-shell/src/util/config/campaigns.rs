...
use xai_nemesis_config::campaigns::{
    CampaignEntry, filter_active_campaigns, ids_touching_paths, merge_campaign_entries,
};
use xai_nemesis_config::config_override::{PatchPath, patch_touches_any};
use xai_nemesis_config::{
    CampaignsState, ConfigLayers, campaigns_state_path, load_dismissed_ids_from_home,
    user_nemesis_home,
};
use xai_nemesis_config_types::{CampaignOverride, RemoteSettings};
...
pub fn dismiss_campaign_ids(ids: impl IntoIterator<Item = String>) {
    let Some(home) = user_nemesis_home() else {
        return;
    };
...
fn dismiss_campaign_ids_at(
    ...
    // several grok processes share `$NEMESIS_HOME`; the in-process mutex alone would
...
pub async fn persist_models_default(
    value: Option<String>,
    reasoning_effort: Option<xai_nemesis_sampling_types::ReasoningEffort>,
) -> anyhow::Result<()> {
...
mod tests {
    ...
    use xai_nemesis_config::ConfigLayers;

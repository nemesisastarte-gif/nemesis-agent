...
use xai_nemesis_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use xai_nemesis_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `xai-nemesis-config-types` (config
...
pub use xai_nemesis_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use xai_nemesis_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.nemesis/config.toml` at each level.
/// Project-scoped `.nemesis/config.toml` entries override global `~/.nemesis/config.toml`
...
/// `.nemesis/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.nemesis/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.nemesis/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.nemesis/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .nemesis/config.toml"
                );
...
pub fn mcp_preferences_path() -> PathBuf {
    xai_nemesis_config::nemesis_home().join("mcp_preferences.json")
}
...
pub fn collect_mcp_setup_configs(
    cwd: &std::path::Path,
    plugin_registry: Option<&xai_nemesis_agent::plugins::PluginRegistry>,
    compat: &CompatConfig,
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    xai_nemesis_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.nemesis/config.toml`.
...
/// Upsert an MCP server entry in `~/.nemesis/config.toml`.
///
...
/// config file, e.g. a project-scoped `.nemesis/config.toml`.
...
/// Delete an MCP server entry from `~/.nemesis/config.toml`.
///
...
/// config file, e.g. a project-scoped `.nemesis/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = xai_nemesis_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.nemesis/config.toml` files.
...
// `.mcp.json` discovery moved to `xai-nemesis-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use xai_nemesis_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.nemesis/config.toml` files from `cwd` up to the
...
fn config_path() -> PathBuf {
    crate::util::nemesis_home::nemesis_home().join("config.toml")
}

/// Path to the user-level config file (`~/.nemesis/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.nemesis/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".nemesis").join("config.toml")
}
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.nemesis/skills", "~/.nemesis/skills/special/SKILL.md"]
ignore = ["~/.nemesis/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.nemesis/skills", "~/.nemesis/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.nemesis/skills/noisy/SKILL.md"]);
    }

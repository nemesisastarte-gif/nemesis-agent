...
/// `xai-nemesis-telemetry`; re-exported here so existing call sites continue
/// to work.
pub use xai_nemesis_telemetry::enums::PermissionMode;
...
/// `xai-nemesis-workspace`.
use xai_nemesis_workspace::permission::resolution::yolo_disabled_by_policy;
...
mod tests {
    ...
    // definition in `xai_nemesis_workspace::permission::claude_compat`.

    #[test]
    fn resolve_launch_yolo_policy_pin_neutralizes_requested_bypass() {
        let warning = xai_nemesis_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;

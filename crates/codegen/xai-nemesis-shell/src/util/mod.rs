...
// existing `crate::util::…` / `xai_nemesis_shell::util::…` users compile
// unchanged.
pub use xai_nemesis_shell_base::util::*;
...
pub(crate) fn is_user_instruction_path(
    ...
    let grok_rules = nemesis_home.join("rules");
    let is_exact_home_surface = parent
        .is_some_and(|parent| parent == nemesis_home || parent == grok_rules)
...
mod is_user_instruction_path_tests {
    ...
    #[test]
    fn nemesis_home_named_file_nested_in_workspace_is_user_scoped() {
    ...
    #[test]
    fn workspace_descendants_under_nemesis_home_stay_project_scoped() {
        assert!(!is_user_instruction_path(

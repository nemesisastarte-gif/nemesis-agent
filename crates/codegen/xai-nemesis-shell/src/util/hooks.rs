...
use xai_nemesis_hooks::discovery::HookSource;
...
pub fn discover_hook_source_paths(
    ...
    // sources. Native .nemesis/hooks/ directories are still scanned (they hold
    ...
    // user_nemesis_home() is None when no home resolves, so inspect lists the same
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".nemesis").join("hooks"));
        if !skip_cursor {
...
pub fn discover_hooks(
    ...
    Vec<xai_nemesis_hooks::error::HookError>,
) {

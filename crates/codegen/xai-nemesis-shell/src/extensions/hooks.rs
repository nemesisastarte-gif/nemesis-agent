...
use xai_nemesis_hooks::event::{HookEventEnvelope, HookEventName};
...
pub fn hook_spec_to_info(spec: &xai_nemesis_hooks::config::HookSpec) -> HookInfo {
    ...
    HookInfo {
        ...
        disabled: xai_nemesis_hooks::trust::is_hook_disabled(&spec.name),
    }
}
...
pub(crate) const ADVERTISED_BLOCKING_EVENTS: &[xai_nemesis_hooks::event::HookEventName] = &[
...
fn parse_hook_group(event: HookEventName, value: &serde_json::Value) -> Option<ClientHookGroup> {
    ...
    let matcher = match group.matcher.as_deref() {
        ...
        Some(pattern)
            if event.traits().matcher == xai_nemesis_hooks::event::MatcherPolicy::Ignored =>
        {
...
mod tests {
    ...
    fn advertised_blocking_events_are_gates() {
        use xai_nemesis_hooks::event::GateKind;
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use xai_nemesis_hooks::event::{HookEventEnvelope, HookPayload};

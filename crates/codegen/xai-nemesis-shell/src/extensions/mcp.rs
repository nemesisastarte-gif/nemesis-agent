...
// rmcp is quarantined in xai-nemesis-mcp; see that crate's docs.
use xai_nemesis_mcp::rmcp;
// `wire::MCP_CALL` is the one cross-SDK contract literal; the agent-only siblings
// live in `mcp_methods` below.
use xai_nemesis_mcp::wire;
...
/// `xai_nemesis_mcp::wire`), these methods are private to the agent↔client channel and
...
pub async fn build_mcp_status(
    ...
    tool_bridge: &Arc<xai_nemesis_tools::bridge::ToolBridge>,
...
//
// Implements the `McpResourceProvider` trait from xai-nemesis-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `xai-nemesis-mcp` directly.
...
impl xai_nemesis_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<xai_nemesis_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(xai_nemesis_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<xai_nemesis_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(xai_nemesis_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(xai_nemesis_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(xai_nemesis_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),

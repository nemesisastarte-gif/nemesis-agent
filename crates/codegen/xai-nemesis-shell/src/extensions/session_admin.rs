...
use xai_nemesis_telemetry::id::agent_id;
...
/// hot-reload watcher when `<cwd>/.nemesis/config.toml`,
...
/// Hot-reload the model catalog from `~/.nemesis/models_cache.json` after an
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .nemesis/plugins under it, plus

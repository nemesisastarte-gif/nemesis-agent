...
pub async fn handle_import(args: &acp::ExtRequest) -> ExtResult {
    ...
    if !has_local_session {
        ...
        crate::util::nemesis_home::ensure_sessions_cwd_dir(&request.cwd)
...
fn sanitize_summary_for_host(summary: &mut serde_json::Map<String, Value>, id: &str, cwd: &str) {
    ...
    set_or_remove(
        summary,
        "nemesis_home",
        crate::session::persistence::nemesis_home_string(),
    );
    set_or_remove(
        summary,
        "sandbox_profile",
        xai_nemesis_sandbox::configured_profile_name().map(String::from),
    );
}

...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    xai_nemesis_config::MANAGED_CONFIG_FILENAME,
    xai_nemesis_config::REQUIREMENTS_FILENAME,
    xai_nemesis_config::signed_policy::SIGNATURE_SIDECAR_FILE,
    xai_nemesis_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE,
];
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            xai_nemesis_config::MANAGED_CONFIG_CACHE_FILE,
    ...
    let atomic_write_tmp_prefixes = [
        format!("{}.", xai_nemesis_config::MANAGED_CONFIG_CACHE_FILE),
        format!(
            "{}.",
            xai_nemesis_config::signed_policy::SIGNATURE_SIDECAR_FILE
        ),
...
fn read_active_team_auth() -> Option<GrokAuth> {
    let home = crate::util::nemesis_home::nemesis_home();
...
/// files (TUI tick vs `nemesis login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `nemesis setup`; runs retries to completion.
    Standard,
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `xai-nemesis-http` classifier.
...
async fn fetch_managed_config_once(
    ...
    if let Some(nonce) = xai_nemesis_config::signed_policy::stored_envelope_nonce(
        &crate::util::nemesis_home::nemesis_home(),
        echo_principal,
    ) && let Ok(value) = reqwest::header::HeaderValue::from_str(&nonce)
    {
        request = request.header(
            xai_nemesis_config::signed_policy::MANAGED_CONFIG_NONCE_ECHO_HEADER,
            value,
        );
    }
...
/// Resolve deployment key from `NEMESIS_API_KEY` env var, then config files.
...
/// Callers doing auto-fetch should check this; explicit user actions (nemesis setup) skip it.
...
/// Fetch managed config + requirements and write to `~/.nemesis/`, trying the
...
/// read-only `nemesis setup --json` both build on this.
...
fn apply_fetched(
    ...
    let verified = if xai_nemesis_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(xai_nemesis_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        xai_nemesis_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        ...
        {
            clear_squatting_dir(
                &home.join(xai_nemesis_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
            xai_nemesis_config::signed_policy::write_managed_identity_sidecar(&home, &claim_sidecar)?;
        }
    }
    ...
    clear_squatting_dir(&home.join(xai_nemesis_config::MANAGED_CONFIG_CACHE_FILE));
...
fn served_principal_of(payload: &xai_nemesis_config::signed_policy::SignedPayload) -> Option<&str> {
    payload
        .deployment_id
        .as_deref()
...
fn verified_claim_sidecar(
...
) -> Option<xai_nemesis_config::signed_policy::SignatureEnvelope> {
    use xai_nemesis_config::signed_policy::now_unix;
    ...
    let claim = match xai_nemesis_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
fn claim_binds_to(
    claim: &xai_nemesis_config::signed_policy::ManagedIdentityClaim,
    served_principal: Option<&str>,
) -> bool {
...
/// Post-login hook for `nemesis login` and the ACP/TUI authenticate flow: clear any
...
/// Whether a credential exists that `nemesis setup` could install config for.
...
/// are the non-forgeable layers. Recovery: reconnect / `nemesis setup`; ceasing to serve
...
fn bump_managed_rollback_floor() {
    ...
    if !xai_nemesis_config::signed_policy::verification_active() {
        return;
    }
    let home = crate::util::nemesis_home::nemesis_home();
    match try_lock_managed_config(&home) {
        Some(_lock) => {
            xai_nemesis_config::bump_rollback_floor(&home);
        }
...
/// Outcome of the `nemesis setup` sync. The caller renders it — CLI presentation
...
/// Result of `nemesis setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `nemesis setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `nemesis setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `nemesis setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `nemesis setup` would refuse
    // is reported as an error, not printed as installable config.
    if source.is_some()
        && xai_nemesis_config::signed_policy::verification_active()
...
/// Run the `nemesis setup` sync for the current principal. The caller must check

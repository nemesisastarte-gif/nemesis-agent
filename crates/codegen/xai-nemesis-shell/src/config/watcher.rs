...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.nemesis/models_cache.json` changed — the on-disk `/v1/models`
...
/// Watches `~/.nemesis/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.nemesis/config.toml`, `.mcp.json`,
...
/// processes (e.g. `nemesis login` in another terminal).
///
/// Adds two **non-recursive** watches per `cwd` argument:
/// `<cwd>/` (catches `.mcp.json` and `.claude.json` at the project root) and
/// `<cwd>/.nemesis/` (catches `<cwd>/.nemesis/config.toml`). Recursing on `<cwd>`
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.nemesis/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let nemesis_home_buf = nemesis_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(nemesis_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(nemesis_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(nemesis_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(nemesis_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %nemesis_home.display(),
                    error = %e,
        ...
        // includes `<cwd>/.nemesis/config.toml` so the loop above
        // watches `<cwd>/.nemesis/`), the call below installs a
        ...
        tracing::info!(
            nemesis_home = %nemesis_home.display(),
    ...
    /// Register `<cwd>/` and `<cwd>/.nemesis/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.nemesis/config.toml` trigger a
    ...
    /// `<cwd>/.nemesis/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.nemesis/` does not yet
/// exist at session-open time, the `.nemesis/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.nemesis/` followed by a
/// write to `<cwd>/.nemesis/config.toml` will NOT be observed — the
...
/// expected for a freshly-opened session whose `<cwd>/.nemesis/` hasn't
...
/// Watches skill directories (`~/.nemesis/skills/`, `<repo>/.nemesis/skills/`, etc.)
...
/// recursively: `nemesis_home` (`~/.grok`, or `$NEMESIS_HOME`) or a known vendor dir
...
fn is_global_config_dir(dir: &Path, nemesis_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, nemesis_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".nemesis", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, nemesis_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(nemesis_home) {
        return true;
    }
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](xai_nemesis_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let dirs_to_watch = xai_nemesis_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &nemesis_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &nemesis_home) {
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only nemesis_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_nemesis_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let nemesis_home = home.join(".nemesis");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &nemesis_home, Some(home));

        // nemesis_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&nemesis_home));
        ...
        assert!(!g(&home.join("repo").join(".nemesis")));
    }

    /// Regression for the ~/.grok inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.nemesis/worktrees/`) must not
    ...
    /// A write to `<nemesis_home>/models_cache.json` must surface as
    ...
    /// A write to `<cwd>/.nemesis/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let nemesis_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_grok = cwd.path().join(".nemesis");
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.nemesis/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.nemesis/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_grok = new_cwd.path().join(".nemesis");
        fs::create_dir_all(&project_grok).unwrap();
        fs::write(project_grok.join("config.toml"), "").unwrap();

...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.nemesis/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.nemesis/config.toml`,
    ...
    /// (`<cwd>/.nemesis/config.toml`, `<cwd>/.mcp.json`, or
    ...
    ProjectMcpServersChanged {
        /// The project root whose `.nemesis/`, `.mcp.json`, or
    ...
    ModelsChanged,
    /// `~/.nemesis/models_cache.json` was rewritten on disk (possibly by another
...
impl ConfigReloader {
    ...
    pub async fn run(
        ...
        loop {
            ...
            if has_auth {
                ...
                match result {
                    Ok(Err(e)) => {
                        ...
                        let path = self.nemesis_home.join("auth.json");
            ...
            // same tick (e.g. `~/.nemesis/config.toml` AND
    ...
    fn reload_auth(&mut self) -> anyhow::Result<()> {
        let auth_path = self.nemesis_home.join("auth.json");
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.nemesis/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.nemesis/config.toml`,
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.nemesis/config.toml` | `<cwd>`           |
...
/// for `.nemesis/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
/// (`~/.nemesis/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
pub(crate) fn parse_skills_config(
    config: &toml::Value,
) -> xai_nemesis_agent::prompt::skills::SkillsConfig {
    config
        .get("skills")
...
fn parse_compat_config(config: &toml::Value) -> xai_nemesis_tools::types::compat::CompatConfigToml {
    config
        .get("compat")
        .and_then(|v| v.clone().try_into().ok())
        .unwrap_or_default()
...
if *c == cwd),
    ...
    /// The hash must reflect ancestor `.nemesis/config.toml` and `.mcp.json`
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".nemesis").join("config.toml"), "x = 1").unwrap();
        let h3 = hash_project_mcp_config(&child).expect("readable");
        assert_ne!(
            h2, h3,
            "ancestor .nemesis/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.nemesis/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.nemesis/skills".to_string()]);
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.nemesis/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.nemesis/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.nemesis/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.nemesis/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(

...
fn with_tools_env<T>(
    ...
    with_env_var_opt(
        ...
        respect_gitignore,
        || with_env_var_opt("NEMESIS_DISABLE_ZDR_INCOMPATIBLE_TOOLS", disable_zdr, f),
    )
}
...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".nemesis/prompts/impl.md"
        "#;
    ...
    assert_eq!(implementer.prompt_file.as_deref(), Some(".nemesis/prompts/impl.md"));
}
#[test]
...
fn validate_roles_accepts_valid_prompt_file() {
    let toml_str = r#"
            ...
            prompt_file = ".nemesis/prompts/ok.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.validate_roles().is_empty());
}
...
fn discover_roles_loads_from_directory() {
    ...
    let roles_dir = tmp.path().join(".nemesis").join("roles");
...
fn personas_parse_from_toml() {
    ...
    assert_eq!(concise.instructions_file.as_deref(), Some(".nemesis/personas/concise.md"));
}
#[test]
...
fn discover_personas_loads_from_directory() {
    let tmp = tempfile::TempDir::new().unwrap();
    let dir = tmp.path().join(".nemesis").join("personas");
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    let bundled = home.join(".nemesis").join("bundled");
    std::fs::create_dir_all(workspace.join(".nemesis").join("roles")).unwrap();
    std::fs::create_dir_all(workspace.join(".nemesis").join("personas")).unwrap();
    std::fs::create_dir_all(home.join(".nemesis").join("roles")).unwrap();
    std::fs::create_dir_all(home.join(".nemesis").join("personas")).unwrap();
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            home.join(".nemesis/roles/reviewer.toml"),
            r#"description = "User reviewer""#,
        )
        .unwrap();
    std::fs::write(
            home.join(".nemesis/personas/reviewer.toml"),
            r#"instructions = "User persona""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".nemesis/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".nemesis/personas/reviewer.toml"),
    ...
    std::fs::remove_file(workspace.join(".nemesis/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(workspace.join(".nemesis/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
    ...
    std::fs::remove_file(home.join(".nemesis/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(home.join(".nemesis/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.nemesis/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
/// a repo `.nemesis/config.toml` structurally cannot supply one.)
#[test]
...
#[test]
fn validate_hooks_path_rejects_outside_nemesis_home() {
    let result = validate_hooks_path("/tmp/evil-hooks");
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
        msg.contains("must be under ~/.nemesis/"),
        "should mention ~/.nemesis/ restriction, got: {msg}"
    );
}
#[test]
fn validate_hooks_path_rejects_traversal_attack() {
    let nemesis_home = crate::util::nemesis_home::nemesis_home();
    let traversal = format!("{}/../evil", nemesis_home.display());
...
fn validate_hooks_path_accepts_grok_hooks_subdir() {
    let nemesis_home = crate::util::nemesis_home::nemesis_home();
    let valid_path = nemesis_home.join("hooks").join("my-hooks");
    let _ = std::fs::create_dir_all(&valid_path);
    let result = validate_hooks_path(valid_path.to_str().unwrap());
    assert!(result.is_ok(), "path under ~/.nemesis/ should be accepted");
}
#[test]
fn managed_settings_disables_features_and_requirements_overrides() {
    use xai_nemesis_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
fn simulate_release_build() -> xai_nemesis_test_support::EnvGuard {
    xai_nemesis_test_support::EnvGuard::set(xai_nemesis_version::TEST_VERSION_ENV, "0.0.0-sim")
}
#[test]
...
fn base_resolver_without_project_cwd_keeps_project_files_out() {
    let tmp = tempfile::tempdir().unwrap();
    write_subagent_definitions(&tmp.path().join(".nemesis"), &[("project", "Project")]);
    let base = SubagentsConfig::resolve_base_with_sources(
        false,
...
/// verdicts untouched). NEMESIS_HOME-isolated + `#[serial]` for folder-trust
/// store hygiene (empty store ⇒ deterministic untrusted;
/// `EnvGuard` restores NEMESIS_HOME even on panic). No user-global
/// `$NEMESIS_HOME/config.toml` is seeded: `nemesis_home()` is `OnceLock`-cached,
...
fn resolve_effective_plugins_config_gates_project_paths_on_folder_trust() {
    ...
    let repo = tempfile::tempdir().unwrap();
    git2::Repository::init(repo.path()).unwrap();
    let grok = repo.path().join(".nemesis");
...
/// in `xai-nemesis-agent`. An ABSOLUTE plugin path is used so the merged
...
/// NEMESIS_HOME-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use xai_nemesis_agent::plugins::{TrustStore, discover_plugins};
    use xai_nemesis_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("NEMESIS_HOME", home.path());
    let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
...
/// arm (store-only reconcile) could never lift. NEMESIS_HOME-isolated (empty

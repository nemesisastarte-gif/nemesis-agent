...
//! # Wiring
//!
//! `xai-nemesis-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`xai_nemesis_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`xai-nemesis-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `xai-nemesis-pager`.
///
...
/// subsequent calls are ignored (see [`xai_nemesis_pager::minimal_hook`]).
pub fn install() {
    xai_nemesis_pager::minimal_hook::install(xai_nemesis_pager::minimal_hook::MinimalHooks { draw });
}

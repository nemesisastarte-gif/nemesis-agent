...
//! persisted to `~/.nemesis/tip_cursor.json`.
...
fn cursor_path(nemesis_home: &Path) -> PathBuf {
    nemesis_home.join(CURSOR_FILE)
}

/// Load the cursor from `~/.nemesis/tip_cursor.json`. Returns 0 on any error.
fn load_cursor(nemesis_home: &Path) -> u64 {
    let text = match std::fs::read_to_string(cursor_path(nemesis_home)) {
        Ok(t) => t,
        Err(_) => return 0,
    };
...
/// Save the cursor to `~/.nemesis/tip_cursor.json`. Silently ignores write errors.
fn save_cursor(nemesis_home: &Path, cursor: u64) {
    if let Ok(text) = serde_json::to_string(&TipState { cursor }) {
        let _ = std::fs::write(cursor_path(nemesis_home), text);
    }
}
...
/// cursor in `~/.nemesis/tip_cursor.json`, so every session sees the next tip
...
pub fn pick_and_advance(tips: &[String], nemesis_home: &Path) -> Option<String> {
    if tips.is_empty() {
        return None;
    }
    let cursor = load_cursor(nemesis_home);
    let tip = tips[cursor as usize % tips.len()].clone();
    save_cursor(nemesis_home, cursor + 1);
    Some(tip)
}

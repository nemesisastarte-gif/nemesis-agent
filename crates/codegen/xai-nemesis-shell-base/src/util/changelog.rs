...
impl ChangelogManager {
    pub fn new() -> Self {
        // Prefer live `$NEMESIS_HOME` so harness-injected homes (PTY e2e) always
    ...
    /// `nemesis_home()` OnceLock). A seeded `$NEMESIS_HOME` set on the pager
    /// process is always honoured even if some earlier init path cached a
    /// different home.
    fn from_env_home() -> Self {
        let home = std::env::var_os("NEMESIS_HOME")
            .map(std::path::PathBuf::from)
            .filter(|p| !p.as_os_str().is_empty())
            .unwrap_or_else(crate::util::nemesis_home::nemesis_home);
        Self {
    ...
    /// `$NEMESIS_HOME` so harness-injected env always applies.
    ///
    ...
    /// without mutating process-global env (`NEMESIS_HOME` /
    ...
    fn fetch_with(&self, offline: bool, base: &str) -> Changelog {
        ...
        // any on-disk seed under `$NEMESIS_HOME` even when offline mode was not
...
mod tests {
    ...
    /// `$NEMESIS_HOME` env so tests never race the parallel harness.

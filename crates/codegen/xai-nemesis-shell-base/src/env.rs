...
//! trust check, `EnvVarGuard`) live in the [`xai_nemesis_env`] leaf crate so
...
pub use xai_nemesis_env::EnvVarGuard;
pub use xai_nemesis_env::{
...
pub const NEMESIS_DISABLE_CUSTOM_BRIDGE_ENV: &str = "NEMESIS_DISABLE_CUSTOM_BRIDGE";
/// `true` when the custom-method bridge passthrough is force-disabled via
/// [`NEMESIS_DISABLE_CUSTOM_BRIDGE_ENV`]. Accepts `1`/`true` (case-insensitive).
pub fn custom_bridge_disabled() -> bool {
    std::env::var(NEMESIS_DISABLE_CUSTOM_BRIDGE_ENV)
        .map(|v| {
            let v = v.trim();
...
mod tests {
    ...
    fn custom_bridge_disabled_defaults_false_when_unset() {
        let _env = EnvVarGuard::remove(NEMESIS_DISABLE_CUSTOM_BRIDGE_ENV);
        assert!(!custom_bridge_disabled());
    }
    #[test]
    fn custom_bridge_disabled_true_for_one_and_true() {
        for v in ["1", "true", "TRUE", " true "] {
            let _env = EnvVarGuard::set(NEMESIS_DISABLE_CUSTOM_BRIDGE_ENV, v);
            assert!(

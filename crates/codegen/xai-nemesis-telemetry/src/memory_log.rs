...
//! file logger that writes to `~/.nemesis/logs/memory.log`.
//!
//! ## When to use
//!
...
//! tail -f ~/.nemesis/logs/memory.log      # watch in another terminal
//! ```
...
mod inner {
    ...
    ///
    /// Writes to `~/.nemesis/logs/memory.log`. Filters to `xai_memory=trace`.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || nemesis_home().join("logs").join("memory.log");

...
//!
//! `xai-nemesis-pager` uses this module to set up OTLP
...
///
/// Built by the binaries (`xai-nemesis-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `xai_nemesis_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-XAI-Token-Auth` header (typically `"xai-nemesis-cli"`).
...
///
/// - `client_name`: binary name (e.g. `"grok-tui"`, `"nemesis-agent"`) -- stored as
...
fn build_export_headers(
    ...
    snapshot: &xai_nemesis_auth::CredentialSnapshot,
...
mod tests {
    ...
    use std::sync::Mutex;
    use xai_nemesis_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("xai-nemesis-cli"),

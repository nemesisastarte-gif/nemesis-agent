...
//! file logger that writes to `~/.nemesis/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `xai_nemesis_hooks` or `xai_nemesis_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! NEMESIS_HOOKS_LOG=1 grok              # enable, write to ~/.nemesis/logs/hooks.log
//! NEMESIS_HOOKS_LOG=/tmp/h.log grok     # write to custom path
//! NEMESIS_HOOKS_LOG=0 grok              # explicitly disable
//! tail -f ~/.nemesis/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "NEMESIS_HOOKS_LOG";
...
///
/// Writes to `~/.nemesis/logs/hooks.log` (or custom path via `NEMESIS_HOOKS_LOG`).
/// Filters to hooks (`xai_nemesis_hooks`) and plugins (`xai_nemesis_agent::plugins`) targets.
/// Set `NEMESIS_HOOKS_LOG=0` to disable, `NEMESIS_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "xai_nemesis_hooks=debug,xai_nemesis_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || nemesis_home().join("logs").join("hooks.log");

...
//! - PerSession (`NEMESIS_DEBUG_LOG=1`): a routing layer fans each session's
//!   firehose to `~/.nemesis/debug/<session_id>.txt` (one file per session), with a
...
//! - SingleFile (explicit path via `NEMESIS_LOG_FILE` or `NEMESIS_DEBUG_LOG=<path>`):
...
use xai_nemesis_config::nemesis_home;
...
impl DebugSource {
    fn label(self) -> &'static str {
        match self {
            Self::GrokLogFile => "NEMESIS_LOG_FILE",
            Self::GrokDebugLog => "NEMESIS_DEBUG_LOG",
        }
    }
}
...
///
/// Lives here (not in `xai-nemesis-pager`) so the firehose directives below and
...
/// safe. See `xai-nemesis-pager/src/tracing.rs` for the consumer side.
...
// Broad firehose filter for the routing/NEMESIS_DEBUG_LOG sources: capture our
...
// until added here.
const FIREHOSE_BASE_DIRECTIVES: &str = "info,xai_nemesis_pager=debug,xai_nemesis_shell=debug,xai_nemesis_tools=debug,xai_nemesis_telemetry=debug,xai_nemesis_agent=debug,xai_nemesis_mcp=debug,xai_acp_lib=debug,sampling_log=off";
...
// NEMESIS_DEBUG_LOG single-file source (mirrors `default_file_filter`).
fn firehose_filter() -> EnvFilter {
    EnvFilter::new(firehose_directives())
}

// RUST_LOG-respecting filter for the NEMESIS_LOG_FILE source: DEBUG default, honor
// RUST_LOG, silence sampling_log (preserves NEMESIS_LOG_FILE back-compat).
...
/// filter by source (NEMESIS_LOG_FILE respects RUST_LOG). Open failures warn AFTER
...
pub(crate) enum DebugTarget {
    /// `NEMESIS_DEBUG_LOG=1` → route per session into `<dir>` (`~/.nemesis/debug`).
    PerSession { dir: PathBuf },
    /// An explicit path → one flat `fmt` file, routing bypassed.
    SingleFile { path: PathBuf, src: DebugSource },
}

/// Resolve the debug target, honoring precedence: explicit NEMESIS_LOG_FILE wins
/// (single file, RUST_LOG filter); else NEMESIS_DEBUG_LOG — a truthy bool routes
/// per session into `~/.nemesis/debug`, an explicit path writes a single file.
///
/// Read via `var_os` (not `var`) so a non-UTF-8 path isn't silently dropped.
pub(crate) fn resolve_debug_target() -> Option<DebugTarget> {
    let grok_log_file = std::env::var_os("NEMESIS_LOG_FILE");
    let grok_debug_log = std::env::var_os("NEMESIS_DEBUG_LOG");
    resolve_debug_target_inner(
        grok_log_file.as_deref(),
        grok_debug_log.as_deref(),
        &nemesis_home().join("debug"),
    )
}
...
/// `~/.nemesis/debug` older than [`LOG_RETENTION`] so the dir doesn't grow
...
pub(crate) fn sweep_old_logs() {
    prune_old_logs(&nemesis_home().join("debug"), LOG_RETENTION);
}
...
mod tests {
        ...
        for v in ["0", "false", "off", "no", "", "  "] {
            assert!(
                resolve_debug_target_inner(None, Some(OsStr::new(v)), Path::new("/debug"))
                    .is_none(),
                "expected None for NEMESIS_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_debug_log_enabled_is_per_session_dir() {
        for v in ["1", "true", "on", "yes"] {
            ...
            assert_eq!(
                ...
                "expected PerSession for NEMESIS_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_empty_log_file_falls_through_to_debug_log() {
        // Empty / whitespace NEMESIS_LOG_FILE is treated as unset (mirrors NEMESIS_DEBUG_LOG).
        for blank in ["", "   "] {
            let target = resolve_debug_target_inner(
    ...
    fn resolve_target_non_utf8_debug_log_path_is_single_file() {
        ...
        // A non-UTF-8 NEMESIS_DEBUG_LOG value is a path, not a bool keyword, and its
    ...
    fn layer_routes_event_by_session_span() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            tracing::info_span!("session", session_id = %"sess-xyz").in_scope(|| {
                tracing::info!(target: "xai_nemesis_shell", "inside session");
            });
            tracing::info!(target: "xai_nemesis_shell", "outside session");
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_under_real_firehose_filter() {
        ...
        // `xai_nemesis_telemetry::session_ctx`) survives the real filter so
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!(
                target: "xai_nemesis_telemetry::session_ctx",
                "session",
                session_id = %"sid-real"
            )
            .in_scope(|| {
                tracing::debug!(target: "xai_nemesis_shell", "filtered routing works");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_two_sessions_to_distinct_files() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!("session", session_id = %"sid-one").in_scope(|| {
                ...
                tracing::info!(target: "xai_nemesis_shell", "one first");
                tracing::info!(target: "xai_nemesis_shell", "one second");
            });
            tracing::info_span!("session", session_id = %"sid-two").in_scope(|| {
                tracing::info!(target: "xai_nemesis_shell", "two only");
            });
        });
        crate::appender::flush_file_log_guards();

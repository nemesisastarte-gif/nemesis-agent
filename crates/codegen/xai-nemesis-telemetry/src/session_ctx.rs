...
//!
//! Extracted from `xai-nemesis-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `xai-nemesis-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `xai-nemesis-workspace` (remote sampler / workspace server).
    Workspace,
}

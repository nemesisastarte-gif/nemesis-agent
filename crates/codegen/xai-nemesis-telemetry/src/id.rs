...
//!
//! Extracted from `xai-nemesis-shell::agent::unique_identifier` so the
//! telemetry engine can stamp events without depending on shell internals.
//! `$NEMESIS_HOME` is resolved through `xai-nemesis-config::nemesis_home`.
...
/// This function caches the result in `$NEMESIS_HOME/agent_id` so subsequent calls
...
fn load_or_compute_agent_id() -> String {
    let cache_path = xai_nemesis_config::nemesis_home().join("agent_id");
...
/// Write `$NEMESIS_HOME/agent_id` as owner-read/write only (Unix 0o600) — it is a
...
fn write_agent_id_cache(path: &std::path::Path, id: &str) -> std::io::Result<()> {
    ...
    xai_nemesis_config::fs_atomic::write_atomically(path, id, Some(0o600))
}

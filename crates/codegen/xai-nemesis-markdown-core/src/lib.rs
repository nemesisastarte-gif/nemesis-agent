//! Headless markdown analysis sharing Nemesis Agent's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `xai-nemesis-markdown` so analysis matches what Grok
...
/// The exact `pulldown-cmark` option set Nemesis Agent uses to render markdown.
///
...
/// Parse `text` with Nemesis Agent's options; count elements and flag structural issues.

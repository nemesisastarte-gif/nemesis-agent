...
use xai_nemesis_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use xai_nemesis_pager::client_identity::PAGER_CLIENT_VERSION;
use xai_nemesis_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use xai_nemesis_shell::agent::config::Config as AgentConfig;
use xai_nemesis_shell::leader::{
...
use xai_nemesis_update::{UpdateConfig, auto_update, enforce_minimum_version_or_exit};
...
fn apply_agent_endpoint_args(
    agent_args: &xai_nemesis_pager::app::AgentArgs,
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   Nemesis Agent server starting...");
    eprintln!();
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use xai_nemesis_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(xai_nemesis_telemetry::sampling_log::layer())
        .with(xai_nemesis_telemetry::instrumentation::layer())
        .with(xai_nemesis_telemetry::hooks_log::layer())
        .with(xai_nemesis_telemetry::otel_layer::build_otel_layer(
            xai_nemesis_telemetry::otel_layer::OtelClientInfo {
                client_name: "nemesis-agent",
                client_version: xai_nemesis_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            xai_nemesis_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    xai_nemesis_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    xai_nemesis_telemetry::external::init(
        xai_nemesis_shell::agent::config::resolve_external_otel_config(
            xai_nemesis_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: xai_nemesis_version::VERSION.to_owned(),
                app_entrypoint: app_entrypoint.to_owned(),
            },
        ),
    );
}
/// `nemesis setup`: rendering + exit codes only; fetch logic lives in `xai_nemesis_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use xai_nemesis_shell::managed_config::{self, SetupOutcome};
    if !managed_config::has_principal() {
        eprintln!("No deployment key or team sign-in found.");
        eprintln!();
        eprintln!("To install managed configuration, sign in with a team using `nemesis login`,");
        eprintln!("or set a deployment key:");
        eprintln!();
        if cfg!(unix) {
            eprintln!("  export NEMESIS_API_KEY=<your-key>");
        } else {
            eprintln!("  $env:NEMESIS_API_KEY=\"<your-key>\"");
        }
        eprintln!("  nemesis setup");
        eprintln!();
        eprintln!("Or add the key to ~/.nemesis/config.toml:");
        eprintln!();
        eprintln!("  [endpoints]");
        eprintln!("  deployment_key = \"<your-key>\"");
        eprintln!();
        eprintln!(
            "If you don't have a deployment key, contact your your NEMESIS administrator."
        );
        std::process::exit(1);
    }
    if json {
    ...
    match managed_config::run_setup().await {
        ...
        SetupOutcome::Skipped => {
            eprintln!(
                "Managed configuration was not applied this run (another process held the apply lock, or the credential changed during the fetch). Run `nemesis setup` again."
            );
        }
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = xai_nemesis_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !xai_nemesis_shell::util::is_grok_process(pid) {
        ...
        if let Err(e) = xai_nemesis_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(xai_nemesis_shell::env::GrokBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, xai_nemesis_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = xai_nemesis_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = xai_nemesis_shell::leader::LeaderClient::connect(
...
fn leader_info_json(
    ...
    info: Option<&xai_nemesis_shell::leader::ControlPayload>,
...
const WORKSPACE_COMMAND_ENV: &str = "NEMESIS_WORKSPACE_COMMAND";
...
/// The `NEMESIS_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&xai_nemesis_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
fn fetch_remote_settings() -> Option<xai_nemesis_shell::util::config::RemoteSettings> {
    join_early_prefetch(xai_nemesis_shell::agent::models::start_early_prefetch(None))
}
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Unknown => {
            anyhow::bail!(
             ...
             network connection (run `nemesis login` if you are signed out), then \
             try again."
            )
        }
    }
...
async fn workspace_control(
    ...
    let raw_config = xai_nemesis_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use xai_nemesis_shell::auth::ensure_authenticated;
    xai_nemesis_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = xai_nemesis_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    if !use_leader {
        anyhow::bail!(
            "`grok workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.nemesis/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.grok_com_config,
        false,
        Some("No cached credentials found. Run `nemesis login` first."),
    )
    .await?;
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    xai_nemesis_telemetry::sentry::flush_on_shutdown();
    xai_nemesis_telemetry::otel_layer::shutdown_otel();
    xai_nemesis_telemetry::debug_log::flush();
...
async fn run_agent_command(
    agent_args: Box<xai_nemesis_pager::app::AgentArgs>,
    ...
    let _signal_flush = tokio::spawn(async {
        #[cfg(unix)]
        {
            ...
            use xai_nemesis_pager::app::signal_handler::next_signal_code;
    ...
    let _otel_guard = xai_nemesis_telemetry::otel_layer::otel_guard();
    xai_nemesis_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => xai_nemesis_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(
    ...
    let early_prefetch = xai_nemesis_shell::agent::models::start_early_prefetch(None);
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "Nemesis Agent (pager) - v{}",
            xai_nemesis_version::display_version_with_commit(
                env!("VERSION_WITH_COMMIT"),
                xai_nemesis_update::channel_label(),
            )
        );
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(xai_nemesis_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = xai_nemesis_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
    ...
    agent_config.default_auto_mode = xai_nemesis_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    agent_config.resolve_runtime_fields(&xai_nemesis_shell::agent::config::RuntimeResolutionContext {
    ...
    let managed_install = is_managed_install(
        ...
        &xai_nemesis_shell::util::nemesis_home::nemesis_home(),
    );
    ...
    if use_leader {
        ...
        let env_urls = xai_nemesis_shell::leader::LeaderEnvUrls::from(&agent_config.grok_com_config);
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = xai_nemesis_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            xai_nemesis_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(xai_nemesis_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = xai_nemesis_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = xai_nemesis_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
/// Sets `NEMESIS_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
...
/// (`[dashboard].enabled = false` / `NEMESIS_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !xai_nemesis_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.nemesis/config.toml and \
             unsetting NEMESIS_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("NEMESIS_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<xai_nemesis_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(xai_nemesis_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<xai_nemesis_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(xai_nemesis_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    xai_nemesis_shell::heap_profile::install(xai_nemesis_shell::heap_profile::HeapProfileHooks {
...
fn main() {
    xai_nemesis_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    xai_nemesis_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        xai_nemesis_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        xai_nemesis_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    if let Some(code) = xai_nemesis_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    xai_nemesis_pager::memory_trace::start(
        xai_nemesis_shell::util::nemesis_home::nemesis_home().join("memtrace"),
    );
    raise_fd_limit();
    if let Err(e) = xai_nemesis_config::validate_requirements() {
        eprintln!("Couldn't start NEMESIS: {e}");
        eprintln!();
        eprintln!(
            "Update NEMESIS to a version the policy allows, or ask your administrator \
             to fix the managed requirements."
        );
        std::process::exit(2);
    }
    let _sentry_guard = xai_nemesis_telemetry::sentry::init(xai_nemesis_telemetry::sentry::Config {
        client: "nemesis-agent",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: xai_nemesis_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    xai_nemesis_pager::docs::extract_user_guide_docs(&xai_nemesis_shell::util::nemesis_home::nemesis_home());
    xai_crash_handler::install_terminal_restore_only();
    if xai_nemesis_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = xai_nemesis_shell::util::nemesis_home::nemesis_home().join("crash");
        if let Some(report) = xai_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("NEMESIS crashed during your last session.");
    ...
    let crashed = xai_nemesis_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
...
async fn async_main() -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("NEMESIS_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("NEMESIS_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(xai_nemesis_shell::agent::chat_modes::NEMESIS_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(xai_nemesis_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("NEMESIS_DEBUG_LOG", path);
            std::env::remove_var("NEMESIS_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("NEMESIS_DEBUG_LOG", "1");
        set_if_unset("NEMESIS_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        xai_nemesis_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return xai_nemesis_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        xai_nemesis_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        xai_nemesis_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    xai_nemesis_shell::config::apply_sandbox(
        None,
    ...
    xai_nemesis_shell::http::set_client_name(if is_interactive {
        xai_nemesis_workspace::permission::ClientType::GrokPager
    } else {
        xai_nemesis_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!(
                        ...
                        xai_nemesis_update::channel_name().unwrap_or("unknown"), }
                    );
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    println!(
            ...
            Command::Inspect { json } => {
                ...
                xai_nemesis_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return xai_nemesis_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = xai_nemesis_telemetry::otel_layer::otel_guard();
                return xai_nemesis_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = xai_nemesis_telemetry::otel_layer::otel_guard();
                let config = xai_nemesis_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_nemesis_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_nemesis_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_nemesis_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_nemesis_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return xai_nemesis_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_nemesis_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return xai_nemesis_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                xai_nemesis_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                xai_nemesis_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = xai_nemesis_shell::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                xai_nemesis_shell::auth::run_cli_logout(&config)?;
                xai_nemesis_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = xai_nemesis_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(xai_nemesis_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some() {
            if args.output_format == xai_nemesis_pager::headless::OutputFormat::Plain {
                args.output_format = xai_nemesis_pager::headless::OutputFormat::Json;
            }
            if args.self_verify {
                anyhow::bail!(
        ...
        return xai_nemesis_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            xai_nemesis_pager::headless::HeadlessOptions {
    ...
    let result = xai_nemesis_pager::app::run(args, bg_update_rx).await;
    xai_nemesis_sandbox::flush();
    match result {
        Ok(true) => {
...
fn build_update_config() -> UpdateConfig {
    let environment = xai_nemesis_shell::env::GrokBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                xai_nemesis_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("NEMESIS_NPM_REGISTRY"))
        .ok()
        .or_else(xai_nemesis_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = xai_nemesis_shell::config::load_effective_config_disk_only()
        && let Some(ch) = xai_nemesis_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    !std::env::var_os("NEMESIS_DISABLE_AUTOUPDATER")
...
/// True when `exe` is the binary `<nemesis_home>/bin/grok` resolves to, the
...
/// npm shim hardcodes `~/.grok`, so a custom `NEMESIS_HOME` skips here too.
fn is_managed_install(exe: Option<std::path::PathBuf>, nemesis_home: &std::path::Path) -> bool {
    if nemesis_home.as_os_str().is_empty() {
        return false;
    }
    let Some(exe) = exe else {
        return false;
    };
    let managed = xai_nemesis_config::grok_application_in(nemesis_home);
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in xai_nemesis_shell::leader::discover_leaders().await {
        if d.classification != xai_nemesis_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match xai_nemesis_shell::leader::LeaderClient::connect(
            socket_path,
            "grok-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(xai_nemesis_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(xai_nemesis_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            xai_nemesis_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: xai_nemesis_shell::heap_profile::JemallocStats) {
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            xai_nemesis_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!xai_nemesis_shell::heap_profile::prof_available());
            return;
        }
        assert!(xai_nemesis_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(xai_nemesis_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(xai_nemesis_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        xai_nemesis_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    fn is_managed_install_matches_only_the_bin_grok_target() {
        ...
        assert!(!is_managed_install(
            Some(home.join("bin").join("nemesis")),
            &home
        ));
        ...
        std::os::unix::fs::symlink(&target, home.join("bin").join("nemesis")).unwrap();
        assert!(is_managed_install(
            Some(home.join("bin").join("nemesis")),
            &home
        ));
    ...
    #[serial_test::serial(NEMESIS_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        let mut args = PagerArgs::try_parse_from(["nemesis", "dashboard"]).unwrap();
        ...
        assert_eq!(
            std::env::var("NEMESIS_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("NEMESIS_OPEN_DASHBOARD_AT_STARTUP") };
    }
    ...
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["nemesis", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    #[serial_test::serial(NEMESIS_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("NEMESIS_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("NEMESIS_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("NEMESIS_WORKSPACE_COMMAND", "off") };

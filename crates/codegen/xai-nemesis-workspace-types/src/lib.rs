//! Wire types for the `xai-nemesis-workspace` API.
//!
...
//! `xai-nemesis-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `xai-nemesis-workspace-types` (instead of `xai-nemesis-workspace` or
/// `xai-nemesis-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `xai_nemesis_workspace::permission`

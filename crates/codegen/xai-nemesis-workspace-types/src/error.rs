...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] xai_nemesis_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `xai-nemesis-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `xai_nemesis_tools::ToolError`) happens

...
//! agent-session config types in `xai-nemesis-config` and friends.
...
/// `xai-nemesis-tools` once the wire surface is firm.
...
///
/// TODO(workspace): align with `xai_nemesis_config::ProjectConfig`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]

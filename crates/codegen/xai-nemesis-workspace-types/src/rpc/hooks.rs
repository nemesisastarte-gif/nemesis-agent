...
//!
//! `xai_nemesis_hooks` pulls in `git2`/`reqwest`/`xai-nemesis-tools`, too heavy for
...
/// Wire mirror of `xai_nemesis_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `xai_nemesis_hooks::config::HookSpec`.
///
...
/// `xai-nemesis-hooks`, so a server-side test (`xai-nemesis-workspace`'s
...
/// Wire mirror of `xai_nemesis_hooks::event::HookEventName`.
///
        ...
        let json = serde_json::json!({
            "hooks": {
                "pre_tool_use": [{
                    ...
                    "url_raw": null,
                    "timeout_ms": 5000,
                    "source_dir": "/home/u/.nemesis/hooks",
                    "extra_env": { "FOO": "bar" }
                }]
            }
        });

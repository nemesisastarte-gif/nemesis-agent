...
//! `xai-nemesis-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    /// cwd/.nemesis/skills
    Local,
    /// repo_root/.nemesis/skills
    Repo,
    /// ~/.nemesis/skills
    User,
    /// ~/.nemesis/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.nemesis/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    // Fixture mirrored field-for-field from the xai-nemesis-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.nemesis/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.nemesis/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.nemesis/plugins/infra-plugin",
            "plugin_data": "/root/.nemesis/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "grok-4",
            "effort": "high",
            "user_invocable": true,

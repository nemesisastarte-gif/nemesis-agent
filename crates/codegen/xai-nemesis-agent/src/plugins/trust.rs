...
//!
//! Plugins from project directories (`.nemesis/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.nemesis/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.nemesis/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.nemesis/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_nemesis_home() so a project's `.nemesis/trusted-plugins` is never
        // read as the user trust store when neither NEMESIS_HOME nor a home dir resolves.
        let Some(grok) = xai_nemesis_config::user_nemesis_home() else {
            return Self {
                trusted: HashSet::new(),
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.nemesis/trusted-plugins`.
    ...
    /// rewrites `~/.nemesis/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.nemesis/trusted-plugins`.

...
/// Default install directory name under `~/.nemesis/`.
...
/// Lives here (not in xai-nemesis-plugin-marketplace) to keep dependency direction sane.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl InstallRegistry {
    ...
    /// 2. Default: `~/.nemesis/installed-plugins/`
    pub fn resolve_install_dir() -> PathBuf {
        if let Some(dir) = Self::read_install_dir_from_config() {
            return dir;
        }

        xai_nemesis_config::nemesis_home().join(DEFAULT_INSTALL_DIR_NAME)
    }

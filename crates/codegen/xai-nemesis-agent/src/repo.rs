...
//! primitive consumed cross-crate by `xai-nemesis-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.nemesis/config.toml`, `.claude/settings.json`,
...
/// from `xai-nemesis-workspace`, which depends on THIS crate) to keep the dep edge

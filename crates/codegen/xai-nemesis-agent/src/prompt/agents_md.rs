...
//!
//! Searches from cwd to repo root, plus `~/.nemesis/`. Also discovers
//! `*.md` files in rules directories: vendor-prefixed `.nemesis/rules/`,
...
//! (`~/.nemesis/rules/`, `~/.claude/rules/`, `~/.cursor/rules/`).
...
/// Find `*.md` files in `.nemesis/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.nemesis/, git repo root, and session cwd.
...
async fn read_agents_config_with_roots(
    ...
    add_discovery_root(&mut home_roots, nemesis_home, true, HOME_RULES_DIRS);
    ...
    candidates
        ...
        .filter_map(|candidate| {
            ...
            let content = if candidate.is_rule {
                xai_nemesis_tools::implementations::skills::skill::extract_skill_body(&content)
            } else {
                content
            };
...
/// Used by `xai-nemesis-shell` to structurally detect legacy untagged AGENTS.md
...
mod tests {
    ...
    async fn home_and_project_rules_have_stable_order_without_doubled_paths() {
        ...
        for path in [
            nemesis_home.join(".nemesis/rules/doubled.md"),
    ...
    async fn vendor_home_agents_and_rules_cells_are_independent() {
        let tmp = tempfile::tempdir().unwrap();
        let nemesis_home = tmp.path().join("grok-home");
        let home = tmp.path().join("home");
        let cwd = tmp.path().join("project");
        fs::create_dir_all(&nemesis_home).unwrap();
    ...
    #[tokio::test]
    async fn nested_nemesis_home_keeps_project_role_in_repo_order() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        let nested = repo.join("nested");
        fs::create_dir_all(nested.join("rules")).unwrap();
        fs::create_dir_all(nested.join(".nemesis/rules")).unwrap();
        init_git_repo(&repo);
        ...
        fs::write(nested.join(".nemesis/rules/project.md"), "nested-project-rule").unwrap();
    ...
    #[tokio::test]
    async fn overlapping_nemesis_home_and_project_root_merges_roles() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        ...
        fs::write(repo.join(".nemesis/rules/project.md"), "project-grok-rule").unwrap();
        fs::write(repo.join(".claude/rules/project.md"), "project-claude-rule").unwrap();
        fs::create_dir_all(repo.join(".nemesis/.nemesis/rules")).unwrap();
        fs::write(repo.join(".nemesis/.nemesis/rules/doubled.md"), "doubled").unwrap();
    ...
    async fn rule_frontmatter_is_stripped_but_named_frontmatter_is_preserved() {
        ...
        for (path, body) in [
            (nemesis_home.join("rules/global.md"), "custom-home-body"),
            (home.join(".claude/rules/global.md"), "claude-body"),
            (home.join(".cursor/rules/global.md"), "cursor-body"),
            (repo.join(".nemesis/rules/project.md"), "grok-project-body"),

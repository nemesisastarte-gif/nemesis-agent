...
//! - `Default` -- the legacy Nemesis Agent prefix (built by the shell layer).
...
use xai_nemesis_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy Nemesis Agent prefix: `<user_info>` + optional `<git_status>`.
...
/// One discovered rule file (AGENTS.md / Claude.md / .nemesis/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `xai_nemesis_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.nemesis/, ~/.claude/).
    ...
    /// (`~/.nemesis/projects/<encoded-cwd>/mcps`). Surfaced in

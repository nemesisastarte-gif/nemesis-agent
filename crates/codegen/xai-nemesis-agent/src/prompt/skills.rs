...
//! live in `xai_nemesis_tools::implementations::skills::discovery`.

use std::collections::{HashMap, HashSet};
use std::path::{Path, PathBuf};

use crate::plugins::discovery::PluginScope;
use xai_nemesis_tools::implementations::skills::types::skill_name_from_path;
pub use xai_nemesis_tools::implementations::skills::types::{SkillInfo, SkillScope};
/// Re-export so agent-side discovery (and the shell) can name the resolved
/// vendor-compat config without reaching into `xai_nemesis_tools` directly.
pub use xai_nemesis_tools::types::compat::CompatConfig;

use xai_nemesis_tools::implementations::skills::discovery::{
...
///
/// Priority order: Local (cwd/.nemesis/skills, cwd/.agents/skills, cwd/.claude/skills) → Intermediate dirs →
/// Repo (repo_root/.nemesis/skills, repo_root/.agents/skills, repo_root/.claude/skills) → User (~/.nemesis/skills, ~/.agents/skills, ~/.claude/skills)
/// → additional paths from `config.paths`
/// → Server (injected `config.server_skill_dirs`)
/// → Bundled (injected `config.bundled_skill_dirs` + `~/.nemesis/bundled`; lowest precedence).
///
...
/// (e.g. `Repo` for `.nemesis/plugins/`). Native skills always win bare-name
...
pub async fn list_skills_with_plugins(
    ...
    let mut skills = list_skills_with_options(
        ...
        &xai_nemesis_tools::util::nemesis_home::nemesis_home(),
        compat,
    )
    .await;
...
pub fn collect_skill_config_dirs(
    ...
    let nemesis_home = global_dir.to_path_buf();
    ...
    // this list equals the historical `[".nemesis", ".agents", ".claude", ".cursor"]`.
...
fn scope_for_config_dir(dir: &Path, cwd: Option<&Path>, git_root: Option<&Path>) -> SkillScope {
    // Home-level dirs (e.g. ~/.nemesis/, ~/.agents/, ~/.claude/) are User scope.
    #[allow(deprecated)]
    if let Some(home) = std::env::home_dir()
...
fn collect_config_skills(config_paths: &[String], git_root: Option<&Path>) -> Vec<SkillInfo> {
    ...
    for skill in &mut skills {
        skill.config_source = Some(
            xai_nemesis_tools::types::config_source::ConfigSource::ConfigToml {
...
fn stamp_plugin_fields(skills: &mut [SkillInfo], plugin: &crate::plugins::LoadedPlugin) {
    ...
    for skill in skills.iter_mut() {
        ...
        skill.config_source = Some(xai_nemesis_tools::types::config_source::ConfigSource::Plugin {
...
pub(crate) fn format_skill_for_injection(skill: &SkillInfo) -> Option<String> {
    skill.body.as_ref().filter(|b| !b.is_empty()).map(|body| {
        xai_nemesis_tools::implementations::skills::skill::build_skill_message(skill, body)
    })
}
    ...
    for name in names {
        ...
        let skill = discovered.iter().find(|s| {
            s.name.eq_ignore_ascii_case(name)
                || xai_nemesis_tools::implementations::skills::skill::format_skill_name(s)
        ...
        match xai_nemesis_tools::implementations::skills::skill::load_skill_with_body(skill).await {
...
mod tests {
    ...
    async fn server_skills_discovered_and_shadowed_by_local() {
        ...
        write_skill_md(&cwd.path().join(".nemesis").join("skills").join("dup"), "dup");
    ...
    fn find_skill_paths_flat_layout() {
        ...
        let grok_dir = tmp.path().join(".nemesis");
    ...
    async fn list_skills_includes_workspace_user_skills() {
        ...
        write_skill_md(
            &user_dir.join(".nemesis").join("skills").join("my-tool"),
            "my-tool",
        );
    ...
    async fn list_skills_workspace_user_dedup_when_cwd_inside_user_dir() {
        ...
        let user_dir = repo_root.join("x").join("testuser");
        write_skill_md(
            &user_dir.join(".nemesis").join("skills").join("dedup-skill"),
            "dedup-skill",
        );
    ...
    async fn list_skills_no_workspace_user_dir_no_extra_skills() {
        ...
        write_skill_md(
            &user_dir.join(".nemesis").join("skills").join("ghost-skill"),
            "ghost-skill",
        );
    ...
    async fn list_skills_workspace_user_dir_with_recursive_skills() {
        ...
        let skills_base = user_dir.join(".nemesis").join("skills");
    ...
    fn collect_config_skills_stamps_config_toml_source() {
        ...
        match &skills[0].config_source {
            Some(xai_nemesis_tools::types::config_source::ConfigSource::ConfigToml { path }) => {
    ...
    async fn list_skills_deduplicates_auto_and_config_overlap() {
        ...
        let auto_dir = repo_root.join(".nemesis").join("skills").join("dup-skill");
    ...
    async fn list_skills_auto_and_config_overlap_keeps_config_toml_source() {
        ...
        let auto_dir = repo_root.join(".nemesis").join("skills").join("overlap-skill");
        ...
        assert!(
            matches!(
                ...
                Some(xai_nemesis_tools::types::config_source::ConfigSource::ConfigToml { .. })
            ),
    ...
    async fn list_skills_ignore_allows_lower_priority_same_name_fallback() {
        ...
        write_skill_md(&cwd.join(".nemesis").join("skills").join("same"), "same");
        let repo_skill_dir = repo_root.join(".nemesis").join("skills").join("same");
    ...
    // these functions now live in xai-nemesis-tools::implementations::skills::discovery
    // and xai-nemesis-tools::types::skill_discovery_tracker, tested there.
    ...
    async fn disabled_config_marks_skill_enabled_false() {
        ...
        write_skill_md(
            &repo_root.join(".nemesis").join("skills").join("commit"),
            "commit",
        );
        write_skill_md(
            &repo_root.join(".nemesis").join("skills").join("review"),
            "review",
        );
    ...
    async fn disabled_config_empty_leaves_all_enabled() {
        ...
        write_skill_md(
            &repo_root.join(".nemesis").join("skills").join("deploy"),
            "deploy",
        );
    ...
    fn collect_skill_config_dirs_gates_vendor_dirs() {
        ...
        for name in [".nemesis", ".agents", ".claude", ".cursor"] {
        ...
        assert!(ends_with(&dirs, ".nemesis"), "grok must remain: {dirs:?}");
    }
    ...
    fn dedupe_same_scope_cross_harness_loser_resurfaces() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "review",
                "/u/.nemesis/skills/review/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "review",
    ...
    fn dedupe_frontmatter_owner_evicts_rekeyed_squatter() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.nemesis/skills/japandi/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi",
                "/u/.nemesis/skills/japandi2/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi2",
    ...
    fn dedupe_cross_scope_shadowing_unchanged() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/repo/.nemesis/skills/japandi/SKILL.md",
                SkillScope::Repo,
            ),
            named_skill("japandi", "/u/skills/japandi2/SKILL.md", SkillScope::User),
        ]);
    ...
    fn dedupe_same_scope_same_basename_still_drops() {
        ...
        // (e.g. ~/.nemesis/skills and ~/.agents/skills): first-seen wins.
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.nemesis/skills/japandi/SKILL.md",
        ...
        assert_eq!(out.len(), 1);
        assert!(out[0].path.contains(".nemesis"));
    }
    ...
    async fn copied_skill_dir_with_stale_frontmatter_name_surfaces_both() {
        ...
        let skills_dir = repo_root.join(".nemesis").join("skills");

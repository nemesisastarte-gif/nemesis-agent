//! Agent definition types — parsed from `.nemesis/agents/*.md` files.
...
use xai_nemesis_tools::implementations::codex;
use xai_nemesis_tools::implementations::grok_build;
use xai_nemesis_tools::implementations::grok_build_concise;
...
use xai_nemesis_tools::implementations::use_tool;
use xai_nemesis_tools::registry::types::{ToolConfig, ToolServerConfig};
...
pub fn grok_build_hashline_toolset(
    hashline_tools: Vec<xai_nemesis_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<xai_nemesis_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
...
/// Nemesis Agent + plan mode toolset.
///
...
/// Nemesis Agent + plan mode toolset WITHOUT subagent tools.
///
...
/// Default Nemesis Agent toolset + `ask_user_question`.
///
...
/// Portable agent identity — parsed from .nemesis/agents/*.md.
...
pub enum AgentScope {
    /// .nemesis/agents/ (project-level, highest priority)
    Project,
    /// ~/.nemesis/agents/ (user-level)
    User,
    /// ~/.nemesis/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
...
pub enum MemoryScope {
    /// `~/.nemesis/agent-memory/<name>/`
    User,
    /// `<project>/.nemesis/agent-memory/<name>/`
    Project,
    /// `<project>/.nemesis/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: xai_nemesis_config::nemesis_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".nemesis/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".nemesis/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `xai_nemesis_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        if path_str.contains(".nemesis/agents/") || path_str.contains(".nemesis\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".nemesis/bundled/agents/")
...
impl AgentDefinition {
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<xai_nemesis_tools::registry::types::ToolConfig>,
    ) {
    ...
    pub fn default_grok_build() -> Self {
        Self::base(
            ...
            "Nemesis Agent agent for software engineering tasks.",
        )
    }
    /// Nemesis Agent Concise agent definition — concise output format for SFT/RL.
    pub fn grok_build_concise() -> Self {
        Self {
            ...
            ..Self::base(
                ...
                "Nemesis Agent agent with concise output format.",
            )
        }
    }
    /// Nemesis Agent agent with plan mode tools.
    pub fn grok_build_plan() -> Self {
        Self {
            tool_config: grok_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::GrokBuildPlan,
                "Nemesis Agent agent with plan mode support.",
            )
        }
    }
    /// Nemesis Agent + plan mode WITHOUT subagent tools.
    pub fn grok_build_plan_no_subagents() -> Self {
        Self {
            ...
            ..Self::base(
                ...
                "Nemesis Agent agent with plan mode (no subagents).",
            )
        }
    }
    /// Default Nemesis Agent agent with the `ask_user_question` tool.
    pub fn grok_build_ask_user() -> Self {
        Self {
            tool_config: grok_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::GrokBuildAskUser,
                "Nemesis Agent agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    /// Nemesis Agent Orchestrator — GBL model with full GrokBuild tools
...
if name == "s"));
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.nemesis/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.nemesis/agent-memory-local/a")
        );

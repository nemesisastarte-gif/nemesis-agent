...
//!
//! Searches `.nemesis/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.nemesis/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.nemesis/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".nemesis/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.nemesis/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.nemesis/agents` / `.claude/agents`) under each
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    let grok = xai_nemesis_config::user_nemesis_home();
...
fn all_subagents_with_home(
    ...
    let discovered = discover_with_home(cwd, home, nemesis_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.nemesis/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.nemesis/agents/` walking from `cwd` up to repo root
/// 2. `~/.nemesis/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.nemesis/bundled/agents/` (bundled, lowest priority)
///
...
/// compat agents, then bundled. `.grok` dirs resolve from `nemesis_home`
/// (NEMESIS_HOME-aware) plus the legacy literal `~/.grok` when NEMESIS_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    nemesis_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.grok, included only when it differs from nemesis_home
    // (i.e. NEMESIS_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_grok = home
        .map(|h| h.join(".nemesis"))
        .filter(|legacy| nemesis_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = nemesis_home {
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, nemesis_home) {
        if dir.is_dir() {
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, nemesis_home);
...
///
/// Project-level `.nemesis/agents/` has highest priority, then falls back
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, nemesis_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, nemesis_home) {
        return Some(def);
    }
...
/// Load project agent definitions from every `.nemesis/agents` / `.claude/agents`
...
mod tests {
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_grok_when_nemesis_home_differs() {
        ...
        assert!(paths.contains(&home.join(".nemesis").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&grok.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".nemesis").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_nemesis_home_is_dot_grok() {
        let home = Path::new("/home/u");
        let grok = home.join(".nemesis");
        ...
        assert_eq!(
            count, 1,
            "no duplicate ~/.nemesis/agents when nemesis_home == ~/.nemesis"
        );
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".nemesis").join("agents");
        let agents_dir_2 = inner_dir.join(".nemesis").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".nemesis").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".nemesis")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".nemesis")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".nemesis").join("agents");
        let bundled_dir = home.join(".nemesis").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".nemesis")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".nemesis")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".nemesis").join("agents");
        let bundled_dir = home.join(".nemesis").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .nemesis/agents/ directory — should fall back to built-in
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.nemesis/agents/) agent named "explore" should NOT shadow

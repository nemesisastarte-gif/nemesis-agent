...
//! `~/.nemesis/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.

use std::path::{Path, PathBuf};

use xai_nemesis_tools::util::nemesis_home::nemesis_home;

/// Scope for a memory write operation.
/// Write-operation scope. Distinct from `xai_nemesis_agent::config::MemoryScope` (agent memory dir).
...
/// `~/.nemesis/memory/`. Workspace-scoped files live under a directory
/// named `{project-slug}-{hash8}`, e.g. `~/.nemesis/memory/xai-a3f7b2c9/`.
#[derive(Debug, Clone)]
pub struct MemoryStorage {
    /// `~/.nemesis/memory/`
    global_dir: PathBuf,
    /// `~/.nemesis/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.nemesis/memory/`.
    ///
    ...
    fn new_inner(cwd: &Path, root_override: Option<&Path>, use_workspace_hash: bool) -> Self {
        let global_dir = root_override
            .map(|p| p.to_path_buf())
            .unwrap_or_else(|| nemesis_home().join("memory"));
    ...
    ///
    /// File path: `~/.nemesis/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`

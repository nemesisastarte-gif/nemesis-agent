//! `paste-latency` — clipboard paste latency benchmark for `xai-nemesis-pager`.
//!
...
//!
//! ```bash
//! cargo bench -p xai-nemesis-pager-pty-harness --bench paste_latency -- --iterations 10
//!
//! # Compare an old release artifact, text mode only, JSON to a file:
//! cargo bench -p xai-nemesis-pager-pty-harness --bench paste_latency -- \
...
use xai_nemesis_pager_pty_harness::{
...
#[command(
    name = "paste-latency",
    about = "Clipboard paste latency benchmark for xai-nemesis-pager (macOS, real pasteboard)",
    long_about = None,
)]
struct Cli {

...
//!
//! ```bash
//! cargo test -p xai-nemesis-pager-pty-harness --test prompt_history_durable_quit \
//!   -- --ignored --nocapture
//! ```
...
use xai_nemesis_pager_pty_harness::{ContentController, PtyHarness, keys, pager_binary};
...
/// The per-CWD history file lives at `<home>/.nemesis/sessions/<enc-cwd>/prompt_history.jsonl`.
fn find_prompt_history(home: &Path) -> Result<PathBuf> {
    let root = home.join(".nemesis").join("sessions");

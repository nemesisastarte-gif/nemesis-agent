//! Scripted TUI scenario runner for xai-nemesis-pager.
//!
...
pub struct EnvironmentConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional `config.toml` written into the run's isolated `$NEMESIS_HOME`
...
impl ScriptedScenarioRunner {
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&nemesis_home)
                .with_context(|| format!("create scenario NEMESIS_HOME {}", nemesis_home.display()))?;
            fs::write(nemesis_home.join("config.toml"), config_toml)

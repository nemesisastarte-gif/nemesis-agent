...
//! [`MockInferenceServer`] from `xai-nemesis-test-support` and provides the
...
pub use xai_nemesis_test_support::mock_server::LogEntry;
pub use xai_nemesis_test_support::mock_server::MockModelEntry as MockModel;
pub use xai_nemesis_test_support::mock_server::StorageUpload;
// SSE event builders for `enqueue_response` scripts (reasoning turns etc.).
pub use xai_nemesis_test_support::sse;
pub use xai_nemesis_test_support::{
...
impl ContentController {
    ...
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ("HOME".into(), home),
            // Explicit NEMESIS_HOME prevents leaking the real user's
            // config.toml when $HOME alone isn't sufficient (e.g. if
            // NEMESIS_HOME is set in the test runner's env).
            ("NEMESIS_HOME".into(), nemesis_home),
...
mod tests {
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(
            get("NEMESIS_HOME").as_deref(),
            content.home().join(".nemesis").to_str()
        );

...
//! elected leader) plus a fixed leader socket beneath its `NEMESIS_HOME`; clients
...
impl LeaderCluster {
    ...
    /// The cluster's sessions root: `NEMESIS_HOME/sessions` (layout below is
    /// `sessions/<encoded-cwd>/<session-id>/updates.jsonl`).
    fn sessions_dir(&self) -> PathBuf {
        self.content.home().join(".nemesis").join("sessions")
    }

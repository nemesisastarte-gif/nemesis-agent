...
//! `xai_nemesis_sampler::shared_http` (one HTTP/2 pooled client plus
...
use xai_nemesis_workspace::permission::ClientType;

/// Startup span timer, local to this crate.
///
/// Replaces `xai_nemesis_shell::instrumentation_timer!`, which cannot be referenced
/// here (it lives in the shell crate, which now depends on this one). This is a
/// behavior-preserving copy: it routes to the same
/// `xai_nemesis_telemetry::instrumentation` API and keeps the Chrome trace
/// span for these startup timings.
macro_rules! startup_timer {
    ($name:literal) => {{
        use xai_nemesis_telemetry::instrumentation::{
...
// `OriginClientInfo` is owned by `xai-nemesis-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `xai-nemesis-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `xai-nemesis-telemetry` consumes
// the same type via `xai_nemesis_sampler::OriginClientInfo`. The shell-specific
// constructors that depended on `ClientType` (a shell-only type) are free
// functions below.
pub use xai_nemesis_sampler::OriginClientInfo;
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn xai_nemesis_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(xai_nemesis_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
mod tests {
    ...
    fn origin_client_info_from_meta_uses_client_type_when_identifier_absent() {
        ...
        assert_eq!(
            origin_client_info_from_meta(Some(&meta)),
            Some(OriginClientInfo {
                product: "nemesis-agent".to_string(),

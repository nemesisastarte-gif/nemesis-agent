...
//! it" buckets.
//!
//! `xai-nemesis-sampler` is intentionally decoupled from `xai-nemesis-shell`
...
/// tools live in `xai-nemesis-tools`
/// (`crates/codegen/xai-nemesis-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `xai_nemesis_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `xai_nemesis_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `xai-nemesis-shell/src/auth/manager.rs` to match, otherwise the

...
use xai_nemesis_sampling_types::error::{try_parse_stream_error, user_facing_api_error_message};
...
pub use xai_nemesis_sampling_types::ApiBackend;
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = xai_nemesis_sampling_types::reported_cost_ticks(
        value
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<xai_nemesis_sampling_types::DoomLoopRecoveryPolicy>,
}
...
impl SamplingClient {
    ...
    pub async fn create_response(
        ...
        xai_nemesis_sampling_types::patch_reasoning_text_types(&mut request_body);
        let http_request = grok_headers
    ...
    pub async fn conversation_stream_responses(
        ...
        let extra_tools = xai_nemesis_sampling_types::extra_raw_tools(&request.hosted_tools);
...
mod tests {
    ...
    use xai_nemesis_sampling_types::types::ChatRequestMessage;
    ...
    fn new_applies_extra_headers() {
        ...
        cfg.extra_headers
            .insert("x-XAI-token-auth".to_string(), "xai-nemesis-cli".to_string());
    ...
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = xai_nemesis_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),

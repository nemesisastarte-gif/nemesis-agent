...
//!
//! This crate provides the common logic used by `xai-nemesis-shell` and
//! `xai-nemesis-pager` for handling announcements (banner notifications).
...
/// Read hidden announcement ids from `~/.nemesis/announcements.json`.
...
/// Write hidden announcement ids to `~/.nemesis/announcements.json`.
...
fn announcements_state_path() -> PathBuf {
    xai_nemesis_tools::util::nemesis_home::nemesis_home().join("announcements.json")
}
